...
@implementation ZGroupModifyNameVC
...
#pragma mark - 界面布局
- (void)setupNavUI {
    self.navTitleStr = MultilingualTranslation(@"修改群名称");
...
- (void)setupUI {
    ...
    tipLabel.text = MultilingualTranslation(@"群名称");
...
- (void)requestChangeGroupName{
    if ([NSString isNil:self.textView.text]) {
        [HUD showMessage:MultilingualTranslation(@"群名称不能为空")];
        return;
    }
    WeakSelf
    ...
    [IMSDKManager changeGroupNameWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (dataResult) {
            ...
            for (UIViewController * vc in self.navigationController.viewControllers) {
                if ([vc isKindOfClass:[ZGroupSetBasicInfoVC class]]) {
                    ZGroupSetBasicInfoVC * infoVc = (ZGroupSetBasicInfoVC *)vc;
