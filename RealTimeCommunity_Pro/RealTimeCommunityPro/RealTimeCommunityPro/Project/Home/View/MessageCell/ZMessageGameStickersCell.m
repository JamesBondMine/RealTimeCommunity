//
//  ZMessageGameStickersCell.m
//  CIMKit
//
//  Created by cusPro on 2023/12/14.
//

#import "ZMessageGameStickersCell.h"

@interface ZMessageGameStickersCell ()
...
@implementation ZMessageGameStickersCell
...
- (void)setConfigMessage:(ZMessageModel *)model {
    ...
    if (isGameAnimationed) {
        ...
        if (model.message.gameSticekersType == ZChatGameStickerTypeFingerGuessing) {
            //石头剪刀布
            contentImgName = [NSString stringWithFormat:@"ichat_message_stoneScissorCloth%@", model.message.gameStickersResut];
        } else if (model.message.gameSticekersType == ZChatGameStickerTypePlayDice) {
            //摇骰子
            contentImgName = [NSString stringWithFormat:@"ichat_message_dice%@", model.message.gameStickersResut];
        } else {
            contentImgName = @"";
        }
        //设置图片
...
- (void)gameStickersStartAnimation {
    ...
    if (self.messageModel.message.gameSticekersType == ZChatGameStickerTypeFingerGuessing) {
        iconAnimationNamePrefix = @"ichat_message_stoneScissorCloth";
        imgNum = 3;
        duringTime = 0.9;
    }
...
- (void)gameStickersStopAnimation {
    if ([self.messageModel.message.msgID isEqualToString:self.msgIdStr]) {
        ...
        if (self.messageModel.message.gameSticekersType == ZChatGameStickerTypeFingerGuessing) {
            iconContentNamePrefix = @"ichat_message_stoneScissorCloth";
        }
        if (self.messageModel.message.gameSticekersType == ZChatGameStickerTypePlayDice) {
            iconContentNamePrefix = @"ichat_message_dice";
        }
