...
@implementation ZMessageVideoCell
{
...
- (void)setupSnapshotUI {
    ...
    _videoSignImgView.image = ImgNamed(@"remsg_img_video_msg_sign");
...
- (void)setConfigMessage:(ZMessageModel *)model {
    ...
    [model setUploadFileFail:^{
        ZFileUploadTask *videoTask = [[ZFileUploadManager sharedInstance] getTaskWithId:weakSelf.messageModel.message.msgID];
        ZFileUploadTask *coverTask = [[ZFileUploadManager sharedInstance] getTaskWithId:[NSString stringWithFormat:@"%@_cover",weakSelf.messageModel.message.msgID]];
        [ZTOOL doInMain:^{
