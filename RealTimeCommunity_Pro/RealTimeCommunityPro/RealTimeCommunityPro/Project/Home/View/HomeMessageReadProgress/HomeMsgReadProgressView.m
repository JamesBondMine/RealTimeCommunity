//
//  HomeMsgReadProgressView.m
//  CIMKit
//
//  Created by cusPro on 2022/11/22.
//

#import "HomeMsgReadProgressView.h"

@interface HomeMsgReadProgressView()
...
@implementation HomeMsgReadProgressView
...
- (instancetype)initWithRadius:(CGFloat)radius fillColor:(UIColor *)fillColor{
    if (self = [super init]) {
        ...
        _statusImgView.image = ImgNamed(@"remsg_img_msg_not_readed");
...
- (void)setProgress:(float)progress{
    ...
    if (_progress <= 0) {
        ...
        _statusImgView.hidden = NO;
        _statusImgView.image = ImgNamed(@"remsg_img_msg_all_readed");
        _borderColor = COLOR_CLEAR;
        _borderWidth = 0;
        _fillColor = COLOR_CLEAR;
    } else {
        _statusImgView.hidden = YES;
        _borderColor = COLOR_81D8CF;
        _borderWidth = 1.5;
        _fillColor = COLOR_81D8CF;
    }
