//
//  ZInputOldPasswordViewController.m
//  CIMKit
//
...
@interface ZInputOldPasswordViewController ()
...
@implementation ZInputOldPasswordViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.navTitleStr = MultilingualTranslation(@"请输入密码");
    [self setupNavBarUI];
    [self setupUI];
}
...
- (void)setupNavBarUI {
    ...
    [self.navBtnRight setTitle:MultilingualTranslation(@"下一步") forState:UIControlStateNormal];
...
- (void)setupUI {
    ...
    titleLab.text = MultilingualTranslation(@"请输入密码");
...
- (void)requestGetEncryptKeyAction {
    if ([NSString isNil:_inputTextField.text]) {
        [HUD showMessage:MultilingualTranslation(@"密码不能为空") inView:self.view];
        return;
    }
...
- (void)requestCheckUserPassword:(NSString *)encryptKey {
    ...
    [IMSDKManager userCheckUserPasswordWith:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (checkResult) {
            ...
            [HUD showMessage:MultilingualTranslation(@"密码校验失败") inView:self.view];
        }
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        @strongify(self)
        [HUD showMessageWithCode:code errorMsg:msg inView:self.view];
...
- (UITextField *)inputTextField {
    if (!_inputTextField) {
        ...
        _inputTextField.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
        _inputTextField.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_F5F6F9_DARK];
        _inputTextField.font = FONTN(16);
//        _inputTextField.placeholder = MultilingualTranslation(@"输入原密码");
        _inputTextField.secureTextEntry = YES;
        _inputTextField.textAlignment = NSTextAlignmentLeft;
        _inputTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:MultilingualTranslation(@"输入原密码")];
