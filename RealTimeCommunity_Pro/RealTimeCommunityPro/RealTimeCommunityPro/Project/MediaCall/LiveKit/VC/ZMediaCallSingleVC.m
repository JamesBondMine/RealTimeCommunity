//
//  ZMediaCallSingleVC.m
//  CIMKit
//
//  Created by cusPro on 2023/1/6.
//

#import "ZMediaCallSingleVC.h"
#import "UIButton+Gradient.h"

@interface ZMediaCallSingleVC () <ZWindowFloatViewDelegate>
...
@property (nonatomic, strong) ZWindowFloatView *viewFloat;//浮窗视频流
@property (nonatomic, strong) MediaZZCallVideoView *viewFloatVideo;

@property (nonatomic, strong) MediaZZCallVideoView *viewPositionVideo;//固定视频流

@property (nonatomic, strong) ZUserModel *userModel;//对方用户信息
@end

@implementation ZMediaCallSingleVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    [ZMediaCallManager sharedManager].currentScreenTrackMe = YES;
    ...
    if ([ZMediaCallManager sharedManager].currentRoomCalling) {
        ...
        if (currentCallOptions.callType == LingIMCallTypeAudio) {
            ...
            if (currentCallOptions.callRoleType == LingIMCallRoleTypeResponse) {
                ...
                //邀请方
                _lblCallTip.text = MultilingualTranslation(@"正在等待对方接受邀请");
                [self btnShowAnimationWith:self.btnMutedAudio];
                [self btnShowAnimationWith:self.btnEnd];
                [self btnShowAnimationWith:self.btnExternal];
            }
...
#pragma mark - 接受通话
- (void)callAccept {
    [HUD showActivityMessage:[NSString stringWithFormat:@"%@...",MultilingualTranslation(@"接听")]];
    
    ZMediaCallOptions *currentCallOptions = [ZMediaCallManager sharedManager].currentCallOptions;
    
    [[ZMediaCallManager sharedManager] mediaCallAcceptWith:currentCallOptions onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
...
//最小化
- (void)btnMiniClick {
    ZWindowFloatView *viewMediaCall = [ZWindowFloatView new];
    ...
    ZMediaCallFloatView *viewFloat = [ZMediaCallFloatView new];
...
- (void)btnEndClick {
    ...
    //调用接口
    [[ZMediaCallManager sharedManager] mediaCallDiscardWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
    ...
    if (self.btnMutedAudio.isSelected) {
        ...
        [[ZMediaCallManager sharedManager] mediaCallAudioMute:NO complete:^(BOOL isMuted) {
            if (!isMuted) {
                ...
                [HUD showMessage:MultilingualTranslation(@"静音关闭") inView:weakSelf.viewHUDBg];
            }
        }];
    } else {
        //执行开启静音
        [[ZMediaCallManager sharedManager] mediaCallAudioMute:YES complete:^(BOOL isMuted) {
            if (isMuted) {
                ...
                [HUD showMessage:MultilingualTranslation(@"静音开启") inView:weakSelf.viewHUDBg];
            }
        }];
    }
}
    ...
    if (self.btnMutedVideo.isSelected) {
        ...
        [[ZMediaCallManager sharedManager] mediaCallVideoMute:NO complete:^(BOOL isMuted) {
            if (!isMuted) {
                ...
                [HUD showMessage:MultilingualTranslation(@"摄像头开启") inView:weakSelf.viewHUDBg];
            }
        }];
    } else {
        //执行开启静默
        [[ZMediaCallManager sharedManager] mediaCallVideoMute:YES complete:^(BOOL isMuted) {
            if (isMuted) {
                ...
                [HUD showMessage:MultilingualTranslation(@"摄像头关闭") inView:weakSelf.viewHUDBg];
            }
        }];
    }
}
...
- (void)btnExternalClick {
    if (self.btnExternal.isSelected) {
        ...
        [HUD showMessage:MultilingualTranslation(@"免提关闭") inView:_viewHUDBg];
    }else {
        //开启免提扬声器
        [[ZMediaCallManager sharedManager] mediaCallAudioSpeaker:YES];
        self.btnExternal.selected = YES;
        [HUD showMessage:MultilingualTranslation(@"免提开启") inView:_viewHUDBg];
    }
}
        ...
        case ConnectionStateConnected:
        {
            ...
            if ([ZMediaCallManager sharedManager].mediaCallState != ZMediaCallStateCall) {
                ...
                [ZTOOL doInMain:^{
                    ...
                    [HUD showMessage:MultilingualTranslation(@"连接成功") inView:weakSelf.viewHUDBg];
                }];
            }
        }
        default:
...
- (void)clickFloatView:(ZWindowFloatView *)floatView {
    [ZMediaCallManager sharedManager].currentScreenTrackMe = ![ZMediaCallManager sharedManager].currentScreenTrackMe;
    [self setupVideoTrack];
...
#pragma mark - 懒加载
- (ZWindowFloatView *)viewFloat {
    if (!_viewFloat) {
        _viewFloat = [[ZWindowFloatView alloc] initWithFrame:CGRectMake(DScreenWidth - DWScale(128), DNavStatusBarH + DWScale(34), DWScale(112), DWScale(200))];
