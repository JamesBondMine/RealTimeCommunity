//
//  ZNetworkDetectionTopHeaderView.m
...
@interface ZNetworkDetectionTopHeaderView ()
...
@implementation ZNetworkDetectionTopHeaderView

// MARK: set/get
- (UIImageView *)iconImgView {
    if (!_iconImgView) {
        _iconImgView = [UIImageView new];
        _iconImgView.image = [UIImage imageNamed:@"icon_network_detection_header"];
    }
    return _iconImgView;
}
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.text = MultilingualTranslation(@"网络检测");
    }
    return _titleLabel;
}
...
- (UILabel *)stateLabel {
    if (!_stateLabel) {
        ...
        // 创建富文本
        [self updateStateLabelText:MultilingualTranslation(@"检测准备就绪，请点击开始检测")
        ...
        if ([x isKindOfClass:[NSDictionary class]]) {
            ...
            switch (status) {
                case ZNetworkDetectionAlready:
                    statusStr = MultilingualTranslation(@"检测准备就绪，请点击开始检测");
                    titleStr = MultilingualTranslation(@"网络检测");
                    topImageName = @"icon_network_detection_header";
                    break;
                case ZNetworkDetecting:
                    statusStr = [NSString stringWithFormat:MultilingualTranslation(@"已检测 %ld%%..."), process];
                    titleStr = MultilingualTranslation(@"网络检测");
                    topImageName = @"icon_network_detection_header";
                    break;
                case ZNetworkDetectFinish: {
                    ...
                    if (count == 0) {
                        statusStr = MultilingualTranslation(@"网络检测完成，当前网络状况良好");
                        titleStr = MultilingualTranslation(@"网络状态正常");
                        topImageName = @"icon_network_detection_header";
                    }else {
                        statusStr = [NSString stringWithFormat:MultilingualTranslation(@"发现 %ld 个异常"), count];
                        titleStr = MultilingualTranslation(@"网络状态异常");
