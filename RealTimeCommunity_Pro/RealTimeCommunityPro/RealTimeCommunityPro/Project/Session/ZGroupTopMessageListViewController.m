...
@property (nonatomic, strong) NSMutableArray<ZMessageModel *> *messageList;
...
- (void)viewDidLoad {
    ...
    self.navTitleStr = MultilingualTranslation(@"置顶消息");
        ...
        if ([dataArray isKindOfClass:[NSArray class]] && dataArray.count > 0) {
            ...
            for (NSDictionary *dict in dataArray) {
                ...
                if (chatMessageModel) {
                    ...
                    ZMessageModel *messageModel = [[ZMessageModel alloc] initWithMessageModel:chatMessageModel isSelf:isSelf];
                    ...
                    if (chatMessageModel.messageType == CIMChatMessageType_AtMessage) {
                        if ([NSString isNil:messageModel.message.showContent] && messageModel.message.atContent.length > 0) {
                            ...
                            if (isSelf) {
                                messageModel.message.showContent = [ZMessageTools atContenTranslateToShowContent:messageModel.message.atContent atUsersDictList:messageModel.message.atUsersInfoList withMessage:messageModel.message isGetShowName:YES];
                            } else {
                                ...
                                messageModel.message.showContent = [ZMessageTools atContenTranslateToShowContent:translateContent atUsersDictList:messageModel.message.atUsersInfoList withMessage:messageModel.message isGetShowName:YES];
                            }
                            // 重新计算 attStr
                            if (messageModel.message.showContent.length > 0) {
                                messageModel.attStr = [[EMChatInputEmojiManager sharedManager] attributedString:messageModel.message.showContent];
...
- (LingIMChatMessageModel *)createChatMessageModelFromDict:(NSDictionary *)dict {
...
- (void)computeVisibleTime {
    ...
    for (NSInteger i = 0; i < self.messageList.count; i++) {
        ZMessageModel *currentModel = [self.messageList objectAtIndex:i];
        ...
        if (i == 0) {
            ...
            BOOL sameDay = [ZMessageTimeTool isSameDay:currentModel.message.sendTime Time2:prevSendTime];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZMessageModel *model = [self.messageList objectAtIndex:indexPath.row];
    ...
    switch (model.message.messageType) {
        case CIMChatMessageType_TextMessage:    //文本消息
        {
            ...
            if (![NSString isNil:model.message.referenceMsgId]) {
                //引用类文本消息
                cell = [[ZMessageReferenceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
            } else {
                //纯文本消息
                cell = [[ZMessageTextCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
            }
        }
            break;
        case CIMChatMessageType_AtMessage:       //@消息
        {
            ...
            if (![NSString isNil:model.message.referenceMsgId]) {
                ...
                //纯@消息
                cell = [[ZMessageAtUserCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
            }
        }
            break;
        case CIMChatMessageType_ImageMessage:   //图片消息
        {
            cell = [[ZMessageImageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        }
            break;
        case CIMChatMessageType_VideoMessage:   //视频消息
        {
            cell = [[ZMessageVideoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        }
            break;
        case CIMChatMessageType_VoiceMessage:   //语音消息
        {
            cell = [[ZMessageVoiceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        }
            break;
        case CIMChatMessageType_FileMessage:     //文件消息
        {
            cell = [[ZMessageFileCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        }
            break;
        case CIMChatMessageType_GeoMessage:     //位置消息
        {
            cell = [[ZMessageGeoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        }
            break;
        case CIMChatMessageType_CardMessage:    //名片消息
        {
            cell = [[ZMessageCardCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        }
            break;
        case CIMChatMessageType_StickersMessage:   //表情消息
        {
            cell = [[ZMessageStickersCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        }
            break;
        case CIMChatMessageType_GameStickersMessage:   //游戏表情消息
        {
            cell = [[ZMessageGameStickersCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        }
            break;

        case CIMChatMessageType_ForwardMessage:   //转发消息
        {
            cell = [[ZMergeMessageRecordCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        }
            break;
        default:
            cell = [[ZMessageTextCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
...
- (void)locationButtonAction:(UIButton *)sender {
    ...
    if (smsgId.length > 0) {
        // 排除删除和撤回的消息
        LingIMChatMessageModel *targetMessage = [IMSDKManager toolGetOneChatMessageWithServiceMessageIDExcludeDeleted:smsgId sessionID:self.groupId];
        if (!targetMessage) {
...
- (void)messageCellLongTapWithIndex:(NSIndexPath *)indexPath {
    ...
    ZMessageModel *msgModel = [self.messageList objectAtIndex:indexPath.row];
