...
@implementation CNAddFriendCell
...
#pragma mark - 界面布局
- (void)setupUI {
    _ivHeader = [[BBBaseImageView alloc] initWithFrame:CGRectMake(DWScale(16), DWScale(16), DWScale(44), DWScale(44))];
    ...
    [_btnAdd setBackgroundColor:COLOR_4791FF];
...
- (void)btnAddClick {
    if (_userModel) {
        ...
        [IMSDKManager checkMyFriendWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
            ...
            if (isMyFriend) {
                [HUD showMessage:MultilingualTranslation(@"该用户已是你的好友")];
            }else {
...
- (void)setUserModel:(ZUserModel *)userModel {
    if (userModel) {
        ...
        if (friendModel) {
            //已是好友
            [_btnAdd setTitle:MultilingualTranslation(@"已添加") forState:UIControlStateNormal];
            [_btnAdd setTkThemebackgroundColors:@[COLOR_99,COLOR_99_DARK]];
            _btnAdd.userInteractionEnabled = NO;
        }else {
            //不是好友
            [_btnAdd setTitle:MultilingualTranslation(@"添加好友") forState:UIControlStateNormal];
            [_btnAdd setTkThemebackgroundColors:@[COLOR_4791FF,COLOR_4791FF]];
            [_btnAdd setTkThemeBackgroundImage:@[[UIImage ImageForColor:COLOR_4069B9],[UIImage ImageForColor:COLOR_4069B9_DARK]] forState:UIControlStateHighlighted];
