...
#import "LingIMSocketManager.h"//长连接工具单例
#import "LingIMMacorHeader.h"//宏定义
#import "LingIMManagerTool.h"//工具
#import "LingIMSDKManager+ChatMessage.h"
#import "LingIMLoganManager.h"//日志
#import "LingIMSDKManager+Logan.h"

// 短连接转长连接分类
#import "LingIMSocketManagerTool+LingImTcpReplaceHttp.h"
#import "LingIMSocketManagerTool+HandleReceiveTcpReplaceHttpMessage.h"
...
@interface LingIMSocketManagerTool ()
...
@implementation LingIMSocketManagerTool

#pragma mark - <<<<<<单例>>>>>>
+ (instancetype)sharedManager {
    
    static LingIMSocketManagerTool *_manager = nil;
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    return [LingIMSocketManagerTool sharedManager];
}
// 防止外部调用copy
...
/// 正在连接服务器
- (void)cimConnecting{
    if ([SOCKETMANAGERTOOL.connectDelegate respondsToSelector:@selector(cimConnecting)]) {
        [SOCKETMANAGERTOOL.connectDelegate cimConnecting];
    }
}
...
- (void)cimConnectSuccess{
    ...
    if ([SOCKETMANAGERTOOL.connectDelegate respondsToSelector:@selector(cimConnectSuccess)]) {
        [SOCKETMANAGERTOOL.connectDelegate cimConnectSuccess];
    }
}
...
- (void)cimReConnectSuccess{
    self.isReconnectSocket = NO;
    if ([SOCKETMANAGERTOOL.connectDelegate respondsToSelector:@selector(cimReConnectSuccess)]) {
        [SOCKETMANAGERTOOL.connectDelegate cimReConnectSuccess];
    }
}

- (void)cimGetSessionIdSuccess {
    // 将首次连接期间缓存的消息发送(http转tcp) 或 将重连期间缓存的消息发送(http转tcp)
...
- (void)cimConnectFailWithError:(NSError * _Nullable)error{
    self.isReconnectSocket = NO;
    if ([SOCKETMANAGERTOOL.connectDelegate respondsToSelector:@selector(cimConnectFailWithError:)]) {
        [SOCKETMANAGERTOOL.connectDelegate cimConnectFailWithError:error];
    }
...
/// 断开服务器连接(告知需要竞速)
- (void)cimDisconnect{
    if ([SOCKETMANAGERTOOL.connectDelegate respondsToSelector:@selector(cimDisconnect)]) {
        [SOCKETMANAGERTOOL.connectDelegate cimDisconnect];
    }
            ...
            if(reSendCount <= 3){
                ...
                [IMSDKManager MessagePushMsg:[sendMessage data] onSuccess:^(id  _Nullable data, NSString * _Nullable traceId) {
                    if ([data isKindOfClass:[NSDictionary class]]) {
                        ...
                        if (chatMessageAck && [weakSelf.messageDelegate respondsToSelector:@selector(cimMessageSendSuccess:)]) {
                            [weakSelf.messageDelegate cimMessageSendSuccess:chatMessageAck];
                        }
                        //移除 我 发送的消息 超时监听
                        //注意此处取值为ackMsgId
...
- (void)sendPingMessageForChatMessageSendFail {
    ...
    pingMessage.msgId = [[LingIMManagerTool sharedManager] getMessageID];
...
- (void)sendChatMessageToAckForReceiveChatMessage:(IMChatMessage *)chatMessage {
    ...
    if (![chatMessage.from isEqualToString:SOCKETMANAGER.socketUserID]) {
        ...
        chatMessageToAck.msgId = [[LingIMManagerTool sharedManager] getMessageID];
    ...
    switch (authMessageAck.code) {
        case LingIMUserAuthStatusSuccess://用户鉴权成功，开始心跳机制
        {
            ...
            if ([self.userDelegate respondsToSelector:@selector(cimUserConnectSuccess)]) {
                [self.userDelegate cimUserConnectSuccess];
            }
        }
            break;
...
//            }
        ...
        case LingIMUserAuthStatusUserDestroy://用户已经注销
        {
            ...
            if (_isBanded == NO) {
                //业务层退出登录
                if ([self.userDelegate respondsToSelector:@selector(cimUserConnectLogoutWithCode:messsage:)]) {
                    [self.userDelegate cimUserConnectLogoutWithCode:authMessageAck.code messsage:authMessageAck.message];
                }
                _isBanded = YES;
            }
        }
        ...
        case LingIMUserAuthStatusUsedIpDisabled://客户端当前IP不在白名单内
        {
            ...
            //业务层退出登录
            if ([self.userDelegate respondsToSelector:@selector(cimUserConnectLogoutWithCode:messsage:)]) {
                [self.userDelegate cimUserConnectLogoutWithCode:90018 messsage:authMessageAck.message];
            }
        }
            break;
        default:
...
- (void)receiveMessageDealForServerMessageWith:(IMMessage *)receiveMessage {
    ...
    switch (serverMessage.sMsgType) {
        ...
        {
            //其他类型的系统通知消息
            if ([_messageDelegate respondsToSelector:@selector(cimMessageSystemReceiveWith:)]) {
                [_messageDelegate cimMessageSystemReceiveWith:serverMessage];
            }
        }
            break;
    }
...
- (void)dealServerMessageOfUserForcedOffline:(IMServerMessage *)serverMessage {
    ...
    //执行用户 强制下线 代理回调
    if ([_userDelegate respondsToSelector:@selector(cimSdkUserForceLogout:message:)]) {
        [_userDelegate cimSdkUserForceLogout:forcedOffLine.type message:forcedOffLine.message];
    }
...
- (void)dealServerMessageOfFriend:(IMServerMessage *)serverMessage {
    ...
    if (serverMessage.sMsgType == IMServerMessage_ServerMsgType_FriendInviteMessage) {
        if ([_userDelegate respondsToSelector:@selector(cimUserFriendInvite:)]) {
            [_userDelegate cimUserFriendInvite:serverMessage.friendInviteMessage];
        }
    }
    
    //对方用户 同意了 我 发起的好友申请
    if (serverMessage.sMsgType == IMServerMessage_ServerMsgType_FriendConfirmMessage) {
        if ([_userDelegate respondsToSelector:@selector(cimUserFriendConfirm:)]) {
            [_userDelegate cimUserFriendConfirm:serverMessage];
        }
    }
    
    //删除好友，后台审核通过  该消息只转发给删除操作的用户
    if (serverMessage.sMsgType == IMServerMessage_ServerMsgType_FriendDelMessage) {
        if ([_userDelegate respondsToSelector:@selector(cimUserFriendDelete:)]) {
            [_userDelegate cimUserFriendDelete:serverMessage.friendDelMessage];
        }
    }
    
    //好友不存在
    if (serverMessage.sMsgType == IMServerMessage_ServerMsgType_NullFriendMessage) {
        if ([_userDelegate respondsToSelector:@selector(cimUserFriendNoneExist:)]) {
            [_userDelegate cimUserFriendNoneExist:serverMessage];
        }
    }
    
    //好友在线状态，(发送给所有在线好友)
    if (serverMessage.sMsgType == IMServerMessage_ServerMsgType_FriendLineStatus) {
        if ([_userDelegate respondsToSelector:@selector(cimUserFriendLineStatus:)]) {
            [_userDelegate cimUserFriendLineStatus:serverMessage];
        }
    }
    
    //好友黑名单 该消息转发给发送消息的用户
    if (serverMessage.sMsgType == IMServerMessage_ServerMsgType_BlackFriendMessage) {
        if ([_userDelegate respondsToSelector:@selector(cimUserFriendBlack:)]) {
            [_userDelegate cimUserFriendBlack:serverMessage];
        }
    }
    
    //好友已注销 该消息转发给发送消息的用户
    if (serverMessage.sMsgType == IMServerMessage_ServerMsgType_UserAccountClose) {
        if ([_userDelegate respondsToSelector:@selector(cimUserAccountClose:)]) {
            [_userDelegate cimUserAccountClose:serverMessage];
        }
    }
...
- (void)dealServerMessageOfGroup:(IMServerMessage *)serverMessage {
    ...
    //群相关提示代理回调
    if ([_groupDelegate respondsToSelector:@selector(cimGroupTipServerMessage:)]) {
        [_groupDelegate cimGroupTipServerMessage:serverMessage];
    }
...
- (void)dealServerMessageOfReadMessage:(IMServerMessage *)serverMessage {
    if ([_messageDelegate respondsToSelector:@selector(cimMessageHaveRead:)]) {
        [_messageDelegate cimMessageHaveRead:serverMessage];
    }
...
- (void)dealServerMessageOfUpdateMessageRead:(IMServerMessage *)serverMessage {
    if ([_messageDelegate respondsToSelector:@selector(cimMessageUpdateMessageRead:)]) {
        [_messageDelegate cimMessageUpdateMessageRead:serverMessage];
    }
}

//处理系统类型消息中的 自定义事件 消息
- (void)dealServerMessageOfCustomEvent:(IMServerMessage *)serverMessage {
    if ([_messageDelegate respondsToSelector:@selector(cimMessageSystemCustomEventWith:)]) {
        [_messageDelegate cimMessageSystemCustomEventWith:serverMessage];
    }
}

//处理系统类型消息中的 消息定时自动删除 消息
- (void)dealServerMessageOfMessageTimeDelete:(IMServerMessage *)serverMessage {
    if ([_messageDelegate respondsToSelector:@selector(cimMessageSystemMessageTimeDeleteWith:)]) {
        [_messageDelegate cimMessageSystemMessageTimeDeleteWith:serverMessage];
    }
}
...
- (void)dealServerMessageOfFriendGroup:(IMServerMessage *)serverMessage {
    if ([_userDelegate respondsToSelector:@selector(cimSdkFriendGroup:)]) {
        [_userDelegate cimSdkFriendGroup:serverMessage];
    }
}

//处理系统消息中的 好友分组 好友管理 相关消息
- (void)dealServerMessageOfFriendGroupForFriend:(IMServerMessage *)serverMessage {
    if ([_userDelegate respondsToSelector:@selector(cimSdkFriendGroupForFriend:)]) {
        [_userDelegate cimSdkFriendGroupForFriend:serverMessage];
    }
}
...
- (void)dealServerMessageOfUpdateSensitive:(IMServerMessage *)serverMessage {
    //通用的系统通知消息处理
    if ([_messageDelegate respondsToSelector:@selector(cimMessageSystemMessageUpdateSensitiveWith:)]) {
        [_messageDelegate cimMessageSystemMessageUpdateSensitiveWith:serverMessage];
    }
}
...
- (void)dealServerMessageOfSignInReminder:(IMServerMessage *)serverMessage {
    //通用的系统通知消息处理
    if ([_messageDelegate respondsToSelector:@selector(cimMessageSystemMessageSignInReminder:)]) {
        [_messageDelegate cimMessageSystemMessageSignInReminder:serverMessage];
    }
}
...
- (void)dealServerMessageOfUserRoleAuthority:(IMServerMessage *)serverMessage {
    //通用的系统通知消息处理
    if ([_messageDelegate respondsToSelector:@selector(cimMessageSystemMessageUserRoleAuthority:)]) {
        [_messageDelegate cimMessageSystemMessageUserRoleAuthority:serverMessage];
    }
}
...
- (void)dealServerMessageUserTranslateConfigChange:(IMServerMessage *)serverMessage {
    //通用的系统通知消息处理
    if ([_userDelegate respondsToSelector:@selector(cimSdkReceiveTranslateConfigUplate:)]) {
        [_userDelegate cimSdkReceiveTranslateConfigUplate:serverMessage];
    }
}

//收到同步类消息
- (void)dealServerMessage_SynchroMessage:(IMServerMessage *)serverMessage {
    if ([_messageDelegate respondsToSelector:@selector(cimMessageSystemMessageSynchroMessageWith:)]) {
        [_messageDelegate cimMessageSystemMessageSynchroMessageWith:serverMessage];
    }
}

//会话 标记已读 / 标记未读
- (void)dealServerMessageDialogReadTagChangeEventMessage:(IMServerMessage *)serverMessage {
    if ([_messageDelegate respondsToSelector:@selector(cimDialogReadTagChangeEventWith:)]) {
        [_messageDelegate cimDialogReadTagChangeEventWith:serverMessage];
    }
}
...
- (void)receiveMessageDealForChatMessageWith:(IMMessage *)receiveMessage {
    ...
    //执行接收到聊天消息的代理回调
    if ([_messageDelegate respondsToSelector:@selector(cimMessageChatReceiveWith:)]) {
        [_messageDelegate cimMessageChatReceiveWith:receiveMessage];
    }
...
- (void)receiveMessageDealForChatMessageAckWith:(IMMessage *)receiveMessage {
    ...
    if (chatMessageAck && [_messageDelegate respondsToSelector:@selector(cimMessageSendSuccess:)]) {
        [_messageDelegate cimMessageSendSuccess:chatMessageAck];
    }
...
- (void)socketConfigLoganWith:(NSString *)errorReason {
    ...
    //写入日志
    LingIMLoganManager *loganManager = [LingIMLoganManager sharedManager];
