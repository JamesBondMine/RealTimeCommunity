//
//  HomeChatHistoryMediaVC.m
//  CIMKit
//
//  Created by cusPro on 2022/11/11.
//

#import "HomeChatHistoryMediaVC.h"
...
@implementation HomeChatHistoryMediaVC
...
- (void)setupUI {
    ...
    [_collectionView registerClass:[ZChatHistoryMediaCell class] forCellWithReuseIdentifier:NSStringFromClass([ZChatHistoryMediaCell class])];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ZChatHistoryMediaCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([ZChatHistoryMediaCell class]) forIndexPath:indexPath];
...
- (NSAttributedString *)titleForEmptyDataSet:(UIScrollView *)scrollView {
    NSString *string = MultilingualTranslation(@"快去发送图片吧");
...
- (void)imageVideoBrowserWith:(NSIndexPath *)indexPath {
    ...
    [_historyList enumerateObjectsUsingBlock:^(LingIMChatMessageModel * _Nonnull chatMessage, NSUInteger idx, BOOL * _Nonnull stop) {
        ZChatHistoryMediaCell *indexCell = (ZChatHistoryMediaCell *)[weakSelf.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:idx inSection:0]];
...
- (void)photoBrowser:(KNPhotoBrowser *)photoBrowser rightBtnOperationActionWithIndex:(NSInteger)index {
    LingIMChatMessageModel *currentModel = [_historyList objectAtIndexSafe:index];
    NSString *imageUrl;
    NSString *videoUrl;
