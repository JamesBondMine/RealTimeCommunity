...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [ZCallManager sharedManager].delegate = self;
...
- (void)updateUIWithRoomState {
    ...
    if (currentCallOptions.zgCallOptions.callType == LingIMCallTypeAudio) {
        //音频通话
        switch ([ZCallManager sharedManager].callState) {
...
- (void)setUserModel:(ZCallUserModel *)userModel {
    if (userModel) {
...
- (void)updateViewVideoTrack {
    ...
    if ([ZCallManager sharedManager].currentCallOptions.zgCallOptions.callType == LingIMCallTypeVideo) {
        //视频通话
        ...
        if ([ZCallManager sharedManager].callState == ZCallStateCalling) {
            if ([_userModel.userUid isEqualToString:UserManager.userInfo.userUID]) {
                //主界面是我
                [[ZCallManager sharedManager] callRoomStartPreviewWith:_viewVideo.sampleViewVideo.viewVideoZG];
            }else {
                //主界面是远端(对方)
                [[ZCallManager sharedManager] callRoomStartPlayingStream:_userModel.streamID with:_viewVideo.sampleViewVideo.viewVideoZG];
            }
        }
