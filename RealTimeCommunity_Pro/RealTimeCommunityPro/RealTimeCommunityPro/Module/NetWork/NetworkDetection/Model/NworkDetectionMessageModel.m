...
@implementation ZNetworkDetectionSubResultModel

@end

@implementation NworkDetectionMessageModel

// MARK: set/get
- (NSMutableArray<ZNetworkDetectionSubResultModel *> *)subFunctionResultArr {
...
- (BOOL)isAllSubFunctionPass {
    ...
    if (self.sectionType == ZNetworkDetectionNetworkConnectSectionType) {
        // 网络连接：要求全部通过
        for (ZNetworkDetectionSubResultModel *subResult in self.subFunctionResultArr) {
            if (!subResult.isPass) {
                return NO;
            }
        }
