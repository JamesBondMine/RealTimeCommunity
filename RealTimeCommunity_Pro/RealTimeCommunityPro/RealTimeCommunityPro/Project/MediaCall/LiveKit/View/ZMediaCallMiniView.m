...
@implementation ZMediaCallMiniView
...
#pragma mark - 界面赋值
- (void)setMediaCallOptions:(ZMediaCallOptions *)mediaCallOptions {
    if (mediaCallOptions) {
...
- (void)getCallRequestUserInfoWith:(NSString *)userUid{
    if (_mediaCallOptions.callRoomType == ZIMCallRoomTypeSingle) {
        ...
        if (friendModel) {
            ...
            [self updateUI];
            [ZMediaCallManager sharedManager].userModel = _userModel;
        }else {
...
- (void)requestUserInfoWith:(NSString *)userUid {
    ...
    [IMSDKManager getUserInfoWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            [weakSelf updateUI];
            [ZMediaCallManager sharedManager].userModel = weakSelf.userModel;
        }
...
- (void)updateUI {
    ...
    if (self.mediaCallOptions.callRoomType == ZIMCallRoomTypeSingle) {
        tipStr = self.mediaCallOptions.callType == LingIMCallTypeAudio ? MultilingualTranslation(@"邀请你进行语音通话") : MultilingualTranslation(@"邀请你进行视频通话");
    }else if (self.mediaCallOptions.callRoomType == ZIMCallRoomTypeGroup) {
        tipStr = self.mediaCallOptions.callType == LingIMCallTypeAudio ? MultilingualTranslation(@"邀请你进行群语音通话") : MultilingualTranslation(@"邀请你进行群视频通话");
    }
...
- (void)callAccept {
    ...
    [[ZMediaCallManager sharedManager] mediaCallAcceptWith:self.mediaCallOptions onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
...
- (void)callGroupAccept {
    WeakSelf
    [[ZMediaCallManager sharedManager] mediaCallGroupAcceptWith:self.mediaCallOptions onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        DLog(@"同意多人音视频通话");
        if ([data isKindOfClass:[NSDictionary class]]) {
