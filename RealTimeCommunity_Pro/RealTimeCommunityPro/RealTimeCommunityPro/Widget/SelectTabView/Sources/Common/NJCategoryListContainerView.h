...
@class NJCategoryListContainerView;
...
@protocol NJCategoryListContainerViewDelegate <NSObject>
/**
 返回list的数量
 ...
 @return list的数量
 */
- (NSInteger)numberOfListsInlistContainerView:(NJCategoryListContainerView *)listContainerView;
 ...
 */
- (id<JXCategoryListContentViewDelegate>)listContainerView:(NJCategoryListContainerView *)listContainerView initListForIndex:(NSInteger)index;
...
/**
 ...
 @param listContainerView NJCategoryListContainerView
 @return 自定义UIScrollView实例
 */
- (Class)scrollViewClassInlistContainerView:(NJCategoryListContainerView *)listContainerView;

/**
 控制能否初始化对应index的列表。有些业务需求，需要在某些情况才允许初始化某些列表，通过通过该代理实现控制。
 */
- (BOOL)listContainerView:(NJCategoryListContainerView *)listContainerView canInitListAtIndex:(NSInteger)index;
...
@interface NJCategoryListContainerView : UIView <JXCategoryViewListContainer>
...
- (instancetype)initWithType:(JXCategoryListContainerType)type delegate:(id<NJCategoryListContainerViewDelegate>)delegate NS_DESIGNATED_INITIALIZER;
