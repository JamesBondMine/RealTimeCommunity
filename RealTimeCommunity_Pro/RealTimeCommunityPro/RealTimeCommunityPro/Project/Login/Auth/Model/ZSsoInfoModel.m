...
#import "ZSsoInfoModel.h"

@implementation ZSsoInfoModel
...
+ (BOOL)isConfigSSO {
    if ([self conformsToProtocol:@protocol(NSCoding)]) {
        ...
        if (data) {
            ZSsoInfoModel *obj = [NSKeyedUnarchiver unarchiveObjectWithData:data];
...
+ (ZSsoInfoModel *)getSSOInfoWithLiceseId:(NSString *)liceseId {
    if ([self conformsToProtocol:@protocol(NSCoding)]) {
        NSString *key = [NSString stringWithFormat:@"%@%@",OSS_LOCAL_CACHE,liceseId];
