...
#import "HomeMassMessageVC.h"
#import <DZNEmptyDataSet/UIScrollView+EmptyDataSet.h>
#import "NSMassMessageTextCell.h"
#import "NSMassMessageAttachmentCell.h"

#import "HomeNewMassMessageVC.h"//新建群发
#import "HomeMassMessageUserVC.h"//查看接收消息用户列表
#import "HomeMassMessageFileDetailVC.h"//文件详情
#import "KNPhotoBrowser.h"//图片视频浏览
#import "ZToolManager.h"//工具类

@interface HomeMassMessageVC () <UITableViewDataSource, UITableViewDelegate, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate, MGSwipeTableCellDelegate, NSMassMessageBaseCellDelegate, KNPhotoBrowserDelegate>
@property (nonatomic, assign) NSInteger pageNumber;
@property (nonatomic, strong) NSMutableArray *messageList;
@property (nonatomic, strong) LIMMassMessageModel *currentDetailMessageModel;//当前查看详情的消息model
@end

@implementation HomeMassMessageVC
...
- (void)setupNavUI {
    self.navTitleStr = MultilingualTranslation(@"群发助手");
...
- (void)setupUI {
    ...
    [self.baseTableView registerClass:[NSMassMessageTextCell class] forCellReuseIdentifier:NSStringFromClass([NSMassMessageTextCell class])];
    [self.baseTableView registerClass:[NSMassMessageAttachmentCell class] forCellReuseIdentifier:NSStringFromClass([NSMassMessageAttachmentCell class])];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (messageModel.mtype == 0) {
        //文本
        NSMassMessageTextCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([NSMassMessageTextCell class]) forIndexPath:indexPath];
        ...
        //1图片2视频5文件
        NSMassMessageAttachmentCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([NSMassMessageAttachmentCell class]) forIndexPath:indexPath];
        cell.messageModel = messageModel;
        cell.delegate = self;
        cell.massMessageDelegate = self;
        return cell;
...
- (NSArray<UIView *> *)swipeTableCell:(MGSwipeTableCell *)cell swipeButtonsForDirection:(MGSwipeDirection)direction swipeSettings:(MGSwipeSettings *)swipeSettings expansionSettings:(MGSwipeExpansionSettings *)expansionSettings {
    ...
    if (direction == MGSwipeDirectionLeftToRight) {
        ...
        MGSwipeButton *btnDelete = [MGSwipeButton buttonWithTitle:@"" icon:ImgNamed(@"com_icon_collection_delete") backgroundColor:COLOR_F5F6F9 callback:^BOOL(MGSwipeTableCell * _Nonnull cell) {
            //执行删除群发消息逻辑
...
#pragma mark - NSMassMessageBaseCellDelegate
//查看全部成员列表
- (void)cellCheckAllReceiverWith:(LIMMassMessageModel *)messageModel {
    HomeMassMessageUserVC *vc = [HomeMassMessageUserVC new];
    vc.allUsers = YES;
...
- (void)cellCheckDetailWith:(LIMMassMessageModel *)messageModel {
    if (messageModel.mtype == 5) {
        //文件
        HomeMassMessageFileDetailVC *vc = [[HomeMassMessageFileDetailVC alloc] init];
        vc.messageModel = messageModel;
        [self.navigationController pushViewController:vc animated:YES];
...
- (void)cellSendAgainWith:(LIMMassMessageModel *)messageModel {
    HomeNewMassMessageVC *vc = [HomeNewMassMessageVC new];
    vc.messageModel = messageModel;
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)navBtnRightClicked {
    HomeNewMassMessageVC *vc = [[HomeNewMassMessageVC alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
