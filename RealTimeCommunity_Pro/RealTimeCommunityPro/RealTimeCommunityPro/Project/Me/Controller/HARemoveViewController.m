...
@interface HARemoveViewController ()
...
@implementation HARemoveViewController
...
- (void)viewDidLoad {
    ...
    self.navTitleStr = MultilingualTranslation(@"验证密码");
    [self setupUI];
...
- (void)setupUI {
    ...
    tipTitleLbl.text = MultilingualTranslation(@"请输入密码:");
    tipTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (void)listenTextFieldEndInputAction {
    [ZAuthInputTools checkPasswordWithText:self.passwordInput.inputText.text];
}
...
- (void)getEncryptKeyAction {
    if ([ZAuthInputTools checkPasswordWithText:self.passwordInput.inputText.text] == NO) {
        return;
    }
...
- (void)LoginActionWithEncryptKey:(NSString *)encryptKey vercode:(NSString *)vercode {
    ...
    [IMSDKManager authDeleteAccountWith:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if(code == LingIMHttpResponseCodeExamineStatus){
            [HUD showMessage:MultilingualTranslation(@"提交成功，系统稍后处理") inView:self.view];
        }else if (code == LingIMHttpResponseCodeNoneExamineStatus){
            [HUD showMessage:MultilingualTranslation(@"您已经提交过申请，请耐心等待审核") inView:self.view];
        }else{
            [HUD showMessageWithCode:code errorMsg:msg inView:self.view];
        }
    }];
...
- (UIButton *)deleteButton {
    if (!_deleteButton) {
        ...
        [_deleteButton setTitle:MultilingualTranslation(@"注销账号") forState:UIControlStateNormal];
