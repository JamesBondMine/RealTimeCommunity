...
@implementation ZChatSetGroupInfoCell
...
- (void)setupUI {
    ...
    // 商务风格：更小的圆角
    viewBg.layer.cornerRadius = DWScale(6);
    viewBg.layer.masksToBounds = NO; // 允许显示阴影
    // 商务风格：添加卡片阴影
    viewBg.layer.shadowColor = [UIColor blackColor].CGColor;
    viewBg.layer.shadowOffset = CGSizeMake(0, 2);
    ...
    _ivGroup = [[BBBaseImageView alloc] init];
    // 商务风格：更小的圆角
    ...
    _ivArrowTop = [[UIImageView alloc] initWithImage:ImgNamed(@"com_c_arrow_right_gray")];
    ...
    */
    ...
    lblMemberTip.text = MultilingualTranslation(@"群成员");
    lblMemberTip.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    UIImageView *ivArrowBottom = [[UIImageView alloc] initWithImage:ImgNamed(@"com_c_arrow_right_gray")];
...
- (void)setGroupModel:(LingIMGroup *)groupModel {
    if (groupModel) {
        ...
        _lblMemberCount.text = [NSString stringWithFormat:MultilingualTranslation(@"%ld人"),groupModel.memberCount];
