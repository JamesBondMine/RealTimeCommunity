...
- (void)dealloc {
    // 移除计时器
    [Logger info:@"[短连接转长连接] 销毁LingIMTcpRequestModel"];
    if (_gcdTimer) {
        [self cancelTimer];
    }
...
- (void)receiveMessage:(NSNotification *)notification {
    ...
    if ([message.responseMessage.requestId isEqualToString:self.msgId]) {
        ...
        [Logger info:[NSString stringWithFormat:@"[短连接转长连接] ✅ 收到匹配响应，消息ID:%@, 请求url:%@, status:%d，响应内容:%@",
...
- (void)beginTimer {
    ...
    dispatch_source_set_event_handler(self.gcdTimer, ^{
        ...
        if (strongSelf.retryCount < maxRetryCount) {
            ...
            [Logger info:[NSString stringWithFormat:@"[短连接转长连接] 消息超时了,重发次数+1,当前重发次数为%ld", strongSelf.retryCount]];
            ...
            [Logger error:[NSString stringWithFormat:@"[短连接转长连接] 消息超时了,重发次数达到上限，取消计时器，超时url = %@, 超时消息id = %@", self.url, self.msgId]];
            // 取消计时器
...
- (void)sendMessageWithParam:(id)param
    ...
    if (!self.msgId || self.msgId.length == 0) {
        NSString *msgId = [[LingIMManagerTool sharedManager] getMessageID];
        self.msgId = msgId;
    }
    ...
    [Logger info:[NSString stringWithFormat:@"[短连接转长连接] 📤 发送请求 - url:%@, requestId:%@, path:%@, method:%@, param:%@, body:%@, headers:%@", url, self.msgId, path, method, param, body, headerDic]];
...
- (void)sendSocketMessage:(IMMessage *)message {
    ...
    if (!self.isReachable) {
        [Logger error:[NSString stringWithFormat:@"[短连接转长连接] 无网，不发送了，消息id = %@", self.msgId]];
...
- (NSMutableDictionary<NSString *, NSString *> *)configHttpHeaderWithFullUrl:(NSString *)fullUrl
    ...
    if ([IMSDKManager myUserToken].length > 0) {
        [Logger info:[NSString stringWithFormat:@"[短连接转长连接] myUserToken获取到了，消息id = %@, url = %@", self.msgId, self.url]];
        [headerDic setObject:[IMSDKManager myUserToken] forKey:@"token"];
    }else {
        [Logger error:[NSString stringWithFormat:@"[短连接转长连接] myUserToken没有获取到，消息id = %@, url = %@", self.msgId, self.url]];
    }
    //loginuseruid
    if ([IMSDKManager myUserID].length > 0) {
        [Logger info:[NSString stringWithFormat:@"[短连接转长连接] myUserID获取到了，消息id = %@, url = %@", self.msgId, self.url]];
        [headerDic setObject:[IMSDKManager myUserID] forKey:@"loginuseruid"];
    }else {
        [Logger error:[NSString stringWithFormat:@"[短连接转长连接] myUserID没有获取到，消息id = %@, url = %@", self.msgId, self.url]];
    }
    //liceseId
    if ([IMSDKManager currentLiceseId].length > 0) {
        [Logger info:[NSString stringWithFormat:@"[短连接转长连接] currentLiceseId获取到了，消息id = %@, url = %@", self.msgId, self.url]];
        [headerDic setObject:[IMSDKManager currentLiceseId] forKey:@"conid"];
    }else {
        [Logger error:[NSString stringWithFormat:@"[短连接转长连接] currentLiceseId没有获取到，消息id = %@, url = %@", self.msgId, self.url]];
    }
    /** 接口验签 */
    //timestamp
