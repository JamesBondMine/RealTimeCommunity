...
#import "NFUserHomePageVC.h" //用户个人资料页
#import "ZAuthPCloginViewController.h"  //授权登录
#import "ZMessageAlertView.h"   //跳转到safari提醒弹窗
#import "ZQrcodeTextContentViewController.h"    //纯文本展示
#import "ZApplyJoinGroupViewController.h"   //扫码进群申请页
#import "NHChatViewController.h"
#import "ZJoinGroupModel.h" //邀请进群信息model
#import "ZMessageAlertView.h"
#import "LXChatEncrypt.h"
#import "AesEncryptUtils.h"
@interface ZQRcodeScanViewController () <AVCaptureMetadataOutputObjectsDelegate, UINavigationControllerDelegate, ZImagePickerVCDelegate>
...
@implementation ZQRcodeScanViewController
...
- (void)checkAVAuthorizationStatus
{
    ...
    [ZTOOL getCameraAuth:^(BOOL granted) {
        ...
        if (granted) {
            ...
            [HUD showMessage:MultilingualTranslation(@"您没有权限访问相机")];
        }
    }];
}
...
- (void)setupUI {
    ...
    label1.text = MultilingualTranslation(@"轻触开灯");
    ...
    label2.text = MultilingualTranslation(@"从相册选择");
...
- (void)imagePickerClipImage:(UIImage *)resultImg localIdenti:(NSString *)localIdenti {
    ...
    if (features.count > 0) {
        ...
        [HUD showMessage:MultilingualTranslation(@"没有识别到二维码")];
    }
}
    ...
    if (contentDic && [[contentDic allKeys] containsObject:@"type"]) {
        ...
        if (type == 5 ) {
            ...
            if (!body) {
                ...
                msgAlertView.lblContent.text = MultilingualTranslation(@"当前二维码无效，确认是否有误");
                ...
                msgAlertView.lblTitle.text = MultilingualTranslation(@"加入服务器");
                msgAlertView.lblContent.text = MultilingualTranslation(@"您正在加入服务器，是否确认？");
                msgAlertView.lblContent.textAlignment = NSTextAlignmentLeft;
...
- (BOOL)checkSacnResultForLicense:(NSString *)content {
    ...
    if (contentDic && [[contentDic allKeys] containsObject:@"type"]) {
        ...
        if (type == 4 && [appId isEqualToString:@"zim"]) {
            ...
            ZMessageAlertView *msgAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeTitle supView:nil];
            msgAlertView.lblTitle.text = MultilingualTranslation(@"加入服务器");
            msgAlertView.lblContent.text = [NSString stringWithFormat: MultilingualTranslation(@"您正在加入“%@”服务器，是否确认？"), liceseId];
            msgAlertView.lblContent.textAlignment = NSTextAlignmentLeft;
            [msgAlertView.btnSure setTitle:MultilingualTranslation(@"是") forState:UIControlStateNormal];
            [msgAlertView.btnSure setTkThemeTitleColor:@[COLORWHITE, COLORWHITE] forState:UIControlStateNormal];
            msgAlertView.btnSure.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF];
...
- (BOOL)checkSacnResultForIpDomainConnect:(NSString *)content {
    ...
    if (contentDic && [[contentDic allKeys] containsObject:@"type"]) {
        ...
        if (type == 1 && [appId isEqualToString:@"zim"]) {
            ...
            ZMessageAlertView *msgAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeTitle supView:nil];
            msgAlertView.lblTitle.text = MultilingualTranslation(@"加入服务器");
            msgAlertView.lblContent.text = [NSString stringWithFormat: MultilingualTranslation(@"您正在加入“%@”服务器，是否确认？"), ipDomainPort];
            msgAlertView.lblContent.textAlignment = NSTextAlignmentLeft;
            [msgAlertView.btnSure setTitle:MultilingualTranslation(@"是") forState:UIControlStateNormal];
            [msgAlertView.btnSure setTkThemeTitleColor:@[COLORWHITE, COLORWHITE] forState:UIControlStateNormal];
            msgAlertView.btnSure.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF];
                ...
                if (dataDic) {
                    ...
                    if (groupStatus == 1) {
                        ...
                        if (isOnGroup) {
                            ...
                            //不在当前群
                            ZJoinGroupModel *applyModel = [ZJoinGroupModel mj_objectWithKeyValues:dataDic];
                            //邀请进群页
                            ZApplyJoinGroupViewController *applyJoinGroupVC = [[ZApplyJoinGroupViewController alloc] init];
        ...
        case ZQrCodeResultPCAuth:
            {
                ...
                if (dataDic) {
                    ...
                    //授权PC登录页
                    ZAuthPCloginViewController *pcAuthVC = [[ZAuthPCloginViewController alloc] init];
        ...
        case ZQrCodeResultUrl:
            {
                ...
                if (![NSString isNil:urlString]) {
                    ...
                    //msgAlertView.lblTitle.text = MultilingualTranslation(@"跳转到Safari浏览器");
                    msgAlertView.lblContent.text = MultilingualTranslation(@"扫码结果为网址，跳转到Safrai并打开");
                    [msgAlertView.btnSure setTitle:MultilingualTranslation(@"是") forState:UIControlStateNormal];
                    [msgAlertView.btnCancel setTitle:MultilingualTranslation(@"否") forState:UIControlStateNormal];
                    [msgAlertView alertShow];
                    msgAlertView.sureBtnBlock = ^(BOOL isCheckBox) {
        ...
        case ZQrCodeResultTxt:
            {
                ...
                if (![NSString isNil:textContent]) {
                    ZQrcodeTextContentViewController *textContentVC = [[ZQrcodeTextContentViewController alloc] init];
