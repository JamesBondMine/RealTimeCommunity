...
//            AVLinearPCMIsFloatKey: @NO
        ...
        if (recorderError) {
            [HUD showMessage:MultilingualTranslation(@"录音组件初始化失败")];
...
- (void)voiceCafToMp3{
    ...
    if([[NSFileManager defaultManager] fileExistsAtPath:srcPath]){
        ...
        /// 将pcm文件转换为mp3文件
        [ZLameTool conventToMp3WithCafFilePath:srcPath mp3FilePath:desPathStr callback:^(BOOL result) {
            StrongSelf
            if (srcPath) {
                // 删除录音文件(pcm临时文件)
            ...
            if (result) {
                ...
                [HUD showMessage:MultilingualTranslation(@"录音出错了，请稍后再试")];
            }
        }];
    } else {
        // 录音文件保存本地沙盒失败
