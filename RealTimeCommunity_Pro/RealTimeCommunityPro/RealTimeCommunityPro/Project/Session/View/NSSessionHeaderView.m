...
@interface NSSessionHeaderView () <UICollectionViewDataSource,UICollectionViewDelegate,BBBaseCollectionCellDelegate>
@property (nonatomic, strong) UICollectionView *collection;
@end

@implementation NSSessionHeaderView
...
- (void)setupUI {
    ...
    [_collection registerClass:[ZSessionHeaderItem class] forCellWithReuseIdentifier:NSStringFromClass([ZSessionHeaderItem class])];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ZSessionHeaderItem *cell = [_collection dequeueReusableCellWithReuseIdentifier:NSStringFromClass([ZSessionHeaderItem class]) forIndexPath:indexPath];
...
- (void)baseCellDidSelectedRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 通过 block 回调将事件传递到父类处理
    if (self.didSelectSessionBlock) {
        self.didSelectSessionBlock(sessionModel);
    }
}
...
@implementation ZSessionHeaderItem
...
- (void)setSessionModel:(LingIMSessionModel *)sessionModel {
    if (sessionModel) {
        ...
        if ([_sessionModel.sessionID isEqualToString:@"100002"]) {
            ...
            _lblName.text = MultilingualTranslation(@"文件助手");
        } else {
            if (sessionModel.sessionType == CIMSessionTypeSingle) {
                //单聊
