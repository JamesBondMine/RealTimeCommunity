...
//  CIMKit
//
//  Created by cusPro on 2023/3/25.
//
...
- (void)setMenuTypeArr:(NSArray *)menuTypeArr {
    ...
    if (_menuTypeArr.count > 0) {
        ...
        if (self.inputViewArr.count > 0) {
            for (MainInputTextView *oldInput in self.inputViewArr) {
...
//                    menuBtn.frame = CGRectMake(DWScale(160) + DWScale(80) * (i - 1) , 0, DWScale(80), DWScale(28));
            ...
            MainInputTextView *accountInput = [[MainInputTextView alloc] init];
            if (authType == UserAuthTypePhone) {
                accountInput.placeholderText = MultilingualTranslation(@"请输入手机号");
                accountInput.inputType = ZMessageInputViewTypePhone;
                accountInput.tipsImgName = @"img_phone_input_tip_reb";
            } else if (authType == UserAuthTypeEmail) {
                accountInput.placeholderText = MultilingualTranslation(@"请输入邮箱");
                accountInput.inputType = ZMessageInputViewTypeNomal;
                accountInput.tipsImgName = @"img_email_input_tip_reb";
            } else if (authType == UserAuthTypeAccount) {
                accountInput.placeholderText = MultilingualTranslation(@"请输入账号");
                accountInput.inputType = ZMessageInputViewTypeNomal;
                accountInput.tipsImgName = @"img_account_input_tip_reb";
            }
...
- (void)mencClickAction:(id)sender {
    ...
    for (int i = 0; i<self.menuBtnArr.count; i++) {
        ...
        MainInputTextView *accountInput = (MainInputTextView *)[self.inputViewArr objectAtIndex:i];
