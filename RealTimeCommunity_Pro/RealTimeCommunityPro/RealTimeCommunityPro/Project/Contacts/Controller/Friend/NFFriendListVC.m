...
@interface NFFriendListVC () <UITableViewDataSource, UITableViewDelegate, CIMToolUserDelegate, ZBaseCellDelegate, SCTableViewSectionIndexDelegate, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate, UIGestureRecognizerDelegate>
...
/// 账号注销的好友列表
@property (nonatomic, strong) NSMutableArray <CMFriendModel *>*friendSignOutList;

/// 排序后的出现过的拼音首字母数组
@property (nonatomic, strong) NSMutableArray *firstLetterArr;

/// 排序好的结果字典
@property (nonatomic, strong) NSMutableDictionary <NSString *, NSMutableArray<CMFriendModel *> *> *sortedModelDict;

@end

@implementation NFFriendListVC
...
- (NSAttributedString *)titleForEmptyDataSet:(UIScrollView *)scrollView {
    NSString *string = MultilingualTranslation(@"暂无好友");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.firstLetterArr.count > indexPath.section) {
        ...
        if ([self.sortedModelDict objectForKey:tempLetter]) {
            NSArray *sectionArr = [self.sortedModelDict objectForKey:tempLetter];
            CMFriendModel *tempModel = [sectionArr objectAtIndexSafe:indexPath.row];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    static NSString *headerID = @"NVFriendListSectionHeaderView";
    NVFriendListSectionHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:headerID];
    if (headerView == nil) {
        headerView = [[NVFriendListSectionHeaderView alloc] initWithReuseIdentifier:headerID];
    }
...
- (void)cellClickAction:(NSIndexPath *)indexPath {
    if (self.firstLetterArr.count > indexPath.section) {
        ...
        if ([self.sortedModelDict objectForKey:tempLetter]) {
            NSArray *sectionArr = [self.sortedModelDict objectForKey:tempLetter];
            CMFriendModel *friendModel = [sectionArr objectAtIndexSafe:indexPath.row];
...
- (void)cimToolUserFriendChange:(LingIMFriendModel *)message {
    ...
    WeakSelf
    dispatch_async(_friendListQueue, ^{
        [weakSelf.friendList.safeArray enumerateObjectsUsingBlock:^(CMFriendModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
//                for (CMFriendModel *model in weakSelf.friendList.safeArray) {
//                    //全转为大写，并取第一个字符
...
//                    CMFriendModel *model = [tempFriendList objectAtIndexSafe:i];
//                    //全转为大写，并取第一个字符
//                    NSString *upperCaseStr = [model.sortName uppercaseString];
//                    NSString *firstLetter = @"";
//                    if (upperCaseStr.length > 0) {
...
- (void)requestFriendListFromDB:(BOOL)fromDB {
    ...
    dispatch_async(self.friendListQueue, ^{
        ...
        NSMutableArray<CMFriendModel *> *tempFriendArray = [NSMutableArray new];
        ...
        // 分组、排序
        NSDictionary<NSString *, NSArray<CMFriendModel *> *> *sortDict = [strongSelf sortFriends:strongSelf.friendList.safeArray safeSignOutList:strongSelf.friendSignOutList];
...
- (NSString *)processedSortNameFromModel:(CMFriendModel *)model {
    // 处理字符串
...
- (NSDictionary<NSString *, NSArray<CMFriendModel *> *> *)sortFriends:(NSMutableArray *)friendList safeSignOutList:(NSArray<CMFriendModel *> *)signOutList {
    // 拼音首字母初始化
    NSMutableArray *firstLetterArr = [NSMutableArray new];
    
    // 排序字典
    NSMutableDictionary<NSString *, NSMutableArray<CMFriendModel *> *> *sortDict = [NSMutableDictionary new];
    
    // 排序数据处理
    NSMutableArray<CMFriendModel *> *specialArr = [NSMutableArray array];
    
    for (CMFriendModel *model in friendList) {
        if (!model) {
            continue;
        }
        // 全转为大写，并取第一个字符
        ...
        if (isLetter) {
            if (!sortDict[firstLetter]) {
                NSMutableArray <CMFriendModel *>*friendModelArr = [NSMutableArray new];
                [friendModelArr addObject:model];
                [sortDict setValue:friendModelArr forKey:firstLetter];
            }else {
                NSMutableArray <CMFriendModel *>*friendModelArr = sortDict[firstLetter];
                [friendModelArr addObject:model];
                [sortDict setValue:friendModelArr forKey:firstLetter];
            }
        } else {
    ...
    for (NSString *firstLetterStr in firstLetterArr) {
        NSMutableArray <CMFriendModel *>*friendModelArr = sortDict[firstLetterStr];
...
/// - Parameter friendModelArr: CMFriendModel数组
- (NSArray *)customCompareArr:(NSMutableArray <CMFriendModel *> *)friendModelArr {
    NSSortDescriptor *sortDescriptor = [NSSortDescriptor sortDescriptorWithKey:@"sortName" ascending:YES comparator:^NSComparisonResult(id obj1, id obj2) {
        return customCompare(obj1, obj2, 0);
    }];
