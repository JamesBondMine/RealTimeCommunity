...
#import "NSMassMessageBaseCell.h"

@implementation NSMassMessageBaseCell
...
- (void)setupBaseUI {
    ...
    _lblReceiver.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _lblSendEnd.text = MultilingualTranslation(@"发送完毕");
...
- (void)setMessageModel:(LIMMassMessageModel *)messageModel {
    ...
    _lblNumber.text = [NSString stringWithFormat:MultilingualTranslation(@"%ld位收件人"), messageModel.totalCount];
    ...
    if (messageModel.status == 2) {
        //发送完成
        _lblSending.hidden = YES;
        _lblSendFail.text = [NSString stringWithFormat:MultilingualTranslation(@"%ld人发送失败"), messageModel.errorCount];
        ...
        _lblSending.text = messageModel.status == 1 ? MultilingualTranslation(@"发送中...") : MultilingualTranslation(@"发送失败");
        _lblSendEnd.hidden = YES;
        _lblSendFail.hidden = YES;
        _btnFail.hidden = YES;
    }
