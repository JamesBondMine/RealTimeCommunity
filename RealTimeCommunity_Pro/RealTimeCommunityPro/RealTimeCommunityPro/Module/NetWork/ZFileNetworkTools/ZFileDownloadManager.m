...
- (void)fileDownloadTaskDidPause:(ZFileDownloadTask *)task {
    
}

//继续下载
- (void)fileDownloadTaskDidResume:(ZFileDownloadTask *)task {
    //将任务 从新添加到队列中
    //创建一个新的 task 对象 ，否则addDownloadTask会报错，
    //这里可以决定是放在 队列末尾 还是放在队列最前面。
    ZFileDownloadTask * newTask = [[ZFileDownloadTask alloc] initWithTaskId:task.taskId sourceURL:task.sourceURL downloadPath:task.downloadPath delegate:nil];
...
//重新下载
- (void)fileDownloadTaskDidRedownload:(ZFileDownloadTask *)task {
    ZFileDownloadTask * newTask = [[ZFileDownloadTask alloc] initWithTaskId:task.taskId sourceURL:task.sourceURL downloadPath:task.downloadPath delegate:nil];
    newTask.delegates = task.delegates;
    [self addDownloadTask:newTask];
    [self.taskCache removeObject:task];
