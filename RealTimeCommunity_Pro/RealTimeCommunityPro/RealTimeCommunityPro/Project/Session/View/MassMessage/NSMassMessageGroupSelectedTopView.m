//
//  NSMassMessageGroupSelectedTopView.m
//  CIMKit
//
...
@interface NSMassMessageGroupSelectedTopView () <UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation NSMassMessageGroupSelectedTopView
...
- (void)setupUI {
    ...
    _titleLbl.text = MultilingualTranslation(@"已选择");
    ...
    _collection.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    _collection.showsHorizontalScrollIndicator = NO;
    [_collection registerClass:[ZMassMessageGroupSelectItem class] forCellWithReuseIdentifier:NSStringFromClass([ZMassMessageGroupSelectItem class])];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ZMassMessageGroupSelectItem *cell = [_collection dequeueReusableCellWithReuseIdentifier:NSStringFromClass([ZMassMessageGroupSelectItem class]) forIndexPath:indexPath];
    ZBaseUserModel *model = [_selectedTopUserList objectAtIndexSafe:indexPath.row];
    cell.model = model;
...
@implementation ZMassMessageGroupSelectItem
...
- (void)setupUI {
    ...
    [_deleteBtn setImage:ImgNamed(@"com_c_delete_blue_icon") forState:UIControlStateNormal];
...
#pragma mark - 数据赋值
- (void)setModel:(ZBaseUserModel *)model {
    if (model) {
        _model = model;
