...
//  CIMKit
//
//  Created by cusPro on 2023/3/30.
//

#import "UpgradeVersionView.h"
#import "ZToolManager.h"

@interface UpgradeVersionView()
...
- (void)setIsCompelUpdate:(BOOL)isCompelUpdate {
    ...
    if (_isCompelUpdate) {
        ...
        [_laterBtn setTitle:MultilingualTranslation(@"退出软件") forState:UIControlStateNormal];
    } else {
        //非强制跟新
        [self.forceUpdateTipsLbl mas_updateConstraints:^(MASConstraintMaker *make) {
            make.height.mas_equalTo(DWScale(0));
        }];
        [_laterBtn setTitle:MultilingualTranslation(@"暂不更新") forState:UIControlStateNormal];
    }
}

- (void)setVersionNumStr:(NSString *)versionNumStr {
    _versionNumStr = versionNumStr;
    self.tipSubTitleLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"升级到最新版本v %@"), versionNumStr];
}
...
- (UIImageView *)backImgView {
    if (!_backImgView) {
        ...
        _backImgView.tkThemeimages = @[ImgNamed(@"com_update_version_bg"), ImgNamed(@"com_update_version_bg_dark")];
...
- (UILabel *)tipTitleLbl {
    if (!_tipTitleLbl) {
        ...
        //富文本
        NSString *tipsTitleStr = MultilingualTranslation(@"发现新版本");
...
- (UILabel *)forceUpdateTipsLbl {
    if (!_forceUpdateTipsLbl) {
        ...
        _forceUpdateTipsLbl.text = MultilingualTranslation(@"如果不更新将无法使用客户端！");
...
- (UIButton *)updateBtn {
    if (!_updateBtn) {
        ...
        [_updateBtn setTitle:MultilingualTranslation(@"立即更新") forState:UIControlStateNormal];
        ...
        _updateBtn.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF_DARK];
