...
#pragma mark - 界面布局
- (void)setupNavUI {
    self.navTitleStr = MultilingualTranslation(@"选择成员");
    self.navBtnRight.hidden = NO;
    [self.navBtnRight setTitle:[NSString stringWithFormat:@"%@(%ld)",MultilingualTranslation(@"完成"), _currentRoomUser.count] forState:UIControlStateNormal];
...
- (void)cellClickAction:(NSIndexPath *)indexPath {
    ...
    if (![model.userUid isEqualToString:UserManager.userInfo.userUID]) {
        ...
        [self.navBtnRight setTitle:[NSString stringWithFormat:@"%@(%ld)",MultilingualTranslation(@"完成"), _groupMemberSelectList.count + 1] forState:UIControlStateNormal];
        CGRect textRect = [self.navBtnRight.titleLabel.text boundingRectWithSize:CGSizeMake(MAXFLOAT, DWScale(32)) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName: FONTR(14)} context:nil];
...
- (void)lkCallRequestForGroup {
    ...
    [[ZMediaCallManager sharedManager] mediaCallGroupRequestWith:callOptions onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
...
- (void)lkCallInviteForGroup {
    ...
    [[ZMediaCallManager sharedManager] mediaCallGroupInviteWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        [HUD showSuccessMessage:MultilingualTranslation(@"操作成功")];
...
- (void)zgCallRequestForGroup {
    ...
    [[ZCallManager sharedManager] requestGroupCallWith:_groupMemberSelectList group:_groupID callType:_callType];
}
...
- (void)zgCallInviteForGroup {
    ...
    WeakSelf
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:[ZCallManager sharedManager].currentCallOptions.zgCallOptions.callID forKey:@"callId"];
    [dict setValue:_groupMemberSelectList forKey:@"friendIds"];
    [[ZCallManager sharedManager] callGroupInviteWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        [HUD showSuccessMessage:MultilingualTranslation(@"操作成功")];
        [weakSelf.navigationController popViewControllerAnimated:YES];
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD showMessageWithCode:code errorMsg:msg];
