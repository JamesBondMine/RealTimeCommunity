...
@interface HomeMessageForwardTipView () <UICollectionViewDataSource,UICollectionViewDelegate>
...
@implementation HomeMessageForwardTipView
...
- (void)setupUI {
    ...
    lblSendTip.text = MultilingualTranslation(@"发送给");
    lblSendTip.tkThemetextColors = @[COLOR_33, COLORWHITE];
    ...
    [_collectionView registerClass:[HomeMessageForwardUserCell class] forCellWithReuseIdentifier:NSStringFromClass([HomeMessageForwardUserCell class])];
    ...
    [btnCancel setTitle:MultilingualTranslation(@"取消") forState:UIControlStateNormal];
    ...
    if (_forwardMsgList.count == 1) {
        ZMessageModel *forwardMsg = (ZMessageModel *)[_forwardMsgList firstObject];
        if (forwardMsg.message.messageType == CIMChatMessageType_TextMessage || forwardMsg.message.messageType == CIMChatMessageType_AtMessage || forwardMsg.message.messageType == CIMChatMessageType_VoiceMessage || forwardMsg.message.messageType == CIMChatMessageType_FileMessage || forwardMsg.message.messageType == CIMChatMessageType_GeoMessage || forwardMsg.message.messageType == CIMChatMessageType_ForwardMessage) {
            ...
            if (forwardMsg.message.messageType == CIMChatMessageType_TextMessage) {
                ...
                _lblContent.text = [NSString stringWithFormat:@"%@：%@", forwardMsg.message.fromNickname, [ZMessageTools forwardMessageAtContenTranslateToShowContent:resultAtContent atUsersDictList:forwardMsg.message.atUsersInfoList]];
            } else if (forwardMsg.message.messageType == CIMChatMessageType_VoiceMessage) {
                //语音消息
                _lblContent.text = [NSString stringWithFormat:@"%@：%@", forwardMsg.message.fromNickname, MultilingualTranslation(@"[语音]")];
                ...
                //地理位置消息
                _lblContent.text = [NSString stringWithFormat:@"%@%@", MultilingualTranslation(@"[位置]"), forwardMsg.message.geoName];
    ...
    //            make.centerX.equalTo(_viewBg);
            ...
            if (forwardMsg.message.messageType == CIMChatMessageType_VideoMessage) {
                _ivVideoTip = [[UIImageView alloc] initWithImage:ImgNamed(@"com_c_video_tip")];
        ...
        if (_multiSelectType == ZMultiSelectTypeMergeForward) {
            _lblContent.text = [NSString stringWithFormat:MultilingualTranslation(@"[合并转发]已选择%ld条消息"), _mergeCount];
        } else if (_multiSelectType == ZMultiSelectTypeSingleForward) {
            _lblContent.text = [NSString stringWithFormat:MultilingualTranslation(@"[逐条转发]已选择%ld条消息"), _forwardMsgList.count];
        } else {
            _lblContent.text = [NSString stringWithFormat:MultilingualTranslation(@"[转发]已选择%ld条消息"), _forwardMsgList.count];
        }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    HomeMessageForwardUserCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([HomeMessageForwardUserCell class]) forIndexPath:indexPath];
