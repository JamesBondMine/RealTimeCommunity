...
@interface ZGestureLockSetVC () <ZGestureLockViewDelegate>
...
@implementation ZGestureLockSetVC
...
- (void)setupUI {
    ...
    _lblTitle.text = MultilingualTranslation(@"请绘制新的解锁图案");
...
- (void)gestureLockViewFinishWith:(NSMutableString *)gesturePassword {
    if (![NSString isNil:_fistPassword]) {
        ...
        if ([gesturePassword isEqualToString:_fistPassword]) {
            ...
            [HUD showMessage:MultilingualTranslation(@"图案绘制的不同\n请重新输入手势密码")];
            
        }
    }else {
        _fistPassword = gesturePassword;
        _lblTitle.text = MultilingualTranslation(@"请绘制相同图案");
    }
}
/*
