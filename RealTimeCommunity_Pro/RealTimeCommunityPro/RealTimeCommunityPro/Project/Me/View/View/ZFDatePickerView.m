...
@interface ZFDatePickerView () <UIPickerViewDataSource,UIPickerViewDelegate>{
    UIView *contentView;
...
- (void)setViewInterface {
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        [button setTitle:i == 0 ? MultilingualTranslation(@"取消") : MultilingualTranslation(@"确定") forState:UIControlStateNormal];
        if (i == 0) {
...
- (NSAttributedString *)pickerView:(UIPickerView *)pickerView attributedTitleForRow:(NSInteger)row forComponent:(NSInteger)component {
    NSDictionary* titleTextAttributes = [NSDictionary dictionaryWithObjectsAndKeys:
                                         COLOR_33, NSForegroundColorAttributeName,
