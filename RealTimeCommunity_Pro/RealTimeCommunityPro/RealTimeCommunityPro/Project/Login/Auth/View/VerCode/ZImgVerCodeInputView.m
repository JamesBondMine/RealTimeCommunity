...
@interface ZImgVerCodeInputView()

@property (nonatomic, strong)ZGraphCodeView *imgCodeView;

@end

@implementation ZImgVerCodeInputView
...
- (void)getImgCodeAction {
    ...
    [IMSDKManager authGetImgVerCodeWith:params 
        ...
        [HUD showMessage:MultilingualTranslation(@"图形验证码获取失败")];
    }];
}
...
- (UITextField *)imgCodeInput {
    if (!_imgCodeInput) {
        ...
        _imgCodeInput.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
        _imgCodeInput.tkThemebackgroundColors = @[COLORWHITE, COLORWHITE_DARK];
        _imgCodeInput.font = FONTN(13);
        _imgCodeInput.placeholder = MultilingualTranslation(@"验证码");
