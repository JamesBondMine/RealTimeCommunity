//
//  ZNetworkDetectionView.m
//  RealTimeCommunityPro
//
//  Created by phl on 2025/10/15.
//  TODO: 网络检测页面

#import "ZNetworkDetectionView.h"
#import "ZNetworkDetectionHandle.h"
#import "ZNetworkDetectionTopHeaderView.h"
#import "NworkDetectionMessageModel.h"
#import "ZNetworkDetectionSectionHeaderView.h"
#import "ZNetworkDetectionMainResultCell.h"
#import "ZNetworkDetectionSubResultCell.h"

@interface ZNetworkDetectionView ()<UITableViewDelegate, UITableViewDataSource>

/// 数据处理
@property (nonatomic, strong) ZNetworkDetectionHandle *dataHandle;

/// 头部视图
@property (nonatomic, strong) ZNetworkDetectionTopHeaderView *headerView;
...
@implementation ZNetworkDetectionView

// MARK: set/get
- (ZNetworkDetectionTopHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[ZNetworkDetectionTopHeaderView alloc] initWithFrame:CGRectZero dataHandle:self.dataHandle];
    }
    return _headerView;
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        _tableView.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
        ...
        [_tableView registerClass:[ZNetworkDetectionSectionHeaderView class] forHeaderFooterViewReuseIdentifier:NSStringFromClass([ZNetworkDetectionSectionHeaderView class])];
    }
    return _tableView;
}
...
- (UIButton *)networkDetectionSwitchBtn {
    if (!_networkDetectionSwitchBtn) {
        ...
        [_networkDetectionSwitchBtn setTitle:MultilingualTranslation(@"开始检测") forState:UIControlStateNormal];
        ...
        _networkDetectionSwitchBtn.tkThemebackgroundColors = @[COLOR_81D8CF, COLOR_81D8CF_DARK];
...
- (void)setupUI {
    self.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
    ...
    [[self.networkDetectionSwitchBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        if (self.dataHandle.networkDetectionStatus == ZNetworkDetectionAlready ||
            ...
            [self.networkDetectionSwitchBtn setTitle:MultilingualTranslation(@"退出检测") forState:UIControlStateNormal];
            [self.dataHandle.startDetectionCommand execute:nil];
        }else {
            [self.networkDetectionSwitchBtn setTitle:MultilingualTranslation(@"开始检测") forState:UIControlStateNormal];
...
- (void)processData {
    ...
    [self.dataHandle.startDetectionCommand.executionSignals.switchToLatest subscribeNext:^(id  _Nullable x) {
        ...
        if (self.dataHandle.networkDetectionStatus == ZNetworkDetectFinish) {
            [self.networkDetectionSwitchBtn setTitle:MultilingualTranslation(@"重新检测") forState:UIControlStateNormal];
        }else {
            [self.networkDetectionSwitchBtn setTitle:MultilingualTranslation(@"开始检测") forState:UIControlStateNormal];
        }
    }];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NworkDetectionMessageModel *sectionModel = [self.dataHandle getSectionModelWithIndex:section];
    if (sectionModel) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (row == 0) {
        ZNetworkDetectionMainResultCell *mainCell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ZNetworkDetectionMainResultCell class])];
        if (!mainCell) {
            mainCell = [[ZNetworkDetectionMainResultCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSStringFromClass([ZNetworkDetectionMainResultCell class])];
        }
    ...
    ZNetworkDetectionSubResultCell *subCell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ZNetworkDetectionSubResultCell class])];
    if (!subCell) {
        subCell = [[ZNetworkDetectionSubResultCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:NSStringFromClass([ZNetworkDetectionSubResultCell class])];
    }
    
    ZNetworkDetectionSubResultModel *cellModel = [self.dataHandle getCellModelWithIndexPath:indexPath];
