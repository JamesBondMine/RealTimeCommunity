...
#import "NFFriendManageVC.h"//好友管理
#import "ZMediaCallManager.h"//音视频通话
#import "ZToolManager.h"
#import "NFFriendApplyPassVC.h"
#import "NFNewFriendListVC.h"
#import "NotesSettingViewController.h"

#import "ZCallManager.h"//新的音视频通话
#import "ZCallSingleVC.h"//单聊

@interface NFUserHomePageVC ()
...
@implementation NFUserHomePageVC
...
- (void)navBtnBackClicked {
    if (self.isFromQRCode) {
        ...
        for (UIViewController *controller in self.navigationController.viewControllers) {
            if ([controller isKindOfClass:[NFFriendApplyPassVC class]]) {
                isHaveApplyVC = YES;
            }
            if ([controller isKindOfClass:[NFNewFriendListVC class]]) {
                isHaveNewFriendVC = YES;
            }
        }
        //判断是否从新添加好友页面返回，如果同时包含NFFriendApplyPassVC和NFNewFriendListVC，则是从新添加好友页面返回
...
- (void)setupNavUI {
    ...
    self.navBtnRight.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        if (themeIndex == 1) {
            //暗黑模式
            [weakSelf.navBtnRight setImage:ImgNamed(@"com_c_nav_more") forState:UIControlStateNormal];
        }else {
            [weakSelf.navBtnRight setImage:ImgNamed(@"com_c_nav_more_black") forState:UIControlStateNormal];
        }
    };
}
- (void)setupUI {
    ...
    [_viewHeader setSetRemarkBtnBlock:^{
        NotesSettingViewController *vc = [[NotesSettingViewController alloc] initWithTitle:MultilingualTranslation(@"备注") 
                                                                                    remark:weakSelf.userModel.remarks 
                                                                               description:weakSelf.userModel.descRemark];
...
//        make.size.mas_equalTo(CGSizeMake(DWScale(56), DWScale(80)));
    ...
    _blackStatusLbl.text = MultilingualTranslation(@"你已将对方拉黑，无法发送消息。如需发送消息，请先关闭加入黑名单或进入个人中心移出黑名单");
...
-(void)navBtnRightClicked {
    NFFriendManageVC *vc = [NFFriendManageVC new];
...
- (UIButton *)btnAudio {
    if (!_btnAudio) {
        ...
        [_btnAudio setImage:ImgNamed(@"recon_call_audio") forState:UIControlStateNormal];
...
- (UIButton *)btnVideo {
    if (!_btnVideo) {
        ...
        [_btnVideo setImage:ImgNamed(@"recon_call_video") forState:UIControlStateNormal];
