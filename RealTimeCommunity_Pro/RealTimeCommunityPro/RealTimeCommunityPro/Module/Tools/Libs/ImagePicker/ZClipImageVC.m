...
//  CIMKit
//
//  Created by cusPro on 2022/11/29.
//

#import "ZClipImageVC.h"

@interface ZClipImageVC ()<UIScrollViewDelegate>{
    CGFloat _selfHeight;
    CGFloat _selfWidth;
    CGRect _cropFrame;
}
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    //防止分母为零
    if (_cropSize.width <= 0) return;
    if (_cropSize.height <= 0) return;
    if (CGRectGetWidth(self.view.frame) <= 0) return;
    if (CGRectGetHeight(self.view.frame) <= 0) return;
    if (_image.size.width <= 0) return;
    if (_image.size.height <= 0) return;
    if (_imageView.frame.size.width <= 0) return;
    if (_imageView.frame.size.height <= 0) return;
...
- (void)setupNavUI {
    ...
    _navBtnBack.frame = CGRectMake(16, DStatusBarH, 24, 44);
    ...
    [_navBtnRight setTitle:MultilingualTranslation(@"完成") forState:UIControlStateNormal];
    [_navBtnRight setTitleColor:UIColor.whiteColor forState:UIControlStateNormal];
    _navBtnRight.titleLabel.font = [UIFont systemFontOfSize:14];
...
-(void)setupData{
    ...
    //防止分母为零
    if (self.cropSize.width <= 0) return;
    if (self.cropSize.height <= 0) return;
    if (self.image.size.width <= 0) return;
    if (self.image.size.height <= 0) return;
...
- (void)navBtnRightClicked {
    if (_delegate && [_delegate respondsToSelector:@selector(clipImageDidFinishedWithImage:)]) {
        [_delegate clipImageDidFinishedWithImage:[self getSubImage]];
    }
