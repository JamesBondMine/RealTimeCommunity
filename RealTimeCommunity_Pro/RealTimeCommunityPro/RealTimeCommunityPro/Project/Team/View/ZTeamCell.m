...
- (void)setupUI {    
    ...
    _subTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (void)setTitleStr:(NSString *)titleStr {
    ...
    if ([_titleStr isEqualToString:MultilingualTranslation(@"团队名称")]) {
        _tipImgView.hidden = NO;
        _tipImgView.image = ImgNamed(@"retm_team_arrow_gray");
        [_tipImgView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.trailing.equalTo(self.contentView).offset(-DWScale(16));
            make.centerY.equalTo(self.contentView);
            make.size.mas_equalTo(CGSizeMake(DWScale(12), DWScale(12)));
        }];
    } else if ([_titleStr isEqualToString:MultilingualTranslation(@"邀请码")]) {
        _tipImgView.hidden = NO;
        _tipImgView.image = ImgNamed(@"retm_team_copy");
        [_tipImgView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.trailing.equalTo(self.contentView).offset(-DWScale(16));
            make.centerY.equalTo(self.contentView);
