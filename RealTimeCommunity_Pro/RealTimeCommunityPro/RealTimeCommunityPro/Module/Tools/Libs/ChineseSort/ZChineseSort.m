...
@implementation ZChineseSortSetting

+(ZChineseSortSetting*)share{
    static ZChineseSortSetting * singleton = nil ;
    if (singleton == nil) {
        singleton = [[ZChineseSortSetting alloc] init];
...
@interface ZChineseSortModel : NSObject
...
@implementation ZChineseSortModel
@end
...
- (NSComparisonResult)mySort:(NSString *)str {
    ...
    NSString*s = [ZChineseSortSetting share].specialCharSectionTitle;
    BOOL b = [ZChineseSortSetting share].specialCharPositionIsFront;
...
+(void)sortAndGroup:(NSArray*)objectArray
    ...
    if (key == nil) {
        ...
        if (![obj isKindOfClass:NSString.class]) {
            [ZChineseSort logMsg:@"数组内元素不是字符串类型,如果是对象类型，请传key"];
    ...
    if (!containKey) {
        [ZChineseSort logMsg:@"数组内元素未包含指定属性"];
        finish(NO,nil,nil,nil);
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (ZChineseSortSetting.share.needStable){
            NSMutableArray *tempArray = [NSMutableArray array];

            [objectArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                
                ZChineseSortModel *model = [weakSelf getModelWithObj:obj key:key];
        ...
        [ZChineseSort logMsg:@""];
        [ZChineseSort logMsg:[NSString stringWithFormat:@"转拼音用时：\t %f s", (state1-start)]];
        ...
        [ZChineseSort logMsg:[NSString stringWithFormat:@"排序用时：\t %f s", (state2-state1)]];
        ...
        for (ZChineseSortModel* object in sortModelArray) {
...
+(ZChineseSortModel*)getModelWithObj:(id)obj key:(NSString*)key{
    
    ZChineseSortModel *model = [[ZChineseSortModel alloc]init];
    ...
    if(model.string == nil || model.string.length == 0){
        model.string = ZChineseSortSetting.share.specialCharSectionTitle;
    }else{
        //过滤 ignoreModelWithPrefix
        NSString *prefix = [model.string substringToIndex:1];
        
        if (![ZChineseSortSetting.share.ignoreModelWithPrefix containsString:prefix]) {
            //是否将字母与汉字拼音一起排序？？？？ （暂不考虑）
            //获取拼音首字母
...
+(void)logMsg:(NSString*)msg{
    if (ZChineseSortSetting.share.logEable == YES) {
        DLog(@"------------------");
        DLog(@"%@", msg);
        DLog(@"------------------");
    }
...
+ (NSString *)getFirstLetter:(NSString *)chinese{
    ...
    //吧多音字先替换
    [ZChineseSortSetting.share.polyphoneMapping enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
...
//    }
    ...
    if ([upperCaseStr characterAtIndex:0] >= 'A' && [upperCaseStr characterAtIndex:0] <= 'Z') {
        ...
        return ZChineseSortSetting.share.specialCharSectionTitle;
    }
}
@end
