//
//  HomeSystemMessagePendReviewVC.m
//  CIMKit
//
...
@implementation HomeSystemMessagePendReviewVC
...
- (void)viewDidLoad {
    ...
    //通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(requestListData) name:@"HomeSystemMessagePendReviewVCReloadListDataNotification" object:nil];
...
- (void)requesPendingJoinGroupApplyFormGropManager {
    ...
    [IMSDKManager groupManagerApplyJoinGroupListWithData:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            NSArray *recordList = [MSSSMessageModel mj_objectArrayWithKeyValuesArray:records];
...
- (void)requestHandleJoinGroupApplyWithStatus:(ZGroupApplyHandleStatus)handleStatus {
    ...
    for (MSSSMessageModel *model in self.selectedApplyList.safeArray) {
...
- (void)operationResultHandleWithData:(NSArray *)resultArr {
    if (resultArr.count > 0) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"HomeSystemMessageAllVCReloadListDataNotification"
                                                            object:self];
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.beStatus == 1) {
        ...
        if (model.selectedStatus) {
            ...
            WeakSelf
            [self.selectedApplyList.safeArray enumerateObjectsUsingBlock:^(MSSSMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)refuseJoinApplyAction {
    ...
    msgAlertView.lblContent.text = MultilingualTranslation(@"是否拒绝?");
...
- (void)agreeJoinApplyAction {
    ...
    ZMessageAlertView *msgAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeNomal supView:nil];
    msgAlertView.lblContent.text = MultilingualTranslation(@"是否同意?");
    [msgAlertView.btnSure setTitle:MultilingualTranslation(@"确认") forState:UIControlStateNormal];
    [msgAlertView.btnSure setTkThemeTitleColor:@[COLORWHITE,COLORWHITE] forState:UIControlStateNormal];
    msgAlertView.btnSure.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF];
    [msgAlertView.btnSure setTkThemeBackgroundImage:@[[UIImage ImageForColor:COLOR_4069B9],[UIImage ImageForColor:COLOR_4069B9_DARK]] forState:UIControlStateSelected];
...
- (ZGroupApplyBottomView *)bottomView {
    if (!_bottomView) {
        _bottomView = [[ZGroupApplyBottomView alloc] initWithFrame:CGRectMake(0, DScreenHeight - DNavStatusBarH - DWScale(40) - DWScale(56) - DHomeBarH, DScreenWidth, DWScale(56) + DHomeBarH)];
