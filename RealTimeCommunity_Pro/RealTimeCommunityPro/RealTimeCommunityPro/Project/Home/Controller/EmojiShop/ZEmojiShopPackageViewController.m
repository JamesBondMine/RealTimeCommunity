//
//  ZEmojiShopPackageViewController.m
//  CIMKit
//
//  Created by cusPro on 2023/10/25.
//

#import "ZEmojiShopPackageViewController.h"
#import <DZNEmptyDataSet/UIScrollView+EmptyDataSet.h>
#import "ZEmojiShopPackagCell.h"
#import "EmojiStorePackageHeaderView.h"
#import "ZEmojiPackageDetailViewController.h"//表情包详情

@interface ZEmojiShopPackageViewController () <UITableViewDataSource, UITableViewDelegate, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate, ZEmojiShopPackagCellDelegate>
...
@implementation ZEmojiShopPackageViewController
...
- (void)setupUI {
    ...
    //cell & header
    [self.baseTableView registerClass:[EmojiStorePackageHeaderView class] forHeaderFooterViewReuseIdentifier:NSStringFromClass([EmojiStorePackageHeaderView class])];
    [self.baseTableView registerClass:[ZEmojiShopPackagCell class] forCellReuseIdentifier:NSStringFromClass([ZEmojiShopPackagCell class])];
}
...
- (void)requestStickersPackageListData {
    ...
    [IMSDKManager imSdkUserFindUnUsedStickersPackageList:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            NSArray *tempPackageList = [LingIMStickersPackageModel mj_objectArrayWithKeyValuesArray:rowList];
...
- (void)requestAddStickersPackageWithStickersSetId:(NSString *)stickersSetId withIndex:(NSInteger)index {
    ...
    [IMSDKManager imSdkUserAddStickersPackage:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        LingIMStickersPackageModel *packageModel = (LingIMStickersPackageModel *)[self.stickersPackageList objectAtIndexSafe:index];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    EmojiStorePackageHeaderView *viewHeader = [tableView dequeueReusableHeaderFooterViewWithIdentifier:NSStringFromClass([EmojiStorePackageHeaderView class])];
    viewHeader.isShow = self.stickersPackageList.count > 0 ? YES : NO;
    return viewHeader;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZEmojiShopPackagCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ZEmojiShopPackagCell class]) forIndexPath:indexPath];
    cell.baseCellIndexPath = indexPath;
    cell.delegate = self;
    LingIMStickersPackageModel *packageModel = (LingIMStickersPackageModel *)[self.stickersPackageList objectAtIndexSafe:indexPath.row];
...
- (void)emojiPackageAddNewEmoji:(NSIndexPath *)cellIndexPath {
    LingIMStickersPackageModel *addPackageModel = (LingIMStickersPackageModel *)[self.stickersPackageList objectAtIndexSafe:cellIndexPath.row];
