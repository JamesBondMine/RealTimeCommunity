...
#import "VVTTeamInviteCustomTextField.h"

@interface ZTeamCreateView()

/// 团队名称标题label
@property (nonatomic, strong) UILabel *teamNameLabel;

/// 团队名称输入
@property (nonatomic, strong) VVTTeamInviteCustomTextField *teamNameInputTF;
...
/// 邀请码输入
@property (nonatomic, strong) VVTTeamInviteCustomTextField *codeInputTF;
...
/// 创建团队处理类
@property (nonatomic, strong) IVTTeamCreateDataHandle *teamListDataHandle;
...
@implementation ZTeamCreateView
...
- (UILabel *)teamNameLabel {
    if (!_teamNameLabel) {
        ...
        _teamNameLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
        _teamNameLabel.font = FONTR(14);
        _teamNameLabel.text = MultilingualTranslation(@"请输入团队名称");
    }
    return _teamNameLabel;
}

- (VVTTeamInviteCustomTextField *)teamNameInputTF {
    if (!_teamNameInputTF) {
        _teamNameInputTF = [VVTTeamInviteCustomTextField new];
        // 为了适配rtl（阿拉伯、波斯语布局，只能用NSMutableAttributedString）
        NSMutableAttributedString *placeHolderAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"请输入团队名称") attributes:@{
        ...
        _teamNameInputTF.textField.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (UILabel *)codeLabel {
    if (!_codeLabel) {
        ...
        _codeLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
        _codeLabel.font = FONTR(14);
        _codeLabel.text = MultilingualTranslation(@"请输入邀请码");
    }
    return _codeLabel;
}
...
- (VVTTeamInviteCustomTextField *)codeInputTF {
    if (!_codeInputTF) {
        _codeInputTF = [VVTTeamInviteCustomTextField new];
        // 为了适配rtl（阿拉伯、波斯语布局，只能用NSMutableAttributedString）
        NSMutableAttributedString *placeHolderAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"请输入邀请码") attributes:@{
            NSForegroundColorAttributeName: COLOR_99
        }];
        ...
        _codeInputTF.textField.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    }
    return _codeInputTF;
}
...
- (UILabel *)codeErrorTipLabel {
    if (!_codeErrorTipLabel) {
        ...
        _codeErrorTipLabel.text = MultilingualTranslation(@"邀请码暂时无法使用，请更换邀请码");
        // 默认隐藏
...
- (UILabel *)topTipLabel {
    if (!_topTipLabel) {
        ...
        _topTipLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
        _topTipLabel.font = FONTR(16);
        _topTipLabel.text = MultilingualTranslation(@"是否设为置顶");
    }
    return _topTipLabel;
}
