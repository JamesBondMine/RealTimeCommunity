...
/// 数据处理
@property (nonatomic, strong, readwrite) GGNBroupNoticeListDataHandle *dataHandle;
...
- (instancetype)initWithFrame:(CGRect)frame
    GroupNoticeListDataHandle:(GGNBroupNoticeListDataHandle *)dataHandle {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    GNoteLocalUserNameModel *groupModel = [self.dataHandle obtainGroupModelWithIndexPath:indexPath];
    if (!groupModel) {
        return 0.0;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    GNoteLocalUserNameModel *groupNoticeModel = [self.dataHandle obtainGroupModelWithIndexPath:indexPath];
...
- (NSAttributedString *)titleForEmptyDataSet:(UIScrollView *)scrollView {
    NSString *string = MultilingualTranslation(@"暂无公告");
