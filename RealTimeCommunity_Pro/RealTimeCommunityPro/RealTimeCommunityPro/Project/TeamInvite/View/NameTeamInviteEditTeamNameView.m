//
//  NameTeamInviteEditTeamNameView.m
//  CIMKit
//
//  Created by phl on 2025/7/25.
//

#import "NameTeamInviteEditTeamNameView.h"
#import "IVTTeamInviteEditTeamNameDataHandle.h"
#import "VVTTeamInviteCustomTextField.h"

@interface NameTeamInviteEditTeamNameView()
...
@property (nonatomic, strong) VVTTeamInviteCustomTextField *teamNameTextField;

@property (nonatomic, strong) IVTTeamInviteEditTeamNameDataHandle *editTeamNameDataHandle;

@end

@implementation NameTeamInviteEditTeamNameView
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.text = MultilingualTranslation(@"团队名称");
...
- (UIButton *)backButton {
    if (!_backButton) {
        ...
        [_backButton setTitle:MultilingualTranslation(@"取消") forState:UIControlStateNormal];
        _backButton.titleLabel.font = FONTR(15);
        _backButton.titleLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (VVTTeamInviteCustomTextField *)teamNameTextField {
    if (!_teamNameTextField) {
        _teamNameTextField = [VVTTeamInviteCustomTextField new];
        _teamNameTextField.textField.font = FONTR(14);
        _teamNameTextField.textField.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (instancetype)initWithFrame:(CGRect)frame editTeamNameDataHandle:(IVTTeamInviteEditTeamNameDataHandle *)dataHandle {
   ...
   // 返回按钮
    ...
    [[self.saveButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        if ([NSString isNil:self.teamNameTextField.textField.text]) {
            [HUD showMessage:MultilingualTranslation(@"团队名称不能为空")];
            return;
        }
