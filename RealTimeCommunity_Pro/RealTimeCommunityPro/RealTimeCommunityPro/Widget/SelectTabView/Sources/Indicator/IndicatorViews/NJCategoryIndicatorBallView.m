...
- (void)jx_contentScrollViewDidScroll:(NJCategoryIndicatorParamsModel *)model {
    ...
    if (percent == 0) {
        ...
        if (percent <= 0.5) {
            targetXOfBigBall = [NJCategoryFactory interpolationFrom:leftX to:(rightX - self.ballScrollOffsetX) percent:percent*2];
            targetWidthOfSmallBall = [NJCategoryFactory interpolationFrom:ballWidth to:ballWidth/2 percent:percent*2];
        }else {
            targetXOfBigBall = [NJCategoryFactory interpolationFrom:(rightX - self.ballScrollOffsetX) to:rightX percent:(percent - 0.5)*2];
            targetWidthOfSmallBall = [NJCategoryFactory interpolationFrom:ballWidth/2 to:0 percent:(percent - 0.5)*2];
            targetXOfSmallBall = [NJCategoryFactory interpolationFrom:leftX to:rightX percent:(percent - 0.5)*2];
        }
    }
