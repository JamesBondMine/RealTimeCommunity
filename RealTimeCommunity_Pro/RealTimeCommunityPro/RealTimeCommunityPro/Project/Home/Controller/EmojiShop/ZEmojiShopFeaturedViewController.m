//
//  ZEmojiShopFeaturedViewController.m
//  CIMKit
//
//  Created by cusPro on 2023/10/25.
//

#import "ZEmojiShopFeaturedViewController.h"
#import <DZNEmptyDataSet/UIScrollView+EmptyDataSet.h>
#import "EmojiStoreFeaturedHeaderView.h"
#import "ZEmojiShopFeaturedCell.h"
#import "EMEmojiMenuPopView.h"

@interface ZEmojiShopFeaturedViewController () <UICollectionViewDelegate, UICollectionViewDataSource, DZNEmptyDataSetSource,DZNEmptyDataSetDelegate, ZEmojiShopFeaturedCellDelegate>
...
@implementation ZEmojiShopFeaturedViewController
...
- (void)setupUI {
    ...
    [_collectionView registerClass:[EmojiStoreFeaturedHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:NSStringFromClass([EmojiStoreFeaturedHeaderView class])];
    [_collectionView registerClass:[ZEmojiShopFeaturedCell class] forCellWithReuseIdentifier:NSStringFromClass([ZEmojiShopFeaturedCell class])];
}
...
- (void)requestFindStickersForName {
    ...
    [IMSDKManager imSdkUserFindStickersForName:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            NSArray *tempStickersList = [LingIMStickersModel mj_objectArrayWithKeyValuesArray:recordsList];
...
//添加表情收藏
- (void)requestAddAStickersToCollectWithModel:(LingIMStickersModel *)strickersModel {
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        EmojiStoreFeaturedHeaderView *headerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:NSStringFromClass([EmojiStoreFeaturedHeaderView class]) forIndexPath:indexPath];
        return headerView;
    }
    return nil;
}
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ZEmojiShopFeaturedCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([ZEmojiShopFeaturedCell class]) forIndexPath:indexPath];
    cell.cellIndexPath = indexPath;
    cell.delegate = self;
    LingIMStickersModel *tempStickersModel = (LingIMStickersModel *)[self.featuredStickersList objectAtIndex:indexPath.row];
...
- (void)shopFeaturedStickerLongTapAction:(NSIndexPath *)indexPath {
    ZEmojiShopFeaturedCell *longTapCell = (ZEmojiShopFeaturedCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
    //计算消息的坐标位置,确定菜单弹窗弹出的位置的坐标
    CGRect targetRect = [self.collectionView convertRect:longTapCell.frame toView:CurrentVC.view];
    
    WeakSelf
    EMEmojiMenuPopView *menuPopView = [[EMEmojiMenuPopView alloc] initWithMenuTitle:MultilingualTranslation(@"存表情") targetRect:targetRect];
    [menuPopView ZEmojiMenuShow];
    [menuPopView setMenuClickBlock:^(void) {
        LingIMStickersModel *addStickersModel = (LingIMStickersModel *)[self.featuredStickersList objectAtIndex:indexPath.row];
