...
@interface ZGroupInviteFriendVC ()<MainSearchViewDelegate,UITableViewDataSource,UITableViewDelegate,ZBaseCellDelegate>
...
@implementation ZGroupInviteFriendVC
...
#pragma mark - 界面布局
- (void)setupNavUI {
    self.navTitleStr = MultilingualTranslation(@"邀请好友");
...
- (void)requestInviteFriendWithReason:(NSString *)reason {
    ...
    if (self.groupInfoModel.isNeedVerify) {
        ...
        [dict setObjectSafe:[NSString stringWithFormat:MultilingualTranslation(@"%@邀请你加入群聊"),UserManager.userInfo.nickname] forKey:@"inviteDesc"];
    }
    __block NSMutableArray *groupMemberList = [NSMutableArray array];
    
    [_selectedList enumerateObjectsUsingBlock:^(ZBaseUserModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    ...
    [IMSDKManager groupInviteFriendWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        for (UIViewController *vc in viewControllers) {
            if ([vc isKindOfClass:[NHChatViewController class]]) {
    ...
    if ([NSString isNil:self.searchStr]) {
        ...
        if ([self.selectedList containsObject:model]) {
            ...
            if (self.selectedList.count >= 500) {
                ...
                viewTip.lblTip.text = [NSString stringWithFormat:MultilingualTranslation(@"最多只能选择%ld人"),500];
