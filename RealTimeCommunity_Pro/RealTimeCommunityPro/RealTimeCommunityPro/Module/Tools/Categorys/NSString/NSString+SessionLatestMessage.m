...
+ (NSMutableAttributedString *)getSessionLatestMessageAttributedStringWith:(LingIMSessionModel *)sessionModel {
    LingIMChatMessageModel *chatMessage = sessionModel.sessionLatestMessage;
    ...
    if (chatMessage) {
        ...
        if (chatMessage.messageType == CIMChatMessageType_TextMessage) {
            if ([chatMessage.fromID isEqualToString:UserManager.userInfo.userUID]) {
                sessionAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:(![NSString isNil:chatMessage.textContent] ? chatMessage.textContent : @"") imageRect:CGRectMake(0, -4, 18, 18)];
            } else {
                ...
                sessionAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:showContent imageRect:CGRectMake(0, -4, 18, 18)];
            }
            [self configAttributedString:sessionAttStr fontNum:12];
        } else if (chatMessage.messageType == CIMChatMessageType_ImageMessage){
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[图片]")];
            [self configAttributedString:sessionAttStr fontNum:12];
        } else if (chatMessage.messageType == CIMChatMessageType_VoiceMessage){
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[语音]")];
            [self configAttributedString:sessionAttStr fontNum:12];
        } else if (chatMessage.messageType == CIMChatMessageType_FileMessage){
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[文件]")];
            [self configAttributedString:sessionAttStr fontNum:12];
        } else if (chatMessage.messageType == CIMChatMessageType_VideoMessage) {
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[视频]")];
            [self configAttributedString:sessionAttStr fontNum:12];
        } else if (chatMessage.messageType == CIMChatMessageType_StickersMessage) {
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[表情]")];
            [self configAttributedString:sessionAttStr fontNum:12];
            ...
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@%@", MultilingualTranslation(@"[个人名片]"), chatMessage.cardNickName]];
            [self configAttributedString:sessionAttStr fontNum:12];
        } else if (chatMessage.messageType == CIMChatMessageType_GeoMessage) {
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@%@", MultilingualTranslation(@"[位置]"), chatMessage.geoName]];
            [self configAttributedString:sessionAttStr fontNum:12];
        } else if (chatMessage.messageType == CIMChatMessageType_ForwardMessage) {
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[会话记录]")];
            [self configAttributedString:sessionAttStr fontNum:12];
        } else if (chatMessage.messageType == CIMChatMessageType_AtMessage) {
            if ([chatMessage.fromID isEqualToString:UserManager.userInfo.userUID]) {
                chatMessage.showContent = [ZMessageTools atContenTranslateToShowContent:(chatMessage.atContent.length > 0 ? chatMessage.atContent : @"") atUsersDictList:chatMessage.atUsersInfoList withMessage:chatMessage isGetShowName:YES];
                
                NSString *atContetnStr = [NSString stringWithString:(![NSString isNil:chatMessage.showContent] ? chatMessage.showContent : @"")];
                sessionAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:atContetnStr imageRect:CGRectMake(0, -4, 18, 18)];
            } else {
                if (![NSString isNil:chatMessage.againAtTranslateContent]) {
                    chatMessage.showTranslateContent = [ZMessageTools atContenTranslateToShowContent:(![NSString isNil:chatMessage.againAtTranslateContent] ? chatMessage.againAtTranslateContent : chatMessage.atTranslateContent) atUsersDictList:chatMessage.atUsersInfoList withMessage:chatMessage isGetShowName:NO];
                    ...
                    chatMessage.showContent = [ZMessageTools atContenTranslateToShowContent:(chatMessage.atTranslateContent.length > 0 ? chatMessage.atTranslateContent : chatMessage.atContent) atUsersDictList:chatMessage.atUsersInfoList withMessage:chatMessage  isGetShowName:YES];
            ...
            if (chatMessage.chatType == CIMChatType_GroupChat) {
                ...
                if (chatMessage.backDelInformSwitch == 2) {
                    ...
                    if (chatMessage.backDelInformSwitch == 0) {
                        resutlContent = [NSString stringWithFormat:MultilingualTranslation(@"%@ 撤回了一条消息"), @""];
                        sessionAttStr = [[NSMutableAttributedString alloc] initWithString:resutlContent];
                    }
                ...
                //撤回消息
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"%@ 撤回了一条消息"), @""]];
            }
            [self configAttributedString:sessionAttStr fontNum:12];
            ...
            //群公告消息
            NSString *tempStr = [NSString stringWithFormat:MultilingualTranslation(@"“%@”设置了群公告"),chatMessage.fromNickname];
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:tempStr];
            [self configAttributedString:sessionAttStr fontNum:12];
            
            NSMutableAttributedString *groupNoticeTip = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"[%@]",MultilingualTranslation(@"群公告")]];
        ...
        if (sessionAttStr.length > 0) {
            ...
            if (chatMessage.messageType == CIMChatMessageType_AtMessage && ![chatMessage.fromID isEqualToString:UserManager.userInfo.userUID]) {
                ...
                if (isAtMe) {
                    //有人 @ 我  @所有人
                    NSMutableAttributedString *atStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[有人@我]")];
                    NSDictionary *dict = @{NSFontAttributeName:FONTR(12), NSForegroundColorAttributeName:COLOR_F93A2F};
        ...
        sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[暂无新消息]")];
        [self configAttributedString:sessionAttStr fontNum:12];
    }
...
+ (NSMutableAttributedString *)dealServerMessage:(LingIMChatMessageModel *)chatMessage{
    NSMutableAttributedString *sessionAttStr = [[NSMutableAttributedString alloc] initWithString:@""];
    ...
    switch (serverMessage.sMsgType) {
        ...
        {
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"您还不是对方好友，请先添加好友。")];
        }
            break;
        case IMServerMessage_ServerMsgType_BlackFriendMessage://好友黑名单
        {
            ...
            if (friendBlack.type == 1) {
                //我拉黑好友
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"已拉黑")];
            }else {
                //好友拉黑我
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"对方已拒绝接受你的消息")];
            }
        }
            break;
        case IMServerMessage_ServerMsgType_UserAccountClose://好友账号已注销
        {
            ...
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"账号已注销")];
        }
            break;
        ...
        case IMServerMessage_ServerMsgType_CreateGroupMessage://创建群聊
        {
            ...
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"%@“%@” 加入了群聊"),user,[invitedUserArr componentsJoinedByString:@"，"]]];
        }
            break;
                    ...
                    if (inviteModel.type == 4) {
                        ...
                        [invitedMemberArr enumerateObjectsUsingBlock:^(UserInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                            if ([obj.uId isEqualToString:UserManager.userInfo.userUID]) {
                                [invitedUserArr addObjectIfNotNil:MultilingualTranslation(@"你")];
                            }else {
                                [invitedUserArr addObjectIfNotNil:obj.uNick];
                            }
                        }];
                        
                        sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”通过扫描二维码加入群聊"), [invitedUserArr componentsJoinedByString:@"，"]]];
                    } else {
                        NSString *user;
                        if ([inviteModel.uid isEqualToString:UserManager.userInfo.userUID]) {
                            ...
                            user = [NSString stringWithFormat:MultilingualTranslation(@"“%@”邀请了 "), groupMemberModel ? groupMemberModel.showName : inviteModel.nick];
                        }
        ...
        {
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"群主开启了全员禁言")];
        }
            break;
        case IMServerMessage_ServerMsgType_NullGroupMessage://群组不存在
        {
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"群组已解散")];
        }
            break;
        case IMServerMessage_ServerMsgType_MemberNoGroupMessage://用户不在群内
        {
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"你已不在该群")];
        }
            break;
        case IMServerMessage_ServerMsgType_MemberGroupForbidMessage://用户在群组内被禁言
        {
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"你已被禁言")];
        }
            break;
        ...
        case IMServerMessage_ServerMsgType_TransferOwnerMessage://转让群主
        {
            ...
            if (groupOwnerTransfer.informUidArray != nil) {
                if (groupOwnerTransfer.informUidArray.count == 0) {
                    sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"群主变更为“%@”"), groupMemberModel ? groupMemberModel.showName : groupOwnerTransfer.nick]];
        ...
        case IMServerMessage_ServerMsgType_EstoppelGroupMessage://告知全部群成员 群禁言 开启/关闭
        {
            ...
            if (groupStatus.status == 1) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”开启了全员禁言"), groupMemberModel ? groupMemberModel.showName : groupStatus.nick]];
            }else {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”关闭了全员禁言"), groupMemberModel ? groupMemberModel.showName : groupStatus.nick]];
            }
        }
            break;
            ...
            if (groupAdmin.informUidArray != nil) {
                if (groupAdmin.informUidArray.count == 0) {
                    ...
                    [adminInfoArr enumerateObjectsUsingBlock:^(UserInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                        if ([obj.uId isEqualToString:UserManager.userInfo.userUID]) {
                            [adminArr addObjectIfNotNil:MultilingualTranslation(@"你")];
                        }else {
                    ...
                    if (groupAdmin.type == 1) {
                        sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”将“%@”设为管理员"),groupMemberOperateModel ? groupMemberOperateModel.showName : groupAdmin.operateNick, [adminArr componentsJoinedByString:@"，"]]];
                    }else {
                        sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”将“%@”从管理员中移除"),groupMemberOperateModel ? groupMemberOperateModel.showName : groupAdmin.operateNick, [adminArr componentsJoinedByString:@"，"]]];
                    }
                } else {
        ...
        case IMServerMessage_ServerMsgType_NameGroupMessage://群名称修改
        {
            ...
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”将群名称修改为“%@”"), groupMemberModel ? groupMemberModel.showName : groupName.nick, groupName.gName]];
        }
            break;
        case IMServerMessage_ServerMsgType_NoticeGroupMessage://群公告设置
        {
            ...
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”设置了群公告"), groupMemberModel ? groupMemberModel.showName : groupNotice.nick]];
        }
            break;
        ...
        case IMServerMessage_ServerMsgType_IsShowHistoryMessage://群禁止私聊
        {
            ...
            if (groupStatusMessage.status == 1) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@” 开启了新成员可查看历史消息"), groupMemberModel ? groupMemberModel.showName : groupStatusMessage.nick]];
            }else {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@” 关闭了新成员可查看历史消息"), groupMemberModel ? groupMemberModel.showName : groupStatusMessage.nick]];
            }
        }
            break;
        case IMServerMessage_ServerMsgType_GroupSingleForbidMessage://群主或管理员 禁言某个群成员
        {
            ...
            if ([memberBanned.toUid isEqualToString:UserManager.userInfo.userUID]) {
                if (memberBanned.status == 1) {
                    sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"你被“%@”%@"), groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick, [NSString convertBannedSendMsgTime:memberBanned.expireTime]]];
                }else {
                    sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"你被“%@”解除禁言"), groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick]];
                }
                
            }else {
                if (memberBanned.status == 1) {
                    sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”被“%@”%@"), groupMemberToModel ? groupMemberToModel.showName : memberBanned.toNick, groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick, [NSString convertBannedSendMsgTime:memberBanned.expireTime]]];
                }else {
                    sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”被“%@”解除禁言"),groupMemberToModel ? groupMemberToModel.showName : memberBanned.toNick, groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick]];
                }
            }
        }
            break;
        ...
        case IMServerMessage_ServerMsgType_InviteJoinGroupNoFriendMessage://邀请好友进群，但是好友不存在，该消息只转发给邀请加入的用户
        {
            ...
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”未添加你为好友，无法邀请进入群聊"), groupMemberModel ? groupMemberModel.showName : inviteJoinGroupNoFriend.operateNick]];
        }
            break;
        case IMServerMessage_ServerMsgType_InviteJoinGroupBlackFriendMessage://邀请好友进群，但是已被拉黑，该消息只转发给邀请加入的用户
        {
            ...
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”拒绝加入群聊"), groupMemberModel ? groupMemberModel.showName : inviteJoinGroupBlackFriend.operateNick]];
        }
            break;
        case IMServerMessage_ServerMsgType_AvatarGroupMessage://变更群头像  该消息只转发给在线的所有群成员
        {
            ...
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”修改了群头像"), groupMemberModel ? groupMemberModel.showName : avatarGroup.nick]];
        }
            break;
        case IMServerMessage_ServerMsgType_DelGroupNotice://删除群公告
        {
            ...
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”删除了群公告"), groupMemberModel ? groupMemberModel.showName : groupNoticeDel.nick]];
        }
            break;
        ...
        case IMServerMessage_ServerMsgType_JoinVerifyGroupMessage://是否进群验证  该消息只转发给在线的所有群成员213
        {
            ...
            if (groupStatus.status == 1) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”已启用群里邀请确认"), userName]];
            }else {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"“%@”已关闭群里邀请确认"), userName]];
            }
        ...
        case IMServerMessage_ServerMsgType_InviteJoinRepGroupMessage://邀请进群申请  该消息发送给群主和群管理员214
        {
            ...
            if ([inviteJoinGroup.uid isEqualToString:UserManager.userInfo.userUID]) {
                inviterName = MultilingualTranslation(@"你");
            }else {
            ...
            if (inviteJoinGroup.inviteUidArray.count > 1) {
                inviteeName = [NSString stringWithFormat:MultilingualTranslation(@"%@等%ld人"),inviteeName, inviteJoinGroup.inviteUidArray.count];
            }
            //群信息
            NSString *groupName = inviteJoinGroup.gName;
            sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"%@邀请%@加入%@"), inviterName, inviteeName, groupName]];
        }
            break;
...
+ (NSMutableAttributedString *)dealServerMessageOfCustomEvent:(LingIMChatMessageModel *)chatMessage {
    NSMutableAttributedString *sessionAttStr = [[NSMutableAttributedString alloc] initWithString:@""];
    ...
    if (customEvent.type == 101) {
        ...
        if (mediaCallModel.mode == 0) {
            //视频通话
            callMode = [NSString stringWithFormat:@"[%@]", MultilingualTranslation(@"视频通话")];
        } else {
            //语音通话
            callMode = [NSString stringWithFormat:@"[%@]", MultilingualTranslation(@"语音通话")];
        }
        ...
        */
        ...
        if (groupMemberModel) {
            if ([actionModel.action isEqualToString:@"new"]) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"%@发起了%@"), groupMemberModel.showName, callMode]];
            } else if ([actionModel.action isEqualToString:@"discard"]) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"%@已经结束"), callMode]];
            }
        } else {
            if ([actionModel.action isEqualToString:@"new"]) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"群%@开始了"), callMode]];
            } else if ([actionModel.action isEqualToString:@"discard"]) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"%@已经结束"), callMode]];
            }
        }
...
+ (NSMutableAttributedString *)dealChatMessageForNetCallMessage:(LingIMChatMessageModel *)chatMessage{
    NSMutableAttributedString *sessionAttStr = [[NSMutableAttributedString alloc] initWithString:@""];
    NSString *callType;
    if (chatMessage.netCallType == 1) {
        //语音通话
        callType = MultilingualTranslation(@"语音通话");
    }else {
        //视频通话
        callType = MultilingualTranslation(@"视频通话");
    }
    ...
    if (chatMessage.netCallChatType == 1) {
        ...
        if (groupMemberModel) {
            if (chatMessage.netCallStatus == 1) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"%@发起了%@"), groupMemberModel.showName, callType]];
            } else if (chatMessage.netCallStatus == 11) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"%@已经结束"), callType]];
            }
        } else {
            if (chatMessage.netCallStatus == 1) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"群%@开始了"), callType]];
            } else if (chatMessage.netCallStatus == 11) {
                sessionAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:MultilingualTranslation(@"%@已经结束"), callType]];
            }
        }
...
+ (NSMutableAttributedString *)getSessionLatestMassMessageAttributedStringWith:(LingIMSessionModel *)sessionModel {
    ...
    if (massMessage) {
        if (massMessage.mtype == 0) {
            //文本
            sessionAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:massMessage.bodyModel.content imageRect:CGRectMake(0, -4, 18, 18)];
            [self configAttributedString:sessionAttStr fontNum:12];
...
+ (NSMutableAttributedString *)getMessageRecordAttributedStringWith:(IMChatMessage * _Nullable)imChatMessage {
    ...
    if (imChatMessage) {
        if (imChatMessage.mType == IMChatMessage_MessageType_TextMessage) {
            recordMsgAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:[NSString stringWithFormat:@"%@：%@", imChatMessage.nick, imChatMessage.textMessage.content] imageRect:CGRectMake(0, -4, 18, 18)];
            [self configAttributedString:recordMsgAttStr fontNum:10];
        } else if (imChatMessage.mType == IMChatMessage_MessageType_ImageMessage){
            recordMsgAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@", imChatMessage.nick, MultilingualTranslation(@"[图片]")]];
            [self configAttributedString:recordMsgAttStr fontNum:10];
        } else if (imChatMessage.mType == IMChatMessage_MessageType_StickersMessage){
            recordMsgAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@", imChatMessage.nick, MultilingualTranslation(@"[表情]")]];
            [self configAttributedString:recordMsgAttStr fontNum:10];
            ...
            recordMsgAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@", imChatMessage.nick, MultilingualTranslation(@"[语音]")]];
            [self configAttributedString:recordMsgAttStr fontNum:10];
        } else if (imChatMessage.mType == IMChatMessage_MessageType_FileMessage){
            recordMsgAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@", imChatMessage.nick, MultilingualTranslation(@"[文件]")]];
            [self configAttributedString:recordMsgAttStr fontNum:10];
        } else if (imChatMessage.mType == IMChatMessage_MessageType_VideoMessage) {
            recordMsgAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@", imChatMessage.nick, MultilingualTranslation(@"[视频]")]];
            [self configAttributedString:recordMsgAttStr fontNum:10];
        } else if (imChatMessage.mType == IMChatMessage_MessageType_CardMessage) {
            recordMsgAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@%@", imChatMessage.nick, MultilingualTranslation(@"[个人名片]"), imChatMessage.cardMessage.name]];
            [self configAttributedString:recordMsgAttStr fontNum:10];
        } else if (imChatMessage.mType == IMChatMessage_MessageType_Geomessage) {
            recordMsgAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@%@", imChatMessage.nick, MultilingualTranslation(@"[位置]"), imChatMessage.geoMessage.name]];
            [self configAttributedString:recordMsgAttStr fontNum:10];
        } else if (imChatMessage.mType == IMChatMessage_MessageType_ForwardMessage) {
            recordMsgAttStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@：%@%@", imChatMessage.nick, MultilingualTranslation(@"[会话记录]"), imChatMessage.forwardMessage.title]];
            [self configAttributedString:recordMsgAttStr fontNum:10];
            ...
            for (NSString *atKey in atKeyArr) {
                if ([atKey isEqualToString:UserManager.userInfo.userUID]) {
                    if ([imChatMessage.from isEqualToString:UserManager.userInfo.userUID]) {
                        showContent = [showContent stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"\v%@\v", atKey] withString:MultilingualTranslation(@"@我自己")];
                    } else {
                        showContent = [showContent stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"\v%@\v", atKey] withString:MultilingualTranslation(@"@我")];
                    }
                } else if ([atKey isEqualToString:@"-1"]) {
                    showContent = [showContent stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"\v%@\v", atKey] withString:MultilingualTranslation(@"@所有人")];
                } else {
            ...
            recordMsgAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:[NSString stringWithFormat:@"%@：%@", imChatMessage.nick, atContetnStr] imageRect:CGRectMake(0, -4, 18, 18)];
            [self configAttributedString:recordMsgAttStr fontNum:10];
        }
    } else {
        recordMsgAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[未知]")];
        [self configAttributedString:recordMsgAttStr fontNum:10];
    }
...
+ (NSMutableAttributedString *)getSessionDraftContentAttributedStringWith:(LingIMSessionModel *)sessionModel {
    ...
    if (sessionModel.draftDict.count > 0) {
        ...
        sessionAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:draftContent imageRect:CGRectMake(0, -4, 18, 18)];
        [self configAttributedString:sessionAttStr fontNum:12];
        
        NSMutableAttributedString *atStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[草稿]")];
        NSDictionary *dict = @{NSFontAttributeName:FONTR(12), NSForegroundColorAttributeName:COLOR_F93A2F};
        [atStr addAttributes:dict range:NSMakeRange(0, atStr.length)];
        [sessionAttStr insertAttributedString:atStr atIndex:0];
    }else {
