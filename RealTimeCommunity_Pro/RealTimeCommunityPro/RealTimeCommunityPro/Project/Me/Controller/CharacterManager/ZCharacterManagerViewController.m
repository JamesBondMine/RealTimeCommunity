//
//  ZCharacterManagerViewController.m
//  CIMKit
//
...
#import "ZCharacterRegisterViewController.h"//注册账户
#import "ZCharacterBindViewController.h"//绑定账户
#import <DZNEmptyDataSet/UIScrollView+EmptyDataSet.h>

@interface ZCharacterManagerViewController ()
//未绑定
...
@implementation ZCharacterManagerViewController
...
- (void)setupUnBindUI {
    ...
    characterImageView.image = ImgNamed(@"remi_character_user");
    ...
    tipLabel.text = MultilingualTranslation(@"您还未绑定翻译账户，请完成绑定");
    tipLabel.font = FONTR(16);
    tipLabel.textAlignment = NSTextAlignmentCenter;
    tipLabel.numberOfLines = 0;
    tipLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (void)setupBindedUI {
    ...
    accountBackView.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    ...
    accountBackTitleLbl.text = MultilingualTranslation(@"翻译账户信息");
    accountBackTitleLbl.font = FONTB(16);
    accountBackTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    nameTitleLab.text = MultilingualTranslation(@"用户名称：");
    nameTitleLab.font = FONTN(14);
    nameTitleLab.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _nameContentLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    accountTypeTitleLab.text = MultilingualTranslation(@"账号类型：");
    accountTypeTitleLab.font = FONTN(14);
    accountTypeTitleLab.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _accountTypeContentLbl.text = MultilingualTranslation(@"普通账号");
    _accountTypeContentLbl.font = FONTN(14);
    _accountTypeContentLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    usedCharBackView.image = ImgNamed(@"remi_img_char_used_back");
    ...
    usedChartTitleLbl.text = MultilingualTranslation(@"已用字符");
    usedChartTitleLbl.font = FONTN(14);
    usedChartTitleLbl.textAlignment = NSTextAlignmentCenter;
    usedChartTitleLbl.tkThemetextColors = @[COLORWHITE, COLOR_33];
    ...
    _usedChartContentLbl.tkThemetextColors = @[COLORWHITE, COLOR_33];
    ...
    remainCharBackView.image = ImgNamed(@"remi_img_char_remain_back");
    ...
    remainChartTitleLbl.text = MultilingualTranslation(@"剩余字符");
    remainChartTitleLbl.font = FONTN(14);
    remainChartTitleLbl.textAlignment = NSTextAlignmentCenter;
    remainChartTitleLbl.tkThemetextColors = @[COLORWHITE, COLOR_33];
    ...
    _remainChartContentLbl.tkThemetextColors = @[COLORWHITE, COLOR_33];
...
- (void)charactersManagetRegisterAction {
    //注册账户
    ZCharacterRegisterViewController *vc = [[ZCharacterRegisterViewController alloc] init];
    vc.isBinded = NO;
    vc.isFromBind = NO;
...
//增加字符
- (void)addChartAction {
    [HUD showMessage:MultilingualTranslation(@"请联系客服")];
}
...
- (NSString *)transitionAccountTypeWithGrade:(NSInteger)grade {
    switch (grade) {
        case 1:
            return MultilingualTranslation(@"普通账号");
            break;
        case 2:
            return MultilingualTranslation(@"企业账号");
            break;
        case 3:
            return MultilingualTranslation(@"企业子账号");
            break;
            
        default:
            return MultilingualTranslation(@"未知");;
            break;
    }
}
...
- (UIView *)unBindBackView {
    if (!_unBindBackView) {
        ...
        _unBindBackView.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
    }
...
- (UIView *)bindedBackView {
    if (!_bindedBackView) {
        ...
        _bindedBackView.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
    }
