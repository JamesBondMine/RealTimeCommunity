...
@implementation LingIMZGCallManager
#pragma mark - 单例>>>>>>
+ (instancetype)sharedManager {
    static LingIMZGCallManager *_manager = nil;
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    return [LingIMZGCallManager sharedManager];
}
// 防止外部调用copy
...
#pragma mark - <<<<<<方法与属性>>>>>>
#pragma mark - SDK基础配置
- (void)configSDKWith:(LingIMZGCallConfig *)config {
    if (config) {
...
- (void)callRoomCreateEngineWithOptions:(LingIMZGCallOptions *)callOptions delegate:(id <ZegoEventHandler>)roomDelegate {
    if (callOptions) {
