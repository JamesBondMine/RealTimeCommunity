...
@interface NSMiniAppWebVC ()
@property (nonatomic, strong) UIButton *btnClose;
@property (nonatomic, strong) UIButton *btnMore;
@end

@implementation NSMiniAppWebVC
...
- (void)setupNavRightUI {
    ...
    [_btnMore setImage:ImgNamed(@"remini_mini_app_more") forState:UIControlStateNormal];
    ...
    [_btnClose setImage:ImgNamed(@"remini_mini_app_close") forState:UIControlStateNormal];
...
- (void)showMoreView {
    WeakSelf
    ZPresentItem *browserItem = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"在浏览器打开") textColor:COLOR_33 font:FONTR(16) itemHeight:DWScale(56) backgroundColor:COLORWHITE];
    ZPresentItem *floatItem = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"浮窗") textColor:COLOR_33 font:FONTR(16) itemHeight:DWScale(56) backgroundColor:COLORWHITE];
    ZPresentItem *copyItem = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"复制链接") textColor:COLOR_33 font:FONTR(16) itemHeight:DWScale(56) backgroundColor:COLORWHITE];
    ZPresentItem *cancelItem = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"取消") textColor:COLOR_99 font:FONTR(16) itemHeight:DWScale(56) backgroundColor:COLORWHITE];
    
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        if (themeIndex == 0) {
            browserItem.textColor = COLOR_33;
            browserItem.backgroundColor = COLORWHITE;
            floatItem.textColor = COLOR_33;
            floatItem.backgroundColor = COLORWHITE;
            copyItem.textColor = COLOR_33;
            ...
            browserItem.backgroundColor = COLOR_33;
            floatItem.textColor = COLORWHITE;
            floatItem.backgroundColor = COLOR_33;
            copyItem.textColor = COLORWHITE;
            copyItem.backgroundColor = COLOR_33;
            cancelItem.textColor = COLOR_99;
    ...
    ZPresentView *viewAlert = [[ZPresentView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DScreenHeight) titleItem:nil selectItems:moreItemArray cancleItem:cancelItem doneClick:^(NSInteger index) {
        //具体的功能实现
...
- (void)goFloatView {
    if (![NSString isNil:self.webViewUrl]) {
        ...
        if (!appDelegate.viewFloatMiniApp) {
            //当前没有小程序浮窗，创建
            SpecMiniAppFloatView *viewFloat = [[SpecMiniAppFloatView alloc] initWithFrame:CGRectMake(0, DWScale(100), DWScale(50), DWScale(50))];
            viewFloat.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];;
            ...
            viewFloat.imageView.image = ImgNamed(@"remini_mini_app_float");
...
- (void)checkNavControllers {
    ...
    [vcList enumerateObjectsUsingBlock:^(UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (idx == vcList.count - 1) {
            ...
            if (![obj isKindOfClass:[NSMiniAppWebVC class]]) {
                [newList addObjectIfNotNil:obj];
            }
        }
    }];
