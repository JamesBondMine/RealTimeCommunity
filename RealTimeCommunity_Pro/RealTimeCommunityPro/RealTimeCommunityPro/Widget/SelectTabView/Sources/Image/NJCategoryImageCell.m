...
#import "NJCategoryImageCell.h"
#import "NJCategoryImageCellModel.h"

@interface NJCategoryImageCell()
@property (nonatomic, strong) id currentImageInfo;
@property (nonatomic, strong) NSString *currentImageName;
@property (nonatomic, strong) NSURL *currentImageURL;
@end

@implementation NJCategoryImageCell
...
- (void)layoutSubviews {
    ...
    NJCategoryImageCellModel *myCellModel = (NJCategoryImageCellModel *)self.cellModel;
...
- (void)reloadData:(NJCategoryBaseCellModel *)cellModel {
    ...
    //因为`- (void)reloadData:(NJCategoryBaseCellModel *)cellModel`方法会回调多次，尤其是左右滚动的时候会调用无数次，如果每次都触发图片加载，会非常消耗性能。所以只会在图片发生了变化的时候，才进行图片加载。
