//
//  ZCharacterRegisterViewController.m
//  CIMKit
//
//  Created by cusPro on 2023/9/15.
//

#import "ZCharacterRegisterViewController.h"
#import "ZCharManagerInput.h"
#import "ZCharacterManagerViewController.h"

@interface ZCharacterRegisterViewController ()

@property (nonatomic, strong) ZCharManagerInput *userNameInput;
@property (nonatomic, strong) ZCharManagerInput *passwordInput;
@property (nonatomic, strong) ZCharManagerInput *confimPWDInput;
@property (nonatomic, strong) UIButton *registerBtn;

@end

@implementation ZCharacterRegisterViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.view.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_F5F6F9_DARK];
    self.navTitleStr = MultilingualTranslation(@"注册账户");
    [self setupUI];
}
...
- (void)checkRegisterBtnAvailable {
    if (self.userNameInput.textLength > 0 && self.passwordInput.textLength > 0 && self.confimPWDInput.textLength > 0) {
        ...
        self.registerBtn.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF_DARK];
    } else {
        self.registerBtn.enabled = NO;
        self.registerBtn.tkThemebackgroundColors = @[[COLOR_4791FF colorWithAlphaComponent:0.3], [COLOR_4791FF_DARK colorWithAlphaComponent:0.3]];
    }
}
    ...
    [IMSDKManager imSdkTranslateRegisterBindAccount:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (weakSelf.isFromBind) {
            for (UIViewController * vc in weakSelf.navigationController.viewControllers) {
                if([vc isKindOfClass:[ZCharacterManagerViewController class]]){
                    ZCharacterManagerViewController *managerVC = (ZCharacterManagerViewController *)vc;
...
#pragma mark - Lazy
- (ZCharManagerInput *)userNameInput {
    if (!_userNameInput) {
        _userNameInput = [[ZCharManagerInput alloc] init];
        _userNameInput.leftTitleStr = MultilingualTranslation(@"用户名");
        _userNameInput.placeholderText = MultilingualTranslation(@"请输入用户名");
...
- (ZCharManagerInput *)passwordInput {
    if (!_passwordInput) {
        _passwordInput = [[ZCharManagerInput alloc] init];
        _passwordInput.clipsToBounds = YES;
        _passwordInput.leftTitleStr = MultilingualTranslation(@"密码");
...
- (ZCharManagerInput *)confimPWDInput {
    if (!_confimPWDInput) {
        _confimPWDInput = [[ZCharManagerInput alloc] init];
        _confimPWDInput.leftTitleStr = MultilingualTranslation(@"请再次输入密码");
        _confimPWDInput.placeholderText = MultilingualTranslation(@"请输入密码");
...
- (UIButton *)registerBtn {
    if (!_registerBtn) {
        ...
        [_registerBtn setTitle:MultilingualTranslation(@"登录") forState:UIControlStateNormal];
        [_registerBtn setTitleColor:COLORWHITE forState:UIControlStateNormal];
        _registerBtn.enabled = NO;
        _registerBtn.tkThemebackgroundColors = @[[COLOR_4791FF colorWithAlphaComponent:0.3], [COLOR_4791FF_DARK colorWithAlphaComponent:0.3]];
