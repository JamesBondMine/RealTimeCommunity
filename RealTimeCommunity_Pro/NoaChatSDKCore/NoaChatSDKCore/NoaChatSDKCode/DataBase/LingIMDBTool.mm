...
@implementation LingIMDBTool
#pragma mark - 单例
+ (instancetype)sharedTool {
    static LingIMDBTool *_tool = nil;
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    return [LingIMDBTool sharedTool];
}
// 防止外部调用copy
...
- (BOOL)configDBWith:(NSString *)userToken userID:(NSString *)userID {
    ...
    if (userID.length > 0 && userToken.length > 0) {
        ...
        if (result) {
            //用户表
            [self createTableWithName:CIMDBUserInfoTableName model:LingIMUserModel.class];
            //好友表
            [self createTableWithName:CIMDBFriendTableName model:LingIMFriendModel.class];
            //群组表
            [self createTableWithName:CIMDBGroupTableName model:LingIMGroupModel.class];
            //会话表
            [self createTableWithName:CIMDBSessionTableName model:LingIMSessionModel.class];
            //好友分组表
            [self createTableWithName:CIMDBFriendGroupTableName model:LingIMFriendGroupModel.class];
            //小程序快应用表
            [self createTableWithName:CIMDBFloatMiniAppTableName model:LingFloatMiniAppModel.class];
        }
...
- (BOOL)createDB {
    ...
    //这行代码 很重要！
    //---------begin----------//
    NSString *dbPath = [documentPath stringByAppendingString:[NSString stringWithFormat:@"/CIMSDK/DB/%@/%@",_userID,CIMDBName]];
    //---------end----------//
    CIMLog(@"数据库地址:%@",dbPath);
    self.cimDB = [[WCTDatabase alloc] initWithPath:dbPath];
    // 使用更安全的加密密钥生成方式
    ...
    [self.cimDB setCipherKey:keyData];
    
    //数据库标记，默认0.(具有相同路径的WCTCore对象共享这个标签，即使他们不是同一个对象)
    self.cimDB.tag = 2000;
    
    // 尝试打开数据库
    if ([self.cimDB canOpen]) {
        CIMLog(@"数据库打开成功");
        return YES;
    } else {
...
- (BOOL)createTableWithName:(NSString *)tableName model:(Class)model {
    ...
    BOOL createResult = [self.cimDB createTable:tableName withClass:model];
...
- (BOOL)isTableStateOkWithName:(NSString *)tableName model:(Class)model {
    //数据库表升级，调用此方法用于自检
    return [self createTableWithName:tableName model:model];
    //此方法，可以单独处理一下，根据本地所有的表来统一执行，但是目前还没有这样做0.0
}
...
- (BOOL)insertModelToTable:(NSString *)tableName model:(id)model {
    ...
    BOOL result = [self.cimDB insertOrReplaceObject:model intoTable:tableName];
    if (result) {
...
- (BOOL)insertMulitModelToTable:(NSString *)tableName modelClass:(id)modelClass list:(NSArray *)list {
    ...
    BOOL result = [self.cimDB insertOrReplaceObjects:list intoTable:tableName];
    if (result) {
        //CIMLog(@"表：%@批量新增数据成功",tableName);
    }else {
        //CIMLog(@"表：%@批量新增数据失败",tableName);
    }
    return result;
}
...
- (BOOL)dropTableWithName:(NSString *)tableName {
    return [self.cimDB dropTable:tableName];
}
...
- (dispatch_queue_t)groupMemberUpdateQueue {
    if (!_groupMemberUpdateQueue) {
        _groupMemberUpdateQueue = dispatch_queue_create("com.CIMSDKCode.groupMemberUpdateQueue", DISPATCH_QUEUE_CONCURRENT);
    }
