...
@property (nonatomic, strong) NSMutableArray <ZFileUploadTask *> * taskCache;
...
SharedInstance(ZFileUploadManager)
    ...
    if (task.uploadType == ZHttpUploadTypeFile) {
        ...
        if ([UserManager.userRoleAuthInfo.upFile.configValue isEqualToString:@"false"]) {
            ...
            if (task.dataLength > 1024 * 1024 * [UserManager.userRoleAuthInfo.upFile.configData integerValue]) {
                NSString *tipsStr = [NSString stringWithFormat: MultilingualTranslation(@"最大可发送%@MB文件"), UserManager.userRoleAuthInfo.upFile.configData];
    ...
    if (task.uploadType == ZHttpUploadTypeImage || task.uploadType == ZHttpUploadTypeImageThumbnail || task.uploadType == ZHttpUploadTypeVideo) {
        ...
        if ([UserManager.userRoleAuthInfo.upImageVideoFile.configValue isEqualToString:@"false"]) {
            ...
            if (task.dataLength > 1024 * 1024 * [UserManager.userRoleAuthInfo.upImageVideoFile.configData integerValue]) {
                NSString *tipsStr = [NSString stringWithFormat: MultilingualTranslation(@"最大可发送%@MB文件"), UserManager.userRoleAuthInfo.upImageVideoFile.configData];
                [HUD showMessage:tipsStr];
                return;
            }
        }
...
- (ZFileUploadTask *)getTaskWithId:(NSString *)taskId{
    ZFileUploadTask * uploadTask;
       for (ZFileUploadTask * task in self.operationQueue.operations) {
           if ([task isKindOfClass:[ZFileUploadTask class]] &&[task.taskId isEqualToString:taskId]) {
               uploadTask = task;
           }
       }
       if(uploadTask == nil){
           for (ZFileUploadTask * task in self.taskCache) {
               if ([task isKindOfClass:[ZFileUploadTask class]] &&[task.taskId isEqualToString:taskId]) {
                   uploadTask = task;
               }
           }
...
#pragma mark - minio上传
- (void)minioUploadWithTask:(ZFileUploadTask *)task {
...
//阿里云/亚马逊云/腾讯云 上传
- (void)thirdOssUploadWithTask:(ZFileUploadTask *)task {
    if(![self.operationQueue.operations containsObject:task]){
...
-(NSMutableArray<ZFileUploadTask *> *)taskCache{
    if (_taskCache == nil) {
