...
@implementation ZMessageSendHander
...
+ (LingIMChatMessageModel *)ZMessageTextSend:(NSString *)textMessage withToUserId:(NSString *)to withChatType:(CIMChatType)chatType referenceMsgId:(NSString *)referenceMsgId {
    LingIMChatMessageModel *textChatMessage = [LingIMChatMessageModel new];
    ...
    textChatMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
...
+ (LingIMChatMessageModel *)ZMessageAtUserSend:(NSString *)content showContent:(NSString *)showContent withAtUsersDicList:(NSArray *)atUsersDicList withToUserId:(NSString *)to withChatType:(CIMChatType)chatType referenceMsgId:(NSString *)referenceMsgId {
    LingIMChatMessageModel *atChatMessage = [LingIMChatMessageModel new];
    ...
    atChatMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
    ...
    for (int i = 0; i < mediaAssetArr.count; ++i) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            if (mediaAsset.mediaType == PHAssetMediaTypeImage) {
                ...
                [ZMessageSendHander assembleImageMessageWithAsset:mediaAsset withToUserId:to withChatType:chatType compelete:^(LingIMChatMessageModel *sendChatMsg) {
                    if (sendChatMsg) {
                ...
                currentSendTime =  [NSDate currentTimeIntervalWithMillisecond];
                [ZMessageSendHander assembleVideoMessageWithAsset:mediaAsset withToUserId:to withChatType:chatType compelete:^(LingIMChatMessageModel *sendChatMsg) {
                    if (sendChatMsg) {
                        sendChatMsg.sendTime = currentSendTime;
                        [dic setValue:sendChatMsg forKey:[NSString stringWithFormat:@"%i",i]];
                    }
...
#pragma mark - 组装图片消息
+ (void)assembleImageMessageWithAsset:(PHAsset *)mediaAsset withToUserId:(NSString *)to withChatType:(CIMChatType)chatType compelete:(void(^)(LingIMChatMessageModel * sendChatMsg))compelete {
    //发送图片
...
#pragma mark - 组装视频消息
+ (void)assembleVideoMessageWithAsset:(PHAsset *)mediaAsset withToUserId:(NSString *)to withChatType:(CIMChatType)chatType compelete:(void(^)(LingIMChatMessageModel * sendChatMsg))compelete {
    //发送视频
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:mediaAsset options:options resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
        ...
        if ([asset isKindOfClass:[AVURLAsset class]]) {
            ...
            if (videoData.length > (200 * 1024 * 1024)) {
                ...
                LingIMChatMessageModel *videoChatMessage = [LingIMChatMessageModel new];
                ...
                videoChatMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
...
+ (void)ZMessageVoiceSend:(NSString *)voicePath fileName:(NSString *)fileName voiceDuring:(float)voiceDuring withToUserId:(NSString *)to withChatType:(CIMChatType)chatType compelete:(void(^)(LingIMChatMessageModel * sendChatMsg))compelete {
    //发送语音
    LingIMChatMessageModel *voiceChatMessage = [LingIMChatMessageModel new];
    ...
    voiceChatMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
...
+ (void)ZMessageLocationSendWithLng:(NSString *)geoLng lat:(NSString *)geoLat name:(NSString *)geoName cImg:(UIImage *)geoImg detail:(NSString *)geoDetail withToUserId:(NSString *)to withChatType:(CIMChatType)chatType compelete:(void(^)(LingIMChatMessageModel * sendChatMsg))compelete {
    ...
    LingIMChatMessageModel *geoChatMessage = [LingIMChatMessageModel new];
    ...
    geoChatMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
...
+ (void)ZMessageAssembleQRcodeImage:(UIImage *)qrImage compelete:(void(^)(LingIMChatMessageModel * sendChatMsg))compelete {
    ...
    //组装消息体
    LingIMChatMessageModel *qrImgChatMessage = [LingIMChatMessageModel new];
    qrImgChatMessage.messageSendType = CIMChatMessageSendTypeSending;
    qrImgChatMessage.messageStatus = 1;  //正常状态
    qrImgChatMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
...
+ (void)ZMessageMergeForwardSendWith:(NSArray *)multiSelectedItmeArr withTitle:(NSString *)title withToUserInfoArr:(NSArray *)userInfoArr compelete:(void(^)(NSArray <LingIMChatMessageModel *> *sendChatMsgList))compelete {
    ...
    for ( NSDictionary *dict in userInfoArr) {
        ...
        for (ZMessageModel *selectedMsgModel in multiSelectedItmeArr) {
            if (selectedMsgModel.isSelf) {
                if (selectedMsgModel.message.messageType == CIMChatMessageType_AtMessage) {
                    selectedMsgModel.message.textContent = [ZMessageTools forwardMessageAtContenTranslateToShowContent:selectedMsgModel.message.atContent atUsersDictList:selectedMsgModel.message.atUsersInfoList];
                ...
                if (selectedMsgModel.message.messageType == CIMChatMessageType_AtMessage) {
                    selectedMsgModel.message.textContent = [ZMessageTools forwardMessageAtContenTranslateToShowContent:![NSString isNil:selectedMsgModel.message.atTranslateContent] ? selectedMsgModel.message.atTranslateContent : selectedMsgModel.message.atContent atUsersDictList:selectedMsgModel.message.atUsersInfoList];
                    selectedMsgModel.message.messageType = CIMChatMessageType_TextMessage;
                } else {
            ...
            IMChatMessage *imChatMessage = [ZMessageTools getIMChatMessageFromLingIMChatMessageModelToMergeForward:selectedMsgModel.message];
            [mergeMsgList addObject:imChatMessage];
        }
        
        LingIMChatMessageModel *recordChatMessage = [LingIMChatMessageModel new];
        ...
        recordChatMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
...
+ (LingIMChatMessageModel *)ZMessageStickersSendContentUrl:(NSString *)stickersImgUrl stickerThumbImgUrl:(NSString *)stickerThumbImgUrl stickerId:(NSString *)stickerId stickerName:(NSString *)stickerName stickerHeight:(float)stickerHeight stickerWidth:(float)stickerWidth stickerSize:(long long)stickerSize isStickersSet:(BOOL)isStickersSet stickerExt:(NSString *)stickerExt withToUserId:(NSString *)to withChatType:(CIMChatType)chatType {
    ...
    LingIMChatMessageModel *stickersChatMessage = [LingIMChatMessageModel new];
    ...
    stickersChatMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
...
+ (LingIMChatMessageModel *)ZMessageGameStickersSendResultContent:(NSString *)resultContent gameStickersType:(ZChatGameStickerType)gameStickersType gameStickerExt:(NSString *)gameStickerExt withToUserId:(NSString *)to withChatType:(CIMChatType)chatType {
    ...
    LingIMChatMessageModel *gameStickersChatMessage = [LingIMChatMessageModel new];
    ...
    gameStickersChatMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
...
+ (LingIMChatMessageModel *)ZMessageReadedWithMsgSidList:(NSArray *)readedMsgSidList withToUserId:(NSString *)to withChatType:(CIMChatType)chatType {
    ...
    LingIMChatMessageModel *haveReadMessage = [LingIMChatMessageModel new];
    ...
    haveReadMessage.msgID = [[LingIMManagerTool sharedManager] getMessageID];
