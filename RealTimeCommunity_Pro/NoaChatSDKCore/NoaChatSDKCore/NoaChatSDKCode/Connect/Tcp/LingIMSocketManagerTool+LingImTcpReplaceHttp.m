//
//  LingIMSocketManagerTool+LingImTcpReplaceHttp.m
//  NoaChatSDKCore
//
...
@implementation LingIMSocketManagerTool (LingImTcpReplaceHttp)
...
- (void)authRefreshTokenExpiredWithSuccessFunc:(LingTcpRequestSuccessCallback)successCallBack
    ...
    [LingIMTcpRequestModel sendTcpRequestWithParam:params Url:Auth_Refresh_Token_Url Method:LingRequestPost SuccessFunc:^(id  _Nullable data, NSString * _Nullable traceId) {
        ...
        if(newToken.length == 0 || newToken == nil) {
            if ([self.userDelegate respondsToSelector:@selector(cimSdkRefreshUsetToken: errorMsg:)]) {
                [self.userDelegate cimSdkRefreshUsetToken:@"" errorMsg:nil];
            }
            ...
            if ([self.userDelegate respondsToSelector:@selector(cimSdkRefreshUsetToken: errorMsg:)]) {
                [self.userDelegate cimSdkRefreshUsetToken:newToken errorMsg:@""];
            }
        ...
        if ([self.userDelegate respondsToSelector:@selector(cimSdkRefreshUsetToken: errorMsg:)]) {
            [self.userDelegate cimSdkRefreshUsetToken:@"" errorMsg:msg];
        }
        ...
        if (code == Auth_User_Account_Banned || code == Auth_User_Device_Banned || code == Auth_User_IPAddress_Banned) {
            if ([self.userDelegate respondsToSelector:@selector(cimSdkRefreshTokenAuthBanned:)]) {
                [self.userDelegate cimSdkRefreshTokenAuthBanned:code];
            }
            return;
        }
