//
//  MGGestureLockCheckAccountPasswordView.m
//  CIMKit
//
...
@interface MGGestureLockCheckAccountPasswordView ()
...
@implementation MGGestureLockCheckAccountPasswordView
...
- (void)setupUI {
    ...
    _lblTitle.text = MultilingualTranslation(@"请输入账户密码");
    ...
    _tfPassword.placeholder = MultilingualTranslation(@"请输入账户密码");
    ...
    [_btnEye setImage:ImgNamed(@"relogimg_icon_eye_off_reb") forState:UIControlStateNormal];
    [_btnEye setImage:ImgNamed(@"relogimg_icon_eye_on_reb") forState:UIControlStateSelected];
    _btnEye.selected = NO;
    _btnEye.hidden = YES;
...
- (void)requestCheckUserPassword:(NSString *)encryptKey {
    ...
    [IMSDKManager userCheckUserPasswordWith:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (checkResult) {
            ...
            self.lblTitle.text = MultilingualTranslation(@"验证账户密码失败");
