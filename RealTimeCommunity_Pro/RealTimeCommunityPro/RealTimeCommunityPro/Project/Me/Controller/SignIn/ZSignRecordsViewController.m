//
//  ZSignRecordsViewController.m
//  CIMKit
//
//  Created by Apple on 2023/8/9.
//

#import "ZSignRecordsViewController.h"
#import "ZSignRecordsTableViewCell.h"
#import "ZFDatePickerView.h"
#import <DZNEmptyDataSet/UIScrollView+EmptyDataSet.h>

@interface ZSignRecordsViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong)NSArray *dataArr;
@property (nonatomic,strong) UIButton * downButtonMenu;
@property(nonatomic,strong) UILabel * getTotlePointLabel;
@end

@implementation ZSignRecordsViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
    self.navTitleStr = MultilingualTranslation(@"签到记录");
    [self setupTableView];
...
- (void)setupTableView {
    ...
    [downButtonMenu setImage:ImgNamed(@"remi_signDownBtn") forState:UIControlStateNormal];
    ...
    [downButtonMenu setTkThemeTitleColor:@[COLOR_33, COLOR_99] forState:UIControlStateNormal];
    ...
    getTotlePointLabel.text = [NSString stringWithFormat:MultilingualTranslation(@"累计获得：%@积分"),self.totalLoyalty];//@"累计获得：234积分";
    ...
    signTimeLabel.text = MultilingualTranslation(@"签到时间");
    ...
    todaySignPointLabel.text = MultilingualTranslation(@"日签积分");
    ...
    todayRewardsPointLabel.text = MultilingualTranslation(@"奖励积分");
    ...
    todayTotlePointLabel.text = MultilingualTranslation(@"日总积分");
    ...
    [self.baseTableView registerClass:[ZSignRecordsTableViewCell class] forCellReuseIdentifier:NSStringFromClass([ZSignRecordsTableViewCell class])];
}
-(void)selectDataPicker{
    WeakSelf;
    ZFDatePickerView *datePickerView = [[ZFDatePickerView alloc]initDatePackerWithSUperView:self.view response:^(NSString *str,NSString* selectYear,NSString* selectMonth) {
        NSString *string = str;
...
- (void)requestSignWithRecord:(NSString*)year selectMonth:(NSString*)month
{
    ...
    [IMSDKManager imSignInWithRecord:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            weakSelf.getTotlePointLabel.text = [NSString stringWithFormat:MultilingualTranslation(@"累计获得：%@积分"),weakSelf.totalLoyalty];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZSignRecordsTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ZSignRecordsTableViewCell class]) forIndexPath:indexPath];
