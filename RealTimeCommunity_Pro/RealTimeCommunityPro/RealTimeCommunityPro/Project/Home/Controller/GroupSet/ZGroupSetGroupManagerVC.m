...
@interface ZGroupSetGroupManagerVC ()<MainSearchViewDelegate,UITableViewDataSource,UITableViewDelegate,ZBaseCellDelegate>
...
@implementation ZGroupSetGroupManagerVC
...
#pragma mark - 界面布局
- (void)setupNavUI {
    self.navTitleStr = MultilingualTranslation(@"设置管理员");
...
- (void)navBtnRightClicked {
    ...
    [IMSDKManager groupSetGroupManagerWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if (data) {
            ...
            [HUD showMessage:MultilingualTranslation(@"设置群管理员成功")];
        }
