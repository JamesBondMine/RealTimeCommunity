...
@interface SpecMiniAppFloatListCell ()
@property (nonatomic, strong) UIImageView *ivLogo;
@property (nonatomic, strong) UILabel *lblTitle;
@property (nonatomic, strong) UIButton *btnDelete;
@end

@implementation SpecMiniAppFloatListCell
...
- (void)setupUI {
    ...
    [_btnDelete setImage:ImgNamed(@"remini_mini_app_delete_black") forState:UIControlStateNormal];
...
-(void)setFloatMiniAppModel:(LingFloatMiniAppModel *)floatMiniAppModel{
    if (floatMiniAppModel) {
        _floatMiniAppModel = floatMiniAppModel;
        __weak typeof(self) weakSelf = self;
        [_ivLogo sd_setImageWithURL:[floatMiniAppModel.headerUrl getImageFullUrl] placeholderImage:ImgNamed(@"remini_mini_app_icon") options:SDWebImageAllowInvalidSSLCertificates completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
            if (!image) {
                strongSelf.ivLogo.image = ImgNamed(@"remini_mini_app_icon");
            }
        }];
