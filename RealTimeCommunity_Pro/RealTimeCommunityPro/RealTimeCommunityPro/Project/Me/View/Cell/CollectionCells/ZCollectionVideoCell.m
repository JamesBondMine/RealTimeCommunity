...
@interface ZCollectionVideoCell()
...
@implementation ZCollectionVideoCell
...
- (void)setupUI {
    ...
    [self.contentImgView addSubview:self.playImgView];
...
#pragma mark - Model
- (void)setModel:(ZMyCollectionModel *)model {
    _model = model;
    
    WeakSelf;
    [self.contentImgView sd_setImageWithURL:[_model.itemModel.body.cImg getImageFullUrl] placeholderImage:[UIImage imageCompressFitSizeScale:DefaultImage targetSize:CGSizeMake(_model.itemWidth, _model.itemHeight)] options:SDWebImageAllowInvalidSSLCertificates progress:nil completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        StrongSelf;
        [strongSelf loadWithImage:image URL:[model.itemModel.body.cImg getImageFullString] error:error];
    }];
...
- (UIImageView *)playImgView {
    if (!_playImgView) {
        ...
        _playImgView.image = ImgNamed(@"remsg_icon_video_msg_play");
    }
    return _playImgView;
}
