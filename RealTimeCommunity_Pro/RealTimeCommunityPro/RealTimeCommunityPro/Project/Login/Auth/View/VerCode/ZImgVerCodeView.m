...
#import "ZGraphCodeView.h"//图形验证码

@interface ZImgVerCodeView()

@property (nonatomic, strong) UIView *bgView;

@property (nonatomic, strong)ZGraphCodeView *imgCodeView;

@property (nonatomic, strong)UITextField *imgCodeInput;

@end

@implementation ZImgVerCodeView
...
- (void)setupUI{
    ...
    titleLab.text = MultilingualTranslation(@"验证码");
    ...
    [relaodBtn setTitle:MultilingualTranslation(@"换一张") forState:UIControlStateNormal];
    ...
    cancelBtn.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    ...
    [sureBtn setTitle:MultilingualTranslation(@"确认") forState:UIControlStateNormal];
    [sureBtn setTkThemeTitleColor:@[COLOR_81D8CF, COLOR_81D8CF_DARK] forState:UIControlStateNormal];
    ...
    sureBtn.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
...
- (void)sureBtnAction {
    if (![NSString isNil:self.imgCodeInput.text]) {
        if ([self.imgCodeStr isEqualToString:[self.imgCodeInput.text lowercaseString]]) {
            ...
            [self getImgCodeAction];
            [HUD showMessage:MultilingualTranslation(@"验证码不正确，请重新输入")];
        }
...
- (ZGraphCodeView *)imgCodeView {
    if (!_imgCodeView) {
        _imgCodeView = [[ZGraphCodeView alloc] init];
...
- (UITextField *)imgCodeInput {
    if (!_imgCodeInput) {
        ...
        _imgCodeInput.tkThemetextColors = @[COLOR_33, COLORWHITE];
        _imgCodeInput.font = FONTN(13);
        _imgCodeInput.placeholder = MultilingualTranslation(@"请输入验证码");
