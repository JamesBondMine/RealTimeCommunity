//
//  NHChatMultiSelectViewController.m
//  CIMKit
//
...
#import "HomeChatMultiSelectedView.h"
#import "HomeChatMultiSelectHeaderView.h"
#import "HomeChatMultiSelectCell.h"
#import "EmptyDataView.h"
#import "ZMessageTools.h"
#import "HomeMessageForwardTipView.h"
#import "ZKnownTipView.h"
#import "SyncMutableArray.h"
#import "HomeChatMultiSelectTipsView.h"
...
@interface NHChatMultiSelectViewController () <MainSearchViewDelegate,UITableViewDataSource,UITableViewDelegate,ZBaseCellDelegate>

@property (nonatomic, strong) EmptyDataView *viewNoData;

@property (nonatomic, strong) MainSearchView *viewSearch;
@property (nonatomic, copy) NSString *searchStr;

@property (nonatomic, strong) CTalkMultiSelectSendHander *multiSendHander;
...
@property (nonatomic, strong) NSMutableArray<ZBaseUserModel *> *selectedList; //已选取的会话
@property (nonatomic, strong) NSMutableArray<ZBaseUserModel *> *disableList;//不符合时间间隔的列表
@end

@implementation NHChatMultiSelectViewController
...
- (void)setupNavUI {
    self.navTitleStr = MultilingualTranslation(@"选择一个聊天");
...
- (void)navBtnRightClicked {
    ...
    if (newSelectedList.count <= 0) {
        NSMutableArray<FMsgPrecheckModel *> *newErrorInfoList = [NSMutableArray array];
        [self.disableList enumerateObjectsUsingBlock:^(ZBaseUserModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ZForwardDialogModel *dialogModel = [[ZForwardDialogModel alloc] init];
            ...
            ZForwardExceptionModel *exceptionModel = [ZForwardExceptionModel new];
            exceptionModel.code = NetWork_Error_Group_Interval;

            FMsgPrecheckModel *precheckModel = [FMsgPrecheckModel new];
        ...
        ZMessageAlertView *forwardFailAlert = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeTitle supView:self.view];
        forwardFailAlert.showClose = YES;
        forwardFailAlert.lblTitle.text = MultilingualTranslation(@"提示");
        ...
        forwardFailAlert.cancelBtnBlock = ^{
            //跳转异常详情
            HomeMessageForwardFailVC *failVC = [[HomeMessageForwardFailVC alloc] init];
    ...
    for (int i = 0; i < self.selectedList.count; i++) {
        ZBaseUserModel *model = [self.selectedList objectAtIndex:i];
    ...
    if (self.multiSelectType == ZMultiSelectTypeSingleForward || self.multiSelectType == ZMultiSelectTypeMergeForward) {
        //转发消息
        HomeMessageForwardTipView *viewTip = [[HomeMessageForwardTipView alloc] initWithForwardMsg:self.forwardMsgList toAvatarList:selectedReceverInfoList mergeMsgCount:self.mergeMsgCount fromSessionId:self.fromSessionId multiSelectType:self.multiSelectType];
        [viewTip viewShow];
        viewTip.sureClick = ^{
            //单条转发、逐条转发、合并转发
    ...
    if (self.multiSelectType == ZMultiSelectTypeRecommentCard) {
        //推荐名片
        NSString *tipsContent = [NSString stringWithFormat:@"%@%@", MultilingualTranslation(@"[个人名片]"), weakSelf.cardFriendInfo.nickname];
        HomeChatMultiSelectTipsView *viewTip = [[HomeChatMultiSelectTipsView alloc] initWithContent:tipsContent toAvatarList:selectedReceverInfoList];
        [viewTip viewShow];
        viewTip.sureClick = ^{
            [weakSelf checkSelectedRecevierComplianceWithRecevierIdArr:[selectedReceverInfoList copy]];
        };
    ...
    if (self.multiSelectType == ZMultiSelectTypeShareQRImg) {
        //分享二维码
        NSString *tipsContent = [NSString stringWithFormat:@"%@%@", MultilingualTranslation(@"[图片]"), MultilingualTranslation(@"二维码")];
        HomeChatMultiSelectTipsView *viewTip = [[HomeChatMultiSelectTipsView alloc] initWithContent:tipsContent toAvatarList:selectedReceverInfoList];
        [viewTip viewShow];
        viewTip.sureClick = ^{
            [weakSelf checkSelectedRecevierComplianceWithRecevierIdArr:[selectedReceverInfoList copy]];
    ...
    if (recevierInfoArr.count > 0) {
        ...
        [IMSDKManager transpondComplianceMessage:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
            ...
            if ([data isKindOfClass:[NSArray class]]) {
                ...
                NSArray *errorInfoList = [FMsgPrecheckModel mj_objectArrayWithKeyValuesArray:dataList];
...
- (void)showPreCheckReceiverErrorWithData:(NSArray *)errorInfoList recevierInfoArr:(NSArray *)recevierInfoArr {
    if (errorInfoList.count > 0 || self.disableList.count > 0) {
        //转发失败，弹窗提示
        WeakSelf
        __block NSMutableArray<FMsgPrecheckModel *> *newErrorInfoList = [NSMutableArray array];
        [self.disableList enumerateObjectsUsingBlock:^(ZBaseUserModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ZForwardDialogModel *dialogModel = [[ZForwardDialogModel alloc] init];
            dialogModel.avatar = obj.avatar;
            dialogModel.nickname = obj.name;
...
- (void)againPreCheckReceiverComplianceWithFailReceiverList:(NSArray *)failReceiverList {
    if (failReceiverList.count > 0) {
        ...
        for (FMsgPrecheckModel *failDialogModel in failReceiverList) {
        ...
        for (int i = 0; i<self.selectedList.count; i++) {
            ZBaseUserModel *tempSelectedModel = [self.selectedList objectAtIndex:i];
...
- (void)showMaxNumTips {
    ...
    viewTip.lblTip.text = [NSString stringWithFormat:MultilingualTranslation(@"最多只能选择%ld个"), Max_Selected_Num];
...
- (void)forwardMessageSendToReceiver:(NSMutableArray *)receiverList message:(NSArray *)messageList {
    ...
    for (int i = 0; i<self.selectedList.count; i++) {
        ZBaseUserModel *tempModel = [self.selectedList objectAtIndex:i];
    ...
    for (int i = 0; i < messageList.count; i++) {
        ZMessageModel *forwardMsg = (ZMessageModel *)[messageList objectAtIndex:i];
        if (forwardMsg.isSelf) {
            if (forwardMsg.message.messageType == CIMChatMessageType_AtMessage) {
                forwardMsg.message.textContent = [ZMessageTools forwardMessageAtContenTranslateToShowContent:forwardMsg.message.atContent atUsersDictList:forwardMsg.message.atUsersInfoList];
            ...
            if (forwardMsg.message.messageType == CIMChatMessageType_AtMessage) {
                ...
                forwardMsg.message.textContent = [ZMessageTools forwardMessageAtContenTranslateToShowContent:resultContent atUsersDictList:forwardMsg.message.atUsersInfoList];
                forwardMsg.message.messageType = CIMChatMessageType_TextMessage;
            }
            if (forwardMsg.message.messageType == CIMChatMessageType_TextMessage) {
                NSString *resultContent = @"";
        ...
        IMMessage *messageModel = [ZMessageTools getIMMessageFromLingIMChatMessageModel:forwardMsg.message withChatObject:resultList.firstObject index:i];
    ...
    for (ZBaseUserModel *receiver in resultList) {
        NSMutableArray *msgIds = [NSMutableArray arrayWithCapacity:1];
        for (int i = 0; i < messageList.count; i++) {
            NSString *msgIdStr = [ZMessageTools getMessageID];
...
- (void)requestForwardMessageWithMessages:(IMChatMessageList *)message {
    ...
    [self.multiSendHander setForwardComleteBlock:^(NSArray<LingIMChatMessageModel *> * _Nullable sendForwardMsgList) {
    ...
    [self.multiSendHander setNavBackActionBlock:^(BOOL isSuccess, NSInteger errorCode, NSString *errorMsg) {
        if (isSuccess) {
            ...
            [HUD showMessage:MultilingualTranslation(@"消息转发失败")];
        }
...
- (void)shareQRcodeImageMessage {
    ...
    [self.multiSendHander setShareQRcodeComleteBlock:^(LingIMChatMessageModel * _Nullable sendShareQRMsg) {
        //如果将消息转发给消息来源的同一个群，需要单独处理
...
- (CTalkMultiSelectSendHander *)multiSendHander {
    if (!_multiSendHander) {
        _multiSendHander = [[CTalkMultiSelectSendHander alloc] init];
    }
...
- (NSMutableArray<ZBaseUserModel *> *)disableList {
    if (_disableList == nil) {
