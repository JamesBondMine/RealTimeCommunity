...
#pragma mark - 界面布局
- (void)setupNavUI {
    self.navTitleStr = MultilingualTranslation(@"群二维码");
...
- (void)setupUI {
    ...
    BBBaseImageView * groupAvatar = [BBBaseImageView new] ;
    ...
    //codeBgview.image =ImgNamed(@"regro_g_qrcode_bg");
...
- (void)shareQRcodeBtnClick {
    ...
    [vc setShareQrCodeMsgSendSuccess:^(LingIMChatMessageModel * _Nonnull sendForwardMsg) {
...
- (void)setExpireTime:(NSInteger)expireTime {
    ...
    self.tipLabel1.text = [NSString stringWithFormat:MultilingualTranslation(@"该二维码7天内（%@月%@日前）有效，重新进入将会更新"), month, day];
}
