...
#import "NJCategoryNumberView.h"

@implementation NJCategoryNumberView
...
- (Class)preferredCellClass {
    return [NJCategoryNumberCell class];
}

- (void)refreshDataSource {
    NSMutableArray *tempArray = [NSMutableArray arrayWithCapacity:self.titles.count];
    for (int i = 0; i < self.titles.count; i++) {
        NJCategoryNumberCellModel *cellModel = [[NJCategoryNumberCellModel alloc] init];
