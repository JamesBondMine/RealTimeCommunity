//
//  TeamTeamInviteEditTeamNameVC.m
//  CIMKit
//
...
@interface TeamTeamInviteEditTeamNameVC ()

@property (nonatomic, strong) IVTTeamInviteEditTeamNameDataHandle *dataHandle;

@property (nonatomic, strong) NameTeamInviteEditTeamNameView *contentView;

@end

@implementation TeamTeamInviteEditTeamNameVC

- (void)dealloc {
    CIMLog(@"%@ dealloc", [self class]);
}

- (NameTeamInviteEditTeamNameView *)contentView {
    if (!_contentView) {
        _contentView = [[NameTeamInviteEditTeamNameView alloc]initWithFrame:CGRectZero
                                                  editTeamNameDataHandle:self.dataHandle];
    }
    return _contentView;
}

- (IVTTeamInviteEditTeamNameDataHandle *)dataHandle {
    if (!_dataHandle) {
        _dataHandle = [[IVTTeamInviteEditTeamNameDataHandle alloc] initWithTeamModel:self.currentTeamModel];
    }
    return _dataHandle;
}
