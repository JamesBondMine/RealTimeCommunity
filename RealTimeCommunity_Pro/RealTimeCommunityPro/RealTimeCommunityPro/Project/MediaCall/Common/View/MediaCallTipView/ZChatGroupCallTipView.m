//
//  ZChatGroupCallTipView.m
//  CIMKit
//
//  Created by cusPro on 2023/2/8.
//

#import "ZChatGroupCallTipView.h"
...
#import "ZCallInfoModel.h"

@interface ZChatGroupCallTipView () <UICollectionViewDataSource, UICollectionViewDelegate, LingIMMediaCallDelegate>
{
    //多人音视频聊天群成员变化队列
...
@property (nonatomic, strong) ZCallInfoModel *zgCallInfoModel;//即构 音视频通话信息
@property (nonatomic, strong) NSMutableArray *zgCallMemberIdList;

@end

@implementation ZChatGroupCallTipView
...
- (void)setupUI {
    ...
    [_collectionView registerClass:[ZChatGroupMemberItem class] forCellWithReuseIdentifier:NSStringFromClass([ZChatGroupMemberItem class])];
    ...
    [_btnAdd setTitle:MultilingualTranslation(@"加入") forState:UIControlStateNormal];
    [_btnAdd setTkThemeTitleColor:@[COLOR_4791FF, COLOR_4791FF_DARK] forState:UIControlStateNormal];
        ...
        if (weakSelf.groupCallInfoModel.participants.count > 0) {
            if ([weakSelf.groupCallInfoModel.participants containsObject:UserManager.userInfo.userUID]) {
                ...
                if (weakSelf.groupCallInfoModel.mode == 1) {
                    ...
                    weakSelf.lblTip.text = [NSString stringWithFormat:MultilingualTranslation(@"%ld人正进行语音通话"),weakSelf.groupCallInfoModel.participants.count];
                }else {
                    //视频通话0
                    weakSelf.ivLogo.image = ImgNamed(@"ms_btn_video_accept_s");
                    weakSelf.lblTip.text = [NSString stringWithFormat:MultilingualTranslation(@"%ld人正进行视频通话"),weakSelf.groupCallInfoModel.participants.count];
                }
            }
        ...
        if (weakSelf.zgCallMemberIdList.count > 0) {
            ...
            if ([weakSelf.zgCallMemberIdList containsObject:mineUserUid]) {
                ...
                if (weakSelf.zgCallInfoModel.callType == LingIMCallTypeAudio) {
                    //音频通话
                    weakSelf.ivLogo.image = ImgNamed(@"ms_btn_accept_s");
                    weakSelf.lblTip.text = [NSString stringWithFormat:MultilingualTranslation(@"%ld人正进行语音通话"), weakSelf.zgCallMemberIdList.count];
                }else {
                    //视频通话
                    weakSelf.ivLogo.image = ImgNamed(@"ms_btn_video_accept_s");
                    weakSelf.lblTip.text = [NSString stringWithFormat:MultilingualTranslation(@"%ld人正进行视频通话"),weakSelf.zgCallMemberIdList.count];
                }
            }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ZChatGroupMemberItem *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([ZChatGroupMemberItem class]) forIndexPath:indexPath];
    if ([ZHostTool.appSysSetModel.video_source_config isEqualToString:@"0"]) {
        //LiveKit
...
- (void)btnAddClick {
    ...
    if ([ZHostTool.appSysSetModel.video_source_config isEqualToString:@"0"]) {
        //LiveKit
        if ([ZMediaCallManager sharedManager].mediaCallState != ZMediaCallStateEnd) {
            [HUD showWarningMessage:MultilingualTranslation(@"当前正在通话中")];
        }else {
        ...
        //即构
        if ([ZCallManager sharedManager].callState != ZCallStateEnd) {
            [HUD showWarningMessage:MultilingualTranslation(@"当前正在通话中")];
        }else {
...
- (void)requestCallGroupState {
    ...
    [[ZMediaCallManager sharedManager] mediaCallGroupState:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
...
- (void)groupCallJoin {
    ...
    [[ZMediaCallManager sharedManager] mediaCallGroupJoinWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            NSDictionary *dataDict = (NSDictionary *)data;
            //房间信息配置
            ...
            ZMediaCallMoreVC *vc = [ZMediaCallMoreVC new];
...
- (void)requestZegoCallGroupState {
    ...
    [[ZCallManager sharedManager] callGetGroupCallInfoWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            NSDictionary *dataDic = (NSDictionary *)data;
            if (dataDic.allKeys.count > 0) {
                //当前群聊 有 正在进行的音视频通话
                weakSelf.zgCallInfoModel = [ZCallInfoModel mj_objectWithKeyValues:dataDic];
                
                [weakSelf.zgCallMemberIdList removeAllObjects];
                [weakSelf.zgCallInfoModel.receiveUserInfo enumerateObjectsUsingBlock:^(ZCallMemberInfoModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)groupCallZGJoin {
    ...
    WeakSelf
    [[ZCallManager sharedManager] callGroupJoinWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            NSDictionary *dataDict = (NSDictionary *)data;
            ZCallInfoModel *joinedCallInfo = [ZCallInfoModel mj_objectWithKeyValues:dataDict];
...
- (void)zgCallJoinForGroupWith:(ZCallInfoModel *)callInfoModel{
    ...
    //1.配置 单聊 音视频通话 参数
    LingIMZGCallOptions *zgCallOptions = [LingIMZGCallOptions new];
    ...
    [receiveUserInfoArray enumerateObjectsUsingBlock:^(ZCallMemberInfoModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ZCallUserModel *model = [ZCallUserModel new];
    ...
    ZCallUserModel *userModelMineInviter = [ZCallUserModel new];
    ...
    __block ZCallOptions *callOptions = [ZCallOptions new];
    ...
    ZCallManager *callManager = [ZCallManager sharedManager];
...
@implementation ZChatGroupMemberItem
- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        CGFloat itemW = (DScreenWidth - DWScale(120)) / 6.0;
