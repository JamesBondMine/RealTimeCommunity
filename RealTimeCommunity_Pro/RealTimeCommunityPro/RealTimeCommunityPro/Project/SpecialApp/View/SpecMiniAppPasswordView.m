...
@interface SpecMiniAppPasswordView ()
...
@implementation SpecMiniAppPasswordView
...
- (void)setupUI {
    ...
    _lblTitle.text = MultilingualTranslation(@"访问密码");
...
- (void)sureBtnAction {
    if (_miniAppModel) {
        ...
        if (![NSString isNil:password]) {
            ...
            [HUD showMessage:MultilingualTranslation(@"请输入访问密码")];
        }
    }
}
