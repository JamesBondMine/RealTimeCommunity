...
//  CIMKit
//
//  Created by cusPro on 2024/7/23.
//

#import "ZCaptchaCodeTools.h"
#import <WebKit/WebKit.h>

@interface ZCaptchaCodeTools() <WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate>

@property (nonatomic, strong) WKWebView *webView;

@end

@implementation ZCaptchaCodeTools
...
- (void)verCaptchaCode {
    ...
    if (ZHostTool.appSysSetModel.captchaChannel == 3) {
        //腾讯云
        NSURL *htmlURL = [[NSBundle mainBundle] URLForResource:@"reht_cq_file" withExtension:@"html"];
        [self.webView loadFileURL:htmlURL allowingReadAccessToURL:htmlURL];
    }
    if (ZHostTool.appSysSetModel.captchaChannel == 4) {
        //阿里云
        self.aliyunVerNum += 1;
        NSURL *htmlURL = [[NSBundle mainBundle] URLForResource:@"reht_cali_filetw" withExtension:@"html"];
        [self.webView loadFileURL:htmlURL allowingReadAccessToURL:htmlURL];
    }
}
...
- (void)secondVerCaptchaCode {
    ...
    if (ZHostTool.appSysSetModel.captchaChannel == 4) {
        //阿里云
        self.aliyunVerNum += 1;
        NSURL *htmlURL = [[NSBundle mainBundle] URLForResource:@"reht_cali_file" withExtension:@"html"];
        [self.webView loadFileURL:htmlURL allowingReadAccessToURL:htmlURL];
    }
