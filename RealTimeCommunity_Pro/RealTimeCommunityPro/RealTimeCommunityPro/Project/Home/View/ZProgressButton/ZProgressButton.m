...
+ (instancetype)crearProgressButtonWithFrame:(CGRect)frame
    ...
    ZProgressButton *progressButton = [[self alloc] init];
...
- (void)drawRect:(CGRect)rect {
    ...
    // 设置进度值
    [self setTitle:[NSString stringWithFormat:MultilingualTranslation(@"正在下载%.0f%%"), self.progress * 100] forState:UIControlStateNormal];
    if (self.isRound) {
