...
//  CIMKit
//
//  Created by cusPro on 2023/2/24.
//

#import "ZMessageCallCell.h"

@implementation ZMessageCallCell
{
...
- (void)setupMediaCallUI {
    ...
    _contentLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    if (model.isSelf) {
        ...
        if (model.message.messageType == CIMChatMessageType_NetCallMessage) {
            ...
            if (model.message.netCallType == 2) {
                //视频通话
                _callTypeImgView.image = ImgNamed(@"remsg_icon_call_video_self");
            } else {
                //语音通话
                _callTypeImgView.image = ImgNamed(@"remsg_icon_call_phone_self");
            }
        ...
        if (model.message.messageType == CIMChatMessageType_NetCallMessage) {
            ...
            if (model.message.netCallType == 2) {
                //视频通话
                _callTypeImgView.image = ImgNamed(@"remsg_icon_call_video_other");
            } else {
                //语音通话
                _callTypeImgView.image = ImgNamed(@"remsg_icon_call_phone_other");
            }
