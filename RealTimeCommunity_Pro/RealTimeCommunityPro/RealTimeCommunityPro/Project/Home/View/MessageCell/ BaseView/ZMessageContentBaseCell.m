...
@interface ZMessageContentBaseCell() <UIGestureRecognizerDelegate>
...
@implementation ZMessageContentBaseCell
...
- (void)setupUI {
    ...
    //气泡
    _viewSendBubble = [[ZBubbleSendView alloc] init];
    ...
    _viewReceiveBubble = [[ZBubbleReceiveView alloc] init];
    ...
    //已读状态(已读人数进度)
    _readedView = [[HomeMsgReadProgressView alloc] initWithRadius:16 fillColor:COLOR_4791FF];
    [_readedView configBorderWithColor:COLOR_4791FF borderWidth:1.5];
    ...
    //头像底部可点击View
    _msgAvatarBackView = [[ZBaseMsgAvatarView alloc] init];
...
- (NSString *)checkGroupActivityRoleShowStatus:(NSInteger)isActivityLevel activityScore:(NSInteger)activityScore role:(NSInteger)role {
    ...
    if (role == 2) {
        ...
        _groupRoleView.backgroundColor = COLOR_4791FF;
    } else {
        roleContent = @"";
    ...
    if (isActivityLevel == 1) {
        if (role == 3) {
            ...
            //开启-显示群活跃等级
            NSString *levelStr = @"";
            for (ZGroupActivityLevelModel *levelConfigInfo in UserManager.activityConfigInfo.sortLevels) {
