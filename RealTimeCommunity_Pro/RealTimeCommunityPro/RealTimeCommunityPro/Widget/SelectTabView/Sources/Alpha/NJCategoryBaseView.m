...
@interface NJCategoryBaseView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>

@property (nonatomic, strong) NJCategoryCollectionView *collectionView;
...
@property (nonatomic, strong) NJCategoryViewAnimator *animator;
...
@implementation NJCategoryBaseView
...
- (void)reloadCellAtIndex:(NSInteger)index {
    ...
    NJCategoryBaseCellModel *cellModel = self.dataSource[index];
    cellModel.selectedType = JXCategoryCellSelectedTypeUnknown;
    [self refreshCellModel:cellModel index:index];
    NJCategoryBaseCell *cell = (NJCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
...
- (void)setDelegate:(id<NJSelectCategoryViewDelegate>)delegate {
    _delegate = delegate;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    NJCategoryBaseCellModel *cellModel = self.dataSource[indexPath.item];
    cellModel.selectedType = JXCategoryCellSelectedTypeUnknown;
    [(NJCategoryBaseCell *)cell reloadData:cellModel];
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    BOOL isTransitionAnimating = NO;
    for (NJCategoryBaseCellModel *cellModel in self.dataSource) {
...
@implementation NJCategoryBaseView (UISubclassingBaseHooks)

- (CGRect)getTargetCellFrame:(NSInteger)targetIndex {
    CGFloat x = [self getContentEdgeInsetLeft];
    for (int i = 0; i < targetIndex; i ++) {
        NJCategoryBaseCellModel *cellModel = self.dataSource[i];
        CGFloat cellWidth;
    ...
    CGFloat width;
    NJCategoryBaseCellModel *selectedCellModel = self.dataSource[targetIndex];
...
- (void)initializeViews {
    ...
    _collectionView = [[NJCategoryCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
...
- (void)refreshState {
    ...
    if (self.isAverageCellSpacingEnabled && totalItemWidth < self.bounds.size.width) {
        ...
        [self.dataSource enumerateObjectsUsingBlock:^(NJCategoryBaseCellModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(NJCategoryBaseCellModel * cellModel, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (BOOL)selectCellAtIndex:(NSInteger)targetIndex selectedType:(JXCategoryCellSelectedType)selectedType {
    ...
    NJCategoryBaseCellModel *lastCellModel = self.dataSource[self.selectedIndex];
    ...
    //刷新当前选中的和将要选中的cell
    NJCategoryBaseCell *lastCell = (NJCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.selectedIndex inSection:0]];
    [lastCell reloadData:lastCellModel];
    NJCategoryBaseCell *selectedCell = (NJCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0]];
    [selectedCell reloadData:selectedCellModel];

    if (self.scrollingTargetIndex != -1 && self.scrollingTargetIndex != targetIndex) {
        NJCategoryBaseCellModel *scrollingTargetCellModel = self.dataSource[self.scrollingTargetIndex];
        ...
        NJCategoryBaseCell *scrollingTargetCell = (NJCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.scrollingTargetIndex inSection:0]];
...
- (void)refreshSelectedCellModel:(NJCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(NJCategoryBaseCellModel *)unselectedCellModel {
    ...
    if (self.isCellWidthZoomEnabled) {
        if (selectedCellModel.selectedType == JXCategoryCellSelectedTypeCode ||
            ...
            self.animator = [[NJCategoryViewAnimator alloc] init];
            ...
            self.animator.progressCallback = ^(CGFloat percent) {
                ...
                selectedCellModel.cellWidthCurrentZoomScale = [NJCategoryFactory interpolationFrom:selectedCellModel.cellWidthNormalZoomScale to:selectedCellModel.cellWidthSelectedZoomScale percent:percent];
                ...
                unselectedCellModel.cellWidthCurrentZoomScale = [NJCategoryFactory interpolationFrom:unselectedCellModel.cellWidthSelectedZoomScale to:unselectedCellModel.cellWidthNormalZoomScale percent:percent];
...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    if (remainderRatio == 0) {
        ...
        if (self.animator.isExecuting) {
            ...
            for (NJCategoryBaseCellModel *model in self.dataSource) {
                if (model.isSelected) {
        ...
        if (self.isCellWidthZoomEnabled && self.isCellWidthZoomScrollGradientEnabled) {
            NJCategoryBaseCellModel *leftCellModel = (NJCategoryBaseCellModel *)self.dataSource[baseIndex];
            NJCategoryBaseCellModel *rightCellModel = (NJCategoryBaseCellModel *)self.dataSource[baseIndex + 1];
            leftCellModel.cellWidthCurrentZoomScale = [NJCategoryFactory interpolationFrom:leftCellModel.cellWidthSelectedZoomScale to:leftCellModel.cellWidthNormalZoomScale percent:remainderRatio];
            leftCellModel.cellWidth = [self getCellWidthAtIndex:leftCellModel.index] * leftCellModel.cellWidthCurrentZoomScale;
            rightCellModel.cellWidthCurrentZoomScale = [NJCategoryFactory interpolationFrom:rightCellModel.cellWidthNormalZoomScale to:rightCellModel.cellWidthSelectedZoomScale percent:remainderRatio];
...
- (Class)preferredCellClass {
    return NJCategoryBaseCell.class;
}
