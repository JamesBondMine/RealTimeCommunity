...
#import "ZCallSingleVC.h"//单聊音视频
#import "ZCallGroupVC.h"//群聊音视频

#define CALL_HEART_BEAT_TIME        10

@interface ZCallManager () <ZegoEventHandler>
...
@implementation ZCallManager

#pragma mark - 单例>>>>>>
+ (instancetype)sharedManager {
    static ZCallManager *_manager = nil;
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    return [ZCallManager sharedManager];
}
// 防止外部调用copy
...
- (void)requestSingleCallWith:(NSMutableDictionary *)inviteeDict callType:(LingIMCallType)callType {
    ...
    //3.配置 单聊 音视频通话 参数
    __block LingIMZGCallOptions *zgCallOptions = [LingIMZGCallOptions new];
...
- (void)requestGroupCallWith:(NSMutableArray *)inviteeList group:(NSString *)groupID callType:(LingIMCallType)callType {
    if (inviteeList.count > 0) {
        ...
        //我在本房间的信息
        ZCallUserModel *userModelMine = [ZCallUserModel new];
        ...
        [callManager callRequestWith:callOptions onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
            ...
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                [callReceiveUserInfo enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    ZCallUserModel *userModel = [ZCallUserModel new];
...
- (void)callSdkConfigWith:(LingIMZGCallConfig *)config {
    [IMSDKManager imSdkZGConfigWith:config];
}
#pragma mark - 创建 ZegoExpressEngine 单例对象并初始化 SDK(第一步)
- (void)callRoomCreateEngineWithOptions:(LingIMZGCallOptions *)callOptions {
...
- (void)callRequestWith:(ZCallOptions *)callOptions onSuccess:(ZCallSuccessBlock)onSuccess onFailure:(ZCallFailureBlock)onFailure {
...
//发起音视频通话
- (void)zegoCallCreateWith:(ZCallOptions *)callOptions onSuccess:(ZCallSuccessBlock)onSuccess onFailure:(ZCallFailureBlock)onFailure {
...
- (void)onPublisherQualityUpdate:(ZegoPublishStreamQuality *)quality streamID:(NSString *)streamID {
    if (quality.level > ZegoStreamQualityLevelMedium){
        [HUD showMessage:MultilingualTranslation(@"当前网络信号差")];
    }
}
...
- (void)onPlayerQualityUpdate:(ZegoPlayStreamQuality *)quality streamID:(NSString *)streamID {
    if (_currentCallOptions.zgCallOptions.callRoomType == LingIMCallRoomTypeSingle) {
        //单聊音视频
        if (quality.level > ZegoStreamQualityLevelMedium){
            [HUD showMessage:MultilingualTranslation(@"对方网络信号差")];
        }
    }else {
        //群聊音视频
    }
...
- (void)onRoomStreamUpdate:(ZegoUpdateType)updateType streamList:(NSArray<ZegoStream *> *)streamList extendedData:(NSDictionary *)extendedData roomID:(NSString *)roomID {
    if (_currentCallOptions.zgCallOptions.callRoomType == LingIMCallRoomTypeSingle) {
        ...
        //更新成员的状态
        for (ZCallUserModel *userModel in self.currentCallOptions.callMemberList) {
