...
@implementation ZChatNavLinkAddView
...
- (void)setupUI {
    ...
    lblNameTitle.text = MultilingualTranslation(@"名称");
    lblNameTitle.tkThemetextColors = @[COLOR_33, COLOR_CCCCCC];
    ...
    lblLinkTitle.text = MultilingualTranslation(@"链接地址");
    lblLinkTitle.tkThemetextColors = @[COLOR_33, COLOR_CCCCCC];
...
- (void)setViewType:(ChatLinkAddViewType)viewType {
    ...
    if (_viewType == ChatLinkAddViewTypeAdd) {
        self.viewTitleLbl.text = MultilingualTranslation(@"添加链接");
    } else {
        self.viewTitleLbl.text = MultilingualTranslation(@"编辑链接");
    }
}

- (void)setEditTagModel:(ZChatTagModel *)editTagModel {
...
- (void)sureBtnAction {
    ...
    if ([NSString isNil:tagName]) {
        [HUD showMessage:MultilingualTranslation(@"名称不可为空")];
        return;
    }
    if ([NSString isNil:tagUrl]) {
        [HUD showMessage:MultilingualTranslation(@"链接地址不可为空")];
        return;
    }
...
- (UILabel *)viewTitleLbl {
    if (!_viewTitleLbl) {
        ...
        _viewTitleLbl.text = @"";
        _viewTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_CCCCCC];
...
- (UITextField *)nameTextField {
    if (!_nameTextField) {
        ...
        _nameTextField.placeholder = MultilingualTranslation(@"请输入名称");
        _nameTextField.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (UITextField *)linkTextField {
    if (!_linkTextField) {
        ...
        _linkTextField.placeholder = MultilingualTranslation(@"请输入链接地址");
        _linkTextField.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
