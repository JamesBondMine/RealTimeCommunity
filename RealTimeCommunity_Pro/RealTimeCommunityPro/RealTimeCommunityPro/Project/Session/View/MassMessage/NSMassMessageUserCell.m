...
@interface NSMassMessageUserCell ()

@end

@implementation NSMassMessageUserCell
...
- (void)setupUI {
    ...
    _lblNickname.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _lblTip = [UILabel new];
    _lblTip.text = MultilingualTranslation(@"发送失败");
...
- (void)setUserModel:(id)userModel {
    ...
    if ([_userModel isKindOfClass:[NSMassMessageUserModel class]]) {
        _lblTip.hidden = YES;
        NSMassMessageUserModel *msgUserModel = (NSMassMessageUserModel *)_userModel;
    ...
    if ([_userModel isKindOfClass:[NSMassMessageErrorUserModel class]]) {
        _lblTip.hidden = NO;
        NSMassMessageErrorUserModel *errorUserModel = (NSMassMessageErrorUserModel *)_userModel;
