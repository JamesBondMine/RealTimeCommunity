...
//#import "MainChatInputCommonEmojiView.h"//常用表情
#import "MainChatInputReferenceView.h"//引用
#import "MainChatInputFunctionView.h"//输入框
#import "MainChatInputMoreView.h"//更多菜单
#import "EMChatImageEmojiView.h"//自定义表情
#import "MainChatInputVoiceView.h"//声音
#import "MainChatInputVoiceStateView.h"
...
@interface ZChatInputView () <FunctionViewDelegate,MoreViewDelegate,MainChatInputReferenceViewDelegate,EMChatImageEmojiViewDelegate,MainChatInputVoiceViewDelegate>

//常用表情
//@property (nonatomic, strong) MainChatInputCommonEmojiView *viewCommonEmoji;
//@property (nonatomic, assign) CGFloat commonEmojiH;

//引用控件
@property (nonatomic, strong) MainChatInputReferenceView *viewReference;
@property (nonatomic, assign) CGFloat referenceH;

//主功能控件(输入框)
@property (nonatomic, strong) MainChatInputFunctionView *viewFunction;
@property (nonatomic, assign) CGFloat functionH;

//表情控件
@property (nonatomic, strong) EMChatImageEmojiView *viewImageEmoji;
@property (nonatomic, assign) CGFloat emojiH;

//录音控件
@property (nonatomic, strong) MainChatInputVoiceView *viewVoice;
...
//录音状态视图控件
@property (nonatomic, strong) MainChatInputVoiceStateView *viewVoiceStateView;


@property (nonatomic, strong) MainChatInputMoreView *viewMore;
...
#pragma mark - 界面布局
- (void)setupUI {
//    _viewCommonEmoji = [[MainChatInputCommonEmojiView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DWScale(48))];
//    _viewCommonEmoji.delegate = self;
//    [self addSubview:_viewCommonEmoji];
    
    _viewReference = [[MainChatInputReferenceView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DWScale(62))];
    _viewReference.delegate = self;
    _viewReference.alpha = 0;
    [self addSubview:_viewReference];
    
    _viewFunction = [[MainChatInputFunctionView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DWScale(56 + 50))];
    _viewFunction.sessionID = self.sessionID;
    _viewFunction.delegate = self;
    [self addSubview:_viewFunction];
    
    _viewImageEmoji = [[EMChatImageEmojiView alloc] initWithFrame:CGRectMake(0, _functionH, DScreenWidth, DWScale(300) + DHomeBarH)];
    _viewImageEmoji.alpha = 0;
    _viewImageEmoji.delegate = self;
    [self addSubview:_viewImageEmoji];
    
    _viewVoice = [[MainChatInputVoiceView alloc] initWithFrame:CGRectMake(0, _functionH, DScreenWidth, DWScale(192) + DHomeBarH)];
    _viewVoice.alpha = 0;
    _viewVoice.delegate = self;
    [self addSubview:_viewVoice];
    
    _viewVoiceStateView = [[MainChatInputVoiceStateView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DWScale(56 + 50 + 48) + DWScale(34))];
...
- (void)setMessageModelReference:(ZMessageModel *)messageModelReference {
...
#pragma mark - MainChatInputCommonEmojiViewDelegate
...
#pragma mark - MainChatInputReferenceViewDelegate
...
- (void)imageGifPackageSelected:(LingIMStickersModel *)stickersModel  {
...
#pragma mark - MainChatInputVoiceViewDelegate
- (void)dragMoving{
    DLog(@"----------dragMoving");
}
...
- (void)voiceEventTouchUpInside{
    ...
    if (self.recordTool.recordTime < 1) {
        [HUD showMessage:MultilingualTranslation(@"说话时间太短")];
    }else {
        WeakSelf
...
#pragma mark - 懒加载
- (MainChatInputMoreView *)viewMore {
    if (!_viewMore) {
        _viewMore = [MainChatInputMoreView new];
    ...
    if (_recordTool == nil) {
        ...
        _recordTool.returnTime = ^(NSTimer *timer,int second) {
            ...
            if (second < 1) {
                ...
                [weakSelf.viewVoice.voiceBtn setImage:ImgNamed(@"recat_Img_chat_voice_input_normal") forState:UIControlStateNormal];
