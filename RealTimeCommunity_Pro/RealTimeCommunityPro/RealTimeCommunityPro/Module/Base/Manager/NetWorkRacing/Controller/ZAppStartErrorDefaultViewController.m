//
//  ZAppStartErrorDefaultViewController.m
//  CIMKit
//
...
@interface ZAppStartErrorDefaultViewController ()

@property (nonatomic, strong)UILabel *errorMsgLbl;

@end

@implementation ZAppStartErrorDefaultViewController
...
- (void)setupNavUI {
    ...
    [self.navBtnRight setTitle:MultilingualTranslation(@"更换") forState:UIControlStateNormal];
    [self.navBtnRight setTkThemeTitleColor:@[COLOR_99, COLOR_99_DARK] forState:UIControlStateNormal];
    [self.navBtnRight setImage:ImgNamed(@"com_c_arrow_right_gray") forState:UIControlStateNormal];
...
- (void)setupUI {
    ...
    defaultErrorImgView.image = ImgNamed(@"com_img_start_error_default");
   ...
   UIButton *relaodButton = [[UIButton alloc] init];
    [relaodButton setTitle:MultilingualTranslation(@"重新加载") forState:UIControlStateNormal];
...
- (void)setupData {
    ...
    switch (self.step) {
        case ZNetRacingStepOss:
        {
            //OSS
            if (self.code == 100000) {
                self.errorMsgLbl.text = MultilingualTranslation(@"服务器连接失败 ，请联系管理员");
            } else {
                if (self.code == 404 || self.code == 403) {
                    self.errorMsgLbl.text = MultilingualTranslation(@"企业号不存在");
                } else {
                    self.errorMsgLbl.text = MultilingualTranslation(@"服务器连接失败");
                }
            }
        }
            break;
        ...
        {
            //Http
            self.errorMsgLbl.text = MultilingualTranslation(@"获取配置失败");
        }
            break;
        case ZNetRacingStepTcp:
        {
            //Tcp
            self.errorMsgLbl.text = MultilingualTranslation(@"IM连接失败");
        }
            break;
...
- (void)navBtnRightClicked {
    ...
    [ZSsoInfoModel clearSSOInfoWithLiceseId:ssoModel.liceseId];
//    NLSsoSetViewController *ssoSetVC = [[NLSsoSetViewController alloc] init];
//    ssoSetVC.isRoot = NO;
