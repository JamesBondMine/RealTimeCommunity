//
//  MaNHChatViewControllerinInputTextView.m
//  CIMKit
//
...
#import "ZBaseVideoPlayerVC.h"
#import "ZChatMessageMoreView.h"
#import "ZMessageAlertView.h"
#import "ZImagePickerVC.h"              //相册
#import "ZFilePickerVC.h"               //文件
#import "ZMessageTools.h"
#import "ZDraftStore.h"
#import "ZMessageSendHander.h"
#import "ZToolManager.h"                //工具类
#import "NHChatMultiSelectViewController.h"  //消息转发选择转发对象
#import "ZChatSingleSetVC.h"            //单聊设置VC
#import "ZChatGroupSetVC.h"             //群聊设置VC
#import "ZMsgAtListViewController.h"    //@用户的列表
#import "NFUserHomePageVC.h"             //用户个人资料
#import "ZAudioPlayManager.h"           //语音消息播放单例
#import "NHChatFileDetailViewController.h"   //文件消息中文件详情
...
#import "HomeMssageTimeDeleteView.h"//消息定时删除配置
#import "CTalkMultiSelectSendHander.h"
#import "HomeMssageMultiBottomView.h" //多选bottom
#import "HomeChatRecordDetailViewController.h" //会话记录详情
#import "ZSensitiveManager.h" //敏感词过滤
#import "ZChatTextUrlListView.h" //topView标签栏相关
#import "ZChatNavLinkAddView.h"
#import "NSMiniAppWebVC.h"//跳转webView
#import "ZMessageTimeTool.h"
#import "ZTranslateSettingVC.h"//翻译配置信息
#import "ZEmojiShopViewController.h"//表情商店
#import "ZEmojiPackageDetailViewController.h"//表情包详情
#import "ZTranslateDefaultModel.h"
#import "ZFileUploadManager.h"
#import "ZMessageSendTask.h"
#import "HomeSessionVC.h"
#import "ZMessageVoiceHudView.h"
#import "HomeChatActivityLevelVC.h"
#import "ZChatAtBannedView.h"
// 群公告列表
#import "ZGroupNoticeListVC.h"
// 群公告详情页面
#import "ZGroupNoticeDetailVC.h"
// 网络检测页面
#import "ZNetworkDetectionVC.h"
...
@interface NHChatViewController () <ZChatInputViewDelegate, UITableViewDelegate, UITableViewDataSource, ZMessageBaseCellDelegate, CIMToolMessageDelegate, CIMToolUserDelegate, ZImagePickerVCDelegate, UIDocumentInteractionControllerDelegate, KNPhotoBrowserDelegate, HomeChatGroupNoticeTipViewDelegate, HomeMssageTimeDeleteViewDelegate, ZMsgMultiSelectDelegate, UIGestureRecognizerDelegate, ZFileUploadTaskDelegate>
...
@property (nonatomic, strong) HomeChatTopView *topView;
@property (nonatomic, strong) ZChatInputView *viewInput;
@property (nonatomic, strong) ZChatAtBannedView *atBannedView;
...
@property (nonatomic, strong) ZUserModel *userModel;//单聊好友信息
@property (nonatomic, strong) ZChatGroupCallTipView *viewGroupCallTip;//音视频通话提示View
//@property (nonatomic, strong) ZFileNetProgressManager *fileUploader;
@property (nonatomic, strong) HomeChatGroupNoticeTipView *viewGroupNotice;//置顶群公告
...
@property (nonatomic, strong) CTalkMultiSelectSendHander *collectionSendHander;
@property (nonatomic, assign) BOOL multiSelectStatus; //是否为多选状态
@property (nonatomic, strong) HomeMssageMultiBottomView *multiSelectBottomView;//多选-底部操作栏
@property (nonatomic, strong) SyncMutableArray *selectedMsgModels;
@property (nonatomic, strong) UIButton *btnBottom;
@property (nonatomic, strong) ZMessageAlertView *groupAlertView;//群状态弹窗
...
@property (nonatomic, strong) HomeMssageMultiSelectView *messageMutiSelectView;
@property (nonatomic, assign) NSInteger pageNumber;

@end

@implementation NHChatViewController
...
- (void)viewWillAppear:(BOOL)animated {
        ...
        [ZTOOL doAsync:^{
           friendModel = [IMSDKManager toolCheckMyFriendWith:weakSelf.sessionID];
            ...
            if (friendModel.disableStatus == 4) {
                weakSelf.topView.chatName = MultilingualTranslation(@"已注销");
            }
    ...
    // 进入时加载本地草稿
    NSDictionary *draft = [ZDraftStore loadDraftForSession:self.sessionID];
    if (draft.count > 0) {
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    if (draft.count > 0) {
        [ZDraftStore saveDraft:draft forSession:self.sessionID];
        if (self.draftDidChange) { self.draftDidChange(self.sessionID, draft); }
    } else {
        [ZDraftStore deleteDraftForSession:self.sessionID];
...
- (void)setupUI {
    ...
    WeakSelf;
    _topView = [[HomeChatTopView alloc] initWithFrame:CGRectMake(0, DStatusBarH, DScreenWidth, 44 + DWScale(40))];
    ...
    _topView.navRightBlock = ^{
        if (weakSelf.chatType == CIMChatType_SingleChat) {
            //单聊-Setting
            ZChatSingleSetVC *vc = [ZChatSingleSetVC new];
            vc.friendUid = weakSelf.sessionID;
            [weakSelf.navigationController pushViewController:vc animated:YES];
        } else {
            //群聊-Setting
            ZChatGroupSetVC *vc = [ZChatGroupSetVC new];
    ...
    _topView.navLinkBlock = ^(NSInteger linkIndex) {
        if (linkIndex == Chat_Top_Nav_Link_Notice) {
            ...
            //跳转Web
            ZChatTagModel *clickTagModel = (ZChatTagModel *)[weakSelf.topView.chatLinkArr objectAtIndex:linkIndex];
    ...
    [self.btnBottom setImage:ImgNamed(@"recat_c_go_bottom") forState:UIControlStateNormal];
...
- (void)reloadBottomView{
    if (self.isAllMemberNotalk) {
        //全员禁言
        _notalkStateLabel.text = MultilingualTranslation(@"全员禁言");
    } else if (self.isCurUserNotalk) {
        //当前用户被禁言
        if (self.isCurUserNotalk) {
            if ([[NSDate getOvertime:[NSString stringWithFormat:@"%ld",self.notalkTime] isShowSecondStr:YES] isEqualToString:MultilingualTranslation(@"禁言")]) {
                ...
                _notalkStateLabel.text = [NSString stringWithFormat:MultilingualTranslation(@"禁言中 %@"),[NSDate getOvertime:[NSString stringWithFormat:@"%ld",self.notalkTime] isShowSecondStr:YES]];
...
- (void)setupMultiSelectedStatusDefaultIsReload:(BOOL)isRelaod {
    ...
    [self.messageModels enumerateObjectsUsingBlock:^(ZMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.view.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[HomeChatGroupNoticeTipView class]]) {
            obj.hidden = NO;
        }
    }];
}
...
- (void)setupInputBottomUI {
    if (!self.viewInput) {
        self.viewInput = [[ZChatInputView alloc] initWithFrame:CGRectMake(0, DScreenHeight - DWScale(56 + 50) - DHomeBarH, DScreenWidth, DWScale(56 + 50) + DHomeBarH)];
...
- (void)setupNotalkBottomUI{
    ...
    _notalkStateLabel.tkThemebackgroundColors = @[COLORWHITE,COLOR_33];
...
- (void)socketReConnectEvent {
    WeakSelf
    [IMSDKManager messageReConnectGetHistoryRecordWith:_sessionID chatType:_chatType lastMessageId:((ZMessageModel *)self.messageModels.firstObject).message.msgID messageId:((ZMessageModel *)self.messageModels.lastObject).message.msgID offset:0 historyList:^(NSArray<LingIMChatMessageModel *> * _Nullable chatMessageHistory, NSInteger offset) {
...
-(void)refreshUploadChatMessageReaded {
    ...
    if (self.unReadSmsgidList.count > 0) {
       LingIMChatMessageModel *uploadReadedMesage = [ZMessageSendHander ZMessageReadedWithMsgSidList:[self.unReadSmsgidList safeArray] withToUserId:self.sessionID withChatType:self.chatType];
...
- (void)requestChatTagList {
    ...
    [IMSDKManager MessageChatTagListWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            weakSelf.topView.chatLinkArr = [ZChatTagModel mj_objectArrayWithKeyValuesArray:tagList];
...
- (void)getUserNotalkState {
    ...
    [[LingIMHttpManager sharedManager] groupGetUserNotalkStateWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
...
- (void)getGroupInfoFromDB {
    ...
    if (groupModel) {
        if (groupModel.groupStatus == 0) {
            //群封禁
            [self showTipAlertView:MultilingualTranslation(@"该群已被封禁，如需申诉请联系管理员") btnName:MultilingualTranslation(@"我知道了") isNeedDelete:NO];
        } else if (groupModel.groupStatus == 1) {
            //群状态 正常
        } else if (groupModel.groupStatus == 2) {
            if (self.groupInfo.groupInformStatus == 1) {
                //群状态 解散
                [self showTipAlertView:MultilingualTranslation(@"群聊已解散") btnName:MultilingualTranslation(@"我知道了") isNeedDelete:YES];
            } else {
                [self deleteSessionAndChatMessage];
                WeakSelf
                [self.navigationController.childViewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if ([obj isKindOfClass:[HomeSessionVC class]]) {
                        HomeSessionVC *vc = (HomeSessionVC *)obj;
...
- (void)getGroupInfoFromServer {
    ...
    [[LingIMSDKManager sharedTool] getGroupInfoWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            if (!weakSelf.groupInfo.userInGroup) {
                ...
                if (weakSelf.groupInfo.groupStatus == 0) {
                    ...
                    if (self.groupInfo.groupInformStatus == 1) {
                        //群解散
                        [weakSelf showTipAlertView:MultilingualTranslation(@"群聊已解散") btnName:MultilingualTranslation(@"我知道了") isNeedDelete:YES];
                    } else {
...
- (void)requestGroupNoticeInfo {
    ...
    [IMSDKManager groupCheckOneGroupNoticeWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            NSDictionary *dataDict = (NSDictionary *)data;
            ZGroupNoteModel *groupNoticeModel = [ZGroupNoteModel mj_objectWithKeyValues:dataDict];
            /**
...
- (void)requestAddStickersToCollectionWithDic:(NSMutableDictionary *)dict  isReloadCollection:(BOOL)isReloadCollection {
    ...
    [IMSDKManager imSdkUserAddStickersToCollectList:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        [HUD showMessage:MultilingualTranslation(@"操作成功") inView:weakSelf.view];
...
- (void)requestHistoryList {
    ZMessageModel *model = [self.messageModels objectAtIndex:0];
    NSString *serviceMsgID = model.message.serviceMsgID;
    
    WeakSelf
    [IMSDKManager messageGetHistoryRecordWith:_sessionID chatType:_chatType serviceMsgID:serviceMsgID offset:self.messageModels.count pageNum:_pageNumber historyList:^(NSArray<LingIMChatMessageModel *> * _Nullable chatMessageHistory, NSInteger offset, BOOL isLocal, NSInteger pageNumber) {
...
//消息处理
- (void)dealChatMessageHistoryWith:(NSArray<LingIMChatMessageModel *> * _Nullable) chatMessageHistory offset:(NSInteger)offset {
    WeakSelf
    dispatch_async(_chatMessageUpdateQueue, ^{
        __block NSMutableArray *newMessageList = [[NSMutableArray alloc] init];
        [chatMessageHistory enumerateObjectsUsingBlock:^(LingIMChatMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj.messageType == CIMChatMessageType_ServerMessage) {
                if (weakSelf.groupInfo.groupInformStatus == 1) {
                    if (obj.backDelInformSwitch == 0) {
                        ZMessageModel *newMsgModel = [[ZMessageModel alloc] initWithMessageModel:obj];
...
- (void)reloadMessageTabView:(NSNotification *)sender{
    //更新聊天页面
    __block ZMessageModel * msgModel;
    ZMessageModel * senderMsgModel = sender.object;
    [self.messageModels enumerateObjectsUsingBlock:^(ZMessageModel *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)requestSelectHistoryList:(NSNotification *)sender {
    ...
    if ([userInfo.allKeys containsObject:@"selectMessageID"]) {
        ...
        if (startTime < nowFirstMessageTime) {
            ...
            [history enumerateObjectsUsingBlock:^(LingIMChatMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ZMessageModel *newMsgModel = [[ZMessageModel alloc] initWithMessageModel:obj];
                [newMessageList addObject:newMsgModel];
...
- (void)shareQRcodeAddLocalMessageToUI:(NSNotification *)sender {
    LingIMChatMessageModel *shareQRcodeMsgModel = sender.object;
    ...
    if ([self.sessionID isEqualToString:shareQRcodeMsgModel.toID]) {
        ZMessageModel *sendMsg = [[ZMessageModel alloc] initWithMessageModel:shareQRcodeMsgModel];
        //刷新并滚动到底部
...
- (void)messageTimeDelete:(NSNotification *)sender {
    ...
    if (messageDeleteType > 0 && [messageDeleteSession isEqualToString:_sessionID]) {
        ...
        dispatch_async(_chatMessageUpdateQueue, ^{
            ...
            [weakSelf.messageModels enumerateObjectsUsingBlock:^(ZMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if (obj.message.sendTime <= messageCanDeleteTimeLine) {
                    [weakSelf.messageModels removeObject:obj];
                }
            }];
            //处理消息是否显示时间
            [weakSelf computeVisibleTime];
...
- (void)groupBanStatusChange:(NSNotification *)notification {
    ...
    if ([groupID isEqualToString:self.sessionID] && [CurrentVC isKindOfClass:[NHChatViewController class]]) {
        //当前群封禁状态改变，且当前显示聊天界面
        ...
        if (self.groupInfo.userGroupRole == 2 || self.groupInfo.userGroupRole == 1) {
            ...
            if(msgSendTime - self.sessionModel.lastSendMsgTime >=
               (ZHostTool.appSysSetModel.groupMessageInterval == 0 ? (2 * 1000) : ZHostTool.appSysSetModel.groupMessageInterval)){
                ...
                [HUD showMessage:MultilingualTranslation(@"发送频次过快，请稍后重试！") inView:self.view];
                return NO;
            }
        } else {
            return YES;
...
- (void)chatInputViewSend:(NSString *)sendStr atUserList:(NSArray *)atUsersDictList atUserSegmentList:(NSArray *)atUserSegmentList {
    ...
    if ([NSString isNil:sendStr]) {
        [HUD showMessage:MultilingualTranslation(@"发送内容不能为空")];
        return;
    }
    NSString *referenceId = @"";
    if (atUsersDictList.count > 0 && atUserSegmentList.count > 0) {
        //@用户消息（按出现顺序逐一范围替换，避免全量误替换）
        NSString *originalStr = [NSString stringWithString:sendStr];
        // 使用 atUserSegmentList 替代正则匹配，段应为按出现顺序的 { uid, range }，range 覆盖“@昵称+空格”
        // 1) 规范化与排序
        NSMutableArray<NSDictionary *> *segments = [NSMutableArray arrayWithCapacity:atUserSegmentList.count];
        for (NSDictionary *seg in atUserSegmentList) {
            if (![seg isKindOfClass:NSDictionary.class]) { continue; }
            id rangeObj = seg[@"range"];
            NSRange segRange = NSMakeRange(NSNotFound, 0);
            if ([rangeObj isKindOfClass:NSString.class]) {
                segRange = NSRangeFromString((NSString *)rangeObj);
            } else if ([rangeObj isKindOfClass:NSValue.class]) {
                segRange = [(NSValue *)rangeObj rangeValue];
            }
            if (segRange.location == NSNotFound || NSMaxRange(segRange) > originalStr.length) { continue; }
            NSString *uid = (NSString *)seg[@"uid"]; // 可为空字符串
            [segments addObject:@{ @"uid": uid ?: @"", @"range": NSStringFromRange(segRange) }];
        }
        [segments sortUsingComparator:^NSComparisonResult(NSDictionary *a, NSDictionary *b) {
            NSRange ra = NSRangeFromString(a[@"range"]);
            NSRange rb = NSRangeFromString(b[@"range"]);
            if (ra.location == rb.location) { return NSOrderedSame; }
            return (ra.location < rb.location) ? NSOrderedAscending : NSOrderedDescending;
        }];
        ...
        for (NSDictionary *seg in segments) {
            NSRange r = NSRangeFromString(seg[@"range"]);
            // 追加非 @ 片段
            if (r.location > lastLocation) {
                [resultStr appendString:[originalStr substringWithRange:NSMakeRange(lastLocation, r.location - lastLocation)]];
            }
            // 取对应的 at 用户（按出现顺序一一映射）
            if (atIndex < atUsersDictList.count) {
                NSDictionary *obj = (NSDictionary *)[atUsersDictList objectAtIndex:atIndex];
                NSArray *atKeyArr = [obj allKeys];
                NSString *atKey = (NSString *)[atKeyArr firstObject];
                [resultStr appendFormat:@"\v%@\v ", atKey];
                atIndex++;
            } else {
                // 安全回退：当列表不足时保留原文
                [resultStr appendString:[originalStr substringWithRange:r]];
            }
            lastLocation = r.location + r.length;
        }
        if (lastLocation < originalStr.length) {
            [resultStr appendString:[originalStr substringFromIndex:lastLocation]];
        }
        ...
        LingIMChatMessageModel *atSendMessage = [ZMessageSendHander ZMessageAtUserSend:ZSensitiveFilter(atContentStr) showContent:@"" withAtUsersDicList:atUsersDictList withToUserId:self.sessionID withChatType:self.chatType referenceMsgId:referenceId];
        //判断是否大于群发消息间隔
        ...
        if (_sessionModel.isSendAutoTranslate == 1 && [UserManager isTranslateEnabled]) {
            ...
            [self requestTranslateActionWithContent:atContentStr atUserDictList:atUsersDictList isSend:YES messageType:atSendMessage.messageType success:^(NSString * _Nullable result) {
                ...
                if (weakSelf.sessionModel) {
                    ...
                    // 清除本地草稿并点对点刷新
                    [ZDraftStore deleteDraftForSession:weakSelf.sessionID];
                    // 清空输入框与 @ 列表，并复位发送按钮
        ...
        LingIMChatMessageModel *textSendMsg = [ZMessageSendHander ZMessageTextSend:ZSensitiveFilter(sendStr) withToUserId:self.sessionID  withChatType:self.chatType referenceMsgId:referenceId];
        //判断是否大于群发消息间隔
...
- (void)requestTranslateActionWithContent:(NSString *)content
    ...
    WeakSelf
    [ZMessageTools translationSplit:content atUserList:atUserList finish:^(NSString * _Nonnull translationString,
            ...
            [IMSDKManager imSdkTranslateYuueeContent:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
               ...
               if (success) {
                ...
                if (code == Translate_yuuee_no_balance_code) {
                    //提示
                    [HUD showMessage:MultilingualTranslation(@"当前账户字符数不足，已关闭翻译功能，请增加字符后使用。") inView:weakSelf.view];
                }
                if (code == Translate_yuuee_unbind_error_code) {
                    [HUD showMessage:MultilingualTranslation(@"您尚未绑定字符账号，无法使用翻译功能，请绑定后使用。") inView:weakSelf.view];
                }
                if (failure) {
...
- (void)chatInputViewVoicePath:(NSString *)vociePath voiceName:(NSString *)voiceName voiceDuration:(CGFloat)voiceDuration {
    WeakSelf
    [ZMessageSendHander ZMessageVoiceSend:vociePath fileName:voiceName voiceDuring:voiceDuration withToUserId:self.sessionID withChatType:self.chatType compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
        //判断是否大于群发消息间隔
        ...
        ZFileUploadTask *audioTask = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:sendChatMsg.localVoicePath originFilePath:@"" fileName:sendChatMsg.localVoiceName fileType:@"" isEncrypt:NO dataLength:audioData.length uploadType:ZHttpUploadTypeVoice beSendMessage:sendChatMsg delegate:self];
        ...
        NSBlockOperation * blockOperation = [NSBlockOperation blockOperationWithBlock:^{
            ...
            if (audioTask.status == FileUploadTaskStatus_Failed) {
                [ZTOOL doInMain:^{
                    [HUD showMessage:MultilingualTranslation(@"上传失败") inView:weakSelf.view];
                }];
            }
        }];
        ...
        [[ZFileUploadManager sharedInstance] addUploadTask:audioTask];
...
- (void)chatInputViewAudioCall {
    ...
    if ([ZHostTool.appSysSetModel.enableAudioAndVideoCalls isEqualToString:@"0"]) {
        [HUD showMessage:MultilingualTranslation(@"该群已关闭音视频功能") inView:self.view];
        return;
    } else {
        if (self.chatType == CIMChatType_GroupChat) {
    ...
    [ZTOOL getMicrophoneAuth:^(BOOL granted) {
        if (weakSelf.chatType == CIMChatType_SingleChat) {
            ...
            if (friendModel && friendModel.disableStatus == 4) {
                [HUD showMessage:MultilingualTranslation(@"账号已注销") inView:weakSelf.view];
                return;
            }
...
- (void)chatInputViewShowImage {
    ...
    [ZTOOL getPhotoLibraryAuth:^(BOOL granted) {
        if (granted) {
            ...
            [HUD showWarningMessage:MultilingualTranslation(@"相册权限未开启，请在设置中选择当前应用，开启相册权限") inView:self.view];
        }
    }];
}
...
- (void)imagePickerVCSelected {
    if (IMAGEPICKER.zSelectedAssets.count > 0) {
        ...
        /// 发送图片、视频消息
        [HUD showActivityMessage:MultilingualTranslation(@"处理中...") inView:self.view];
        //目录路径
        NSString *customPath = [NSString stringWithFormat:@"%@-%@", UserManager.userInfo.userUID, self.sessionID];
        WeakSelf
        [ZMessageSendHander ZMessageMediaSend:IMAGEPICKER.zSelectedAssets withToUserId:self.sessionID withChatType:self.chatType compelete:^(NSArray <LingIMChatMessageModel *> * sendChatMsgArr) {
            
            __block NSMutableArray * taskArray = [NSMutableArray array];
            [sendChatMsgArr enumerateObjectsUsingBlock:^(LingIMChatMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                //生成上传任务，开始上传
                if (obj.messageType == CIMChatMessageType_ImageMessage) {
                    ...
                    ZFileUploadTask *thumbImgTask = [[ZFileUploadTask alloc] initWithTaskId:[NSString stringWithFormat:@"%@_thumb", obj.msgID] filePath:localThumbImgPath originFilePath:localImgPath fileName:obj.localthumbImgName fileType:@"" isEncrypt:YES dataLength:thumbImgData.length uploadType:ZHttpUploadTypeImageThumbnail beSendMessage:obj delegate:self];
                    thumbImgTask.messageTaskType = FileUploadMessageTaskTypeThumbImage;
                    [taskArray addObject:thumbImgTask];
                    
                    //图片
                    ZFileUploadTask *imgTask = [[ZFileUploadTask alloc] initWithTaskId:obj.msgID filePath:localImgPath originFilePath:@"" fileName:obj.localImgName fileType:@"" isEncrypt:YES dataLength:obj.imgSize uploadType:ZHttpUploadTypeImage beSendMessage:obj delegate:self];
                ...
                if (obj.messageType == CIMChatMessageType_VideoMessage) {
                    ...
                    ZFileUploadTask *coverTask = [[ZFileUploadTask alloc] initWithTaskId:[NSString stringWithFormat:@"%@_cover", obj.msgID] filePath:localCoverPath originFilePath:@"" fileName:obj.localVideoCover fileType:@"" isEncrypt:YES dataLength:obj.videoCoverSize uploadType:ZHttpUploadTypeImage beSendMessage:obj delegate:self];
                    ...
                    ZFileUploadTask *videoTask = [[ZFileUploadTask alloc] initWithTaskId:obj.msgID filePath:localVideoPath originFilePath:@"" fileName:obj.localVideoName fileType:@"" isEncrypt:YES dataLength:obj.videoSize uploadType:ZHttpUploadTypeVideo beSendMessage:obj delegate:self];
                ...
                if (obj.messageType == CIMChatMessageType_FileMessage) {
                    ...
                    ZFileUploadTask *fileTask = [[ZFileUploadTask alloc] initWithTaskId:obj.msgID filePath:localFilePath originFilePath:@"" fileName:obj.fileName fileType:obj.fileType isEncrypt:YES dataLength:obj.fileSize uploadType:ZHttpUploadTypeFile beSendMessage:obj delegate:self];
            ...
            ZMessageSendTask *messageSendTask = [[ZMessageSendTask alloc] init];
            messageSendTask.uploadTask = taskArray;
            [taskArray enumerateObjectsUsingBlock:^(ZFileUploadTask *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                [[ZFileUploadManager sharedInstance] addUploadTask:obj];
            }];

            [[ZFileUploadManager sharedInstance].operationQueue addOperation:messageSendTask];
            ...
            [sendChatMsgArr enumerateObjectsUsingBlock:^(LingIMChatMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                //将配置好的消息 1.存储到数据库 2.添加到UI上展示(此时展示的图片或视频还没上传完成，正在上传中...)
                [IMSDKManager toolInsertOrUpdateChatMessageWith:obj];
    ...
    [ZTOOL getPhotoLibraryAuth:^(BOOL granted) {
        if (granted) {
            [ZTOOL doInMain:^{
                ...
                ZFilePickerVC *vc = [ZFilePickerVC new];
                vc.sessionFoldPath = fileFolderPath;
                [weakSelf.navigationController pushViewController:vc animated:YES];
                //直接选择 手机储存的文件
                vc.savePhoneFileSuccess = ^(ZFilePickModel *selectFileModel) {
                    //判断是否大于群发消息间隔
                    if(![self isExceedSendMsgTimeInterval:[NSDate currentTimeIntervalWithMillisecond]]){
                        return;
                    }
            ...
            [HUD showWarningMessage:MultilingualTranslation(@"相册权限未开启，请在设置中选择当前应用，开启相册权限") inView:weakSelf.view];
        }
    }];
}

//发送文件类型消息时上传文件并组合消息体
- (void)recombineFileSendData:(ZFilePickModel *)sendFileModel sendFileDataList:(NSArray <ZFilePickModel *> *)sendFileDataList {
    [HUD showActivityMessage:MultilingualTranslation(@"处理中...") inView:self.view];
    WeakSelf
    if (sendFileDataList == nil) {
        //手机中存储的文件(每次只发1个文件)
        [ZMessageSendHander ZMessageFileSendData:sendFileModel withToUserId:self.sessionID withChatType:self.chatType compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
            //目录
            NSString *customPath = [NSString stringWithFormat:@"%@-%@", UserManager.userInfo.userUID, self.sessionID];
            NSString *localFilePath = [NSString getPathWithFileName:sendChatMsg.fileName CustomPath:customPath];
            //上传文件
            ZFileUploadTask *fileTask = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:localFilePath originFilePath:@"" fileName:sendChatMsg.fileName fileType:sendChatMsg.fileType isEncrypt:YES dataLength:sendChatMsg.fileSize uploadType:ZHttpUploadTypeFile beSendMessage:sendChatMsg delegate:self];
            fileTask.messageTaskType = FileUploadMessageTaskTypeFile;
            ...
            [[ZFileUploadManager sharedInstance] addUploadTask:fileTask];
            [[ZFileUploadManager sharedInstance].operationQueue addOperation:blockOperation];
        ...
        [sendFileDataList enumerateObjectsUsingBlock:^(ZFilePickModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            dispatch_group_enter(myGroup);
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [ZMessageSendHander ZMessageFileSendData:obj withToUserId:weakSelf.sessionID withChatType:weakSelf.chatType compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
                    //目录
                    NSString *customPath = [NSString stringWithFormat:@"%@-%@", UserManager.userInfo.userUID, weakSelf.sessionID];
                    NSString *localFilePath = [NSString getPathWithFileName:sendChatMsg.fileName CustomPath:customPath];
                    //上传文件
                    ZFileUploadTask *fileTask = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:localFilePath originFilePath:@"" fileName:sendChatMsg.fileName fileType:sendChatMsg.fileType isEncrypt:YES dataLength:sendChatMsg.fileSize uploadType:ZHttpUploadTypeFile beSendMessage:sendChatMsg delegate:weakSelf];
                    fileTask.messageTaskType = FileUploadMessageTaskTypeFile;
                    [taskArray addObject:fileTask];
        ...
        dispatch_group_notify(myGroup, dispatch_get_main_queue(), ^{
            ...
            [sengMessageArr enumerateObjectsUsingBlock:^(LingIMChatMessageModel *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                //将配置好的消息 1.存储到数据库 2.添加到UI上展示
                [IMSDKManager toolInsertOrUpdateChatMessageWith:obj];
                [weakSelf chatListAppendMessage:obj];
            }];
...
- (void)chatInputViewAtUser {
    if (_chatType == CIMChatType_GroupChat) {//群聊
        ...
        if (!_groupInfo) {
            //用户没选择了艾特的成员，相当于输入艾特
            [self.viewInput inputViewInsertAtUserInfo:nil];
            return;
        }
...
- (void)showAtUserListView {
    ZMsgAtListViewController *atUserVC = [[ZMsgAtListViewController alloc] init];
    ...
    atUserVC.AtUserSelectClick = ^(id atModel) {
        ...
        if (atModel) {
            NSDictionary *atUser;
            if (weakSelf.chatType == CIMChatType_SingleChat) {
                ZUserModel *clickModel = (ZUserModel *)atModel;
                atUser = @{
...
- (void)sendGeoLocationMessageWithLat:(NSString *)lat lng:(NSString *)lng name:(NSString *)name cImg:(UIImage *)cImg detail:(NSString *)detail {
    WeakSelf
    [ZMessageSendHander ZMessageLocationSendWithLng:lng lat:lat name:name cImg:cImg detail:detail withToUserId:self.sessionID withChatType:self.chatType compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
        //判断是否大于群发消息间隔
        ...
        ZFileUploadTask *geoImgTask = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:geoImgPath originFilePath:@"" fileName:sendChatMsg.localGeoImgName fileType:@"" isEncrypt:YES dataLength:geoImgData.length uploadType:ZHttpUploadTypeImage beSendMessage:sendChatMsg delegate:self];
        ...
        [[ZFileUploadManager sharedInstance] addUploadTask:geoImgTask];
        [[ZFileUploadManager sharedInstance].operationQueue addOperation:blockOperation];
...
- (void)chatInputViewCollection {
    ...
    WeakSelf
    [myCollectionVC setSendCollectionMsgBlock:^(ZMyCollectionItemModel * _Nonnull collectionMessage) {
...
- (void)sendCollectionUserForwardAction:(ZMyCollectionItemModel *)collectionMsg {
    ...
    WeakSelf
    [self.collectionSendHander setCollectionSendCompleteBlock:^(BOOL isSuccess, LingIMChatMessageModel * _Nullable sendCollectionMsg) {
        if (isSuccess) {
            [HUD showMessage:MultilingualTranslation(@"已发送") inView:weakSelf.view];
        } else {
            [HUD showMessage:MultilingualTranslation(@"操作失败") inView:weakSelf.view];
        }
        ...
        if ([weakSelf.sessionID isEqualToString:sendCollectionMsg.toID]) {
            //刷新并滚动到底部
            [ZTOOL doInMain:^{
                ZMessageModel *sendMsg = [[ZMessageModel alloc] initWithMessageModel:sendCollectionMsg];
...
- (void)chatInputViewTranslate {
    ZTranslateSettingVC *translateSetVC = [[ZTranslateSettingVC alloc] init];
...
- (void)chatInputViewPlayGameStickerAction:(ZChatGameStickerType)gameType {
    ...
    //生成消息体
    LingIMChatMessageModel *gameStickersMessageModel = [ZMessageSendHander ZMessageGameStickersSendResultContent:resultContent gameStickersType:gameStickersType gameStickerExt:@"" withToUserId:self.sessionID withChatType:self.chatType];
    //发送游戏表情消息
...
- (void)imagePickerClipImage:(UIImage *)resultImg localIdenti:(NSString *)localIdenti {
    ...
    ZFileUploadTask *stickerThumbTask = [[ZFileUploadTask alloc] initWithTaskId:thumbnailFileName filePath:stickerThumbPath originFilePath:@"" fileName:thumbnailFileName fileType:@"" isEncrypt:YES dataLength:thumbnailImgData.length uploadType:ZHttpUploadTypeStickers beSendMessage:nil delegate:self];
    stickerThumbTask.messageTaskType = FileUploadMessageTaskTypeStickerThumb;
    [taskArray addObject:stickerThumbTask];
    //上传原图
    ZFileUploadTask *stickerTask = [[ZFileUploadTask alloc] initWithTaskId:fileName filePath:stickerPath originFilePath:@"" fileName:fileName fileType:@"" isEncrypt:YES dataLength:stickerData.length uploadType:ZHttpUploadTypeStickers beSendMessage:nil delegate:self];
...
- (void)sendChatRecordMessage:(NSMutableArray *)msgModels {
    ...
    vc.messageRecordReceverListBlock = ^(NSArray * _Nonnull selectedReceverInfoArr) {
        ...
        if (weakSelf.chatType == CIMChatType_SingleChat) {
            //单聊
            recordTitle = [NSString stringWithFormat:MultilingualTranslation(@"%@与%@的会话记录"), UserManager.userInfo.nickname, weakSelf.chatName];
        } else {
            recordTitle = MultilingualTranslation(@"群聊会话记录");
        }
        //根据消息时间进行升序排列
        NSArray *sortResultArr = [NSMutableArray sortMultiSelectedMessageArr:msgModels];
        WeakSelf
        [ZMessageSendHander ZMessageMergeForwardSendWith:sortResultArr withTitle:recordTitle withToUserInfoArr:selectedReceverInfoArr compelete:^(NSArray <LingIMChatMessageModel *> *sendChatMsgList) {
            //发送消息
            [sendChatMsgList enumerateObjectsUsingBlock:^(LingIMChatMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
//任务状态改变回调
-(void)fileUploadTask:(ZFileUploadTask *)task didChangTaskStatus:(FileUploadTaskStatus)status error:(NSError *)error {}

//任务上传进度回调
-(void)fileUploadTask:(ZFileUploadTask *)task didChangTaskProgress:(float)progress {}

//任务暂停
-(void)fileUploadTaskDidPause:(ZFileUploadTask *)task {}

//任务继续
-(void)fileUploadTaskDidResume:(ZFileUploadTask *)task {}

//重新上传
-(void)fileUploadTaskDidReupload:(ZFileUploadTask *)task {}

#pragma mark - 消息列表追加消息
- (void)chatListAppendMessage:(LingIMChatMessageModel *)newMessage {
    if (newMessage != nil) {
        //删除 要撤回的消息
        ...
        if (newMessage.chatType == CIMChatType_GroupChat) {
            ...
            newMsgModel = [[ZMessageModel alloc] initWithMessageModel:newMessage];
        }
...
- (ZMessageModel *)handleMessageWithGroupInformStatusAndInformSwitch:(LingIMChatMessageModel *)newMessage {
    ZMessageModel *newMsgModel = nil;
...
- (void)cimToolChatMessageReceive:(LingIMChatMessageModel *)message {
    ...
    if (message.serverMessage.sMsgType == IMServerMessage_ServerMsgType_KickGroupMessage) {
        ...
        if (self.chatType == CIMChatType_GroupChat && [message.serverMessage.kickGroupMessage.gid isEqualToString:self.groupInfo.groupId]) {
            if ([message.serverMessage.kickGroupMessage.uid isEqualToString:UserManager.userInfo.userUID]) {
                if (self.groupInfo.groupInformStatus == 1) {
                    [self showTipAlertView:MultilingualTranslation(@"你已被移出群聊") btnName:MultilingualTranslation(@"我知道了") isNeedDelete:NO];
                } else {
                    [self deleteSessionAndChatMessage];
                    [weakSelf.navigationController.childViewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                        if ([obj isKindOfClass:[HomeSessionVC class]]) {
                ...
                if([message.toID isEqualToString:self.sessionID]){
                    ...
                    if (message.serverMessage.kickGroupMessage.msgDel) {
                        ...
                        for (int i = 0; i<tempMessageArr.count; i++) {
                            ZMessageModel *msgModel = [tempMessageArr objectAtIndex:i];
    ...
    if (message.serverMessage.sMsgType == IMServerMessage_ServerMsgType_DelGroupMessage) {
        ...
        if (![message.serverMessage.delGroupMessage.uid isEqualToString:UserManager.userInfo.userUID] && [message.serverMessage.delGroupMessage.gid isEqualToString:self.sessionID]) {
            if (self.groupInfo.groupInformStatus == 1) {
                //群解散
                [self showTipAlertView:MultilingualTranslation(@"群聊已解散") btnName:MultilingualTranslation(@"我知道了") isNeedDelete:NO];
            } else {
                [self deleteSessionAndChatMessage];
                [self.navigationController.childViewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if ([obj isKindOfClass:[HomeSessionVC class]]) {
    ...
    if (message.serverMessage.sMsgType == IMServerMessage_ServerMsgType_OutGroupMessage) {
        if ([CurrentVC isKindOfClass:[NHChatViewController class]]) {
            if ([message.serverMessage.delGroupMessage.uid isEqualToString:UserManager.userInfo.userUID]) {
                [self showTipAlertView:MultilingualTranslation(@"你已退出该群聊") btnName:MultilingualTranslation(@"我知道了") isNeedDelete:NO];
            } else {
                if([message.toID isEqualToString:self.sessionID]){
                    [IMSDKManager imSdkDeleteGroupMemberWith:message.serverMessage.kickGroupMessage.uid groupID:self.groupInfo.groupId];
                    [self getGroupInfoFromServer];
...
- (void)cimToolChatMessageSendSuccess:(IMChatMessageACK *)messageACK {
    ...
    dispatch_async(_chatMessageUpdateQueue, ^{
        for (int i = 0; i<weakSelf.messageModels.count; i++) {
            ZMessageModel *msgModel = [weakSelf.messageModels objectAtIndex:i];
    ...
    dispatch_async(_chatMessageUpdateQueue, ^{
        for (int i = 0; i<weakSelf.messageModels.count; i++) {
            ...
            if ([msgModel.message.msgID isEqualToString:messageID]) {
                //更新本地数据
                ZMessageModel *newMsgModel = [[ZMessageModel alloc] initWithMessageModel:msgModel.message];
...
- (void)tableViewScrollToBottom:(BOOL)animated duration:(CGFloat)duration {
    if (self.messageModels.count > 0) {
        ...
        @try {
            ...
            DLog(@"tableViewScrollToBottom>>>>>异常");
        }
    };
}
...
- (void)cimToolChatMessageUpdate:(LingIMChatMessageModel *)message {
    WeakSelf
    dispatch_async(_chatMessageUpdateQueue, ^{
        for (int i = 0; i<weakSelf.messageModels.count; i++) {
            ZMessageModel *msgModel = [weakSelf.messageModels objectAtIndex:i];
            if ([msgModel.message.serviceMsgID isEqualToString:message.serviceMsgID]) {
                ZMessageModel *updateMessage = [[ZMessageModel alloc] initWithMessageModel:message];
...
- (void)showTipAlertView:(NSString *)content btnName:(NSString *)btnName isNeedDelete:(BOOL)isNeedDelete {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // 创建新的弹窗
        self.groupAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeNomal supView:nil];
...
- (void)deleteSessionAndChatMessage {
    ...
    if (model) {
        ...
        //删除会话
        [[LingIMSDKManager sharedTool] deleteServerConversation:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {} onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {}];
    }
    
    //删除会话
    [IMSDKManager toolDeleteMySessionWith:self.sessionID deleteAllChatMessage:YES];

    if (_chatType == CIMChatType_SingleChat) {
        //删除好友
        [IMSDKManager toolDeleteMyFriendWith:self.sessionID];
    } else if (_chatType == CIMChatType_GroupChat) {
        //删除群组
...
//    WeakSelf
//    dispatch_async(_chatMessageUpdateQueue, ^{
        //对界面上的消息数组进行时间排序，刷新界面
        NSArray *sortResultArr = [NSMutableArray sortMultiSelectedMessageArr:self.messageModels.safeArray];
        [self.messageModels removeAllObjects];
        [self.messageModels addObjectsFromArray:sortResultArr];
//        [ZTOOL doInMain:^{
//            [weakSelf.baseTableView reloadData];
//            //加载滚动到底部
...
- (void)updateDeleteMessageAndRefreshMsgWithOriginalMsg:(NSString *)originalMsgId {
    ...
    dispatch_async(_chatMessageUpdateQueue, ^{
        for (int i = 0; i<weakSelf.messageModels.count; i++) {
            ZMessageModel *tempMessageModel = [weakSelf.messageModels objectAtIndex:i];
            ...
            if (tempMessageModel.message.referenceMsgId != nil && [tempMessageModel.message.referenceMsgId isEqualToString:originalMsgId]) {
                ZMessageModel *refreshMsg = [[ZMessageModel alloc] initWithMessageModel:tempMessageModel.message];
    ...
    dispatch_async(_chatMessageUpdateQueue, ^{
        for (int i = 0; i<weakSelf.messageModels.count; i++) {
            ...
            if (msgModel.message.referenceMsgId != nil && [msgModel.message.referenceMsgId isEqualToString:originalMsgId]) {
                ZMessageModel *refreshMsg = [[ZMessageModel alloc] initWithMessageModel:msgModel.message];
                [weakSelf.messageModels replaceObjectAtIndex:i withObject:refreshMsg];
                //处理消息是否显示时间
                [weakSelf computeVisibleTime];
                [ZTOOL doInMain:^{
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZMessageModel *model = [self.messageModels objectAtIndex:indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row < self.messageModels.count) {
        ...
        switch (model.message.messageType) {
            ...
            {
                //文件消息
                ZMessageFileCell *fileCell = [tableView dequeueReusableCellWithIdentifier:[ZMessageFileCell cellIdentifier]];
                if (fileCell == nil) {
                    fileCell = [[ZMessageFileCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:[ZMessageFileCell cellIdentifier]];
                }
            ...
            case CIMChatMessageType_CardMessage:    //名片消息
            {
                ...
                if (cell == nil) {
                    cell = [[ZMessageCardCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"nameCardCell"];
                }
                cell.delegate = self;
                cell.sessionId = self.sessionID;
                [cell setConfigMessage:model];
            ...
            case CIMChatMessageType_GroupNotice:   //群公告消息
            {
                ...
                if (!cell) {
                    cell = [[ZMessageGroupNoticeCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"groupNoticeCell"];
                }
                cell.delegate = self;
                cell.sessionId = self.sessionID;
                [cell setConfigMessage:model];
            ...
            case CIMChatMessageType_NetCallMessage://即构 音视频通话
            {
                ...
                if (cell == nil) {
                    cell = [[ZMessageCallCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"mediaCallCell"];
                }
                cell.delegate = self;
                cell.sessionId = self.sessionID;
                [cell setConfigMessage:model];
            ...
            case CIMChatMessageType_ServerMessage:  //系统通知类消息
            {
                ...
                if (cell == nil) {
                    cell = [[ZMessageSystemCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"systemTipsCell"];
                }
                cell.delegate = self;
                cell.sessionId = self.sessionID;
                [cell setConfigMessage:model];
...
- (void)checkHistoryListForUnreadMessage:(NSArray *)historyList {
    if (historyList.count > 0) {
        WeakSelf
        [historyList enumerateObjectsUsingBlock:^(ZMessageModel * _Nonnull chatMessage, NSUInteger idx, BOOL * _Nonnull stop) {
            StrongSelf
...
- (void)userAvatarLongTapClickAtAndBanned:(NSString *)userId nickname:(NSString *)nickname role:(NSInteger)role cellIndex:(nonnull NSIndexPath *)cellIndex{
    ...
    ZMessageBaseCell *avatarLongTapCell = [self.baseTableView cellForRowAtIndexPath:cellIndex];
    ...
    [self.atBannedView setCleanUserMessageCallback:^{
        ...
        ZMessageAlertView *msgAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeTitle supView:nil];
        msgAlertView.lblTitle.text = MultilingualTranslation(@"清空用户消息");
        msgAlertView.lblTitle.font = FONTB(18);
        msgAlertView.lblTitle.textAlignment = NSTextAlignmentCenter;
        msgAlertView.lblContent.text = [NSString stringWithFormat:MultilingualTranslation(@"此操作将永久删除用户“%@”在本群的所有发言记录，且所有群成员将无法查看"), self.atBannedView.userName];
        msgAlertView.lblContent.font = FONTN(14);
        msgAlertView.lblContent.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
        msgAlertView.lblContent.textAlignment = NSTextAlignmentCenter;
        [msgAlertView.btnSure setTitle:MultilingualTranslation(@"确认") forState:UIControlStateNormal];
...
- (void)refreshMessageListAfterDeleteMemberHistHistory:(NSArray *)memberUidList {
    ...
    NSPredicate *keepPredicate = [NSPredicate predicateWithBlock:^BOOL(ZMessageModel *messageModel, NSDictionary *bindings) {
...
- (void)messageCellLongTapWithIndex:(NSIndexPath *)cellIndex {
    ...
    ZMessageBaseCell *longTapCell = [self.baseTableView cellForRowAtIndexPath:cellIndex];
    ZMessageModel *longTapModel = [self.messageModels objectAtIndex:cellIndex.row];
    //配置弹窗里的菜单选项
    ...
    //显示出弹窗
    ZChatMessageMoreView *msgMoreMenu = [[ZChatMessageMoreView alloc] initWithMenu:menuArr targetRect:targetRect isFromMy:longTapModel.isSelf isBottom:isBottom msgContentSize:CGSizeMake(longTapModel.messageWidth, longTapModel.messageHeight)];
    WeakSelf;
...
- (void)messageReSendClick:(NSIndexPath *)cellIndex {
    ...
    ZMessageModel *reSendMsgModel = [self.messageModels objectAtIndex:cellIndex.row];
    WeakSelf
    if (reSendMsgModel.message.messageType == CIMChatMessageType_FileMessage) {
        ZPresentItem *saveItem = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"重新上传") textColor:COLOR_33 font:FONTR(17) itemHeight:DWScale(54) backgroundColor:COLORWHITE];
        ...
        msgAlertView.lblContent.text = MultilingualTranslation(@"是否重新发送");
        [msgAlertView.btnSure setTitle:MultilingualTranslation(@"确认") forState:UIControlStateNormal];
        [msgAlertView alertShow];
        msgAlertView.sureBtnBlock = ^(BOOL isCheckBox) {
            [weakSelf reSendFailMessageWithReSendMsg:reSendMsgModel Index:cellIndex.row];
...
- (void)systemMessageNotFriendAlert:(NSIndexPath *)cellIndex {
    ...
    ZMessageAlertView *msgAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeNomal supView:nil];
    msgAlertView.lblContent.text = MultilingualTranslation(@"点击确认发送好友验证申请，对方同意后即可对话聊天。");
    [msgAlertView.btnSure setTitle:MultilingualTranslation(@"确认") forState:UIControlStateNormal];
    [msgAlertView alertShow];
    WeakSelf
    msgAlertView.sureBtnBlock = ^(BOOL isCheckBox) {
...
- (void)voiceMessageClick:(NSIndexPath *)cellIndex {
    ...
    ZMessageModel *voiceMsgModel = [self.messageModels objectAtIndex:cellIndex.row];
    if (ZAudioPlayerTOOL.isPlaying) {
        [ZAudioPlayerTOOL stop];
    }
...
- (void)voicePlay:(ZMessageVoiceCell *)clickVoiceCell voiceMsgModel:(ZMessageModel *)voiceMsgModel {
            ...
            if (self.isReachable) {
                ...
                [ZMessageTools downloadAudioWith:voiceMsgModel.message.voiceName AudioCachePath:downloadVoicePath completion:^(BOOL success, NSString * _Nonnull audioPath) {
                    if (success) {
                        ...
                        [HUD showMessage:MultilingualTranslation(@"语音播放失败，请稍后再试") inView:self.view];
                    }
                }];
            } else {
                //无网络
                [HUD showMessage:MultilingualTranslation(@"网络错误,播放失败") inView:self.view];
            }
        }
    } else {
        //本地音频文件路径
            ...
            if (self.isReachable) {
                ...
                [ZMessageTools downloadAudioWith:voiceMsgModel.message.voiceName AudioCachePath:meLocalPath completion:^(BOOL success, NSString * _Nonnull audioPath) {
                    if (success) {
                        ...
                        [HUD showMessage:MultilingualTranslation(@"语音播放失败，请稍后再试") inView:weakSelf.view];
                    }
                }];
            } else {
                //无网络
...
- (void)messageBubbleClick:(NSIndexPath *)cellIndex {
    ...
    ZMessageModel *bubbleMsgClickModel = [self.messageModels objectAtIndex:cellIndex.row];
...
- (void)messageCellClick:(NSIndexPath *)cellIndex {
    ...
    ZMessageModel *multiClickModel = [self.messageModels objectAtIndex:cellIndex.row];
    ...
    if (!multiClickModel.multiSelected && self.selectedMsgModels.count >= Multi_Selected_Max_Num) {
        [HUD showMessage:MultilingualTranslation(@"最多选择100条消息") inView:self.view];
        return;
    }
    ...
    if (multiClickModel.multiSelected) {
        ...
        WeakSelf
        [self.selectedMsgModels enumerateObjectsUsingBlock:^(ZMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)messageTextReTranslateClick:(NSIndexPath *)cellIndex {
    ZMessageModel *reTranslateMsgModel = [self.messageModels objectAtIndex:cellIndex.row];
    WeakSelf
    //At消息
...
- (void)gameMessageAnimationComplete:(NSIndexPath *)cellIndex {
    ZMessageModel *msgModel = [self.messageModels objectAtIndex:cellIndex.row];
...
- (void)groupMemberActivityLevelTagClick:(NSIndexPath *)cellIndex {
    if (self.groupInfo.isActiveEnabled == 1) {
        HomeChatActivityLevelVC *vc = [[HomeChatActivityLevelVC alloc] init];
...
- (void)messageCellClickGroupNotice:(NSIndexPath *)cellIndex {
    ...
    ZMessageModel *groupNoticMsgModel = [self.messageModels objectAtIndex:cellIndex.row];
    ...
    [IMSDKManager groupCheckOneGroupNoticeWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        [HUD hideHUD];
        if (code == 41034) {
            [HUD showMessage:MultilingualTranslation(@"公告已删除") inView:self.view];
        }else {
...
#pragma mark - 消息重新发送
- (void)reSendFailMessageWithReSendMsg:(ZMessageModel *)reSendMsgModel Index:(NSInteger)index {
    WeakSelf
    if (![NetWorkStatusManager shared].getConnectStatus) {
        return;
    }
    [ZMessageSendHander ZMessageReSendWithFailMsg:reSendMsgModel compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
        ...
        if (oldMsgID.length > 0) {
            for (NSInteger i = weakSelf.messageModels.count - 1; i >= 0; i--) {
                ZMessageModel *m = [weakSelf.messageModels objectAtIndex:i];
        ...
        if (sendChatMsg.messageType == CIMChatMessageType_TextMessage || sendChatMsg.messageType == CIMChatMessageType_AtMessage) {
            ...
            if (sendChatMsg.messageType == CIMChatMessageType_ImageMessage) {
                ...
                //缩略图
                NSData *thumbImgData = [NSData dataWithContentsOfFile:localThumbImgPath];
                ZFileUploadTask *thumbImgTask = [[ZFileUploadTask alloc] initWithTaskId:[NSString stringWithFormat:@"%@_thumb", sendChatMsg.msgID] filePath:localThumbImgPath originFilePath:imagePath fileName:sendChatMsg.localthumbImgName fileType:@"" isEncrypt:YES dataLength:thumbImgData.length uploadType:ZHttpUploadTypeImageThumbnail beSendMessage:sendChatMsg delegate:self];
                thumbImgTask.messageTaskType = FileUploadMessageTaskTypeThumbImage;
                [taskArray addObject:thumbImgTask];
                [[ZFileUploadManager sharedInstance] addUploadTask:thumbImgTask];

                //图片
                ZFileUploadTask *imgTask = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:imagePath originFilePath:@"" fileName:sendChatMsg.localImgName fileType:@"" isEncrypt:YES dataLength:sendChatMsg.imgSize uploadType:ZHttpUploadTypeImage beSendMessage:sendChatMsg delegate:self];
                imgTask.messageTaskType = FileUploadMessageTaskTypeImage;
                [taskArray addObject:imgTask];
                [[ZFileUploadManager sharedInstance] addUploadTask:imgTask];
                ...
                ZFileUploadTask *coverTask = [[ZFileUploadTask alloc] initWithTaskId:[NSString stringWithFormat:@"%@_cover", sendChatMsg.msgID] filePath:coverPath originFilePath:@"" fileName:sendChatMsg.localVideoCover fileType:@"" isEncrypt:YES dataLength:sendChatMsg.videoCoverSize uploadType:ZHttpUploadTypeImage beSendMessage:sendChatMsg delegate:self];
                coverTask.messageTaskType = FileUploadMessageTaskTypeCover;
                [taskArray addObject:coverTask];
                [[ZFileUploadManager sharedInstance] addUploadTask:coverTask];
                
                //视频文件上传
                NSString *videoPath = [NSString getPathWithVideoName:sendChatMsg.localVideoName CustomPath:customPath];
                ZFileUploadTask *videoTask = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:videoPath originFilePath:@"" fileName:sendChatMsg.localVideoName fileType:@"" isEncrypt:YES dataLength:sendChatMsg.videoSize uploadType:ZHttpUploadTypeVideo beSendMessage:sendChatMsg delegate:self];
                videoTask.messageTaskType = FileUploadMessageTaskTypeVideo;
                [taskArray addObject:videoTask];
                [[ZFileUploadManager sharedInstance] addUploadTask:videoTask];
            } else if (sendChatMsg.messageType == CIMChatMessageType_VoiceMessage) {
                //音频
                NSData *audioData = [NSData dataWithContentsOfFile:sendChatMsg.localVoicePath options:NSDataReadingMappedIfSafe error:nil];
                ZFileUploadTask *voiceTask = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:sendChatMsg.localVoicePath originFilePath:@"" fileName:sendChatMsg.localVoiceName fileType:@"" isEncrypt:NO dataLength:audioData.length uploadType:ZHttpUploadTypeVoice beSendMessage:sendChatMsg delegate:self];
                voiceTask.messageTaskType = FileUploadMessageTaskTypeVoice;
                [taskArray addObject:voiceTask];
                [[ZFileUploadManager sharedInstance] addUploadTask:voiceTask];
                ...
                ZFileUploadTask *fileTask = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:filePath originFilePath:@"" fileName:sendChatMsg.fileName fileType:sendChatMsg.fileType isEncrypt:YES dataLength:sendChatMsg.fileSize uploadType:ZHttpUploadTypeFile beSendMessage:sendChatMsg delegate:self];
                fileTask.messageTaskType = FileUploadMessageTaskTypeFile;
                [taskArray addObject:fileTask];
                [[ZFileUploadManager sharedInstance] addUploadTask:fileTask];
                ...
                ZFileUploadTask *geoImgTask = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:geoImgPath originFilePath:@"" fileName:sendChatMsg.localImgName fileType:@"" isEncrypt:YES dataLength:geoImgData.length uploadType:ZHttpUploadTypeImage beSendMessage:sendChatMsg delegate:self];
                geoImgTask.messageTaskType = FileUploadMessageTaskTypeImage;
...
- (void)deleteSelectedMessageAction {
    ...
    WeakSelf
    ZMessageAlertView *msgAlertView;
    if (self.chatType == CIMChatType_SingleChat) {  //单聊
        msgAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeNomal supView:nil];
        //msgAlertView.checkboxLblContent.text = MultilingualTranslation(@"从我和当前好友的设备删除");
    } else {    //群聊
        //1管理员;2群主
        if (self.groupInfo.userGroupRole == 2 || self.groupInfo.userGroupRole == 1) {
            msgAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeCheckBox supView:nil];
            msgAlertView.checkboxLblContent.text = MultilingualTranslation(@"从所有人设备中删除");
        } else if (self.groupInfo.userGroupRole == 0) { //自己只能删除自己，单向删除
            msgAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeNomal supView:nil];
        }
    }
    msgAlertView.lblContent.text = MultilingualTranslation(@"删除所选消息");
    [msgAlertView.btnSure setTitle:MultilingualTranslation(@"确认") forState:UIControlStateNormal];
    [msgAlertView.btnCancel setTitle:MultilingualTranslation(@"取消") forState:UIControlStateNormal];
    [msgAlertView alertShow];
    msgAlertView.sureBtnBlock = ^(BOOL isCheckBox) {
...
//复制
- (void)messageCopyActionWithModel:(ZMessageModel *)menuModel {
    ...
    [HUD showMessage:MultilingualTranslation(@"复制成功") inView:self.view];
}

//复制原文
- (void)messageCopyContentActionWithModel:(ZMessageModel *)menuModel {
    UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
    if (menuModel.message.messageType == CIMChatMessageType_TextMessage) {
...
//复制译文
- (void)messageCopyTranslateActionWithModel:(ZMessageModel *)menuModel {
    UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
    if (menuModel.message.messageType == CIMChatMessageType_TextMessage) {
...
- (void)messageForwardActionWithMsgList:(NSArray *)msgModelList multiSelectType:(ZMultiSelectType)multiSelectType {
    ...
    [self.navigationController pushViewController:vc animated:YES];
    WeakSelf
    vc.forwardMsgSendSuccess = ^(NSArray<LingIMChatMessageModel *> * _Nullable sendForwardMsgList) {
        [HUD showMessage:MultilingualTranslation(@"转发成功") inView:weakSelf.view];
        //转发消息是转发给当前聊天对象
        [sendForwardMsgList enumerateObjectsUsingBlock:^(LingIMChatMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([weakSelf.sessionID isEqualToString:obj.toID]) {
                ZMessageModel *sendMsg = [[ZMessageModel alloc] initWithMessageModel:obj];
                //刷新并滚动到底部
                [weakSelf.messageModels addObject:sendMsg];
            }
        }];
...
//仅本地删除
- (void)messageDeleteActionWithMsg:(ZMessageModel *)msgModel index:(NSInteger)msgIndex {
    ZMessageModel *deleteMsgModel = [self.messageModels objectAtIndex:msgIndex];
...
//撤回
- (void)messageRevokeActionWithMsg:(ZMessageModel *)msgModel {
    ZMessageAlertView *msgAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeNomal supView:nil];
...
//引用
- (void)messageReferenceActionWithMsg:(ZMessageModel *)msgModel {
    self.viewInput.messageModelReference = msgModel;
    [self.viewInput inputViewBecomeFirstResponder];
}

//收藏
- (void)messageCollectionActionWithMsg:(ZMessageModel *)msgModel {
    //调用收藏消息接口
    //消息的会话类型
    ...
    [IMSDKManager MessageCollectionSave:dic onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (result) {
            [HUD showMessage:MultilingualTranslation(@"收藏成功") inView:weakSelf.view];
        } else {
            [HUD showMessage:MultilingualTranslation(@"收藏失败") inView:weakSelf.view];
        }
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD showMessageWithCode:code errorMsg:msg inView:weakSelf.view];
    }];
...
- (void)messageMultiSelectAction {
    ...
    [self.messageMutiSelectView setSelectCallback:^{
        for (NSInteger i = 0; i < weakSelf.messageModels.count; i++) {
            ZMessageModel *multiClickModel = [weakSelf.messageModels objectAtIndex:i];
        ...
        [weakSelf.baseTableView.indexPathsForVisibleRows enumerateObjectsUsingBlock:^(NSIndexPath * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([cell isKindOfClass:[ZMessageContentBaseCell class]]) {
                ZMessageContentBaseCell *contentCell = (ZMessageContentBaseCell *)cell;
...
- (void)multiSelectManage:(NSIndexPath *)indexPath {
    ...
    for (NSInteger i = indexPath.row; i < self.messageModels.count; i++) {
        ZMessageModel *multiClickModel = [self.messageModels objectAtIndex:i];
...
//存为
- (void)messageTextContentUrlAddTagWithMsg:(ZMessageModel *)msgModel {
    NSArray *urlArr;
    ...
    if (urlArr.count > 0) {
        ZChatTextUrlListView *textUrlListView = [[ZChatTextUrlListView alloc] initWithDataList:urlArr];
...
//翻译
- (void)messageContentTranslateActionWithMsg:(ZMessageModel *)msgModel {
    if ([NSString isNil:self.sessionModel.receiveTranslateChannel] && [NSString isNil:self.sessionModel.receiveTranslateLanguage]) {
        [HUD showMessage:MultilingualTranslation(@"请选择消息翻译的通道和语种") inView:self.view];
        ZTranslateSettingVC *vc = [[ZTranslateSettingVC alloc] init];
        vc.sessionModel = _sessionModel;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if ([NSString isNil:self.sessionModel.receiveTranslateLanguage]) {
        [HUD showMessage:MultilingualTranslation(@"请选择消息翻译的语种") inView:self.view];
        ZTranslateSettingVC *vc = [[ZTranslateSettingVC alloc] init];
        vc.sessionModel = _sessionModel;
        [self.navigationController pushViewController:vc animated:YES];
        return;
...
//隐藏译文
- (void)messageContentHiddenTranslateActionWithMsg:(ZMessageModel *)msgModel {
...
//添加表情到收藏
- (void)messageStickersAddToCollectionActionWithMsg:(ZMessageModel *)msgModel {
    //组装接口数据
    NSMutableDictionary *stickersDic = [NSMutableDictionary dictionary];
...
//听筒播放语音
- (void)messageMutePlaybackActionWithMsg:(ZMessageModel *)msgModel cellIndex:(NSIndexPath *)cellIndex {
    ZMessageVoiceHudView *voiceHudView = [ZMessageVoiceHudView new];
...
//查找当前表情所属表情包
- (void)messageStickersSearchPackageActionWithMsg:(ZMessageModel *)msgModel {
    //跳转表情包详情
    ZEmojiPackageDetailViewController *vc = [[ZEmojiPackageDetailViewController alloc] init];
    vc.stickersId = ![NSString isNil:msgModel.message.stickersId] ? msgModel.message.stickersId : @"";
    [self.navigationController pushViewController:vc animated:YES];
}

//替换messageModel
- (void)replaceMessageModelWithNewMsgModel:(LingIMChatMessageModel *)chatMessageModel {
    for(int i = 0; i<self.messageModels.count;i++) {
        ZMessageModel *tempMessagModel = (ZMessageModel *)[self.messageModels objectAtIndex:i];
        if ([tempMessagModel.message.msgID isEqualToString:chatMessageModel.msgID]) {
            ZMessageModel *newMessagModel = [[ZMessageModel alloc] initWithMessageModel:chatMessageModel];
...
- (void)messageDeleteWithSMgIds:(NSArray *)sMsgIdsList withDeleteType:(ZMsgDeleteType)deleteType {
    ...
    WeakSelf
    [HUD showActivityMessage:MultilingualTranslation(@"删除中...") inView:self.view];
    [[LingIMSDKManager sharedTool] deleteMessage:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        [weakSelf.selectedMsgModels enumerateObjectsUsingBlock:^(ZMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            //接口成功后，删除本地
        ...
        [HUD showMessage:MultilingualTranslation(@"消息删除成功") inView:weakSelf.view];
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD showMessage:MultilingualTranslation(@"消息删除失败") inView:weakSelf.view];
        [weakSelf setupMultiSelectedStatusDefaultIsReload:YES];
    }];
}

//消息撤回
- (void)messageReCallWithMsg:(ZMessageModel *)msgModel {
    ...
    [IMSDKManager recallMessage:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (isRecalled) {
            ...
            [HUD showMessage:MultilingualTranslation(@"消息撤回失败") inView:weakSelf.view];
        }
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD showMessageWithCode:code errorMsg:msg inView:weakSelf.view];
    }];
    ...
    dispatch_async(_chatMessageCalculateTimeQueue, ^{
        ...
        for (int i = 0; i < weakSelf.messageModels.count; i++) {
            ...
            if (i==0) {
                ...
                if ([ZMessageTimeTool isSameDay:temoMsgModel.message.sendTime Time2:prevSendTime]){
                    //不显示时间
...
- (void)computeOneMessageVisibleTimeWithMessage:(ZMessageModel *)currentMsg currentIndex:(NSInteger)currentIndex {
    if (currentIndex < self.messageModels.count) {
        if (currentIndex == 0) {
            currentMsg.isShowSendTime = YES;
        } else {
            ZMessageModel *prevMessage = [self.messageModels objectAtIndex:currentIndex - 1];
            //如果聊天消息是同一天的，不显示间隔的日期
            if ([ZMessageTimeTool isSameDay:currentMsg.message.sendTime Time2: prevMessage.message.sendTime]){
                //不显示时间
...
- (void)checkVisibleCellDoAnimation {
    ...
    ZMessageModel *selectedModel = [self.messageModels objectAtIndex:_chatHistorySelectIndex.row];
    WeakSelf
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [ZTOOL doInMain:^{
            ...
            [visibleList enumerateObjectsUsingBlock:^(NSIndexPath * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ZMessageModel *model = [weakSelf.messageModels objectAtIndex:obj.row];
                if ([selectedModel.message.msgID isEqualToString:model.message.msgID]) {
                    //需要执行动画的cell
                    ZMessageBaseCell *cellBase = [weakSelf.baseTableView cellForRowAtIndexPath:weakSelf.chatHistorySelectIndex];
...
- (void)imageVideoBrowserWith:(LingIMChatMessageModel *)messageModel {
    ...
    [currentImageVideoMessageList enumerateObjectsUsingBlock:^(LingIMChatMessageModel * _Nonnull chatMessage, NSUInteger idx, BOOL * _Nonnull stop) {
        //找到被点击的消息 下标
...
- (void)photoBrowser:(KNPhotoBrowser *)photoBrowser rightBtnOperationActionWithIndex:(NSInteger)index {
    LingIMChatMessageModel *currentModel = [_currentImageVideoMessageList objectAtIndexSafe:index];
...
- (void)saveVideoToAlbumWithUrl:(NSString *)videoUrl {
    [HUD showActivityMessage:MultilingualTranslation(@"正在保存...") inView:self.view];
...
#pragma mark - HomeChatGroupNoticeTipViewDelegate 群公告提示代理方法
...
- (void)messageTimeDeleteViewShow {
    ...
    HomeMssageTimeDeleteView *timeDeleteView;
    if (_messageTimeDeleteType == 0) {
        timeDeleteView = [[HomeMssageTimeDeleteView alloc] initWithShowCloseView:NO];
    }else {
        timeDeleteView = [[HomeMssageTimeDeleteView alloc] initWithShowCloseView:YES];
    }
    timeDeleteView.delegate = self;
    [timeDeleteView viewShow];
}

#pragma mark - HomeMssageTimeDeleteViewDelegate
...
- (void)lkCallRequestForSingleWith:(LingIMCallType)callType {
    ...
    ZMediaCallOptions *callOptions = [ZMediaCallOptions new];
    ...
    [ZMediaCallManager sharedManager].currentCallOptions = callOptions;
    [ZMediaCallManager sharedManager].userModel = self.userModel;
    [[ZMediaCallManager sharedManager] mediaCallRequestWith:callOptions onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        [ZMediaCallManager sharedManager].mediaCallState = ZMediaCallStateBegin;
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [ZMediaCallManager sharedManager].mediaCallState = ZMediaCallStateEnd;
    }];
}
...
- (void)zgCallRequestForSingleWith:(LingIMCallType)callType {
    if (!_userModel) return;
    if ([ZCallManager sharedManager].callState != ZCallStateEnd) return;
    ...
    [[ZCallManager sharedManager] requestSingleCallWith:dict callType:callType];
}
...
- (ZChatGroupCallTipView *)viewGroupCallTip {
    if (!_viewGroupCallTip) {
        _viewGroupCallTip = [[ZChatGroupCallTipView alloc] initWithFrame:CGRectMake(DWScale(17), DWScale(10) + DNavStatusBarH + DWScale(40), DScreenWidth - DWScale(34), DWScale(52))];
        _viewGroupCallTip.groupID = _sessionID;
    }
    return _viewGroupCallTip;
}

- (HomeChatGroupNoticeTipView *)viewGroupNotice {
    if (!_viewGroupNotice) {
        _viewGroupNotice = [[HomeChatGroupNoticeTipView alloc] initWithFrame:CGRectMake(DWScale(DWScale(17)), DStatusBarH + 44 + DWScale(40) + DWScale(10), DScreenWidth - DWScale(34), DWScale(42))];
        _viewGroupNotice.delegate = self;
    }
    return _viewGroupNotice;
}

- (CTalkMultiSelectSendHander *)collectionSendHander {
    if (!_collectionSendHander) {
        _collectionSendHander = [[CTalkMultiSelectSendHander alloc] init];
    }
    return _collectionSendHander;
}

- (HomeMssageMultiBottomView *)multiSelectBottomView {
    if (!_multiSelectBottomView) {
        _multiSelectBottomView = [[HomeMssageMultiBottomView alloc] initWithFrame:CGRectMake(0, DScreenHeight - DWScale(56) - DHomeBarH, DScreenWidth, DWScale(56) + DHomeBarH)];
...
- (HomeMssageMultiSelectView *)messageMutiSelectView{
    if (_messageMutiSelectView == nil) {
        _messageMutiSelectView = [[HomeMssageMultiSelectView alloc] init];
    }
    return _messageMutiSelectView;
}

- (ZChatAtBannedView *)atBannedView {
    if (_atBannedView == nil) {
        _atBannedView = [[ZChatAtBannedView alloc] init];
    }
