//
//  ZSafeCodeCloseViewController.m
//  CIMKit
//
...
@interface ZSafeCodeCloseViewController ()

@property (nonatomic, strong)MainInputTextView *safeCodePwdInput;
@property (nonatomic, assign)NSInteger securityCodeFailNum;

@end

@implementation ZSafeCodeCloseViewController
...
- (void)setupNavBarUI {
    self.navTitleStr = MultilingualTranslation(@"关闭安全码");
...
- (void)setupUI {
    ...
    originTitleLab.text = MultilingualTranslation(@"请输入安全码");
    ...
    if (self.operatorType == SafeCodeOperatorTypeClose) {
        originTitleLab.text = MultilingualTranslation(@"请输入安全码");
        self.safeCodePwdInput.placeholderText = MultilingualTranslation(@"请输入安全码");
    ...
    if (self.operatorType == SafeCodeOperatorTypePassword) {
        originTitleLab.text = MultilingualTranslation(@"请输入登录密码");
        self.safeCodePwdInput.placeholderText = MultilingualTranslation(@"请输入登录密码");
...
- (void)navBtnRightClicked {
    //关闭安全码(验证原安全码)
    if (self.operatorType == SafeCodeOperatorTypeClose) {
        if (![ZSafeSettingTools checkInputDeviceSafeCodeEndWithText:self.safeCodePwdInput.inputText.text]) {
...
- (void)requestCheckDeviceSafeCodeWithEncryptkey:(NSString *)encryptKey {
    ...
    [IMSDKManager authCloseSecurityCodeWith:params onSuccess:^(id  _Nullable data, NSString * _Nullable traceId) {
        ...
        if (result) {
            [HUD showMessage:MultilingualTranslation(@"关闭成功") inView:self.view];
        ...
        if (code == Auth_Original_Security_Code_Error_Code) {
            ...
            if (self.securityCodeFailNum < SECURITY_CODE_FAIL_MAX_NUM) {
                ...
                [HUD showMessage:MultilingualTranslation(@"原安全码输入错误3次需要验证登录密码") inView:self.view];
                ZSafeCodeCloseViewController *vc = [[ZSafeCodeCloseViewController alloc] init];
...
#pragma mark - Lazy
- (MainInputTextView *)safeCodePwdInput {
    if (!_safeCodePwdInput) {
        _safeCodePwdInput = [[MainInputTextView alloc] init];
        _safeCodePwdInput.clipsToBounds = YES;
        _safeCodePwdInput.isPassword = YES;
        _safeCodePwdInput.isShowBoard = NO;
        _safeCodePwdInput.placeholderText = MultilingualTranslation(@"请输入安全码");
