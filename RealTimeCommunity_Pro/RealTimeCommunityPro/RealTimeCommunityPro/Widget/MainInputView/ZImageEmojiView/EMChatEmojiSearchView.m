...
#import "ZChatEmojiSearchCell.h"
#import "MainSearchView.h"
#import "EMEmojiMenuPopView.h"

@interface EMChatEmojiSearchView() <UICollectionViewDelegate, UICollectionViewDataSource, DZNEmptyDataSetSource,DZNEmptyDataSetDelegate, MainSearchViewDelegate, UIGestureRecognizerDelegate, ZChatEmojiSearchCellDelegate>
...
@implementation EMChatEmojiSearchView
...
- (void)setupUI {
    ...
    _backView.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    [_backView round:DWScale(20) RectCorners:UIRectCornerTopLeft | UIRectCornerTopRight];
    [self addSubview:_backView];

    UILabel *titleLbl = [[UILabel alloc] init];
    titleLbl.text = MultilingualTranslation(@"表情搜索");
    ...
    [_collectionView registerClass:[ZChatEmojiSearchCell class] forCellWithReuseIdentifier:NSStringFromClass([ZChatEmojiSearchCell class])];
    ...
    [_moreEmojiBtn setTitle:MultilingualTranslation(@"更多表情 >") forState:UIControlStateNormal];
    [_moreEmojiBtn setTkThemeTitleColor:@[COLOR_81D8CF, COLOR_81D8CF] forState:UIControlStateNormal];
...
- (void)searchStickerResultLongTapAction:(NSIndexPath *)indexPath {
    ZChatEmojiSearchCell *longTapCell = (ZChatEmojiSearchCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
    ...
    [menuPopView setMenuClickBlock:^(void) {
        LingIMStickersModel *model = (LingIMStickersModel *)[self.searchResultList objectAtIndex:indexPath.row];
        //组装接口数据
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ZChatEmojiSearchCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([ZChatEmojiSearchCell class]) forIndexPath:indexPath];
    cell.delegate = self;
...
- (UIImage *)imageForEmptyDataSet:(UIScrollView *)scrollView {
    if (![NSString isNil:self.searchContent]) {
        ...
        return ImgNamed(@"recat_emoji_search_no_data");
    }
}
...
- (NSAttributedString *)titleForEmptyDataSet:(UIScrollView *)scrollView {
    NSString *desStr;
    if (![NSString isNil:self.searchContent]) {
        desStr = MultilingualTranslation(@"暂无数据");
    } else {
        desStr = MultilingualTranslation(@"请在上方输入搜索词条，找到那个能表达您真情实感的GIF。");
    }
