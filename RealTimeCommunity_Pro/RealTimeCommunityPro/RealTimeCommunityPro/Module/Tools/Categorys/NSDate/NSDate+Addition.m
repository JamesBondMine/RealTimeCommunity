...
//  ZIMChatService
//
//  Created by cusPro on 2022/7/13.
//
...
+ (NSString*)getOvertime:(NSString*)mStr isShowSecondStr:(BOOL)isShowSecond{
    ...
    if (d > 0)
    {
        _dStr = [NSString stringWithFormat:MultilingualTranslation(@"%ld天"),d];
    }
      
    if (h > 0)
    {
        _hStr = [NSString stringWithFormat:MultilingualTranslation(@"%ld小时"),h];
    }
    if (m > 0)
    {
        _mStr = [NSString stringWithFormat:MultilingualTranslation(@"%ld分钟"),m];
    }
    if (isShowSecond) {
        if (s > 0)
        {
            _sStr = [NSString stringWithFormat:MultilingualTranslation(@"%ld秒"),s];
        }
    }
    if (d>30) {
        _tStr = MultilingualTranslation(@"禁言");
    }else{
...
+ (NSString *)timeDescriptionFromTimestamp:(NSTimeInterval)timestamp {
    ...
    if (diff < 60) {
        return [NSString stringWithFormat:MultilingualTranslation(@"%@分钟前在线"), @"1"];
    } else if (diff < 60 * 60) {
        return [NSString stringWithFormat:MultilingualTranslation(@"%@分钟前在线"), [NSString stringWithFormat:@"%ld",(NSInteger)floor(diff / 60)]];
    } else if (diff < 60 * 60 * 24) {
        return [NSString stringWithFormat:MultilingualTranslation(@"%@小时前在线"), [NSString stringWithFormat:@"%ld",(NSInteger)floor(diff / (60 * 60))]];
    } else if (diff < 60 * 60 * 24 * 30) {
        return [NSString stringWithFormat:MultilingualTranslation(@"%@天前在线"), [NSString stringWithFormat:@"%ld",(NSInteger)floor(diff / (60 * 60 * 24))]];
    } else {
        return MultilingualTranslation(@"1月前在线");
    }
}
