...
#import "LJCategoryTitleView.h"
#import "NJCategoryFactory.h"

@implementation LJCategoryTitleView
...
- (Class)preferredCellClass {
    return [LJCategoryTitleCell class];
}

- (void)refreshDataSource {
    NSMutableArray *tempArray = [NSMutableArray arrayWithCapacity:self.titles.count];
    for (int i = 0; i < self.titles.count; i++) {
        LJCategoryTitleCellModel *cellModel = [[LJCategoryTitleCellModel alloc] init];
...
- (void)refreshSelectedCellModel:(NJCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(NJCategoryBaseCellModel *)unselectedCellModel {
    ...
    LJCategoryTitleCellModel *myUnselectedCellModel = (LJCategoryTitleCellModel *)unselectedCellModel;
    LJCategoryTitleCellModel *myselectedCellModel = (LJCategoryTitleCellModel *)selectedCellModel;
...
- (void)refreshLeftCellModel:(NJCategoryBaseCellModel *)leftCellModel rightCellModel:(NJCategoryBaseCellModel *)rightCellModel ratio:(CGFloat)ratio {
    ...
    LJCategoryTitleCellModel *leftModel = (LJCategoryTitleCellModel *)leftCellModel;
    LJCategoryTitleCellModel *rightModel = (LJCategoryTitleCellModel *)rightCellModel;

    if (self.isTitleLabelZoomEnabled && self.isTitleLabelZoomScrollGradientEnabled) {
        leftModel.titleLabelCurrentZoomScale = [NJCategoryFactory interpolationFrom:self.titleLabelZoomScale to:1.0 percent:ratio];
        rightModel.titleLabelCurrentZoomScale = [NJCategoryFactory interpolationFrom:1.0 to:self.titleLabelZoomScale percent:ratio];
    }

    if (self.isTitleLabelStrokeWidthEnabled) {
        leftModel.titleLabelCurrentStrokeWidth = [NJCategoryFactory interpolationFrom:leftModel.titleLabelSelectedStrokeWidth to:leftModel.titleLabelNormalStrokeWidth percent:ratio];
        rightModel.titleLabelCurrentStrokeWidth = [NJCategoryFactory interpolationFrom:rightModel.titleLabelNormalStrokeWidth to:rightModel.titleLabelSelectedStrokeWidth percent:ratio];
    }

    if (self.isTitleColorGradientEnabled) {
        leftModel.titleCurrentColor = [NJCategoryFactory interpolationColorFrom:self.titleSelectedColor to:self.titleColor percent:ratio];
        rightModel.titleCurrentColor = [NJCategoryFactory interpolationColorFrom:self.titleColor to:self.titleSelectedColor percent:ratio];
    }
}
...
- (void)refreshCellModel:(NJCategoryBaseCellModel *)cellModel index:(NSInteger)index {
    ...
    LJCategoryTitleCellModel *model = (LJCategoryTitleCellModel *)cellModel;
