...
@interface CActivityLevelHeaderView()
...
@implementation CActivityLevelHeaderView
...
- (void)setupUI {
    ...
    [levelBackImgView setImage:ImgNamed(@"recat_img_myLevel_score_back")];
    ...
    [attributedString addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"RoDINPlus-Bold" size:40] range:NSMakeRange(0, levelContent.length)];//设置字体
    ...
    [attributedString addAttribute:NSForegroundColorAttributeName value:COLOR_81D8CF range:NSMakeRange(0, levelContent.length)];//设置填充颜色为蓝色
    ...
    _scoreLbl.text = [NSString stringWithFormat:@"%@%d", MultilingualTranslation(@"分值: "), 0];
    ...
    tips1.text = MultilingualTranslation(@"群活跃等级根据你在群内发消息的情况变化，规则如下");
    tips1.font = FONTN(14);
    ...
    talkBackView.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    ...
    [talkIcon setImage:ImgNamed(@"recat_icon_group_sendmsg_score")];
    ...
    talkTitleLbl.text = MultilingualTranslation(@"群内发言");
    talkTitleLbl.font = FONTN(16);
    talkTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _groupSendMsgTipsLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"在群内每发一条消息得%ld分"), 1];
    ...
    onlineBackView.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    ...
    [onlineIcon setImage:ImgNamed(@"recat_icon_group_readed_score")];
    ...
    onlineTitleLbl.text = MultilingualTranslation(@"群内在线");
    onlineTitleLbl.font = FONTN(16);
    onlineTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _groupReadMsgTipsLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"在群内首次读消息得%ld分"), 1];
    ...
    tips2.text = MultilingualTranslation(@"每日分值上限");
    tips2.font = FONTN(14);
    ...
    dayMaxScroeBackView.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    ...
    _dayMaxScroeLbl.text = MultilingualTranslation(@"群活跃分值每日上限：");
    _dayMaxScroeLbl.font = FONTN(16);
    _dayMaxScroeLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    tips3.text = MultilingualTranslation(@"群活跃等级体系");
    tips3.font = FONTN(14);
...
- (void)setActivityInfoModel:(GHomeActivityInfoModel *)activityInfoModel {
    ...
    if (_activityInfoModel) {
        ...
        _scoreLbl.text = [NSString stringWithFormat:@"%@%ld", MultilingualTranslation(@"分值: "), (long)_myLevelScroe];
        
        ZGroupActivityActionModel *talkActionModel = (ZGroupActivityActionModel *)[_activityInfoModel.actions objectAtIndexSafe:0];
        if (talkActionModel) {
            _groupSendMsgTipsLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"在群内每发一条消息得%ld分"), (long)talkActionModel.score];
        }
        ZGroupActivityActionModel *onlineActionModel = (ZGroupActivityActionModel *)[_activityInfoModel.actions objectAtIndexSafe:1];
        if (onlineActionModel) {
            _groupReadMsgTipsLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"在群内首次读消息得%ld分"), (long)onlineActionModel.score];
        }
        
        _dayMaxScroeLbl.text = [NSString stringWithFormat:@"%@%ld", MultilingualTranslation(@"群活跃分值每日上限："), (long)_activityInfoModel.dailyLimit];
    }
}

- (NSString *)computeCurrentUserLevel {
    NSString *levelStr = @"--";
    for (ZGroupActivityLevelModel *levelConfigInfo in _activityInfoModel.sortLevels) {
