...
@property (nonatomic, strong) ZUserModel *userModel;//邀请我的用户信息
...
@property (nonatomic, strong) ZMediaCallMoreLayout *layoutV;//垂直方向布局
...
@implementation ZMediaCallMoreVC
...
- (void)setupMoreCallUI {
    ...
    //具体的UI相关实现
    //模糊头像背景
    _ivHeaderBg = [[BBBaseImageView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DScreenHeight)];
    ...
    //对方头像
    _ivHeader = [[BBBaseImageView alloc] initWithImage:DefaultAvatar];
    ...
    //当前通话信息
    ZMediaCallOptions *currentCallOptions = [ZMediaCallManager sharedManager].currentCallOptions;
    if (currentCallOptions.callType == LingIMCallTypeAudio) {
        ...
        [self.btnEnd setTitle:MultilingualTranslation(@"挂断") forState:UIControlStateNormal];
        ...
        [self.btnRefuse setTitle:MultilingualTranslation(@"拒绝") forState:UIControlStateNormal];
        ...
        [self.btnAccept setTitle:MultilingualTranslation(@"接听") forState:UIControlStateNormal];
...
- (void)setupMoreUIWithCallState {
    ...
    if ([ZMediaCallManager sharedManager].currentRoomCalling) {
        //当前通话已连接成功
        ...
        if (currentCallOptions.callType == LingIMCallTypeAudio) {
            ...
            //开启扬声器
            [[ZMediaCallManager sharedManager] mediaCallAudioSpeaker:YES];
        }
        ...
        if (currentCallOptions.callType == LingIMCallTypeAudio) {
            ...
            if (currentCallOptions.callRoleType == LingIMCallRoleTypeResponse) {
                //被邀请方
                _lblCallTip.text = MultilingualTranslation(@"邀请你进行语音通话");
            ...
            if (currentCallOptions.callRoleType == LingIMCallRoleTypeResponse) {
                //被邀请方
                _lblCallTip.text = MultilingualTranslation(@"邀请你进行视频通话");
                [self btnShowAnimationWith:self.btnRefuse];
                [self btnShowAnimationWith:self.btnAccept];
                _viewCallBg.hidden = YES;
                _viewBaseBg.hidden = NO;
...
- (void)updateUIWithUserModel {
    //获取对方用户信息
    _userModel = [ZMediaCallManager sharedManager].userModel;
    if (_userModel) {
        //对方头像
...
- (void)mediaCallRoomGroupMemberUpdate {
    ...
    dispatch_async(self->_mediaCallMoreMemberChangeQueue, ^{
        ...
        [currentParticipantList enumerateObjectsUsingBlock:^(MCMediaCallGroupMemberModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)mediaCallRoomGroupMemberLeave:(NSNotification *)notification {
    ...
    MCMediaCallGroupMemberModel *leaveModel = [MCMediaCallGroupMemberModel mj_objectWithKeyValues:dict];
    if (![leaveModel.groupID isEqualToString:[ZMediaCallManager sharedManager].currentCallOptions.groupId]) return;
    ...
    dispatch_async(self->_mediaCallMoreMemberChangeQueue, ^{
        ...
        if ([weakSelf.modelBig.userUid isEqualToString:leaveModel.userUid]) {
            ...
            [weakSelf.callUserList enumerateObjectsUsingBlock:^(MCMediaCallGroupMemberModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    MCMediaCallGroupMemberModel *selectModel = [_callUserList objectAtIndex:indexPath.row];
...
- (void)showBigMemberWith:(MCMediaCallGroupMemberModel *)selectModel {
    WeakSelf
    dispatch_async(self->_mediaCallMoreMemberChangeQueue, ^{
...
- (void)mediaCallRoomJoin {
    if ([ZMediaCallManager sharedManager].currentRoomCalling) return;
    
    LingIMCallOptions *callOptions = [LingIMCallOptions new];
    callOptions.callRoomUrl = [ZMediaCallManager sharedManager].currentCallOptions.callRoomUrl;
    callOptions.callRoomToken = [ZMediaCallManager sharedManager].currentCallOptions.callRoomToken;
    callOptions.callType = [ZMediaCallManager sharedManager].currentCallOptions.callType;
    callOptions.callRoleType = [ZMediaCallManager sharedManager].currentCallOptions.callRoleType;
    callOptions.callMicState = [ZMediaCallManager sharedManager].currentCallOptions.callMicState;
    callOptions.callCameraState = [ZMediaCallManager sharedManager].currentCallOptions.callCameraState;
    
    [[ZMediaCallManager sharedManager] mediaCallConnectRoomWith:callOptions delegate:self];
    
}
#pragma mark - 交互事件
//最小化
- (void)btnMiniClick {
    ZWindowFloatView *viewMediaCall = [[ZWindowFloatView alloc] initWithFrame:CGRectMake(0, DNavStatusBarH, DWScale(86), DWScale(101))];
    viewMediaCall.isKeepBounds = YES;
    [CurrentWindow addSubview:viewMediaCall];
    
    ZMediaCallMoreFloatView *viewFloat = [[ZMediaCallMoreFloatView alloc] initWithFrame:CGRectMake(0, 0, DWScale(86), DWScale(101))];
...
- (void)btnInviteClick {
    ...
    [_callUserList enumerateObjectsUsingBlock:^(MCMediaCallGroupMemberModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)btnEndClick {
    ...
    //调用接口
    [[ZMediaCallManager sharedManager] mediaCallGroupDiscardWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        
        [weakSelf dismissViewControllerAnimated:YES completion:nil];
        
        if ([ZMediaCallManager sharedManager].currentRoomCalling) {
            //断开连接
            [[ZMediaCallManager sharedManager] mediaCallDisconnect];
        }
        ...
        if ([NSString isNil:discardReason]) {
            //销毁一下定时器，挂断操作
            [[ZMediaCallManager sharedManager] deallocCurrentCallDurationTimer];
        }
...
- (void)btnMutedAudioClick {
    ...
    if (self.btnMutedAudio.isSelected) {
        //执行关闭静音
        [[ZMediaCallManager sharedManager] mediaCallAudioMute:NO complete:^(BOOL isMuted) {
            if (!isMuted) {
                weakSelf.btnMutedAudio.selected = NO;
                [HUD showMessage:MultilingualTranslation(@"静音关闭")];
            }
        }];
    } else {
        //执行开启静音
        [[ZMediaCallManager sharedManager] mediaCallAudioMute:YES complete:^(BOOL isMuted) {
            if (isMuted) {
                weakSelf.btnMutedAudio.selected = YES;
                [HUD showMessage:MultilingualTranslation(@"静音开启")];
            }
        }];
    }
}
...
- (void)btnMutedVideoClick {
    ...
    if (self.btnMutedVideo.isSelected) {
        //执行关闭静默
        [[ZMediaCallManager sharedManager] mediaCallVideoMute:NO complete:^(BOOL isMuted) {
            if (!isMuted) {
                weakSelf.btnMutedVideo.selected = NO;
                [HUD showMessage:MultilingualTranslation(@"摄像头开启")];
            }
        }];
        
    } else {
        //执行开启静默
        [[ZMediaCallManager sharedManager] mediaCallVideoMute:YES complete:^(BOOL isMuted) {
            if (isMuted) {
                weakSelf.btnMutedVideo.selected = YES;
                [HUD showMessage:MultilingualTranslation(@"摄像头关闭")];
            }
        }];
...
- (void)btnExternalClick {
    ...
    if (self.btnExternal.isSelected) {
        //关闭免提扬声器
        [[ZMediaCallManager sharedManager] mediaCallAudioSpeaker:NO];
        self.btnExternal.selected = NO;
        [HUD showMessage:MultilingualTranslation(@"免提关闭")];
    }else {
        //开启免提扬声器
        [[ZMediaCallManager sharedManager] mediaCallAudioSpeaker:YES];
        self.btnExternal.selected = YES;
        [HUD showMessage:MultilingualTranslation(@"免提开启")];
    }
...
- (void)btnCameraSwitchClick {
    [[ZMediaCallManager sharedManager] mediaCallVideoCameraSwitch:^(BOOL success) {
        DLog(@"切换摄像头");
    }];
}
...
- (void)callGroupAccept {
    WeakSelf
    ZMediaCallOptions *currentCallOptions = [ZMediaCallManager sharedManager].currentCallOptions;
    [[ZMediaCallManager sharedManager] mediaCallGroupAcceptWith:currentCallOptions onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            ZMediaCallOptions *callOptions = [ZMediaCallManager sharedManager].currentCallOptions;
...
- (void)room:(Room *)room didUpdateConnectionState:(enum ConnectionState)connectionState oldConnectionState:(enum ConnectionState)oldConnectionState {
    switch (connectionState) {
        ...
        {
            DLog(@"当前房间-连接成功-3");
            if ([ZMediaCallManager sharedManager].mediaCallState != ZMediaCallStateCall) {
                
                [ZMediaCallManager sharedManager].mediaCallState = ZMediaCallStateCall;
                //更新UI 主线程
                WeakSelf
                [ZTOOL doInMain:^{
                    [weakSelf setupMoreUIWithCallState];
                    [[ZMediaCallManager sharedManager] createCurrentCallDurationTimer];
                    [weakSelf mediaCallRoomGroupMemberUpdate];
                    [HUD showMessage:MultilingualTranslation(@"连接成功")];
                }];
...
- (void)room:(Room *)room participant:(RemoteParticipant *)participant didSubscribePublication:(RemoteTrackPublication *)publication track:(Track *)track {
    ...
    dispatch_async(self->_mediaCallMoreMemberChangeQueue, ^{
        ...
        if (!userInclude) {
            ...
            MCMediaCallGroupMemberModel *newModel = [MCMediaCallGroupMemberModel new];
...
#pragma mark - 懒加载
- (ZMediaCallMoreLayout *)layoutV {
    if (!_layoutV) {
        _layoutV = [ZMediaCallMoreLayout new];
