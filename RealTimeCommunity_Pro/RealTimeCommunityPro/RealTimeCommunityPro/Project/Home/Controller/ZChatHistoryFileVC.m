//
//  ZChatHistoryFileVC.m
//  CIMKit
//
//  Created by cusPro on 2023/2/2.
//

#import "ZChatHistoryFileVC.h"
#import "MainSearchView.h"
#import "TalkCellChatHistoryFileCell.h"
...
@implementation ZChatHistoryFileVC
...
- (void)setupUI {
    ...
    [self.baseTableView registerClass:[TalkCellChatHistoryFileCell class] forCellReuseIdentifier:[TalkCellChatHistoryFileCell cellIdentifier]];
}
...
- (NSAttributedString *)titleForEmptyDataSet:(UIScrollView *)scrollView {
    NSString *string = MultilingualTranslation(@"快去发送文件吧");
...
- (NSArray<UIView *> *)swipeTableCell:(MGSwipeTableCell *)cell swipeButtonsForDirection:(MGSwipeDirection)direction swipeSettings:(MGSwipeSettings *)swipeSettings expansionSettings:(MGSwipeExpansionSettings *)expansionSettings {
    ...
    LingIMChatMessageModel *model = [_showHistoryList objectAtIndexSafe:cellIndex.row];
    ZMessageModel *newMsgModel = [[ZMessageModel alloc] initWithMessageModel:model];
    WeakSelf
...
- (void)cellClickAction:(NSIndexPath *)indexPath {
    LingIMChatMessageModel *model = [_showHistoryList objectAtIndexSafe:indexPath.row];
    ZMessageModel *fileMsgModel = [[ZMessageModel alloc] initWithMessageModel:model];
    //文件消息-文件详情
...
//删除
- (void)messageDeleteActionWithMsg:(ZMessageModel *)msgModel {
    WeakSelf
    ...
    msgAlertView.lblContent.text = MultilingualTranslation(@"文件删除后将不会出现在您的聊天记录里，确认删除吗？");
...
//删除消息(单向/双向)
- (void)messageDeleteWithMsg:(ZMessageModel *)msgModel withDeleteType:(ZMsgDeleteType)deleteType {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TalkCellChatHistoryFileCell *cell = [tableView dequeueReusableCellWithIdentifier:[TalkCellChatHistoryFileCell cellIdentifier] forIndexPath:indexPath];
    cell.delegate = self;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [TalkCellChatHistoryFileCell defaultCellHeight];
}
