...
@implementation CNNewFriendListCell
...
#pragma mark - 界面赋值
- (void)setModel:(CMFriendApplyModel *)model {
    if (model) {
        _model = model;
        NSString *imgUrl;
        ...
        switch (model.beStatus) {
            case 0://申请中
            {
                if ([model.fromUserUid isEqualToString:UserManager.userInfo.userUID]) {
                    ...
                    [self.statusButton setTitle:MultilingualTranslation(@"等待验证") forState:UIControlStateNormal];
            ...
            case 1://已通过
                {
                    [self.statusButton setTitle:MultilingualTranslation(@"已添加") forState:UIControlStateNormal];
                    self.statusButton.tkThemebackgroundColors = @[COLOR_CCCCCC, COLOR_EEEEEE_DARK];
                    [_statusButton setTkThemeTitleColor:@[COLORWHITE, COLOR_CCCCCC_DARK] forState:UIControlStateNormal];
                    [self.statusButton mas_remakeConstraints:^(MASConstraintMaker *make) {
                        make.top.equalTo(self.contentView).offset(18);
            ...
            case 2://已过期
                {
                    [self.statusButton setTitle:MultilingualTranslation(@"已过期") forState:UIControlStateNormal];
                    self.statusButton.tkThemebackgroundColors = @[COLOR_CCCCCC, COLOR_EEEEEE_DARK];
                    [_statusButton setTkThemeTitleColor:@[COLORWHITE, COLOR_CCCCCC_DARK] forState:UIControlStateNormal];
                    [self.statusButton mas_remakeConstraints:^(MASConstraintMaker *make) {
                        make.top.equalTo(self.contentView).offset(18);
...
- (UILabel *)nickNameLabel {
    if (!_nickNameLabel) {
        ...
        _nickNameLabel.text = @"";
        _nickNameLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
