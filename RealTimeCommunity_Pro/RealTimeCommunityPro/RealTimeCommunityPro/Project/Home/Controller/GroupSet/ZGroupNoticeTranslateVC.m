...
@interface ZGroupNoticeTranslateVC () <UITableViewDelegate, UITableViewDataSource, ZNoticeTranslateCellDelegate>

@property (nonatomic, strong)NSMutableArray *dataArr;

@end

@implementation ZGroupNoticeTranslateVC
...
- (void)setupNavUI {
    self.navTitleStr = MultilingualTranslation(@"群公告翻译");
    self.navBtnRight.hidden = NO;
    [self.navBtnRight setTitle:MultilingualTranslation(@"发布") forState:UIControlStateNormal];
...
- (void)setupData {
    //原文
    ZNoticeTranslateModel *originModel = [[ZNoticeTranslateModel alloc] init];
    originModel.originNotice = _originNoticeContent;
    originModel.languageName = MultilingualTranslation(@"原文");
    ...
    if (_languageCodeArr.count > 0) {
        ...
        for(int i = 0; i< _languageCodeArr.count; i++) {
            ...
            ZNoticeTranslateModel *translateModel = [[ZNoticeTranslateModel alloc] init];
...
- (void)checkReleaseStatus {
    ...
    if (translateArr != nil && translateArr.count > 0) {
        ...
        for (ZNoticeTranslateModel *model in translateArr) {
    ...
    if (translateArr != nil && translateArr.count > 0) {
        ...
        for (ZNoticeTranslateModel *model in translateArr) {
            if ([model.languageCode isEqual:@"en"] && model.translateNotice.length <= 0) {
                [HUD showMessage:MultilingualTranslation(@"英文内容不可为空")];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    viewHeader.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    ...
    if (section == 1) {
        ...
        sectionTitleLbl.text = MultilingualTranslation(@"译文");
        sectionTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //保证cell不进行复用
    ZNoticeTranslateCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell) {
        cell = [[ZNoticeTranslateCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
    }
    cell.delegate = self;
    cell.baseCellIndexPath = indexPath;
    NSArray *contentArr = (NSArray *)[self.dataArr objectAtIndexSafe:indexPath.section];
    ZNoticeTranslateModel *model = (ZNoticeTranslateModel *)[contentArr objectAtIndexSafe:indexPath.row];
    cell.model = model;
    return cell;
}
