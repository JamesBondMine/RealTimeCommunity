...
@interface MainChatInputFunctionView () <UITextViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, MainChatInputActionCellDelegate>
...
@implementation MainChatInputFunctionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        NSDictionary *videoCall = @{
            @"actionTitle" : MultilingualTranslation(@"视频通话"),
        ...
        NSDictionary *audioCall =                 @{
            @"actionTitle" : MultilingualTranslation(@"语音通话"),
        ...
        NSDictionary *photoAlbum = @{
            @"actionTitle" : MultilingualTranslation(@"相册"),
        ...
        self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
            if (themeIndex == 1) {
                ...
                weakSelf.typingAttributes = @{
                    NSFontAttributeName:FONTR(16),
                    NSForegroundColorAttributeName:COLOR_33
                };
            }
        };
...
- (void)setupUI {
    ...
    [_btnMore setTkThemeImage:@[ImgNamed(@"com_c_input_more"), ImgNamed(@"com_c_input_more_dark")] forState:UIControlStateNormal];
    ...
    [_btnEmoji setImage:ImgNamed(@"com_c_input_emoji") forState:UIControlStateNormal];
    [_btnEmoji setTkThemeImage:@[ImgNamed(@"com_c_input_emoji_s"), ImgNamed(@"com_c_input_emoji_s_dark")] forState:UIControlStateSelected];
    ...
    [_btnVoice setImage:ImgNamed(@"com_c_input_voice") forState:UIControlStateNormal];
    [_btnVoice setImage:ImgNamed(@"com_c_input_send") forState:UIControlStateSelected];
    [_btnVoice setImage:ImgNamed(@"com_c_input_send") forState:UIControlStateSelected | UIControlStateHighlighted];
    ...
    [_actionCollectionView registerClass:[MainChatInputActionCell class] forCellWithReuseIdentifier:NSStringFromClass([MainChatInputActionCell class])];
...
- (void)inputAtUserInfo:(NSDictionary *)atUserDict {
    if (atUserDict) {
        ...
        if ([key isEqualToString:UserManager.userInfo.userUID]) {
            value = MultilingualTranslation(@"我自己");
        } else {
    ...
    if (_btnVoice.selected) {
        ...
        [ZTOOL getMicrophoneAuth:^(BOOL granted) {
            ...
            if (granted) {
                ...
                [HUD showMessage:MultilingualTranslation(@"需要获取麦克风权限")];
            }
        }];
    }
}
    ...
    if (newText.length < 1) {
        ...
        self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
            ...
            if (safeLength > 0) {
                if (themeIndex == 1) {
                    ...
                    [string addAttribute:NSForegroundColorAttributeName value:COLOR_33 range:NSMakeRange(0, safeLength)];
                }
            }
        };
        ...
        if (self.atSegments && self.atSegments.count > 0) {
            for (NSDictionary *seg in self.atSegments) {
                ...
                [string addAttribute:NSForegroundColorAttributeName value:COLOR_81D8CF range:highlightRange];
            }
        }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    MainChatInputActionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([MainChatInputActionCell class]) forIndexPath:indexPath];
...
#pragma mark - MainChatInputActionCellDelegate
        ...
        case ZChatInputActionTypePhotoAlbum://相册
        {
            ...
            [IMSDKManager userGetUserAuthorityWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
                ...
                if (resultData) {
                    ...
                    //没有操作权限
                    [HUD showMessage:MultilingualTranslation(@"无操作权限")];
                }
...
- (ZChatTextView *)tvContent {
    if (!_tvContent) {
        _tvContent = [ZChatTextView new];
        _tvContent.isCanPerform = YES;
        _tvContent.backgroundColor = UIColor.clearColor;
        _tvContent.font = FONTR(16);
        _tvContent.tkThemetextColors = @[COLOR_33, COLORWHITE];
