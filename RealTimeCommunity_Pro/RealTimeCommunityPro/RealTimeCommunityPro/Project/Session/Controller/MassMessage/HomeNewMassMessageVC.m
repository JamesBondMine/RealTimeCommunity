//
//  HomeNewMassMessageVC.m
//  CIMKit
//
//  Created by cusPro on 2023/4/17.
//

#import "HomeNewMassMessageVC.h"
#import "NSNewMassMessageContentView.h"
#import "NSMassMessageUserHeaderCell.h"
...
@interface HomeNewMassMessageVC () <UICollectionViewDataSource, UICollectionViewDelegate, ZMassMessageSelectUserDelegate, NSNewMassMessageContentViewDelegate>
@property (nonatomic, strong) UIView *viewUser;//接受消息人
@property (nonatomic, strong) UICollectionView *collectionView;

@property (nonatomic, strong) NSNewMassMessageContentView *viewMessage;//消息内容
...
@property (nonatomic, strong) NSArray<ZBaseUserModel *> *selectedUserList;//选中的用户信息
...
@implementation HomeNewMassMessageVC

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.navTitleStr = MultilingualTranslation(@"群发");
...
- (void)setupUI {
    ...
    viewHeader.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
    ...
    [_collectionView registerClass:[NSMassMessageUserHeaderCell class] forCellWithReuseIdentifier:NSStringFromClass([NSMassMessageUserHeaderCell class])];
    ...
    _viewMessage = [[NSNewMassMessageContentView alloc] initWithFrame:CGRectMake(DWScale(16), DWScale(32) + viewUserH, DScreenWidth - DWScale(32), DWScale(200))];
    ...
    _tfTarget.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    _tfTarget.font = FONTR(12);
    NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:MultilingualTranslation(@"设置群发标签（非必填）") attributes:
    ...
    [_btnSend setTitle:MultilingualTranslation(@"立即发送") forState:UIControlStateNormal];
    _btnSend.titleLabel.font = FONTR(16);
    [_btnSend setTkThemeTitleColor:@[COLORWHITE, COLORWHITE_DARK] forState:UIControlStateNormal];
    [_btnSend setTkThemebackgroundColors:@[COLOR_81D8CF, COLOR_81D8CF_DARK]];
...
- (void)btnSendClick {
    ...
    if (_massMessageSelectType == MassMessageSelectTypeText) {
        ...
        [HUD showActivityMessage:MultilingualTranslation(@"发送中...")];
...
- (void)groupHairSendHairGroupMessageWithJsonStr:(NSString *)jsonStr {
    ...
    [IMSDKManager GroupHairSendHairMessageWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        [HUD showMessage:MultilingualTranslation(@"发送成功")];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    NSMassMessageUserHeaderCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([NSMassMessageUserHeaderCell class]) forIndexPath:indexPath];
    ...
    if (_selectedUserList.count > 11 && indexPath.row == 0) {
        ...
        cell.lblNumber.text = [NSString stringWithFormat:MultilingualTranslation(@"%ld人"), _selectedUserList.count];
    }else {
...
- (void)goSelectReceiveMessageUser {
    ...
    HomeMassMessageSelectUserVC *vc = [HomeMassMessageSelectUserVC new];
...
- (void)massMessageSelectedUserList:(NSArray<ZBaseUserModel *> *)selectedUserList {
    ...
    WeakSelf
    [_selectedUserList enumerateObjectsUsingBlock:^(ZBaseUserModel* _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.isGroup) {
...
#pragma mark - NSNewMassMessageContentViewDelegate
    ...
    [[PHImageManager defaultManager] requestImageDataAndOrientationForAsset:mediaAsset options:options resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, CGImagePropertyOrientation orientation, NSDictionary * _Nullable info) {
        ...
        if ([[info valueForKey:@"PHImageResultIsDegradedKey"] integerValue] == 0){
            ...
            [ZTOOL doAsync:^{
                ...
                //上传缩略图task
                ZFileUploadTask *thumbTask = [[ZFileUploadTask alloc] initWithTaskId:thumbImgFileName filePath:thumbnailImagePath originFilePath:imagePath fileName:thumbImgFileName fileType:@"" isEncrypt:YES dataLength:thumbImageData.length uploadType:ZHttpUploadTypeImageThumbnail beSendMessage:nil delegate:nil];
                thumbTask.messageTaskType = FileUploadMessageTaskTypeNoamlImgThumb;
                [taskArr addObject:thumbTask];
                
                //上传原图
                ZFileUploadTask *task = [[ZFileUploadTask alloc] initWithTaskId:originImageName filePath:imagePath originFilePath:@"" fileName:originImageName fileType:@"" isEncrypt:YES dataLength:imageData.length uploadType:ZHttpUploadTypeImage beSendMessage:nil delegate:nil];
                task.messageTaskType = FileUploadMessageTaskTypeNoamlImg;
                [taskArr addObject:task];
                
                __block NSInteger taskNum = 0;
                NSBlockOperation *blockOperation = [NSBlockOperation blockOperationWithBlock:^{
                    for (ZFileUploadTask *task in taskArr) {
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:mediaAsset options:options resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
        if ([asset isKindOfClass:[AVURLAsset class]]) {
            ...
            [ZTOOL doAsync:^{
                ...
                //封面
                ZFileUploadTask *coverTask = [[ZFileUploadTask alloc] initWithTaskId:coverImgName filePath:coverImgPath originFilePath:@"" fileName:coverImgName fileType:@"" isEncrypt:YES dataLength:coverImgData.length uploadType:ZHttpUploadTypeImage beSendMessage:nil delegate:nil];
                coverTask.messageTaskType = FileUploadMessageTaskTypeCover;
                [taskArray addObject:coverTask];
                
                //视频
                ZFileUploadTask *videoTask = [[ZFileUploadTask alloc] initWithTaskId:videoName filePath:videoPath originFilePath:@"" fileName:videoName fileType:@"" isEncrypt:YES dataLength:videoData.length uploadType:ZHttpUploadTypeVideo beSendMessage:nil delegate:nil];
...
- (void)uploadFile {
    ...
    ZFilePickModel *fileModel = (ZFilePickModel *)_attachment;
    ...
    if (fileModel.fileSource == ZMsgFileSourceTypePhone) {
        ...
        [ZTOOL doAsync:^{
            ...
            ZFileUploadTask *fileTask = [[ZFileUploadTask alloc] initWithTaskId:extensionFileName filePath:fileSaxboxPath originFilePath:@"" fileName:extensionFileName fileType:fileType isEncrypt:YES dataLength:fileData.length uploadType:ZHttpUploadTypeFile beSendMessage:nil delegate:nil];
        ...
        [[PHImageManager defaultManager] requestAVAssetForVideo:fileModel.videoAsset options:options resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
            if ([asset isKindOfClass:[AVURLAsset class]]) {
                ...
                [ZTOOL doAsync:^{
                    ...
                    //上传
                    ZFileUploadTask *fileTask = [[ZFileUploadTask alloc] initWithTaskId:fileName filePath:fileSaxboxPath originFilePath:@"" fileName:fileName fileType:fileType isEncrypt:YES dataLength:fileVideoData.length uploadType:ZHttpUploadTypeFile beSendMessage:nil delegate:nil];
