...
- (void)configTipAttributedString {
    ...
    if (_model.memberState == ZCallUserStateTimeOut) {
        tipAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"对方正忙")];
    }else if (_model.memberState == ZCallUserStateRefuse) {
        tipAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"对方拒绝")];
    }else if (_model.memberState == ZCallUserStateHangup) {
        tipAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"对方离开")];
            ...
            if (model.callType == LingIMCallTypeAudio) {
                ...
                if ([ZHostTool.appSysSetModel.video_source_config isEqualToString:@"0"]) {
                    ...
                    if (model.callUserModel) {
                        ...
                        if ([modelUserUid isEqualToString:mineUserUid]) {
                            ...
                            [[ZCallManager sharedManager] callRoomStartPlayingStream:streamIDStr with:weakSelf.sampleViewVideo.viewVideoZG];
                        }
                    }
                }
                ...
                if ([ZHostTool.appSysSetModel.video_source_config isEqualToString:@"0"]) {
                    ...
                    if (model.callUserModel) {
                        ...
                        if ([modelUserUid isEqualToString:mineUserUid]) {
                            //本地 渲染 视频
                            [[ZCallManager sharedManager] callRoomStartPreviewWith:weakSelf.sampleViewVideo.viewVideoZG];
                        }else {
                            //拉流 渲染远端 视频
                            NSString *streamIDStr = [NSString stringWithFormat:@"%@", model.callUserModel.streamID];
                            [[ZCallManager sharedManager] callRoomStartPlayingStream:streamIDStr with:weakSelf.sampleViewVideo.viewVideoZG];
