...
@interface HomeMessageForwardFailCell()

@property (nonatomic, strong)BBBaseImageView *ivGroupHeader;//群头像
@property (nonatomic, strong)UILabel *ivGroupNameLbl;//群名称
@property (nonatomic, strong)UILabel *ivReasonLbl;//转发失败原因

@end

@implementation HomeMessageForwardFailCell
...
#pragma mark - 界面布局
- (void)setupUI {
    _ivGroupHeader = [BBBaseImageView new];
    ...
    _ivGroupNameLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
#pragma mark - Data
- (void)setPreCheckFailModel:(FMsgPrecheckModel *)preCheckFailModel {
    ...
    switch (_preCheckFailModel.exceptionInfo.code) {
        case NetWork_Error_Friend_Delete:
            _ivReasonLbl.text = MultilingualTranslation(@"已被对方删除");
            break;
        case NetWork_Error_Friend_Blacklist:
            _ivReasonLbl.text = MultilingualTranslation(@"已将对方拉黑");
            break;
        case NetWork_Error_Friend_BeBlacklist:
            _ivReasonLbl.text = MultilingualTranslation(@"已被对方拉黑");
            break;
        case NetWork_Error_Group_Nonentity:
            _ivReasonLbl.text = MultilingualTranslation(@"群组不存在");
            break;
        case NetWork_Error_Group_Single_Silent:
            _ivReasonLbl.text = MultilingualTranslation(@"已被群组禁言");
            break;
        case NetWork_Error_Group_All_Silent:
            _ivReasonLbl.text = MultilingualTranslation(@"群组开启全员禁言");
            break;
        case NetWork_Error_Group_Single_ShutDown:
            _ivReasonLbl.text = MultilingualTranslation(@"群组已封停");
            break;
        case NetWork_Error_Group_Not_In:
            _ivReasonLbl.text = MultilingualTranslation(@"已不在群组");
            break;
        case NetWork_Error_Group_Interval:
            _ivReasonLbl.text = MultilingualTranslation(@"发送频次过快");
            break;
        case NetWork_Error_Group_Number_Limit:
            _ivReasonLbl.text = MultilingualTranslation(@"转发数量超过限制");
            break;
        default:
            _ivReasonLbl.text = MultilingualTranslation(@"未知");
            break;
    }
}
