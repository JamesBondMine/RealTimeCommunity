//
//  IVTTeamInviteDetailDataHandle.m
//  CIMKit
//
...
@interface IVTTeamInviteDetailDataHandle()

/// 从上个页面传入的团队信息
@property (nonatomic, strong, readwrite) NTMTeamModel *currentTeamModel;

/// 获取到的团队详情信息
@property (nonatomic, strong, readwrite) IVTTeamDetailModel *teamDetailModel;

@end

@implementation IVTTeamInviteDetailDataHandle
            ...
            return [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
                ...
                [IMSDKManager imTeamDetailWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
                    if ([data isKindOfClass:[NSDictionary class]]) {
                        ...
                        self.teamDetailModel = [IVTTeamDetailModel mj_objectWithKeyValues:dataDict];
                        if([self.teamDetailModel.teamName isEqualToString:@"默认团队"]){
                            self.teamDetailModel.teamName = MultilingualTranslation(@"默认团队");
                        }
                    }
        ...
        _editTeamDetailInfoCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal * _Nonnull(id  _Nullable input) {
            ...
            return [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
                ...
                [IMSDKManager imTeamEditWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
                    [HUD showMessage:MultilingualTranslation(@"置顶成功")];
                    [subscriber sendNext:@(YES)];
                    [subscriber sendCompleted];
                } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
                    [HUD showMessageWithCode:code errorMsg:msg];
