...
#import <MMKV/MMKV.h>
#import "LingIMLoganManager.h"
#import <Sentry/Sentry.h>
#import "Sentry-Swift.h"

@interface LingIMHttpManager()

@end

@implementation LingIMHttpManager
{
    NSCache *_interfaceTimeConsumingCatche;//接口耗时统计 NSCache是线程安全的
}

static LingIMHttpManager *manager = nil;
...
//网络请求配置
+ (LingIMHttpManager *)setConfig {
    //基本网络地址
    NSURLSessionConfiguration *config = [NSURLSessionConfiguration defaultSessionConfiguration];
    config.connectionProxyDictionary = @{}; // 关闭系统代理
    LingIMHttpManager *httpManager = [[LingIMHttpManager alloc] initWithBaseURL:[NSURL URLWithString:@"https://www.baidu.com"] sessionConfiguration:config];
...
- (NSDictionary *)configHttpHeaderWithUrl:(NSString *)url {
    ...
    //日志跟踪
    [headerDic setObject:[[LingIMManagerTool sharedManager] getMessageID] forKey:@"ZTID"];
    //版本号
    [headerDic setObject:[LingIMDeviceTool appVersion] forKey:@"version"];
    //租户信息
...
- (void)netRequestWithType:(LingIMHttpRequestType)type path:(NSString *)path parameters:(NSDictionary *)parameters onSuccess:(LingIMSuccessCallback)onSuccess onFailure:(LingIMFailureCallback)onFailure {
    ...
    __block NSURLSessionDataTask *task =  [self dataTaskWithHTTPMethod:medthStr URLString:fullPath parameters:parameters headers:hearder uploadProgress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        LingIMHttpResponse *resp = [LingIMHttpResponse mj_objectWithKeyValues:responseObject];
        if (resp.isHttpSuccess) {
            ...
            if (resp.code == LingIMHttpResponseCodeTokenOutTime ||
                ...
                //执行用户 强制下线 代理回调
                if ([weakSelf.userDelegate respondsToSelector:@selector(cimSdkUserForceLogout:message:)]) {
                    [weakSelf.userDelegate cimSdkUserForceLogout:999 message:@""];
                }
            } else if (resp.code == LingIMHttpResponseCodeUsedIpDisabled) {
                //执行用户 强制下线 代理回调并给出提示语
                if ([weakSelf.userDelegate respondsToSelector:@selector(cimSdkUserForceLogout:message:)]) {
                    [weakSelf.userDelegate cimSdkUserForceLogout:LingIMHttpResponseCodeUsedIpDisabled message:resp.message];
                }
                if (onFailure) {
                    ...
                    strongSelf.updateBaseHostOpeartion = [NSBlockOperation blockOperationWithBlock:^{
                        ...
                        if (nodeResultList != nil && nodeResultList.count > 0) {
                            ...
                            if (fastNodeHost != nil && fastNodeHost.length > 0) {
                                ...
                                if ([strongSelf.userDelegate respondsToSelector:@selector(cimSdkUpdateHttpNodeWith:)]) {
                                    [strongSelf.userDelegate cimSdkUpdateHttpNodeWith:requestHost];
                                }
                            } else {
                                ...
                                if ([strongSelf.userDelegate respondsToSelector:@selector(cimSdkUnableHttpNodeWith:)]) {
                                    [strongSelf.userDelegate cimSdkUnableHttpNodeWith:@"网络连接异常 请联系管理员"];
                                }
                            }
                        } else {
                            [strongSelf.netWorkingQueue cancelAllOperations];
...
- (void)netRequestForwardWithPath:(NSString *)path paramData:(NSData *)paramData onSuccess:(LingIMSuccessCallback)onSuccess onFailure:(LingIMFailureCallback)onFailure {
    ...
    //日志跟踪
    NSString *ztid = [[LingIMManagerTool sharedManager] getMessageID];
    [request addValue:ztid forHTTPHeaderField:@"ZTID"];
    //租户信息
    [request addValue:IMSDKManager.orgName forHTTPHeaderField:@"orgName"];
    //版本号
    [request addValue:[LingIMDeviceTool appVersion] forHTTPHeaderField:@"version"];
    //loginuseruid
...
- (void)authRefreshTokenOnSuccess:(LingIMSuccessCallback)onSuccess onFailure:(LingIMFailureCallback)onFailure {
    ...
    [self netRequestWithType:LingIMHttpRequestTypePOST path:Auth_Refresh_Token_Url parameters:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if([newToken isEqualToString:IMSDKManager.myUserToken] || newToken.length == 0 || newToken == nil){
            ...
            if ([weakSelf.userDelegate respondsToSelector:@selector(cimSdkRefreshUsetToken: errorMsg:)]) {
                [weakSelf.userDelegate cimSdkRefreshUsetToken:@"" errorMsg:nil];
            }
            if (onFailure) {
                onFailure(0, @"", ztid);
            }
            ...
            //更新Kit层token
            if ([weakSelf.userDelegate respondsToSelector:@selector(cimSdkRefreshUsetToken: errorMsg:)]) {
                [weakSelf.userDelegate cimSdkRefreshUsetToken:newToken errorMsg:@""];
            }
        ...
        [weakSelf.netWorkingQueue cancelAllOperations];
        if ([weakSelf.userDelegate respondsToSelector:@selector(cimSdkRefreshUsetToken: errorMsg:)]) {
            [weakSelf.userDelegate cimSdkRefreshUsetToken:@"" errorMsg:msg];
        }
        //账号封禁、设备封禁、IP封禁
        if (code == Auth_User_Account_Banned || code == Auth_User_Device_Banned || code == Auth_User_IPAddress_Banned) {
            if ([weakSelf.userDelegate respondsToSelector:@selector(cimSdkRefreshTokenAuthBanned:)]) {
                [weakSelf.userDelegate cimSdkRefreshTokenAuthBanned:code];
            }
            return;
        }
        //traceId
