...
@implementation CNGlobalSearchCell
...
#pragma mark - 界面布局
- (void)setupUI {
    _ivHeader = [[BBBaseImageView alloc] initWithFrame:CGRectMake(DWScale(16), DWScale(12), DWScale(44), DWScale(44))];
    ...
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        ...
        switch (themeIndex) {
            ...
            {
                //暗黑
                weakSelf.lblTitle.textColor = COLOR_33_DARK;
            ...
            {
                //浅色
                weakSelf.lblTitle.textColor = COLOR_33;
...
- (void)globalSearchConfigWith:(NSIndexPath *)cellIndex model:(id)model search:(NSString *)searchStr{
    ...
    //聊天记录
    if (cellIndex.section == 2 && [model isKindOfClass:[LingIMChatMessageModel class]]) {
...
- (void)updateFriendUI {
    ...
    if (friendModel.disableStatus == 4) {
        ...
        _lblTitle.text = MultilingualTranslation(@"账号已注销");
        _lblTitleSub.text =  [NSString stringWithFormat:@"%@%@",MultilingualTranslation(@"账号："), @"-"];
        _lblUserRoleName.hidden = YES;
    } else {
        ...
        if(![NSString isNil:friendModel.remarks]){
            titleContent = [NSString stringWithFormat:MultilingualTranslation(@"%@ (%@)"), friendModel.remarks, friendModel.nickname];
        }else{
        ...
        NSString *subContent = [NSString stringWithFormat:MultilingualTranslation(@"账号：%@"),friendModel.userName];
...
- (void)updateHistoryMessageUI {
    ...
    LingIMChatMessageModel *historyMsgModel = (LingIMChatMessageModel *)_cellModel;
