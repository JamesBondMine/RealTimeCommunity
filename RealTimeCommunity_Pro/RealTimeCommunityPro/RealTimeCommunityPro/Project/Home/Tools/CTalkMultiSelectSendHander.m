...
@interface CTalkMultiSelectSendHander()

@end

@implementation CTalkMultiSelectSendHander
...
- (void)chatMultiSelectSendForwardMessageList:(NSArray *)forwardMsgList imMessage:(IMChatMessageList *)imMessage {
    ...
    WeakSelf
    [[LingIMSDKManager sharedTool] transpondMessage:messageData onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
...
- (void)insertForwardMessageToDataBaseWithDic:(NSDictionary *)dic forwardMessageList:(NSArray *)forwardMsgList imMessageList:(IMChatMessageList *)imMessageList {
    ...
    for (int i = 0; i<toMessages.count; i++) {
        ...
        for (int j = 0; j<forwardMsgList.count; j++) {
            ...
            ZMessageModel *forwardMessage = (ZMessageModel *)[forwardMsgList objectAtIndex:j];
            IMChatMessage *messageChat = [ZMessageTools getIMChatMessageFromLingIMChatMessageModelToMergeForward:forwardMessage.message];
            messageChat.from = UserManager.userInfo.userUID;
            LingIMChatMessageModel *forwardSendMessage = [[LingIMModelTool sharedTool] getChatMessageModelFromIMChatMessage:messageChat];
            ...
            if (forwardMessage.isSelf) {
                if (forwardMessage.message.messageType == CIMChatMessageType_AtMessage) {
                    forwardSendMessage.textContent = [ZMessageTools forwardMessageAtContenTranslateToShowContent:forwardMessage.message.atContent atUsersDictList:forwardMessage.message.atUsersInfoList];
                ...
                if (forwardMessage.message.messageType == CIMChatMessageType_AtMessage) {
                    ...
                    forwardSendMessage.textContent = [ZMessageTools forwardMessageAtContenTranslateToShowContent:![NSString isNil:forwardMessage.message.atTranslateContent] ? forwardMessage.message.atTranslateContent : forwardMessage.message.atContent atUsersDictList:forwardMessage.message.atUsersInfoList];
                    forwardSendMessage.messageType = CIMChatMessageType_TextMessage;
                }
                if (forwardMessage.message.messageType == CIMChatMessageType_TextMessage) {
...
- (void)chatMultiSelectRecommendFriendCard:(NSString *)friendUid receiverList:(NSArray *)receiverList {
    ...
    for (ZBaseUserModel *receiverModel in receiverList) {
...
- (void)insertCardMessageToDataBaseWithDic:(NSDictionary *)dic {
    ...
    for (NSDictionary *tempResultDic in receiveUserMsgInfosArr) {
        ...
        if (status == NO) {
            LingIMChatMessageModel *cardSendMessage = [LingIMChatMessageModel new];
...
- (void)chatMultiSelectShareQRcodeMessage:(UIImage *)qrImage selectObjectList:(NSArray *)selectObjectList {
    //先上传二维码图片
    WeakSelf
    [ZMessageSendHander ZMessageAssembleQRcodeImage:qrImage compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
        ...
        //缩略图
        ZFileUploadTask *thumbTask = [[ZFileUploadTask alloc] initWithTaskId:[NSString stringWithFormat:@"thumbnail_%@", sendChatMsg.msgID] filePath:thumbImagePath originFilePath:imagePath fileName:thumbImageName fileType:@"" isEncrypt:YES dataLength:thumbImageData.length uploadType:ZHttpUploadTypeImageThumbnail beSendMessage:nil delegate:nil];
        thumbTask.messageTaskType = FileUploadMessageTaskTypeNoamlImgThumb;
        [taskArray addObject:thumbTask];
        
        //原图
        ZFileUploadTask *task = [[ZFileUploadTask alloc] initWithTaskId:sendChatMsg.msgID filePath:imagePath originFilePath:@"" fileName:imageName fileType:@"" isEncrypt:YES dataLength:imageData.length uploadType:ZHttpUploadTypeImage beSendMessage:nil delegate:nil];
...
//组装消息体
- (void)assembleQRcodeMessageContentWithMessage:(LingIMChatMessageModel *)qrMsg selectObjectList:(NSArray *)selectObjectList {
    ...
    IMMessage *messageModel = [ZMessageTools getIMMessageFromLingIMChatMessageModel:qrMsg withChatObject:selectObjectList.firstObject index:0];
    ...
    for (ZBaseUserModel *receiver in selectObjectList) {
...
- (void)insertShareQrcodeMessageToLocalDatabaseDic:(NSDictionary *)dic qrCodeMessage:(LingIMChatMessageModel *)qrCodeMessage imMessageList:(IMChatMessageList *)imMessageList {
    NSArray *failIdArray = dic[@"failId"];
    NSArray * sMsgIdTempArr = dic[@"sMsgId"];
    ...
    for (int i = 0; i<toMessages.count; i++) {
        ToMessage *toRecever = [toMessages objectAtIndex:i];
        NSArray *sMsgIdArr = [sMsgIdTempArr objectAtIndex:i];
        LingIMChatMessageModel *qrcodeSendMessage = [LingIMChatMessageModel new];
...
- (void)chatCollectionMessagSendWith:(ZMyCollectionItemModel *)collectionMsg chatType:(CIMChatType)chatType sessionId:(NSString *)sessionId {
    ...
    IMMessage *sendImMessage = [ZMessageTools getIMMessageFromCollection:collectionMsg withChatType:chatType chatSessionId:sessionId];
