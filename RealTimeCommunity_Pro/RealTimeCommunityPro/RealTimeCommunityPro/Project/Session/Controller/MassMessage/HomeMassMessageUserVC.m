//
//  HomeMassMessageUserVC.m
//  CIMKit
//
//  Created by cusPro on 2023/4/19.
//

#import "HomeMassMessageUserVC.h"
#import "NSMassMessageUserCell.h"
#import "NSMassMessageUserModel.h"
#import "NSMassMessageErrorUserModel.h"

@interface HomeMassMessageUserVC () <UITableViewDataSource, UITableViewDelegate>
@property (nonatomic, assign) NSInteger pageNumber;
@property (nonatomic, strong) NSMutableArray *userList;
@end

@implementation HomeMassMessageUserVC
...
- (void)setupUI {
    ...
    [self.baseTableView registerClass:[NSMassMessageUserCell class] forCellReuseIdentifier:[NSMassMessageUserCell cellIdentifier]];
    ...
    if (_allUsers) {
        [IMSDKManager GroupHairGetGroupHairUserListWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                [recordsArr enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    NSMassMessageUserModel *model = [NSMassMessageUserModel mj_objectWithKeyValues:obj];
        ...
        [IMSDKManager GroupHairGetGroupHairErrorUserListWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                [recordsArr enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    NSMassMessageErrorUserModel *model = [NSMassMessageErrorUserModel mj_objectWithKeyValues:obj];
                    [weakSelf.userList addObjectIfNotNil:model];
                }];
                [weakSelf.baseTableView reloadData];
                if (weakSelf.userList.count >= totalCount) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    NSMassMessageUserCell *cell = [tableView dequeueReusableCellWithIdentifier:[NSMassMessageUserCell cellIdentifier] forIndexPath:indexPath];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [NSMassMessageUserCell defaultCellHeight];
}
/*
