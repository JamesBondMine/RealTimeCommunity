//
//  EmojiUpdatePackageDetailHeaderView.m
//  CIMKit
//
...
@interface EmojiUpdatePackageDetailHeaderView ()
...
@implementation EmojiUpdatePackageDetailHeaderView
...
#pragma mark - Data
- (void)setModel:(LingIMStickersPackageModel *)model {
    _model = model;
    if (_model) {
        ...
        if (_model.isDownLoad) {
            [_addPackageBtn setTitle:MultilingualTranslation(@"已添加") forState:UIControlStateNormal];
            _addPackageBtn.tkThemebackgroundColors = @[COLOR_99, COLOR_99_DARK];
            _addPackageBtn.enabled = NO;
        } else {
            [_addPackageBtn setTitle:MultilingualTranslation(@"添加") forState:UIControlStateNormal];
            _addPackageBtn.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF];
...
- (UILabel *)desTitleLbl {
    if (!_desTitleLbl) {
        ...
        _desTitleLbl.text = @"--";
        _desTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
