...
@interface NTTeamUpdateNameView ()
...
@implementation NTTeamUpdateNameView
...
- (void)setupUI {
    ...
    _lblTitle.text = MultilingualTranslation(@"修改名称");
    ...
    _tfUpdateName.placeholder = MultilingualTranslation(@"请输入团队名称");
    _tfUpdateName.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
#pragma mark - Data
- (void)setModel:(NTMTeamModel *)model {
    _model = model;
    if (_model) {
...
- (void)requestUpdateTeamNameWithNewTeamName:(NSString *)newTeamName {
    ...
    if (_model.isSystemCreate == 1) {
        teamName = MultilingualTranslation(@"默认团队");
    }else {
    ...
    if (![newTeamName isEqualToString:teamName]) {
        ...
        [IMSDKManager imTeamEditWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
            [HUD showMessage:MultilingualTranslation(@"修改成功")];
