...
@interface ZEmojiShopPackagCell()
...
@implementation ZEmojiShopPackagCell
...
- (void)setModel:(LingIMStickersPackageModel *)model {
    ...
    if (_model.isDownLoad) {
        [_packageAddBtn setTitle:MultilingualTranslation(@"已添加") forState:UIControlStateNormal];
...
- (UILabel *)packageNameLbl {
    if (!_packageNameLbl) {
        ...
        _packageNameLbl.text = MultilingualTranslation(@"表情包名称");
        _packageNameLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
