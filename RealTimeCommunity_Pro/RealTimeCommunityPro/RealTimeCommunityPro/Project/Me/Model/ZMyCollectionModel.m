...
@implementation ZMyCollectionModel

- (instancetype)initWithCollectionModel:(ZMyCollectionItemModel *)itemModel {
    self = [super init];
    if (self) {
...
- (void)calculateTextMessage {
    ...
    self.attStr = [[EMChatInputEmojiManager sharedManager] attributedString:_itemModel.body.content];
    ...
    if (![NSString isNil:atContentStr]) {
        ...
        for (NSDictionary *atUsersDict in atUsersDictList) {
            ...
            if ([uidKey isEqualToString:@"-1"]) {
                showContent = [showContent stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"\v%@\v", uidKey] withString:MultilingualTranslation(@"@所有人")];
            } else {
