...
@interface ZChatLinkCollectCell ()
...
@implementation ZChatLinkCollectCell
...
- (void)setTagModel:(ZChatTagModel *)tagModel {
    ...
    [self.contentIcon sd_setImageWithURL:[_tagModel.tagIcon getImageFullUrl] placeholderImage:ImgNamed(@"remini_mini_app_icon") options:SDWebImageAllowInvalidSSLCertificates];
...
- (UIImageView *)contentIcon  {
    if (!_contentIcon) {
        ...
        _contentIcon.image = ImgNamed(@"remini_mini_app_icon");
...
- (UIButton *)contentBtn {
    if (!_contentBtn) {
        ...
        [_contentBtn setTkThemeTitleColor:@[COLOR_33, COLORWHITE] forState:UIControlStateNormal];
        _contentBtn.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
