...
@implementation HomeSessionCell
...
- (void)setupUI {
    ...
    _lblTitle.tkThemetextColors = @[COLOR_33,COLORWHITE];
    [self.contentView addSubview:_lblTitle];

    _lblContent = [UILabel new];
    _lblContent.textColor = COLOR_99;
    [self.contentView addSubview:_lblContent];
    
    _lblTime = [UILabel new];
    _lblTime.font = FONTR(12);
    _lblTime.tkThemetextColors = @[COLOR_99, COLOR_99];
    ...
    _viewOnline.layer.cornerRadius = DWScale(4);
    ...
    [_viewOnline mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_ivHeader);
        make.trailing.equalTo(_ivHeader).offset(-DWScale(6));
        make.size.mas_equalTo(CGSizeMake(DWScale(8), DWScale(8)));
    }];
...
- (void)setModel:(LingIMSessionModel *)model {
    ...
    if (model) {
        ...
        if (model.sessionType == CIMSessionTypeSingle) {
            //单聊
            [self loadAvatarImageWithURL:[model.sessionAvatar getImageFullUrl].absoluteString placeholder:DefaultAvatar];
            ...
            dispatch_async_main_queue(^{
                ...
                if (friendModel) {
                    ...
                    [weakSelf.ivHeader loadAvatarWithUserImgContent:avatarUri defaultImg:DefaultAvatar];
                    ...
                    [weakSelf loadAvatarImageWithURL:[model.sessionAvatar getImageFullUrl].absoluteString placeholder:DefaultAvatar];
            ...
            //会话最新消息处理
            // 优先显示本地草稿
            NSDictionary *localDraft = [ZDraftStore loadDraftForSession:_model.sessionID];
            if (localDraft.count > 0) {
                [self showLocalDraftDict:localDraft];
            } else {
                LingIMChatMessageModel *lastMessage = _model.sessionLatestMessage;
                if (lastMessage) {
                    self.model.sessionLatestMessage = lastMessage;
                    [self showLocalLatestMessageWith:self.model.sessionLatestMessage];
                } else {
                    LingIMChatMessageModel *lastMessage = [DBTOOL getLatestChatMessageWithTableName:weakSelf.model.sessionTableName];
                    self.model.sessionLatestMessage = lastMessage;
                    [self showLocalLatestMessageWith:self.model.sessionLatestMessage];
                }
            }
            ...
            //群聊
            [self loadAvatarImageWithURL:[model.sessionAvatar getImageFullUrl].absoluteString placeholder:DefaultGroup];
            _lblTitle.text = _model.sessionName;
            
            //会话最新消息处理
            NSDictionary *localDraft2 = [ZDraftStore loadDraftForSession:_model.sessionID];
            ...
            _lblTitle.text = MultilingualTranslation(_model.sessionName);
            
            //会话最新消息处理
            _lblContent.attributedText = [NSString getSessionDefaultLastMsgContentAttributedStringWith:MultilingualTranslation(@"今日未签到-前去签到")];
        }
...
// 设置图片并显示
- (void)loadAvatarImageWithURL:(NSString *)imageURL placeholder:(UIImage *)placeholder {
    if (!imageURL || imageURL.length == 0) {
        self.ivHeader.image = placeholder;
        return;
    }
    ...
    if (cachedImage) {
        self.ivHeader.image = cachedImage;
        return;
    }
    ...
    [[SDImageCache sharedImageCache] diskImageExistsWithKey:imageURL completion:^(BOOL isInCache) {
        if (isInCache) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                self.ivHeader.image = diskImage;
            });
        } else {
            // 没有缓存，尝试网络加载
            [self.ivHeader sd_setImageWithURL:[NSURL URLWithString:imageURL]
...
#pragma mark - 会话最新消息
//展示本地存储的最新消息
- (void)showLocalLatestMessageWith:(LingIMChatMessageModel *)latestMessage {
    if (latestMessage) {
        ...
        self.contentView.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
            ...
            if (safeLength > 0) {
                switch (themeIndex) {
                    ...
                    {
                        //暗黑
                        [attriStr addAttribute:NSForegroundColorAttributeName value:COLOR_33_DARK range:NSMakeRange(0, safeLength)];
                    }
                        break;
                        
                    default:
                    {
                        [attriStr addAttribute:NSForegroundColorAttributeName value:COLOR_33 range:NSMakeRange(0, safeLength)];
                    }
                        break;
                }
            }
...
- (NSMutableAttributedString *)sessionNoNewMessageTip {
    ...
    NSMutableAttributedString *sessionAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[暂无新消息]")];
...
- (void)specialSessionTitleConfig {
    if (_model) {
        if ([_model.sessionID isEqualToString:@"100001"]) {
            //群发助手
            _lblTitle.text = MultilingualTranslation(@"群发助手");
            _ivHeader.image = ImgNamed(@"session_hair_group_logo");
        }else if ([_model.sessionID isEqualToString:@"100002"]) {
            //文件助手
            _lblTitle.text = MultilingualTranslation(@"文件助手");
            ...
            //群助手
            _lblTitle.text = MultilingualTranslation(@"群助手");
            _ivHeader.image = ImgNamed(@"session_helper_group_logo");
        }else if ([_model.sessionID isEqualToString:@"100009"]) {
            //支付通知
            _lblTitle.text = MultilingualTranslation(@"支付通知");
