...
@interface ZMessageReferenceCell() <MImageBrowserVCDelegate>

@end

@implementation ZMessageReferenceCell
{
...
- (void)setupReferenceTextUI {
    ...
    _referenceTipImgView.image = ImgNamed(@"remsg_img_msg_reference_tip");
    ...
    _referenceNameLabel.tkThemetextColors = @[COLOR_33, COLORWHITE];
    ...
    _referenceContentLabel.tkThemetextColors = @[COLOR_33, COLOR_99];
    ...
    _referencePlayImgView.image = ImgNamed(@"remsg_icon_video_msg_play");
    ...
    [_reTranslateBtn setTitle:MultilingualTranslation(@"翻译失败") forState:UIControlStateNormal];
    [_reTranslateBtn setImage:ImgNamed(@"remsg_icon_msg_translate_fail") forState:UIControlStateNormal];
    [_reTranslateBtn setIconInLeftWithSpacing:DWScale(2)];
    [_reTranslateBtn setTkThemeTitleColor:@[COLOR_4791FF, COLOR_4791FF] forState:UIControlStateNormal];
...
-(void)setupThemeUpdate {
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        ZMessageReferenceCell * cell = itself;
        [ZTOOL doInMain:^{
...
- (void)setConfigMessage:(ZMessageModel *)model {
    ...
    if (model.referenceMsg == nil) {
        _referenceNameLabel.text = MultilingualTranslation(@"未知");
        ...
        if (model.referenceMsg.messageType == CIMChatMessageType_TextMessage || model.referenceMsg.messageType == CIMChatMessageType_AtMessage) {
            ...
            if (model.referenceMsg.messageStatus == 1) { //正常
                ...
                if (model.referenceMsg.gameSticekersType == ZChatGameStickerTypeFingerGuessing) {
                    //石头剪刀布
                    contentImgName = [NSString stringWithFormat:@"ichat_message_stoneScissorCloth%@", model.referenceMsg.gameStickersResut];
                } else if (model.referenceMsg.gameSticekersType == ZChatGameStickerTypePlayDice) {
                    //摇骰子
                    contentImgName = [NSString stringWithFormat:@"ichat_message_dice%@", model.referenceMsg.gameStickersResut];
                } else {
                    contentImgName = @"";
                }
                //设置图片
    ...
    if (urlArr.count > 0) {
        for (NSString *urlStr in urlArr) {
            ...
            if (model.isSelf) {
                ...
                [model.attStr yy_setTextHighlightRange:[model.attStr.string rangeOfString:urlStr] color:COLOR_4791FF backgroundColor:COLOR_CLEAR tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
                    [weakSelf textMsgUrlClick:urlStr];
                }];
            }
        }
    ...
    if (model.message.translateStatus == CIMTranslateStatusSuccess) {
        ...
        if (translateUrlArr.count > 0) {
            for (NSString *translateUrlStr in translateUrlArr) {
                ...
                if (model.isSelf) {
                    ...
                    [model.attStr yy_setTextHighlightRange:[model.translateAttStr.string rangeOfString:translateUrlStr] color:COLOR_4791FF backgroundColor:COLOR_CLEAR tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
                        [weakSelf textMsgUrlClick:translateUrlStr];
                    }];
                }
            }
        ...
        if (model.isSelf) {
            [_reTranslateBtn setImage:ImgNamed(@"remsg_icon_msg_translate_fail_white") forState:UIControlStateNormal];
    ...
    if (model.isSelf) {
        ...
        _referenceTipImgView.image = ImgNamed(@"remsg_img_msg_reference_tip_blue");
        _referenceLineView.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF_DARK];
        _referenceNameLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
