//
//  HomeTranslateChannelLanguageView.m
//  CIMKit
//
...
#import "HomeChannelLanguageCell.h"

@interface HomeTranslateChannelLanguageView()<UITableViewDelegate, UITableViewDataSource, MainSearchViewDelegate>
...
@implementation HomeTranslateChannelLanguageView
...
- (void)setupUI {
    ...
    [closeBtn setTkThemeImage:@[ImgNamed(@"remsg_icon_chat_refresh_close"), ImgNamed(@"remsg_icon_chat_refresh_close_dark")] forState:UIControlStateNormal];
    ...
    NSString *sureTitleStr = MultilingualTranslation(@"确定");
    ...
    if (_translateType == ZSendMsgTranslateTypeChannel || _translateType == ZReceiveMsgTranslateTypeChannel) {
        titleLbl.text = MultilingualTranslation(@"翻译通道");
    } else {
        titleLbl.text = MultilingualTranslation(@"翻译语种");
    }
    ...
    _searchView = [[MainSearchView alloc] initWithPlaceholder:MultilingualTranslation(@"搜索")];
    ...
    [self.baseTableView registerClass:[HomeChannelLanguageCell class] forCellReuseIdentifier:NSStringFromClass([HomeChannelLanguageCell class])];
}
...
- (void)requestGetTranslateChannel {
    ...
    [IMSDKManager imSdkTranslateGetChannelLanguage:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if ([data isKindOfClass:[NSArray class]]) {
            ...
            weakSelf.showDataList = [MeTranslateChannelLanguageModel mj_objectArrayWithKeyValuesArray:dataArr];
...
- (void)searchViewTextValueChanged:(NSString *)searchStr {
    ...
    if (self.searchContent.length > 0) {
        //通道
        if (_translateType == ZSendMsgTranslateTypeChannel || _translateType == ZReceiveMsgTranslateTypeChannel) {
            for (MeTranslateChannelLanguageModel *tempChannelModel in self.defaultDataList) {
        ...
        if (_translateType == ZSendMsgTranslateTypeLanguage || _translateType == ZReceiveMsgTranslateTypeLanguage) {
            for (ZTranslateLanguageModel *tempLangueModel in self.defaultDataList) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    HomeChannelLanguageCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([HomeChannelLanguageCell class]) forIndexPath:indexPath];
    if (_translateType == ZSendMsgTranslateTypeChannel || _translateType == ZReceiveMsgTranslateTypeChannel) {
        //通道
        MeTranslateChannelLanguageModel *channelModel = (MeTranslateChannelLanguageModel *)[self.showDataList objectAtIndexSafe:indexPath.row];
    ...
    if (_translateType == ZSendMsgTranslateTypeLanguage || _translateType == ZReceiveMsgTranslateTypeLanguage) {
        //语种
        ZTranslateLanguageModel *languageModel = (ZTranslateLanguageModel *)[self.showDataList objectAtIndexSafe:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_translateType == ZReceiveMsgTranslateTypeChannel) {
        //通道-收到消息
        MeTranslateChannelLanguageModel *clickChannelModel = (MeTranslateChannelLanguageModel *)[self.showDataList objectAtIndexSafe:indexPath.row];
        self.selectOption = clickChannelModel.channelId;
        self.selectOptionName = clickChannelModel.name;
    }
    if (_translateType == ZReceiveMsgTranslateTypeLanguage) {
        //语种-收到消息
        ZTranslateLanguageModel *clickLanguageModel = (ZTranslateLanguageModel *)[self.showDataList objectAtIndexSafe:indexPath.row];
