...
#import "NFNewFriendListVC.h"
#import "MainSearchView.h"
#import "CNNewFriendListCell.h"
#import "EmptyDataView.h"

#import "CMFriendApplyModel.h"
#import "CMFriendReqModel.h"
#import "NFFriendApplyPassVC.h"//好友申请验证通过
#import "NFUserHomePageVC.h"//好友主页

@interface NFNewFriendListVC () <UITableViewDataSource,UITableViewDelegate,MGSwipeTableCellDelegate,CIMToolUserDelegate,CNNewFriendListCellDelegate>
@property (nonatomic, assign) NSInteger pageNum;
@property (nonatomic, strong) EmptyDataView *viewNoData;
@property (nonatomic, strong) EmptyDataView *viewNoMore;
...
@implementation NFNewFriendListVC
...
-(void)initNavBar{
    ...
    self.navTitleLabel.text = MultilingualTranslation(@"新朋友");
}
    ...
    [IMSDKManager getFriendApplyListFromServerWith:param onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            [reqArr enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                CMFriendApplyModel *model = [CMFriendApplyModel mj_objectWithKeyValues:obj];
            ...
            if (weakSelf.requestList.count == 0) {
                weakSelf.baseTableView.tableFooterView = self.viewNoData;
            }
...
- (void)makeApplyRead {
    WeakSelf
    [self.requestList enumerateObjectsUsingBlock:^(CMFriendApplyModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
//    LingIMSessionModel *model = [_sessionList objectAtIndexSafe:cellIndex.row];
    ...
    if (direction == MGSwipeDirectionRightToLeft) {
        MGSwipeButton *btnDelete = [MGSwipeButton buttonWithTitle:MultilingualTranslation(@"删除") icon:ImgNamed(@"s_session_delete") backgroundColor:HEXCOLOR(@"FF504E") callback:^BOOL(MGSwipeTableCell * _Nonnull cell) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    CNNewFriendListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CNNewFriendListCell"];
    if (cell == nil){
        cell = [[CNNewFriendListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"CNNewFriendListCell"];
        cell.delegate = self;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    }
    CMFriendApplyModel *model = [self.requestList objectAtIndexSafe:indexPath.row];
    cell.model = model;
    WeakSelf
    cell.stateBtnClick = ^{
        ...
        if(NSMaxRange(range) <= self.requestList.count){
            ...
            for (int i = 0; i < indexMessageArr.count; i++) {
                for (int j = 0; j < list.count; j++) {
                    CMFriendApplyModel *model = [indexMessageArr objectAtIndexSafe:i];
                    CMFriendReqModel *reqModel = [list objectAtIndexSafe:j];
...
#pragma mark - CNNewFriendListCellDelegate
...
//头像点击事件
- (void)headerFriend:(CMFriendApplyModel *)model {
    NSString *userID;
    ...
    if (friendModel) {
        ...
        //不是好友
        NFFriendApplyPassVC *vc = [NFFriendApplyPassVC new];
        vc.applyModel = model;
        [self.navigationController pushViewController:vc animated:YES];
    }
...
//好友申请处理
- (void)dealFriendApply:(CMFriendApplyModel *)model {
    if (model.beStatus == 0) {
        //申请中
        if (![model.fromUserUid isEqualToString:UserManager.userInfo.userUID]) {
            //对方发起的好友申请，通过验证
...
//发送好友添加成功的通知
- (void)postNotificationWith:(CMFriendApplyModel *)model {
...
- (EmptyDataView *)viewNoData {
    if (!_viewNoData) {
        _viewNoData = [[EmptyDataView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DWScale(30))];
        _viewNoData.lblNoDataTip.text = MultilingualTranslation(@"暂无好友申请");
        _viewNoData.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    }
    return _viewNoData;
}
- (EmptyDataView *)viewNoMore {
    if (!_viewNoMore) {
        _viewNoMore = [[EmptyDataView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DWScale(30))];
        _viewNoMore.lblNoDataTip.text = MultilingualTranslation(@"我是有底线的");
        _viewNoData.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    }
    return _viewNoMore;
}
