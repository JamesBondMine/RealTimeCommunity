...
@interface NSNewMassMessageContentView () <ZImagePickerVCDelegate>
@end

@implementation NSNewMassMessageContentView
...
- (void)setupUI {
    ...
    _lblMessageTip.text = MultilingualTranslation(@"粘贴或输入需要发送的内容");
    ...
    _tvMessage.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    [_btnSelectAttachment setImage:ImgNamed(@"com_c_square_add") forState:UIControlStateNormal];
    [_btnSelectAttachment setTitle:MultilingualTranslation(@"选择需要发送的图片、视频、文件") forState:UIControlStateNormal];
    ...
    _ivPlay = [[UIImageView alloc] initWithImage:ImgNamed(@"remsg_icon_video_msg_play")];
    _ivPlay.hidden = YES;
    ...
    _lblFileName.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (void)showAttachmentTypeView {
    WeakSelf
    ZPresentItem *imageItem = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"图片/视频") textColor:COLOR_33 font:FONTR(17) itemHeight:DWScale(56) backgroundColor:COLORWHITE];
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        if (themeIndex == 0) {
            imageItem.textColor = COLOR_33;
            imageItem.backgroundColor = COLORWHITE;
        }else {
            imageItem.textColor = COLORWHITE;
            imageItem.backgroundColor = COLOR_33;
        }
    };
    
    ZPresentItem *fileItem = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"文件") textColor:COLOR_33 font:FONTR(17) itemHeight:DWScale(56) backgroundColor:COLORWHITE];
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        if (themeIndex == 0) {
            fileItem.textColor = COLOR_33;
            fileItem.backgroundColor = COLORWHITE;
        }else {
            fileItem.textColor = COLORWHITE;
            fileItem.backgroundColor = COLOR_33;
        }
    };
    ...
    if ([UserManager.userRoleAuthInfo.upImageVideoFile.configValue isEqualToString:@"true"] && [UserManager.userRoleAuthInfo.upFile.configValue isEqualToString:@"false"]) {
        viewAlert = [[ZPresentView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DScreenHeight) titleItem:nil selectItems:@[imageItem] cancleItem:cancelItem doneClick:^(NSInteger index) {
            if (index == 0) {
                //图片/视频
        ...
        viewAlert = [[ZPresentView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DScreenHeight) titleItem:nil selectItems:@[fileItem] cancleItem:cancelItem doneClick:^(NSInteger index) {
            if (index == 0) {
                //文件
        ...
        viewAlert = [[ZPresentView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DScreenHeight) titleItem:nil selectItems:@[imageItem, fileItem] cancleItem:cancelItem doneClick:^(NSInteger index) {
            if (index == 0) {
                //图片/视频
                [weakSelf selectPhotoLibrary];
            }else {
        ...
        if (granted) {
            [ZTOOL doInMain:^{
                ...
                vc.saveLingXinFileSuccess = ^(NSArray * _Nonnull sendSelectFileArr) {
                    //App中的文件或者相册视频
                    ZFilePickModel *selectFileModel = sendSelectFileArr.firstObject;
                    [weakSelf showSelectedFile:selectFileModel];
                };
            }];
        } else {
            ...
            [[PHImageManager defaultManager] requestAVAssetForVideo:mediaAsset options:options resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
                if ([asset isKindOfClass:[AVURLAsset class]]) {
                    ...
                    if (videoData.length > (200 * 1024 * 1024)) {
                        ZFilePickModel *tempAssetModel = [[ZFilePickModel alloc] init];
...
#pragma mark - 文件选择
- (void)showSelectedFile:(ZFilePickModel *)fileModel{
