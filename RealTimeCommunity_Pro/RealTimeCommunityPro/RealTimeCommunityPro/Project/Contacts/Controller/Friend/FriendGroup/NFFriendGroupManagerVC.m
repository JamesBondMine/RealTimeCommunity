//
//  NFFriendGroupManagerVC.m
//  CIMKit
//
//  Created by cusPro on 2023/7/3.
//

#import "NFFriendGroupManagerVC.h"
#import "NFFriendGroupManagerCell.h"

#import "NFFriendGroupAddView.h"
#import "NFFriendGroupDeleteView.h"

@interface NFFriendGroupManagerVC () <UITableViewDataSource, UITableViewDelegate, UITableViewDragDelegate, UITableViewDropDelegate, NFFriendGroupManagerCellDelegate, NFFriendGroupDeleteViewDelegate, ZBaseCellDelegate, CIMToolUserDelegate>
@property (nonatomic, strong) UIButton *btnFinish;
@property (nonatomic, strong) NSMutableArray *friendGroupList;
@end

@implementation NFFriendGroupManagerVC
...
- (void)setupNavUI {
    if (self.friendGroupCanEdit) {
        ...
        self.navTitleStr = MultilingualTranslation(@"分组管理");
        
        _btnFinish = [UIButton buttonWithType:UIButtonTypeCustom];
        [_btnFinish setTitle:MultilingualTranslation(@"完成") forState:UIControlStateNormal];
        ...
        [_btnFinish setTkThemeTitleColor:@[COLOR_81D8CF, COLOR_81D8CF_DARK] forState:UIControlStateSelected];
        ...
        self.navTitleStr = MultilingualTranslation(@"移动分组");
    }
...
- (void)setupUI {
    ...
    lblAdd.text = MultilingualTranslation(@"添加分组");
    lblAdd.tkThemetextColors = @[COLOR_81D8CF, COLOR_81D8CF_DARK];
    ...
    [self.baseTableView registerClass:[NFFriendGroupManagerCell class] forCellReuseIdentifier:[NFFriendGroupManagerCell cellIdentifier]];
...
#pragma mark - 交互事件
- (void)btnAddClick {
    NFFriendGroupAddView *viewAdd = [NFFriendGroupAddView new];
    [viewAdd addViewShow];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NFFriendGroupManagerCell *cell = [tableView dequeueReusableCellWithIdentifier:[NFFriendGroupManagerCell cellIdentifier] forIndexPath:indexPath];
    cell.delegate = self;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [NFFriendGroupManagerCell defaultCellHeight];
}
...
#pragma mark - NFFriendGroupManagerCellDelegate
- (void)friendGroupManagerDelete:(LingIMFriendGroupModel *)friendGroupModel {
    NFFriendGroupDeleteView *viewDelete = [NFFriendGroupDeleteView new];
...
- (void)friendGroupManagerChangeName:(LingIMFriendGroupModel *)friendGroupModel newFriendGroupName:(nonnull NSString *)friendGroupName{
    if (![NSString isNil:friendGroupName]) {
        ...
        [HUD showMessage:MultilingualTranslation(@"请输入分组名")];
        [self.baseTableView reloadData];
    }
}

#pragma mark - NFFriendGroupDeleteViewDelegate
