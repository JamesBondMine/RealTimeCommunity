...
- (void)setupUI {
    ...
    self.singleIndicatorBar.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF_DARK];
    ...
    self.topLabel.text = MultilingualTranslation(@"置顶消息");
    self.topLabel.tkThemetextColors = @[COLOR_4791FF, COLOR_4791FF_DARK];
...
- (void)updateIndicatorBarsForCount:(NSInteger)count {
    ...
    for (NSInteger i = 0; i < displayCount; i++) {
        ...
        bar.tkThemebackgroundColors = @[[COLOR_4791FF colorWithAlphaComponent:0.3], [COLOR_4791FF_DARK colorWithAlphaComponent:0.3]];
...
- (NSMutableAttributedString *)getDisplayContentFromData:(NSDictionary *)data {
    ...
    if (self.sessionID.length > 0 && smsgId.length > 0) {
        LingIMChatMessageModel *messageModel = [IMSDKManager toolGetOneChatMessageWithServiceMessageID:smsgId sessionID:self.sessionID];
        ...
        if (messageModel) {
            ...
            if (messageModel.messageType == CIMChatMessageType_AtMessage) {
                ...
                if ([NSString isNil:messageModel.showContent]) {
                    if (isSelf) {
                        messageModel.showContent = [ZMessageTools atContenTranslateToShowContent:![NSString isNil:messageModel.atContent] ? messageModel.atContent : @"" atUsersDictList:messageModel.atUsersInfoList withMessage:messageModel isGetShowName:YES];
                    } else {
                        messageModel.showContent = [ZMessageTools atContenTranslateToShowContent:![NSString isNil:messageModel.atTranslateContent] ? messageModel.atTranslateContent : messageModel.atContent atUsersDictList:messageModel.atUsersInfoList withMessage:messageModel isGetShowName:YES];
                    }
                }
                ...
                if ([NSString isNil:messageModel.showTranslateContent]) {
                    if (isSelf) {
                        messageModel.showTranslateContent = [ZMessageTools atContenTranslateToShowContent:![NSString isNil:messageModel.atTranslateContent] ? messageModel.atTranslateContent : @"" atUsersDictList:messageModel.atUsersInfoList withMessage:messageModel isGetShowName:YES];
                    } else {
                        messageModel.showTranslateContent = [ZMessageTools atContenTranslateToShowContent:![NSString isNil:messageModel.againAtTranslateContent] ? messageModel.againAtTranslateContent : messageModel.atTranslateContent atUsersDictList:messageModel.atUsersInfoList withMessage:messageModel isGetShowName:YES];
                    }
                }
    ...
    NSMutableAttributedString *attributedString = [[EMChatInputEmojiManager sharedManager] yy_emojiAttributedString:displayText];
    ...
    if (attributedString) {
        ...
        // 设置颜色（使用主题颜色方法）
        [attributedString configAttStrLightColor:COLOR_33 darkColor:COLOR_33_DARK range:NSMakeRange(0, attributedString.length)];
    } else {
...
- (void)updateIndicatorHighlight {
    ...
    for (NSInteger i = 0; i < self.indicatorBars.count; i++) {
        ...
        if (i == barIndex) {
            // 当前索引对应的竖条：高亮
            bar.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF_DARK];
        } else {
            // 其他竖条：半透明
            bar.tkThemebackgroundColors = @[[COLOR_4791FF colorWithAlphaComponent:0.3], [COLOR_4791FF_DARK colorWithAlphaComponent:0.3]];
        }
