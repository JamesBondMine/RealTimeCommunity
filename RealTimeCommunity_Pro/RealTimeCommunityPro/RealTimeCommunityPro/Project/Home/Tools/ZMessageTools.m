    ...
    if (message) {
        ...
        switch (message.messageType) {
            ...
            case IMChatMessage_MessageType_AtMessage:
            {
                chatMessage.mType = IMChatMessage_MessageType_TextMessage;
                TextMessage *textMessage = [[TextMessage alloc] init];
                textMessage.content = [ZMessageTools forwardMessageAtContenTranslateToShowContent:message.atContent atUsersDictList:message.atUsersInfoList];
                textMessage.translate = @"";
                chatMessage.textMessage = textMessage;
            }
                break;
...
+ (IMMessage *)getIMMessageFromLingIMChatMessageModel:(LingIMChatMessageModel *)message withChatObject:(ZBaseUserModel *)chatObject index:(int)index {
    if (message) {
        ...
        IMChatMessage *chatMessage = [[IMChatMessage alloc] init];
        chatMessage.msgId = [ZMessageTools getMessageID];
...
+ (NSString *)atContenTranslateToShowContent:(NSString *)atContentStr atUsersDictList:(NSArray *)atUsersDictList withMessage:(nonnull LingIMChatMessageModel *)chatMessage isGetShowName:(BOOL)isGetShowName {
        ...
        for (NSInteger i = matches.count - 1; i >= 0; i--) {
            ...
            if ([uid isEqualToString:UserManager.userInfo.userUID]) {
                if ([chatMessage.fromID isEqualToString:UserManager.userInfo.userUID]) {
                    replaceText = MultilingualTranslation(@"@我自己");
                } else {
                    replaceText = MultilingualTranslation(@"@我");
                }
            } else if ([uid isEqualToString:@"-1"]) {
                replaceText = MultilingualTranslation(@"@所有人");
            } else {
...
+ (void)translationSplit:(NSString *)messageStr
    ...
    if (messageStr.length > 0) {
        ...
        for (NSTextCheckingResult *match in matchs) {
            [emojiArray addObject:[translationString safeSubstringWithRange:match.range]];
        }
        //匹配 表情字符串
