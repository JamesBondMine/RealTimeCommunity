...
@interface MainChatInputVoiceStateView ()
...
@implementation MainChatInputVoiceStateView
...
- (void)setupUI {
    ...
    micImgView.image = ImgNamed(@"recat_Img_chat_voice_input_mic");
    ...
    self.stateLabel.text = MultilingualTranslation(@"上滑取消发送");
...
- (void)updateViewState:(InputVoiceStateType)stateType{
    if(stateType == InputVoiceStateSend){
        ...
        self.stateLabel.text = MultilingualTranslation(@"松开取消发送");
    }
}
