...
- (NSArray <LingIMGroupMemberModel *> *)getAllMemberWithTabName:(NSString *)tabName {
    return [self.cimDB getObjectsOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.role != 3];
}

/// 查询当前表里群成员中 群主和管理员 信息  role: 1管理员  2群主
/// @param tabName 群名称
- (NSArray <LingIMGroupMemberModel *> *)getGroupOwnerAndManagerWithTabName:(NSString *)tabName {
    return [self.cimDB getObjectsOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.role == 1 || LingIMGroupMemberModel.role == 2];
}

/// 查询当前表里群成员 除了群主和群机器人
/// @param tabName 群名称
- (NSArray <LingIMGroupMemberModel *> *)imSdkGetGroupMemberExceptOwnerWith:(NSString *)tabName {
    return [self.cimDB getObjectsOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.role != 2 && LingIMGroupMemberModel.role != 3];
}
...
- (LingIMGroupMemberModel *)getGroupOwnerWithTabName:(NSString *)tabName exceptUserId:(NSString *)exceptUserId {
    if (exceptUserId.length > 0) {
        return [self.cimDB getObjectOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.role == 2 && LingIMGroupMemberModel.userUid != exceptUserId];

    } else {
        return [self.cimDB getObjectOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.role == 2];
    }
}
...
- (NSArray <LingIMGroupMemberModel *> *)getGroupManagerWithTabName:(NSString *)tabName exceptUserId:(NSString *)exceptUserId {
    if (exceptUserId.length > 0) {
        return [self.cimDB getObjectsOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.role == 1 && LingIMGroupMemberModel.userUid != exceptUserId];
    } else {
        return [self.cimDB getObjectsOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.role == 1];
    }
}
...
- (NSArray <LingIMGroupMemberModel *> *)getGroupNomalMemberWithTabName:(NSString *)tabName exceptUserId:(NSString *)exceptUserId {
    if (exceptUserId.length > 0) {
        return [self.cimDB getObjectsOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.role == 0 && LingIMGroupMemberModel.role != 3 && LingIMGroupMemberModel.userUid != exceptUserId];
    } else {
        return [self.cimDB getObjectsOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.role == 0 && LingIMGroupMemberModel.role != 3];
    }
}
...
- (LingIMGroupMemberModel *)checkGroupMemberWithTabName:(NSString *)tabName memberId:(NSString *)memberId{
    ...
    return [self.cimDB getObjectOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.userUid == memberId];
}
...
- (BOOL)insertOrUpdateMultiActiviteScoreWithTabName:(NSString *)tabName scoreList:(NSArray <LingIMGroupActiviteScoreModel *> *)scoreList {
    for (LingIMGroupActiviteScoreModel *model in scoreList) {
        BOOL result = [self.cimDB updateTable:tabName setProperty:LingIMGroupMemberModel.activityScroe toValue:@(model.activityScore) where:LingIMGroupMemberModel.userUid == model.memberUid];
...
- (BOOL)deleteGroupMemberWithTabName:(NSString *)tabName memberId:(NSString *)memberId{
    return [self.cimDB deleteFromTable:tabName where:LingIMGroupMemberModel.userUid == memberId];
}
...
- (NSArray <LingIMGroupMemberModel *> *)checkGroupMemberWithTabName:(NSString *)tabName searchContent:(NSString *)searchContent exceptUserId:(NSString *)exceptUserId {
    ...
    searchContent = [[LingIMManagerTool sharedManager] stringReplaceSpecialCharacterWith:searchContent];
    if (searchContent.length > 0) {
        NSString *likeStr = [NSString stringWithFormat:@"%%%@%%",searchContent];
        if (exceptUserId.length > 0) {
            return [self.cimDB getObjectsOfClass:LingIMGroupMemberModel.class fromTable:tabName where:(LingIMGroupMemberModel.remarks.like(likeStr) || LingIMGroupMemberModel.nicknameInGroup.like(likeStr) || LingIMGroupMemberModel.userNickname.like(likeStr) || LingIMGroupMemberModel.userName.like(likeStr)) && LingIMGroupMemberModel.userUid != exceptUserId];
        } else {
            return [self.cimDB getObjectsOfClass:LingIMGroupMemberModel.class fromTable:tabName where:LingIMGroupMemberModel.remarks.like(likeStr) || LingIMGroupMemberModel.nicknameInGroup.like(likeStr) || LingIMGroupMemberModel.userNickname.like(likeStr) || LingIMGroupMemberModel.userName.like(likeStr)];
        }
    } else {
        return @[];
    }
