...
@interface ZFilePickerHeaderView ()
...
@implementation ZFilePickerHeaderView
...
- (void)setUnFlod:(BOOL)unFlod {
    ...
    if (_unFlod) {
        ...
        self.arrowImgView.image = ImgNamed(@"com_c_arrow_down_darkgray");
        ...
        self.arrowImgView.image = ImgNamed(@"com_c_arrow_right_darkgray");
        [self.arrowImgView mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.backView);
            make.trailing.equalTo(self.backView).offset(-16);
            make.width.mas_equalTo(DWScale(8));
...
- (UIImageView *)arrowImgView {
    if (!_arrowImgView) {
        ...
        _arrowImgView.image = ImgNamed(@"com_c_arrow_right_darkgray");
    }
