...
@implementation ZMessageFileCell
...
- (void)setupFileUI {
    ...
    _fileIconImgView.image = ImgNamed(@"remsg_icon_file_unknow");
    ...
    _fileTitleLbl.text = @"";
    _fileTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _fileSubTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_99_DARK];
    ...
    [_downUploadBtn setImage:ImgNamed(@"refile_c_file_goonBtn") forState:UIControlStateNormal];
    ...
    if(model.isSelf) {
        ...
        [model setUploadFileSuccess:^{
            [ZTOOL doInMain:^{
                ...
                weakSelf.fileSubTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_99_DARK];
        ...
        */
        [model setUploadFileFail:^{
            [ZTOOL doInMain:^{
                ...
                weakSelf.fileSubTitleLbl.text = MultilingualTranslation(@"上传失败");
                weakSelf.downUploadBtn.hidden = YES;
                [weakSelf.downUploadBtn setTitle:@"" forState:UIControlStateNormal];
            }];
        }];
        ...
        if (model.message.messageSendType == CIMChatMessageSendTypeSuccess) {
            self.fileSubTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_99_DARK];
        ...
        if (model.message.messageSendType == CIMChatMessageSendTypeFail) {
            ...
            self.fileSubTitleLbl.text = MultilingualTranslation(@"上传失败");
            self.downUploadBtn.hidden = YES;
            [self.downUploadBtn setTitle:@"" forState:UIControlStateNormal];
        }
        if (model.message.messageSendType == CIMChatMessageSendTypeSending) {
