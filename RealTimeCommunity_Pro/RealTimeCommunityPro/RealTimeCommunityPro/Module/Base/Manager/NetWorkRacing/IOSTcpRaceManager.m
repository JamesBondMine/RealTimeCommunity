...
//  CIMKit
//
//  Created by cusPro on 2025/1/15.
//
...
@property (nonatomic, strong) ZUrlHostModel *bucket;
...
- (BOOL)establishConnection {
    ...
    // 获取代理配置
    ZProxySettings *proxyConfig = nil;
    if (self.useProxy) {
        if ([ZTOOL getCurrentProxyType] == ProxyTypeSOCKS5) {
            proxyConfig = [[MMKV defaultMMKV] getObjectOfClass:[ZProxySettings class] forKey:SOCKS_PROXY_KEY];
        }
    }
...
// 通过SOCKS5代理连接
- (BOOL)connectThroughSocks5Proxy:(NSString *)host port:(int)port proxyConfig:(ZProxySettings *)proxyConfig {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
    if (sock < 0) {
        NSLog(@"❌ 创建socket失败");
        return NO;
