...
#import "HomeChatTopView.h"//顶部自定义导航栏
#import "ZChatInputView.h"//底部输入框
#import "ZChatMessageMoreView.h"//消息长按 更多功能
#import "ZImagePickerVC.h"//相册
#import "ZFilePickerVC.h"//文件
#import "ZMessageSendHander.h"//消息发送工具
...
#import "ZNMMyCollectionViewController.h"//收藏
...
#import "ZMessageAlertView.h"//弹窗提示类的控件
#import "ZMessageTools.h"//消息工具类
#import "FileFileHelperSetVC.h"//文件助手 设置VC
...
@interface FileFileHelperVC () <ZChatInputViewDelegate, UITableViewDelegate, UITableViewDataSource, ZMessageBaseCellDelegate, CIMToolMessageDelegate, ZImagePickerVCDelegate, UIDocumentInteractionControllerDelegate, KNPhotoBrowserDelegate, ZMsgMultiSelectDelegate, ZFileUploadTaskDelegate>
//显示刷新界面的专门队列(消息更新)
...
@property (nonatomic, strong) HomeChatTopView *topView;//顶部导航栏(自定义)
@property (nonatomic, strong) ZChatInputView *viewInput;//底部输入框
...
@property (nonatomic, strong) CTalkMultiSelectSendHander *collectionSendHander;//收藏回调
...
@implementation FileFileHelperVC
...
- (void)setupUI {
    ...
    _topView = [[HomeChatTopView alloc] initWithFrame:CGRectMake(0, DStatusBarH, DScreenWidth, 44)];
    _topView.chatName = MultilingualTranslation(@"文件助手");
    ...
    _topView.navRightBlock = ^{
        FileFileHelperSetVC *vc = [FileFileHelperSetVC new];
...
- (void)requestHistoryList {
    ...
    WeakSelf
    [IMSDKManager messageGetHistoryRecordWith:_sessionID chatType:CIMChatType_SingleChat serviceMsgID:serviceMsgID offset:self.messageModels.count pageNum:_pageNumber historyList:^(NSArray<LingIMChatMessageModel *> * _Nullable chatMessageHistory, NSInteger offset, BOOL isLocal, NSInteger pageNumber) {
...
- (void)reloadMessageTabView:(NSNotification *)sender{
    //更新聊天页面
    ZMessageModel * msgModel;
    ZMessageModel * senderMsgModel = sender.object;
    for (ZMessageModel * messageModel in self.messageModels.safeArray) {
...
- (void)chatInputViewSend:(NSString *)sendStr atUserList:(NSArray *)atUsersDictList atUserSegmentList:(NSArray *)atUserSegmentList {
    ...
    LingIMChatMessageModel *textSendMsg = [ZMessageSendHander ZMessageTextSend:ZSensitiveFilter(sendStr) withToUserId:self.sessionID  withChatType:CIMChatType_SingleChat referenceMsgId:referenceId];
...
- (void)chatInputViewVoicePath:(NSString *)vociePath voiceName:(NSString *)voiceName voiceDuration:(CGFloat)voiceDuration{
    WeakSelf
    [ZMessageSendHander ZMessageVoiceSend:vociePath fileName:voiceName voiceDuring:voiceDuration withToUserId:self.sessionID withChatType:CIMChatType_SingleChat compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
        //上传的语音文件
        ...
        NSBlockOperation * blockOperation = [NSBlockOperation blockOperationWithBlock:^{
            ...
            if (audioTask.status == FileUploadTaskStatus_Failed) {
                [ZTOOL doInMain:^{
                    [HUD showMessage:MultilingualTranslation(@"上传失败")];
                }];
            }
        }];
...
- (void)imagePickerVCSelected {
    ...
    if (IMAGEPICKER.zSelectedAssets.count > 0) {
        ...
        WeakSelf
        [ZMessageSendHander ZMessageMediaSend:IMAGEPICKER.zSelectedAssets withToUserId:self.sessionID withChatType:CIMChatType_SingleChat compelete:^(NSArray <LingIMChatMessageModel *> * sendChatMsgArr) {
            
            NSMutableArray * taskArray = [NSMutableArray array];
            for (LingIMChatMessageModel *beSendMessageModel in sendChatMsgArr) {
                //生成上传任务，开始上传
            ...
            for (LingIMChatMessageModel *sendMessageModel in sendChatMsgArr) {
                //将配置好的消息 1.存储到数据库 2.添加到UI上展示
                [IMSDKManager toolInsertOrUpdateChatMessageWith:sendMessageModel];
...
- (void)recombineFileSendData:(ZFilePickModel *)sendFileModel sendFileDataList:(NSArray <ZFilePickModel *> *)sendFileDataList {
    [HUD showActivityMessage:MultilingualTranslation(@"处理中...")];
    WeakSelf
    if (sendFileDataList == nil) {
        //手机中存储的文件(每次只发1个文件)
        [ZMessageSendHander ZMessageFileSendData:sendFileModel withToUserId:self.sessionID withChatType:CIMChatType_SingleChat compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
            //目录
            NSString *customPath = [NSString stringWithFormat:@"%@-%@", UserManager.userInfo.userUID, self.sessionID];
        ...
        for (ZFilePickModel *filePickModel in sendFileDataList) {
            dispatch_group_enter(myGroup);
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [ZMessageSendHander ZMessageFileSendData:filePickModel withToUserId:self.sessionID withChatType:CIMChatType_SingleChat compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
                    //目录
                    NSString *customPath = [NSString stringWithFormat:@"%@-%@", UserManager.userInfo.userUID, self.sessionID];
                    NSString *localFilePath = [NSString getPathWithFileName:sendChatMsg.fileName CustomPath:customPath];
                    //上传文件
        ...
        dispatch_group_notify(myGroup, dispatch_get_main_queue(), ^{
            ...
            for (LingIMChatMessageModel *sendMessageModel in sengMessageArr) {
                //将配置好的消息 1.存储到数据库 2.添加到UI上展示
                [IMSDKManager toolInsertOrUpdateChatMessageWith:sendMessageModel];
                [weakSelf chatListAppendMessage:sendMessageModel];
            }
...
- (void)sendGeoLocationMessageWithLat:(NSString *)lat lng:(NSString *)lng name:(NSString *)name cImg:(UIImage *)cImg detail:(NSString *)detail {
    WeakSelf
    [ZMessageSendHander ZMessageLocationSendWithLng:lng lat:lat name:name cImg:cImg detail:detail withToUserId:self.sessionID withChatType:CIMChatType_SingleChat compelete:^(LingIMChatMessageModel * _Nonnull sendChatMsg) {
        //地理位置地图截图
...
- (void)chatInputViewPlayGameStickerAction:(ZChatGameStickerType)gameType {
    ...
    //生成消息体
    LingIMChatMessageModel *gameStickersMessageModel = [ZMessageSendHander ZMessageGameStickersSendResultContent:resultContent gameStickersType:gameStickersType gameStickerExt:@"" withToUserId:self.sessionID withChatType:CIMChatType_SingleChat];
    //发送游戏表情消息
...
- (void)sendChatRecordMessage {
    ...
    vc.messageRecordReceverListBlock = ^(NSArray * _Nonnull selectedReceverInfoArr) {
        //文件助手合并转发
       NSString *recordTitle  = [NSString stringWithFormat:MultilingualTranslation(@"%@的会话记录"), UserManager.userInfo.nickname];
        //根据消息时间进行升序排列
        ...
        [ZMessageSendHander ZMessageMergeForwardSendWith:sortResultArr withTitle:recordTitle withToUserInfoArr:selectedReceverInfoArr compelete:^(NSArray <LingIMChatMessageModel *> *sendChatMsgList) {
            //发送消息
            for (LingIMChatMessageModel *sendChatMsg in sendChatMsgList) {
                [IMSDKManager toolSendChatMessageWith:sendChatMsg];
...
- (void)chatListAppendMessage:(LingIMChatMessageModel *)newMessage {
    if (newMessage != nil) {
        ...
        //追加消息
        ZMessageModel *newMsgModel = [[ZMessageModel alloc] initWithMessageModel:newMessage];
    ...
    if (self.multiSelectStatus) {
        ...
        if (multiClickModel.message.messageType != CIMChatMessageType_GroupNotice && multiClickModel.message.messageType != CIMChatMessageType_CardMessage  && multiClickModel.message.messageType != CIMChatMessageType_GameStickersMessage && multiClickModel.message.messageType != CIMChatMessageType_ServerMessage && multiClickModel.message.messageSendType == CIMChatMessageSendTypeSuccess) {
            ...
            if (multiClickModel.multiSelected == NO && self.selectedMsgModels.count >= Multi_Selected_Max_Num) {
                [HUD showMessage:MultilingualTranslation(@"最多选择100条消息")];
                return;
            }
            ...
            if (multiClickModel.multiSelected) {
                ...
                WeakSelf
                [self.selectedMsgModels.safeArray enumerateObjectsUsingBlock:^(ZMessageModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)messageForwardActionWithMsgList:(NSArray *)msgModelList multiSelectType:(ZMultiSelectType)multiSelectType {
    ...
    vc.forwardMsgSendSuccess = ^(NSArray<LingIMChatMessageModel *> * _Nullable sendForwardMsgList) {
        [HUD showMessage:MultilingualTranslation(@"转发成功")];
        //转发消息是转发给当前聊天对象
...
- (void)messageCollectionActionWithMsg:(ZMessageModel *)msgModel {
    ...
    [IMSDKManager MessageCollectionSave:dic onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (result) {
            [HUD showMessage:MultilingualTranslation(@"收藏成功")];
        } else {
            [HUD showMessage:MultilingualTranslation(@"收藏失败")];
        }
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD hideHUD];
        [HUD showMessageWithCode:code errorMsg:msg];
...
- (void)messageDeleteWithSMgIds:(NSArray *)sMsgIdsList withDeleteType:(ZMsgDeleteType)deleteType {
    [HUD showActivityMessage:MultilingualTranslation(@"删除中...")];
    ...
    WeakSelf
    [[LingIMSDKManager sharedTool] deleteMessage:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        [HUD showMessage:MultilingualTranslation(@"消息删除成功")];
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD showMessage:MultilingualTranslation(@"消息删除失败")];
    }];
}
...
- (void)messageReCallWithMsg:(ZMessageModel *)msgModel {
    ...
    WeakSelf
    [[LingIMSDKManager sharedTool] recallMessage:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (isRecalled) {
            ...
            [HUD showMessage:MultilingualTranslation(@"消息撤回失败")];
        }
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD showMessageWithCode:code errorMsg:msg];
    }];
...
- (void)imageVideoBrowserWith:(LingIMChatMessageModel *)messageModel {
    ...
    if (messageModelIndex == 0 && currentImageVideoMessageList.count > 0) {
        LingIMChatMessageModel *first = [currentImageVideoMessageList firstObject];
        if (![first.msgID isEqualToString:messageModel.msgID]) {
            ...
            [fix enumerateObjectsUsingBlock:^(LingIMChatMessageModel * _Nonnull chatMessage, NSUInteger idx, BOOL * _Nonnull stop) {
                if (chatMessage.messageType == CIMChatMessageType_ImageMessage) {
                    KNPhotoItems *item = [[KNPhotoItems alloc] init];
                    item.isVideo = false;
                    if (chatMessage.localImgName) {
