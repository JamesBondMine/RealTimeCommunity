...
@implementation ZMessageTimeTool
...
+ (NSString*)getTimeStringAutoShort2:(NSDate *)dt mustIncludeTime:(BOOL)includeTime{
    ...
    // 要额外显示的时间分钟
    NSString *timeExtraStr = (includeTime ? [ZMessageTimeTool getTimeString:dt format:@"HH:mm"] : @"");
//    // 当年
...
//    }
    ...
    if(currentYear == srcYear) {
        // 当年
        ret = [NSString stringWithFormat:@"%@", [ZMessageTimeTool getTimeString:dt format:@"M-d"]];
    } else {
        // 往年
        ret = [NSString stringWithFormat:@"%@", [ZMessageTimeTool getTimeString:dt format:@"yyyy-M-d"]];
    }
    return ret;
}
...
+ (NSString*)getTimeString:(NSDate*)dt format:(NSString*)fmt{
    ...
    return [format stringFromDate:(dt == nil ? [ZMessageTimeTool getIOSDefaultDate] : dt)];
}
...
+ (long)getIOSTimeStamp_l:(NSDate*)dat {
    return [[NSNumber numberWithDouble:[ZMessageTimeTool getIOSTimeStamp:dat]] longValue];
}
