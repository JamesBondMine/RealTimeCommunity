...
@interface ZFileDownloadTask()

@property (nonatomic, strong) dispatch_semaphore_t semaphore;

@end

@implementation ZFileDownloadTask
...
- (void)startDonwloadTask {
    ...
    __block NSURLSessionDownloadTask *task = [manager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ZFileDownloadTask *downloadTask = [[ZFileDownloadManager sharedInstance] getTaskWithId:taskId];
...
- (BOOL)isEqual:(id)object{
    ...
    return [self.taskId isEqualToString:((ZFileDownloadTask *)object).taskId];
}
