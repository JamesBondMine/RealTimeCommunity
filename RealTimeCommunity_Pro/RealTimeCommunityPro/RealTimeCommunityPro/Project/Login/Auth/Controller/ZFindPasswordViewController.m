//
//  ZFindPasswordViewController.m
//  CIMKit
//
//  Created by cusPro on 2023/3/28.
//
...
@interface ZFindPasswordViewController ()

@property (nonatomic, strong)MainInputTextView *accountInput;
@property (nonatomic, strong)MainInputTextView *vercodeInput;
@property (nonatomic, strong)MainInputTextView *passwordInput;
@property (nonatomic, strong)MainInputTextView *confimPasswordInput;
...
@implementation ZFindPasswordViewController
...
#pragma mark - UI
- (void)setupUI {
    NSString *titleStr = [NSString stringWithFormat:MultilingualTranslation(@"使用%@设置新密码"),[NSString getAuthContetnWithAuthType:self.findPasswordWay]];
...
- (void)listenAccountInput {
    if (self.findPasswordWay == UserAuthTypePhone) {
        [ZAuthInputTools loginCheckPhoneWithText:self.accountInput.inputText.text];
    }
    if (self.findPasswordWay == UserAuthTypeEmail) {
        [ZAuthInputTools loginCheckEmailWithText:self.accountInput.inputText.text];
    }
}

//验证码监听
- (void)listenVercodeInput {
    [ZAuthInputTools checkVerCodeWithText:self.vercodeInput.inputText.text];
}

//密码监听
- (void)listenPasswordInput {
    if ([ZAuthInputTools checkCreatPasswordEndWithTextLength:self.passwordInput.inputText.text] == NO) {
        self.dynamicTipLbl.hidden = NO;
        self.dynamicTipLbl.text = MultilingualTranslation(@"密码长度6-16");
    ...
    } else if ([ZAuthInputTools checkCreatPasswordEndWithTextFormat:self.passwordInput.inputText.text] == NO) {
        self.dynamicTipLbl.hidden = NO;
        self.dynamicTipLbl.text = MultilingualTranslation(@"密码须包含字母、数字");
        [self.dynamicTipLbl mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.passwordInput.mas_bottom).offset(DWScale(4));
            make.height.mas_equalTo(DWScale(16));
        }];
...
- (void)setFindPasswordWay:(int)findPasswordWay {
    ...
    switch (_findPasswordWay) {
        ...
        {
            self.accountInput.placeholderText = MultilingualTranslation(@"请输入手机号");
            self.accountInput.inputType = ZMessageInputViewTypePhone;
            self.accountInput.tipsImgName = @"img_phone_input_tip_reb";
        }
            break;
        case UserAuthTypeEmail:
        {
            self.accountInput.placeholderText = MultilingualTranslation(@"请输入邮箱");
            self.accountInput.inputType = ZMessageInputViewTypeNomal;
            self.accountInput.tipsImgName = @"img_email_input_tip_reb";
        }
            break;
...
- (void)loginAction {
    ...
    if (self.findPasswordWay == UserAuthTypePhone) {
        if ([ZAuthInputTools loginCheckPhoneWithText:self.accountInput.inputText.text] == NO ||
            [ZAuthInputTools checkVerCodeWithText:self.vercodeInput.inputText.text] == NO) {
            return;
        }
    }
    if (self.findPasswordWay == UserAuthTypeEmail) {
        if ([ZAuthInputTools loginCheckEmailWithText:self.accountInput.inputText.text] == NO ||
            [ZAuthInputTools checkVerCodeWithText:self.vercodeInput.inputText.text] == NO) {
            return;
        }
    }
    if ( [ZAuthInputTools checkCreatPasswordEndWithTextLength:self.passwordInput.inputText.text] == NO ||
        [ZAuthInputTools checkCreatPasswordEndWithTextFormat:self.passwordInput.inputText.text] == NO ||
        [ZAuthInputTools checkCreatPasswordEndWithTextLength:self.confimPasswordInput.inputText.text] == NO ||
        [ZAuthInputTools checkCreatPasswordEndWithTextFormat:self.confimPasswordInput.inputText.text] == NO) {
        
        [HUD showMessage:MultilingualTranslation(@"密码长度6-16位，须包含字母、数字") inView:self.view];
        return;
    }
    if (![self.passwordInput.inputText.text isEqualToString:self.confimPasswordInput.inputText.text]) {
        [HUD showMessage:MultilingualTranslation(@"密码不一致") inView:self.view];
...
- (void)requestGetVercode:(NSString *)code ticket:(NSString *)ticket randstr:(NSString *)randstr captchaVerifyParam:(NSString *)captchaVerifyParam {
    if ( self.accountInput.isEmpty) {
        [HUD showMessage:MultilingualTranslation(@"账号")];
        return;
    }
...
- (void)resetPasswordActionWithEncryptKey:(NSString *)encryptKey {
    ...
    [IMSDKManager authResetPasswordWith:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        [ZTOOL doInMain:^{
            ...
            [ZUserModel savePreAccount:self.loginInfo Type:self.findPasswordWay];
...
#pragma mark - Lazy
- (MainInputTextView *)accountInput {
    if (!_accountInput) {
        _accountInput = [[MainInputTextView alloc] init];
        _accountInput.placeholderText = MultilingualTranslation(@"输入手机号");
        ...
        _accountInput.tipsImgName = @"img_phone_input_tip_reb";
    }
    return _accountInput;
}

- (MainInputTextView *)vercodeInput {
    if (!_vercodeInput) {
        _vercodeInput = [[MainInputTextView alloc] init];
        _vercodeInput.placeholderText = MultilingualTranslation(@"输入验证码");
        if (@available(iOS 12.0, *)) {
        ...
        _vercodeInput.tipsImgName = @"img_vercode_input_tip_reb";
    }
...
- (MainInputTextView *)passwordInput {
    if (!_passwordInput) {
        ...
        _passwordInput.placeholderText = MultilingualTranslation(@"请设置新密码");
...
- (MainInputTextView *)confimPasswordInput {
    if (!_confimPasswordInput) {
        _confimPasswordInput = [[MainInputTextView alloc] init];
        _confimPasswordInput.isPassword = YES;
        _confimPasswordInput.placeholderText = MultilingualTranslation(@"请再次输入新密码");
        ...
        _confimPasswordInput.tipsImgName = @"img_password_input_tip_reb";
    }
...
- (UILabel *)tipLbl {
    if (!_tipLbl) {
        ...
        if (ZHostTool.appSysSetModel.checkEnglishSymbol) {
            _tipLbl.text = MultilingualTranslation(@"密码至少6个字符，同时包含字母、数字、符号");
        } else {
            _tipLbl.text = MultilingualTranslation(@"密码至少6个字符，不能全是字母或数字");
        }
        _tipLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
