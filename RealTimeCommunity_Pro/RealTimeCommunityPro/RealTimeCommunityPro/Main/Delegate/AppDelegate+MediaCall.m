...
/// 开始拖动
- (void)beganDragFloatView:(ZWindowFloatView *)floatView {
    
}
/// 拖动中...
- (void)duringDragFloatView:(ZWindowFloatView *)floatView {
    
}
/// 结束拖动
- (void)endDragFloatView:(ZWindowFloatView *)floatView {
    ...
    if ([ZHostTool.appSysSetModel.video_source_config isEqualToString:@"0"]) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (currentCallOptions.callRoomType == ZIMCallRoomTypeSingle) {
                ZMediaCallSingleVC *callVC = [ZMediaCallSingleVC new];
                callVC.modalPresentationStyle = UIModalPresentationFullScreen;
                [CurrentVC presentViewController:callVC animated:YES completion:nil];
            }else {
                ZMediaCallMoreVC *callVC = [ZMediaCallMoreVC new];
                NTNavigationController *nav = [[NTNavigationController alloc] initWithRootViewController:callVC];
        ...
        switch ([ZCallManager sharedManager].callSDKType) {
...
- (void)imSdkMediaCallSingleInviteeReceiveRequestWith:(LIMMediaCallModel *)mediaCallModel {
    ...
    if ([ZMediaCallManager sharedManager].mediaCallState != ZMediaCallStateEnd) {
        ...
        ZMediaCallMiniView *viewCall = [ZMediaCallMiniView new];
...
- (void)imSdkMediaCallSingleInviteeAcceptRequestWith:(LIMMediaCallModel *)mediaCallModel {
    ...
    [[ZMediaCallManager sharedManager] mediaCallConfirmWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            
            NSDictionary *dataDict = (NSDictionary *)data;
            
            LIMMediaCallSingleModel *newCallModel = [LIMMediaCallSingleModel mj_objectWithKeyValues:dataDict];
            [ZMediaCallManager sharedManager].currentCallOptions.callMediaModel = newCallModel;
...
- (void)imSdkMediaCallSingleInviterConfirmRoomWith:(LIMMediaCallModel *)mediaCallModel {
    ...
    LIMMediaCallSingleModel *singleModel = mediaCallModel.callSingleModel;
    //更新房间信息
    [ZMediaCallManager sharedManager].currentCallOptions.callMediaModel = singleModel;
...
- (void)imSdkMediaCallGroupInviteWith:(LIMMediaCallModel *)mediaCallModel {
    ...
    [currentOptions.callMediaGroupMemberList enumerateObjectsUsingBlock:^(MCMediaCallGroupMemberModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)imSdkMediaCallGroupLeaveWith:(LIMMediaCallModel *)mediaCallModel {
    ...
    if ([groupModel.reason isEqualToString:@"refused"]) {
        ...
        [currentOptions.callMediaGroupMemberList enumerateObjectsUsingBlock:^(MCMediaCallGroupMemberModel * _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        //通知UI修改
        MCMediaCallGroupMemberModel *leaveModel = [MCMediaCallGroupMemberModel new];
...
- (void)imSdkCallInviteeReceiveRequestWith:(IMChatMessage *)chatMessageCall {
    ...
    if ([ZCallManager sharedManager].callState == ZCallStateEnd) {
        ...
        if (callMessage.chatType == 1) {
            ...
            ZCallMiniView *viewCall = [ZCallMiniView new];
            [viewCall mediaCallMiniViewShow];
...
- (void)zgCallInviteeWith:(IMChatMessage *)chatMessageCall {
    ...
    ZCallUserModel *inviteeUserModel = [ZCallUserModel new];
    ...
    //2.配置 单聊 音视频通话 邀请者信息
    ZCallUserModel *inviterUserModel = [ZCallUserModel new];
...
- (void)zgCallGroupInviteeWith:(IMChatMessage *)chatMessageCall {
    ...
    //3.邀请者信息
    ZCallUserModel *inviterModel = [ZCallUserModel new];
...
- (void)zgCallGroupMemberChangeWith:(IMChatMessage *)chatMessageCall {
    ...
    switch (netCallMessage.status) {
        ...
        case 12://加入房间超时
        {
            ...
            [currentCallOptions.callMemberList enumerateObjectsUsingBlock:^(ZCallUserModel * _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
