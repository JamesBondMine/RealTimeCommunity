...
@interface MainInputTextView() <UITextFieldDelegate>
...
@implementation MainInputTextView
...
- (void)configVercodeBtnCountdown {
    ...
    [self.vercodeBtn startCountDownTime:60 styleIndex:2 withCountDownBlock:^{
        weakSelf.vercodeBtn.tkThemebackgroundColors = @[COLOR_81D8CF, COLOR_81D8CF_DARK];
        [weakSelf.vercodeBtn setTitle:MultilingualTranslation(@"获取验证码") forState:UIControlStateNormal];
    }];
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    {   //当输入为空格时
        ...
        if (_isSSO) {
            ...
            if ([ZAuthInputTools checkCreatPasswordInputWithText:string]) {
                return YES;
            } else {
                return NO;
            }
...
- (void)textFieldDidPaste:(UITextField *)textField {
    ...
    if (_isPassword) {
        NSString *pastedString = [UIPasteboard generalPasteboard].string;
        if ([ZAuthInputTools checkCreatPasswordEndWithTextFormat:pastedString]) {
...
- (UIButton *)areaCodeBtn {
    if (!_areaCodeBtn) {
        ...
        [_areaCodeBtn setTkThemeTitleColor:@[COLOR_81D8CF, COLOR_81D8CF_DARK] forState:UIControlStateNormal];
...
- (UITextField *)inputText {
    if (!_inputText) {
        ...
        _inputText.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (UIButton *)vercodeBtn {
    if (!_vercodeBtn) {
        ...
        [_vercodeBtn setTitle:MultilingualTranslation(@"获取验证码") forState:UIControlStateNormal];
        ...
        _vercodeBtn.tkThemebackgroundColors = @[COLOR_81D8CF, COLOR_81D8CF_DARK];
...
- (UIButton *)eyeBtn {
    if (!_eyeBtn) {
        ...
        [_eyeBtn setImage:ImgNamed(@"relogimg_icon_eye_off_reb") forState:UIControlStateNormal];
        [_eyeBtn setImage:ImgNamed(@"relogimg_icon_eye_on_reb") forState:UIControlStateSelected];
        _eyeBtn.selected = NO;
        _eyeBtn.hidden = YES;
...
- (UIButton *)clearButton {
    if (!_clearButton) {
        ...
        [_clearButton setImage:ImgNamed(@"relogimg_icon_text_clear_reb") forState:UIControlStateNormal];
