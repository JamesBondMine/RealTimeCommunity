//
//  ZInputNewPasswordViewController.m
//  CIMKit
//
...
@interface ZInputNewPasswordViewController ()
...
@implementation ZInputNewPasswordViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.navTitleStr = MultilingualTranslation(@"输入新密码");
    [self setupNavBarUI];
    [self setupUI];
}
...
- (void)setupUI {
    ...
    newTitleLab.text = MultilingualTranslation(@"输入新密码");
    ...
    confirmTitleLab.text = MultilingualTranslation(@"再次输入密码");
...
- (void)checkNextBtnAvailable {
    if ([ZAuthInputTools checkCreatPasswordEndWithTextLength:self.passwordInput.inputText.text] &&
        [ZAuthInputTools checkCreatPasswordEndWithTextFormat:self.passwordInput.inputText.text] &&
        [ZAuthInputTools checkCreatPasswordEndWithTextLength:self.confimPasswordInput.inputText.text] &&
...
- (void)requestResetPassword:(NSString *)encryptKey {
    ...
    [IMSDKManager userResetPasswordWith:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (resetResult) {
            [HUD showMessage:MultilingualTranslation(@"修改密码成功") inView:self.view];
            [ZTOOL setupLoginUI];
        } else {
            [HUD showMessage:MultilingualTranslation(@"修改密码失败") inView:self.view];
        }
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD showMessageWithCode:code errorMsg:msg inView:self.view];
    }];
...
- (MainInputTextView *)passwordInput {
    if (!_passwordInput) {
        ...
        _passwordInput.placeholderText = MultilingualTranslation(@"输入6-16位密码");
...
- (MainInputTextView *)confimPasswordInput {
    if (!_confimPasswordInput) {
        ...
        _confimPasswordInput.placeholderText = MultilingualTranslation(@"再次输入密码");
