...
@interface ZGroupManageVC ()<UITableViewDataSource,UITableViewDelegate,ZBaseCellDelegate>
...
- (void)viewDidLoad {
    ...
    self.navTitleStr = MultilingualTranslation(@"群管理");
...
-(void)requestRobotCount {
    ...
    [[LingIMSDKManager sharedTool] groupGetRobotCountWith:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        DLog(@"查询群机器人数量===%@",data);
    ...
    [IMSDKManager groupGetUserNotalkStateWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        [IMSDKManager groupGetNotalkListWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
            ...
            for (NSDictionary * notalkMemberDic in data) {
                HomeGroupNotalkMemberModel *model = [HomeGroupNotalkMemberModel mj_objectWithKeyValues:notalkMemberDic];
...
- (void)cancelNotalkReq:(HomeGroupNotalkMemberModel *)model{
    WeakSelf;
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    ...
    [IMSDKManager groupSetNotalkMemberWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if (data) {
            ...
            [HUD showMessage:MultilingualTranslation(@"成员解除禁言成功")];
        }
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD showMessageWithCode:code errorMsg:msg];
    }];
...
- (void)openAllNoTalkReq{
    ...
    [IMSDKManager groupSetNotalkAllWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if (data) {
            [HUD showMessage:self.isOpenAllMenmberNotalk ?MultilingualTranslation(@"关闭全员禁言") :MultilingualTranslation(@"开启全员禁言")];
            [weakSelf requsetGroupNotalkState];
        }
    } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
        [HUD showMessageWithCode:code errorMsg:msg];
...
- (void)changeGroupNewMemberCheckHistoryStatus {
    ...
    WeakSelf
    [[LingIMSDKManager sharedTool] groupNewMemberCheckHistoryStatusWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
...
- (void)changeGroupRemindSwitchStatus {
    ...
    WeakSelf
    [[LingIMSDKManager sharedTool] groupUpdateGroupRemindSwitchStatusWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        BOOL isResult = [data boolValue];
        if (isResult) {
...
- (void)changeGroupQRCodeSwitchStatus {
    ...
    WeakSelf
    [[LingIMSDKManager sharedTool] groupUpdateIsShowQrCodeWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        BOOL isResult = [data boolValue];
        if (isResult) {
...
- (void)changeAudioAndVideoCallEnableStatus {
    ...
    WeakSelf
    [[LingIMSDKManager sharedTool] groupUpdateAudioAndVideoCallStatusWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        BOOL isResult = [data boolValue];
        if (isResult) {
...
- (void)changeActivityLevelEnableStatus {
    ...
    WeakSelf
    [[LingIMSDKManager sharedTool] groupUpdateActivityLevelEnableStatusWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        BOOL isResult = [data boolValue];
        if (isResult) {
...
- (void)cellClickAction:(NSIndexPath *)indexPath {
    if (self.groupInfoModel.userGroupRole == 1) {
        ...
        if (indexPath.section == 0) {
            //群机器人
            ZRobotListViewController * robotVc = [ZRobotListViewController new];
            ...
            if (indexPath.row == 0) {
                ...
                //单人禁言操作
                ZGroupSetNotalkMemberVC * vc = [ZGroupSetNotalkMemberVC new];
        ...
        if ([ZHostTool.appSysSetModel.enableAudioAndVideoCalls isEqualToString:@"1"]) {
            ...
            if (indexPath.section == 0) {
                //设置群主/管理员操作
                ZGroupSetManagerOwnerVC * vc = [ZGroupSetManagerOwnerVC new];
                vc.groupInfoModel = self.groupInfoModel;
                [self.navigationController pushViewController:vc animated:YES];
            } else if (indexPath.section == 1) {
                //群机器人
...
- (void)navToJoinGroupApplyList {
    HomeSystemMessageVC *joinApplyVC = [[HomeSystemMessageVC alloc] init];
    ...
    if (self.groupInfoModel.userGroupRole == 1) {
        ...
        if (indexPath.section == 0) {
            ...
            if (indexPath.row == 0) {
                ...
                [cell setTapCancelNotalkBlock:^(HomeGroupNotalkMemberModel * _Nonnull model) {
            ...
            if (indexPath.row == 0) {
                //群私密，禁止群内私聊
                ZGroupManageContentCell *cell = [tableView dequeueReusableCellWithIdentifier:[ZGroupManageContentCell cellIdentifier] forIndexPath:indexPath];
                ...
                if (self.groupInfoModel.isNeedVerify) {
                    if (indexPath.row == 2) {
                        ...
                        cell.baseDelegate = self;
                        [cell setCornerRadiusWithIsShow:NO location:CornerRadiusLocationBottom];
                        [cell cellConfigWith:GroupManageCellCommon itemStr:MultilingualTranslation(@"邀请进群申请") model:self.groupInfoModel];
                        cell.viewLine.hidden = NO;
                        return cell;
                    } else {
                        //新成员可查看历史记录
