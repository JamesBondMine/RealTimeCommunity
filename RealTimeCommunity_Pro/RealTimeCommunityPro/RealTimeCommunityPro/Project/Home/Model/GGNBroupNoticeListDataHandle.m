//
//  GGNBroupNoticeListDataHandle.m
//  CIMKit
//
...
@interface GGNBroupNoticeListDataHandle()
...
@implementation GGNBroupNoticeListDataHandle

- (void)dealloc {
    CIMLog(@"%@ dealloc", [self class]);
}

- (NSMutableArray<GNoteLocalUserNameModel *> *)topGroupNoteModelList {
...
- (NSMutableArray<GNoteLocalUserNameModel *> *)normalGroupNoteModelList {
            ...
            return [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
                ...
                [IMSDKManager groupCheckGroupNoticeWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
                    if ([data isKindOfClass:[NSDictionary class]]) {
                        //数据处理
                        NSMutableArray <GNoteLocalUserNameModel *>*normalGroupNoteModelList = [NSMutableArray new];
                        NSMutableArray <GNoteLocalUserNameModel *>*topGroupNoteModelList = [NSMutableArray new];
                        ...
                        [groupListTemp enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                            GNoteLocalUserNameModel *groupNoticeModel = [GNoteLocalUserNameModel mj_objectWithKeyValues:obj];
    ...
    if (!_requestNoticeDetailCommand) {
        ...
        _requestNoticeDetailCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal * _Nonnull(id  _Nullable input) {
            ...
            return [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
                ...
                GNoteLocalUserNameModel *groupNoticeModel = [self obtainGroupModelWithIndexPath:selectIndexPath];
                if (!groupNoticeModel) {
                ...
                [IMSDKManager groupCheckOneGroupNoticeWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
                    ZGroupNoteModel *groupNoticeModel = nil;
                    if ([data isKindOfClass:[NSDictionary class]]) {
                        NSDictionary *dataDict = (NSDictionary *)data;
                        groupNoticeModel = [ZGroupNoteModel mj_objectWithKeyValues:dataDict];
                    }
                    [subscriber sendNext:groupNoticeModel];
                    [subscriber sendCompleted];
                } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
                    if (code == 41034) {
                        [HUD showMessage:MultilingualTranslation(@"公告已删除")];
                    }else {
                        [HUD showMessageWithCode:code errorMsg:msg];
                    }
                    [subscriber sendNext:nil];
    ...
    if (!_deleteDataCommand) {
        ...
        _deleteDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal * _Nonnull(id  _Nullable input) {
            ...
            return [RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
                ...
                GNoteLocalUserNameModel *groupNoticeModel = [self obtainGroupModelWithIndexPath:indexPath];
                if (!groupNoticeModel) {
                    [subscriber sendNext:@(false)];
                    [subscriber sendCompleted];
                    return [RACDisposable disposableWithBlock:^{
...
- (GNoteLocalUserNameModel *)obtainGroupModelWithIndexPath:(NSIndexPath *)indexPath {
...
- (void)sortGroupNoteModelListsByCreateTime:(NSMutableArray<GNoteLocalUserNameModel *> *)list {
