...
#import "SpecMiniAppFloatListCell.h"

#import "NSMiniAppWebVC.h"

@interface SpecMiniAppFloatListView () <UITableViewDelegate, UITableViewDataSource, ZBaseCellDelegate, SpecMiniAppFloatListCellDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, strong) BBBaseTableView *miniAppTableView;
@property (nonatomic, strong) UIButton *btnDeleteAll;//清空
@property (nonatomic, strong) NSMutableArray *miniAppList;

@end

@implementation SpecMiniAppFloatListView
...
- (void)setupUI {
    ...
    _miniAppTableView = [[BBBaseTableView alloc] initWithFrame:CGRectMake(DScreenWidth, 0, DScreenWidth, DScreenHeight) style:UITableViewStyleGrouped];
    _miniAppTableView.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
    ...
    viewFooter.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
    _miniAppTableView.tableFooterView = viewFooter;
    
    [_miniAppTableView registerClass:[SpecMiniAppFloatListCell class] forCellReuseIdentifier:[SpecMiniAppFloatListCell cellIdentifier]];
    
    UILabel *lblTip = [UILabel new];
    lblTip.text = MultilingualTranslation(@"链接");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    SpecMiniAppFloatListCell *cell = [tableView dequeueReusableCellWithIdentifier:[SpecMiniAppFloatListCell cellIdentifier] forIndexPath:indexPath];
    LingFloatMiniAppModel *model = [_miniAppList objectAtIndexSafe:indexPath.row];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [SpecMiniAppFloatListCell defaultCellHeight];
}
#pragma mark - ZBaseCellDelegate
- (void)cellClickAction:(NSIndexPath *)indexPath {
    LingFloatMiniAppModel * floadModel = [_miniAppList objectAtIndexSafe:indexPath.row];
    
    if ([CurrentVC isKindOfClass:[NSMiniAppWebVC class]]) {
        NSMiniAppWebVC *currentVC = (NSMiniAppWebVC *)CurrentVC;
...
- (void)navToNewMiniWebVC:(LingFloatMiniAppModel *)floadModel {
    NSMiniAppWebVC *oldVC = [ZWebCachesTOOL.caches objectForKey:floadModel.floladId];
    if (oldVC) {
...
#pragma mark - SpecMiniAppFloatListCellDelegate
- (void)miniAppDeleteWith:(NSIndexPath *)cellIndex {
    
    LingFloatMiniAppModel *model = [_miniAppList objectAtIndexSafe:cellIndex.row];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    ...
    if (![touchViewClass isEqualToString:@"BBBaseTableView"]) {
        return NO;
    }
