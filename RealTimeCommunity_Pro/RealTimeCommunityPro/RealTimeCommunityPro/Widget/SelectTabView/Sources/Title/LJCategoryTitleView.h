...
@class LJCategoryTitleView;

@protocol LJCategoryTitleViewDataSource <NSObject>
@optional
// 如果将LJCategoryTitleView嵌套进UITableView的cell，每次重用的时候，LJCategoryTitleView进行reloadData时，会重新计算所有的title宽度。所以该应用场景，需要UITableView的cellModel缓存titles的文字宽度，再通过该代理方法返回给LJCategoryTitleView。
// 如果实现了该方法就以该方法返回的宽度为准，不触发内部默认的文字宽度计算。
- (CGFloat)categoryTitleView:(LJCategoryTitleView *)titleView widthForTitle:(NSString *)title;
@end


@interface LJCategoryTitleView : NJCategoryIndicatorView

@property (nonatomic, weak) id<LJCategoryTitleViewDataSource> titleDataSource;
