...
@implementation ZGroupManageContentCell
...
- (void)setupUI {
    ...
    [_btnSwitch setImage:ImgNamed(@"com_c_switch_off") forState:UIControlStateNormal];
    [_btnSwitch setImage:ImgNamed(@"com_c_switch_on") forState:UIControlStateSelected];
...
- (void)updateCellUIWith:(NSInteger)currentRow totalRow:(NSInteger)totalRow {
    ...
    if (currentRow == 0) {
        _lblTitle.text = MultilingualTranslation(@"群私密");
        _lblContent.text = MultilingualTranslation(@"禁止成员查看其他成员资料");
        _viewContent.height = DWScale(80);
    }else if (currentRow == 1) {
        _lblTitle.text = MultilingualTranslation(@"邀请确认");
        _lblContent.text = MultilingualTranslation(@"邀请成员入群需管理员或群主确认");
        _viewContent.height = DWScale(80);
    }
