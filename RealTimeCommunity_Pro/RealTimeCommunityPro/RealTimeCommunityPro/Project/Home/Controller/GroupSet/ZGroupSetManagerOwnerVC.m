...
@interface ZGroupSetManagerOwnerVC ()<UITableViewDataSource,UITableViewDelegate,ZBaseCellDelegate>
...
@implementation ZGroupSetManagerOwnerVC
...
- (void)viewDidLoad {
    ...
    self.navTitleStr = MultilingualTranslation(@"设置群主/管理员");
...
- (void)cancelGroupManagerReq:(LingIMGroupMemberModel *)model{
    ...
    [IMSDKManager groupSetGroupManagerWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if (data) {
            ...
            [HUD showMessage:MultilingualTranslation(@"取消群管理员成功")];
...
- (void)cellClickAction:(NSIndexPath *)indexPath {
    switch (indexPath.section) {
        ...
        {
            if (indexPath.row == 0) {
                ZGroupSetGroupManagerVC * vc = [ZGroupSetGroupManagerVC new];
                vc.groupInfoModel = self.groupInfoModel;
        ...
        case 1:
        {
            if (indexPath.row == 0) {
                ...
                [cell setTapCancelManagerBlock:^(LingIMGroupMemberModel * _Nonnull model) {
                    ...
                    msgAlertView.lblTitle.text = MultilingualTranslation(@"取消管理员设置");
                    msgAlertView.lblContent.text = [NSString stringWithFormat:MultilingualTranslation(@"您正在取消 “%@” 的管理权限。"),model.nicknameInGroup];
...
//                    msgAlertView.btnCancel.tkThemebackgroundColors = @[COLOR_81D8CF, COLOR_81D8CF_DARK];
