...
@implementation HomeSystemMessageAllVC
...
- (void)viewDidLoad {
    ...
    //通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(requestListData) name:@"HomeSystemMessageAllVCReloadListDataNotification" object:nil];
...
- (void)requestHandleJoinGroupApplyWithStatus:(ZGroupApplyHandleStatus)handleStatus model:(MSSSMessageModel *)model {
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
...
- (void)operationResultHandleWithModel:(MSSSMessageModel *)model resultArr:(NSArray *)resultArr {
    if (resultArr.count > 0) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"HomeSystemMessagePendReviewVCReloadListDataNotification"
                                                            object:self];
    }
}
