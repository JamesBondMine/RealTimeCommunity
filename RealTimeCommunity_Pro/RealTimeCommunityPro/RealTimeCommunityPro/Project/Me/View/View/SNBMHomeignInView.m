...
#import "SNBMHomeignInView.h"
#import "KLSignInModel.h"
#import "KLConst.h"

@interface SNBMHomeignInView ()
/* 积分 */
...
@implementation SNBMHomeignInView
- (void)setSignModel:(KLSignInModel *)signModel {
    ...
    self.integrationsLb.text = [NSString stringWithFormat:MultilingualTranslation(@"积分：%@"), signModel.integral];
    if (signModel.continuous.length >0) {
        self.continuousDayLb.text = [NSString stringWithFormat:MultilingualTranslation(@"已连续签到%@天"), signModel.continuous];
...
-(void)awakeFromNib
{
    ...
    // 连续签到天数
    self.continuousDayLb.text = [NSString stringWithFormat:MultilingualTranslation(@"已连续签到%@天"), @"2"];
    ...
    // 积分
    self.integrationsLb.text = [NSString stringWithFormat:MultilingualTranslation(@"积分：%@"), @"200000"];
...
- (void)signIn {
    ...
    if ([self isYesterdaySign]) {
        self.continuousDayLb.text = [NSString stringWithFormat:MultilingualTranslation(@"已连续签到%@天"), [self.continuous intValue]+1];
    } else {
        self.continuousDayLb.text = [NSString stringWithFormat:MultilingualTranslation(@"已连续签到%@天"), 1];
    }
