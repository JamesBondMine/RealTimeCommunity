...
//  CIMKit
//
//  Created by cusPro on 2024/2/17.
//
...
- (void)setupUI {
    ...
    [self shadow:COLOR_33 opacity:0.15 radius:5 offset:CGSizeMake(0, 0)];
...
- (void)requestGetTranslateChannel {
    ...
    [IMSDKManager imSdkTranslateGetChannelLanguage:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if ([data isKindOfClass:[NSArray class]]) {
            ...
            weakSelf.dataList = [MeTranslateChannelLanguageModel mj_objectArrayWithKeyValuesArray:dataArr];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_translateType == ZGroupNoticeTranslateTypeChannel) {
        MeTranslateChannelLanguageModel *model = (MeTranslateChannelLanguageModel *)[self.dataList objectAtIndexSafe:indexPath.row];
        cell.itemTitleStr = model.name;
        cell.statusImgView.image = ImgNamed(@"remi_icon_selected_blue");
    ...
    if (_translateType == ZGroupNoticeTranslateTypeLanguage) {
        ZTranslateLanguageModel *model = (ZTranslateLanguageModel *)[self.dataList objectAtIndexSafe:indexPath.row];
        cell.itemTitleStr = model.name;
        cell.statusImgView.hidden = NO;
        if ([_selectedItemsCode containsObject:model.slug]) {
            cell.statusImgView.image = ImgNamed(@"com_c_select_yes");
        } else {
            cell.statusImgView.image = ImgNamed(@"com_c_select_no");
        }
    }
    ...
    if (_translateType == ZGroupNoticeTranslateTypeLanguage) {
        ...
        if ([_selectedItemsCode containsObject:model.slug]) {
            ...
            if (_selectedItemsCode.count >= 15) {
                [HUD showMessage:MultilingualTranslation(@"最多可选15个语种")];
                return;
            }
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        _tableView.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
