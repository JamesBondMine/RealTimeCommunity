...
#import "Nav.pbobjc.h"
#import <ifaddrs.h>
#import <NoaChatSDKCore/LingIMSDKManager.h>

@interface ZNetworkQualityDetector ()
...
/// 网络质量结果缓存
@property (nonatomic, strong) NSMutableDictionary<NSString *, ZNetworkQualityResult *> *qualityResultsCache;
...
@implementation ZNetworkQualityDetector

#pragma mark - 单例

+ (instancetype)sharedDetector {
    static ZNetworkQualityDetector *detector = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        detector = [[ZNetworkQualityDetector alloc] init];
    });
    return detector;
}
...
- (void)setupInitialConfiguration {
    // 设置默认配置
    _config = [ZNetworkQualityConfig defaultConfig];
...
/// 获取排序后的网络质量结果（统一的数据源）
- (NSArray<ZNetworkQualityResult *> *)getSortedQualityResults {
    @synchronized(self.qualityResultsCache) {
        NSArray<ZNetworkQualityResult *> *allResults = [self.qualityResultsCache allValues];
...
- (IMServerEndpoint *)getOptimalServer {
    // 使用统一的数据源获取排序后的结果
    NSArray<ZNetworkQualityResult *> *sortedResults = [self getSortedQualityResults];
    ...
    // 检查第一个结果是否有效
    ZNetworkQualityResult *firstResult = sortedResults.firstObject;
...
- (NSArray<IMServerEndpoint *> *)getUnavailableServers {
    ...
    for (ZNetworkQualityResult *result in sortedResults) {
...
- (void)performNetworkQualityProbe {
    ...
    NSMutableArray<ZNetworkQualityResult *> *allResults = [NSMutableArray array];
    
    // TCP服务器检测
    for (IMServerEndpoint *tcpServer in self.tcpServers) {
        dispatch_group_enter(probeGroup);
        dispatch_async(self.monitorQueue, ^{
            ZNetworkQualityResult *result = [self probeTCPServer:tcpServer];
...
- (ZNetworkQualityResult *)probeTCPServer:(IMServerEndpoint *)server {
...
- (ZNetworkQualityResult *)probeTCPServer:(IMServerEndpoint *)server retryCount:(NSInteger)retryCount {
    // 输入参数验证
    ...
    ZNetworkQualityResult *result = [[ZNetworkQualityResult alloc] init];
...
- (void)checkExceptionConditions:(ZNetworkQualityResult *)result {
    // 空指针检查
    ...
    if (result.latency > self.config.maxAcceptableLatency) {
        ZNetworkQualityException *exception = [[ZNetworkQualityException alloc] init];
        ...
        if (newCount.integerValue >= self.config.maxConsecutiveHighLatency) {
            ZNetworkQualityException *consecutiveException = [[ZNetworkQualityException alloc] init];
...
- (void)handleProbeResults:(NSArray<ZNetworkQualityResult *> *)results {
    NSLog(@"[网络检测] 处理检测结果，共 %lu 个结果", (unsigned long)results.count);
    
    // 对结果进行排序，最优的网络在前
    NSArray<ZNetworkQualityResult *> *sortedResults = [self sortResultsByQuality:results];
...
- (NSTimeInterval)getTCPConnectionLatency:(NSString *)hostname port:(int)port timeout:(NSTimeInterval)timeout {
    ...
    [[LingIMSDKManager sharedTool] probeECDHConnectivityWithHost:hostname port:portValue timeout:timeout type:1 completion:^(BOOL success, LingIMSDKManagerProbeECDHConnectStatus status) {
        testResult = success;
...
- (NSArray<ZNetworkQualityResult *> *)sortResultsByQuality:(NSArray<ZNetworkQualityResult *> *)results {
    // 首先过滤出可用的服务器
    NSArray<ZNetworkQualityResult *> *availableResults = [results filteredArrayUsingPredicate:
    ...
    // 对可用的服务器按质量进行排序
    NSArray<ZNetworkQualityResult *> *sortedResults = [availableResults sortedArrayUsingComparator:^NSComparisonResult(ZNetworkQualityResult *obj1, ZNetworkQualityResult *obj2) {
    ...
    for (NSInteger i = 0; i < sortedResults.count; i++) {
        ZNetworkQualityResult *result = sortedResults[i];
...
#pragma mark - ZNetworkQualityConfig Implementation

@implementation ZNetworkQualityConfig

+ (instancetype)defaultConfig {
    ZNetworkQualityConfig *config = [[ZNetworkQualityConfig alloc] init];
...
#pragma mark - ZNetworkQualityResult Implementation

@implementation ZNetworkQualityResult

- (NSString *)description {
    return [NSString stringWithFormat:@"<ZNetworkQualityResult: %@:%d, 类型: %@, 延迟: %.1fms, 可用: %@, 评分: %ld, 连续失败: %ld, 连续高延迟: %ld>",
...
#pragma mark - ZNetworkQualityException Implementation

@implementation ZNetworkQualityException

- (NSString *)description {
    return [NSString stringWithFormat:@"<ZNetworkQualityException: 类型: %ld, 描述: %@, 时间: %.0f, 触发重新竞速: %@, 立即触发: %@, 触发级别: %ld>",
...
#pragma mark - NSError (ZNetworkQualityDetector) Implementation

@implementation NSError (ZNetworkQualityDetector)
...
+ (instancetype)networkQualityErrorWithCode:(ZNetworkQualityErrorCode)code
    ...
    return [NSError errorWithDomain:@"ZNetworkQualityDetector"
                               code:code
                           userInfo:errorUserInfo];
}

- (BOOL)isNetworkQualityError {
    return [self.domain isEqualToString:@"ZNetworkQualityDetector"];
}
