...
@interface ZGroupMemberListCell ()

@property (nonatomic, strong) UILabel *groupRoleLabel;

@end

@implementation ZGroupMemberListCell
...
- (void)setupUI {
    ...
    _lblUserName.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (void)cellConfigWithmodel:(LingIMGroupMemberModel *)model searchStr:(NSString *)searchStr activityInfo:(GHomeActivityInfoModel *)activityInfo isActivityEnable:(NSInteger)isActivityEnable {
    ...
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        switch (themeIndex) {
            ...
            {
                //暗黑
                attStrName.yy_color = COLOR_33_DARK;
            }
                break;
...
- (void)checkGroupRoleContentWithRole:(NSInteger)role {
    if(role == 1) { //群管理
        ...
        self.groupRoleLabel.text = MultilingualTranslation(@"管理员");
        self.groupRoleLabel.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF];

        CGSize roleTextSize = [MultilingualTranslation(@"管理员") sizeWithFont:FONTR(10) constrainedToSize:CGSizeMake(10000, DWScale(16))];
        ...
        self.groupRoleLabel.hidden = NO;
        self.groupRoleLabel.text = MultilingualTranslation(@"群主");
        self.groupRoleLabel.tkThemebackgroundColors = @[COLOR_FF9327, COLOR_FF9327];

        CGSize roleTextSize = [MultilingualTranslation(@"群主") sizeWithFont:FONTR(10) constrainedToSize:CGSizeMake(10000, DWScale(16))];
        [self.groupRoleLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(_ivHeader);
            make.width.mas_equalTo(roleTextSize.width + DWScale(20));
            make.height.mas_equalTo(DWScale(16));
...
- (void)checkGroupActivityRoleShowStatus:(NSInteger)isActivityLevel activityScore:(NSInteger)activityScore role:(NSInteger)role activityInfo:(GHomeActivityInfoModel *)activityInfo {
    
    NSString *groupRoleLabelStr;
    NSString *roleContent = @"";
    if (role == 2) {
        roleContent =  MultilingualTranslation(@"群主");
        self.groupRoleLabel.tkThemebackgroundColors = @[COLOR_FF9327, COLOR_FF9327];
    } else if (role == 1) {
        roleContent =  MultilingualTranslation(@"管理员");
        self.groupRoleLabel.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF];
    } else {
        roleContent = @"";
        self.groupRoleLabel.tkThemebackgroundColors = @[COLOR_FF9C9C, COLOR_FF9C9C];
    ...
    if (isActivityLevel == 1) {
        ...
        if (role == 3) {
            ...
            for (ZGroupActivityLevelModel *levelConfigInfo in activityInfo.sortLevels) {
