...
#import "NTTeamHeaderCollectCell.h"

@interface NTTeamHomeHeaderView () <UICollectionViewDataSource, UICollectionViewDelegate, ZTeamHeaderCollectCellDelegate>
...
@implementation NTTeamHomeHeaderView
...
- (void)setupUI {
    ...
    _teamListTipImg.image = ImgNamed(@"retm_img_team_tip");
    ...
    _teamListArrowImg.image = ImgNamed(@"retm_team_arrow_gray");
    ...
    _teamListTitleLbl.text = MultilingualTranslation(@"团队列表");
    _teamListTitleLbl.font = FONTR(14);
    _teamListTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _teamListCollection.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    ...
    [_teamListCollection registerClass:[NTTeamHeaderCollectCell class] forCellWithReuseIdentifier:NSStringFromClass([NTTeamHeaderCollectCell class])];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    NTTeamHeaderCollectCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([NTTeamHeaderCollectCell class]) forIndexPath:indexPath];
    NTMTeamModel *model = [_headerTeamList objectAtIndexSafe:indexPath.row];
...
- (void)selectedTeamForDefaultAction:(NTMTeamModel *)teamModel {
   //设置为默认
