//
//  NMChangeUserInfoViewController.m
//  CIMKit
//
...
@interface NMChangeUserInfoViewController ()
...
@implementation NMChangeUserInfoViewController
...
- (void)viewDidLoad {
    ...
    if (self.changeType == changeUserInfoTypeAccount) {
        self.navTitleStr = MultilingualTranslation(@"修改账号");
        self.maxNum = 16;
        self.inputText.text = UserManager.userInfo.userName;
    } else {
        self.navTitleStr = MultilingualTranslation(@"修改昵称");
        self.maxNum = 30;
...
- (void)setupUI {
    ...
    if (self.changeType == changeUserInfoTypeAccount) {
        ...
        titleLab.text = MultilingualTranslation(@"账号只能修改一次");
        ...
        subTitleLab.text = MultilingualTranslation(@"*账号格式：2个字母+字母/数字,长度：6-16");
    ...
    [_clearBtn setImage:ImgNamed(@"remi_icon_userinfo_text_clear") forState:UIControlStateNormal];
...
- (void)navBtnRightClicked {
    if (self.changeType == changeUserInfoTypeAccount) {
        ...
        if ([NSString isNil:[_inputText.text trimString]]) {
            [HUD showMessage:MultilingualTranslation(@"账号不能为空")];
            return;
        }
        if ([[_inputText.text trimString] isEqualToString:UserManager.userInfo.userUID]) {
            [HUD showMessage:MultilingualTranslation(@"与当前账号相同")];
            return;
        }
        if ([[_inputText.text trimString] checkUserAccountFormat] == NO) {
            [HUD showMessage:MultilingualTranslation(@"账号格式错误")];
            return;
        }
        //调用修改账号接口
        ...
        //修改昵称
        if ([NSString isNil:[_inputText.text trimString]]) {
            [HUD showMessage:MultilingualTranslation(@"昵称不能为空")];
            return;
        }
        if ([[_inputText.text trimString] isEqualToString:UserManager.userInfo.userName]) {
            [HUD showMessage:MultilingualTranslation(@"与当前昵称相同")];
            return;
        }
        //调用修改昵称接口
...
- (void)requestCheckUserNameExit {
    ...
    [IMSDKManager authUserExistWith:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if (!exist) {
            ...
            //账号已使用
            [HUD showMessage:MultilingualTranslation(@"账号已被使用")];
        }
...
- (void)requestChangeUserName {
    ...
    [IMSDKManager userAccountChangeWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        //修改账号成功
        [ZUserModel savePreAccount:weakSelf.inputText.text Type:UserAuthTypeAccount];
