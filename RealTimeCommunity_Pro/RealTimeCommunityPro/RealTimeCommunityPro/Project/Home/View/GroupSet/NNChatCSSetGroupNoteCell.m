...
//    viewBg.frame = CGRectMake(DWScale(16), 0, DScreenWidth - DWScale(32), DWScale(84));
    ...
    self.viewBg.layer.shadowOpacity = 0.06;
    ...
    self.lblNote.text = MultilingualTranslation(@"群公告");
    self.lblNote.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _lblGroupNote.text = MultilingualTranslation(@"暂无公告");
    ...
    if (_groupModel) {
        if (![NSString isNil:groupModel.groupNotice.groupId]) {
            ...
            if (![NSString isNil:_groupModel.groupNotice.translateContent]) {
                ...
                groupNoticeStr = ![NSString isNil:_groupModel.groupNotice.content] ? _groupModel.groupNotice.content : MultilingualTranslation(@"群公告");
            }
...
- (void)layoutSubviews {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        // 商务风格：更小的圆角
        [self.viewBg round:DWScale(6) RectCorners:self.isShowLine ? UIRectCornerBottomLeft | UIRectCornerBottomRight : UIRectCornerAllCorners];
    });
}
