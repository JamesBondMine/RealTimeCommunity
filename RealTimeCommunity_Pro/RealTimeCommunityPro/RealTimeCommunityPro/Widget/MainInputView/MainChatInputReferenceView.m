//
//  MainChatInputReferenceView.m
//  CIMKit
//
//  Created by cusPro on 2022/9/27.
//

#import "MainChatInputReferenceView.h"

@interface MainChatInputReferenceView ()
...
@implementation MainChatInputReferenceView
...
#pragma mark - 界面布局
- (void)setupUI {
    UIImageView *ivReference = [[UIImageView alloc] initWithImage:ImgNamed(@"com_c_input_share")];
    ...
    _lblReferenceUser.textColor = COLOR_81D8CF;
    _lblReferenceUser.text = MultilingualTranslation(@"引用：");
    ...
    [_btnClose setTkThemeImage:@[ImgNamed(@"remsg_icon_chat_refresh_close"), ImgNamed(@"remsg_icon_chat_refresh_close_dark")] forState:UIControlStateNormal];
...
#pragma mark - Data
- (void)setReferenceMsgModel:(ZMessageModel *)referenceMsgModel {
    _referenceMsgModel = referenceMsgModel;
    
    //UI赋值
    _lblReferenceUser.text = [NSString stringWithFormat:@"%@%@", MultilingualTranslation(@"引用："),  _referenceMsgModel.message.fromNickname];
