...
@implementation TeamTeamInviteDetailVC
...
- (IVTTeamInviteDetailDataHandle *)teamDetailDataHandle {
    if (!_teamDetailDataHandle) {
        _teamDetailDataHandle = [[IVTTeamInviteDetailDataHandle alloc] initWithTeamModel:self.currentTeamModel];
    }
...
/// 界面布局
- (void)configNavUI {
    self.navTitleStr = MultilingualTranslation(@"团队详情");
...
- (void)processData {
    ...
    [self.teamDetailView.editTeamNameSubject subscribeNext:^(id  _Nullable x) {
        @strongify(self)
        TeamTeamInviteEditTeamNameVC *editTeamNameVC = [TeamTeamInviteEditTeamNameVC new];
    ...
    [self.teamDetailDataHandle.jumpAllGroupPeoplePageSubject subscribeNext:^(id  _Nullable x) {
        @strongify(self)
        TeamTeamTotalNumberListVC *totalVC = [TeamTeamTotalNumberListVC new];
