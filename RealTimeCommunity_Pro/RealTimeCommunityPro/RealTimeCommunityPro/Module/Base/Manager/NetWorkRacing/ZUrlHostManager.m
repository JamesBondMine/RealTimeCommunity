...
@property (nonatomic, strong)ZNodePreferTools *nodePreferTools;  //节点定时择优
...
/// 兜底导航存储
@property (nonatomic, strong) ZFallbackEndpointStore *fallbackStore;

/// 网络质量检测器
@property (nonatomic, strong) ZNetworkQualityDetector *networkQualityDetector;
...
- (ZNetworkQualityDetector *)networkQualityDetector {
    if (!_networkQualityDetector) {
        _networkQualityDetector = [ZNetworkQualityDetector sharedDetector];
...
+ (instancetype)shareManager{
    static ZUrlHostManager *_manager = nil;
    dispatch_once(&onceToken, ^{
        ...
        // 初始化兜底存储
        _manager.fallbackStore = [ZFallbackEndpointStore shared];
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    return [ZUrlHostManager shareManager];
}
...
- (void)QRcodeSacnNav:(IMServerListResponseBody *)serverResponse {
    ...
    if ([ZTOOL isNetworkAvailable]) {
        ...
        NSMutableArray <ZNetRacingItemModel *>* httpArray = [NSMutableArray new];
        ...
        ZNetRacingModel *racingModel = [[ZNetRacingModel alloc] init];
...
- (void)hostNodeRaceWithNetworkQualityTrigger:(BOOL)isNetworkQualityTrigger {
    ...
    if (![NSString isNil:ssoModel.liceseId]) {
        ...
        void (^consumeList)(NSArray<ZUrlHostModel *> *, NSString *sourceTag) = ^(NSArray<ZUrlHostModel *> *list, NSString *sourceTag){
            ...
            NSArray<ZUrlHostModel *> *normalized = [weakSelf normalizeDomainToIP:list];
...
- (void)getDirectOssUrlListFromTencentDoHAAAAComplete:(void(^)(NSArray<ZUrlHostModel *> *ossUrlList))complete {
    ...
    requestNext = ^{
        ...
        NSURLSessionDataTask *task = [manager GET:url parameters:nil headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            NSMutableArray<ZUrlHostModel *> *list = [NSMutableArray array];
...
- (void)getDirectOssUrlListFromCloudflareTXTComplete:(void(^)(NSArray<ZUrlHostModel *> *ossUrlList))complete {
    __block BOOL finished = NO;
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer = [AFJSONResponseSerializer serializer];
    NSMutableSet *contentTypes = [NSMutableSet setWithSet:manager.responseSerializer.acceptableContentTypes];
...
- (void)getDirectOssUrlListFromCloudflareAAAAComplete:(void(^)(NSArray<ZUrlHostModel *> *ossUrlList))complete {
    __block BOOL finished = NO;
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer = [AFJSONResponseSerializer serializer];
    NSMutableSet *contentTypes = [NSMutableSet setWithSet:manager.responseSerializer.acceptableContentTypes];
...
// 阿里 DoH TXT（JSON API）
- (void)getDirectOssUrlListFromAliDoHTXTComplete:(void(^)(NSArray<ZUrlHostModel *> *ossUrlList))complete {
    __block BOOL finished = NO;
    AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
    manager.responseSerializer = [AFJSONResponseSerializer serializer];
    NSMutableSet *contentTypes = [NSMutableSet setWithSet:manager.responseSerializer.acceptableContentTypes];
...
// 阿里resolver dns
- (void)getDirectOssUrlListFromALIDNSComplete:(void(^)(NSArray<ZUrlHostModel *> *ossUrlList))complete {
    ...
    [[DNSResolver share] clearHostCache:nil];
    __block NSMutableArray<ZUrlHostModel *> *ossUrlList = [NSMutableArray array];
    WeakSelf
...
// 阿里resolver dns
- (void)getDirectOssUrlListFromDNSComplete:(void(^)(NSArray<ZUrlHostModel *> *ossUrlList))complete {
    DNSResolver *resolver = [DNSResolver share];
    [resolver setAccountId:DirectId andAccessKeyId:DirectKeyId andAccesskeySecret:DirectKeySecret];
    resolver.cacheEnable = NO;
    resolver.scheme = DNSResolverSchemeHttp;
...
- (NSArray<ZUrlHostModel *> *)normalizeDomainToIP:(NSArray<ZUrlHostModel *> *)list {
    if (list.count <= 0) return list;
    NSMutableArray<ZUrlHostModel *> *result = [NSMutableArray arrayWithCapacity:list.count];
    // 使用 ip[:port] 组合去重键（IPv6在有端口时使用 [ip]:port 表示）
    NSMutableSet<NSString *> *comboSet = [NSMutableSet set];
    for (ZUrlHostModel *item in list) {
...
- (void)ossRacingDirectFailResultHandle:(NSInteger)ossNum
    ...
    if (ossNum >= totalNum) {
        ...
        if (iscontinue) {
            ...
            NSMutableArray<ZUrlHostModel *> *fallbackOss = [NSMutableArray array];
...
// Http竞速
- (void)netWorkQRScanFiltrateBestHttpRacingWithList:(ZNetRacingModel *)racingModel index:(NSInteger)index{
    WeakSelf
    if (index < racingModel.httpArr.count) {
        ...
        for (ZNetRacingItemModel *httpItem in subArr) {
            if (hasResult) {
                break;
            }
...
// Http竞速
- (void)netWorkFiltrateBestHttpRacingWithList:(ZNetRacingModel *)racingModel
                      isNetworkQualityTrigger:(BOOL)isNetworkQualityTrigger {
    
    NSArray<ZNetRacingItemModel *> *subArr = racingModel.httpArr;
    ZNetRacingItemModel *httpItem = subArr.firstObject;
    NSString *requestHost;
    if (![httpItem.ip containsString:@"http"]) {
        requestHost = [NSString stringWithFormat:@"https://%@", httpItem.ip];
    } else {
...
// Tcp竞速
- (void)netWorkFiltrateBestTcpRacingWithList:(ZNetRacingModel *)racingModel
                     isNetworkQualityTrigger:(BOOL)isNetworkQualityTrigger {
    ...
    if (optimalServerPoint.ip && optimalServerPoint.ip.length > 0 && optimalServerPoint.port > 0) {
        ...
        [Logger verbose:[NSString stringWithFormat:@"调用netWorkFiltrateBestTcpRacingWithList:(ZNetRacingModel *)racingModel isNetworkQualityTrigger:(BOOL)isNetworkQualityTrigger,发现有最优节点，使用最优节点, ip = %@, port = %ld", opt.socketHost, opt.socketPort]];
        cim_function_configSocketHost(opt);
        
        // 配置用户信息
        LingIMSDKUserOptions *uOpt = [LingIMSDKUserOptions new];
        ...
        // 配置企业号 & 验证渠道
        ZSsoInfoModel *sso = [ZSsoInfoModel getSSOInfo];
    ...
    for (ZNetRacingItemModel *tcpItem in subArr) {
        if (hasResult) break;
...
- (void)clearConnectLocalStartHostNodeRaceWithIsNetworkQualityTrigger:(BOOL)isNetworkQualityTrigger {
    ...
    ZSsoInfoModel *ssoInfoModel = [ZSsoInfoModel getSSOInfo];
    [[MMKV defaultMMKV] removeValueForKey:[NSString stringWithFormat:@"%@%@",CONNECT_LOCAL_CACHE,ssoInfoModel.liceseId]];
    [ZSsoInfoModel clearSSOInfoWithLiceseId:ssoInfoModel.liceseId];
}
...
- (void)tcpNodePickCompanyIdWithSsoModel:(ZNetRacingModel *)racingModel index:(NSInteger)index  {
    WeakSelf
            ...
            [self checkTcpConnectivityWithHost:host port:port completion:^(BOOL success, CFWriteStreamRef writeStream) {
                ...
                if (success && !hasResult) {
                    @synchronized (weakSelf) {
                        ...
                        opt.socketPort = [port integerValue];
                        opt.socketOrgName = Z_OrgName;
                        [Logger verbose:[NSString stringWithFormat:@"调用tcpNodePickCompanyIdWithSsoModel:(ZNetRacingModel *)racingModel index:(NSInteger)index回调, ip = %@, port = %ld", opt.socketHost, opt.socketPort]];
                        cim_function_configSocketHost(opt);
                        ...
                        userOption.userAvatar = UserManager.userInfo.avatar;
                        [IMSDKManager configSDKUserWith:userOption];
                        [Logger verbose:[NSString stringWithFormat:@"调用IMSDKManager configSDKUserWith - (void)tcpNodePickCompanyIdWithSsoModel:(ZNetRacingModel *)racingModel index:(NSInteger)index, ip = %@, port = %@", host, port]];
                        [ZTOOL doInMain:^{
                            [HUD hideHUD];
                        }];
                    }
...
- (void)requestSystemConfigInfo {
    ...
    [IMSDKManager appGetSystemConfigInfoWithBaseUrl:self.apiHost
        ...
        ZSystemSettingModel *newModel = [ZSystemSettingModel mj_objectWithKeyValues:data];
...
- (void)checkNetWorkWithUrl:(ProxyType)currentType compelete:(void(^)(NSInteger code, NSString *msg, NSData *data, NSString *traceId))compelete {
    ...
    if (currentType == ProxyTypeHTTP) {
        ZProxySettings *setting = [[MMKV defaultMMKV] getObjectOfClass:[ZProxySettings class] forKey:HTTP_PROXY_KEY];
        ...
        ZProxySettings *setting = [[MMKV defaultMMKV] getObjectOfClass:[ZProxySettings class] forKey:SOCKS_PROXY_KEY];
        config.connectionProxyDictionary = @{
                          ...
                          completion:(void(^)(BOOL success, CFWriteStreamRef writeStream))completion
{
    ...
    [[LingIMSDKManager sharedTool] probeECDHConnectivityWithHost:host port:portValue timeout:-1 type:0 completion:^(BOOL success, LingIMSDKManagerProbeECDHConnectStatus status) {
        if (completion) {
...
- (void)scheduledNodePreferAction {
    ...
    if (ssoModel.ossRacingModel.is_merge_version) {
        ...
        if (self.nodePreferTools == nil) {
            self.nodePreferTools = [[ZNodePreferTools alloc] init];
        }
...
- (void)callSDKConfig {
    if (_appSysSetModel) {
        if ([_appSysSetModel.video_source_config isEqualToString:@"0"]) {
            ...
            //即构 SDK
            LingIMZGCallConfig *config = [LingIMZGCallConfig new];
            ...
            [ZCallManager sharedManager].callSDKType = LingIMCallSDKTypeZego;
            [[ZCallManager sharedManager] callSdkConfigWith:config];
        }
    }
}
    ...
    [IMSDKManager imGetRoleConfigInfoWith:nil onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSArray class]]) {
            ...
            if (roleConfigArr != nil && roleConfigArr.count > 0) {
                ...
                for (ZRoleConfigModel *roleModel in roleConfigArr) {
...
- (void)networkQualityDetector:(ZNetworkQualityDetector *)detector didCompleteProbeWithResults:(NSArray<ZNetworkQualityResult *> *)results {
    ...
    for (NSInteger i = 0; i < results.count; i++) {
        ZNetworkQualityResult *result = results[i];
...
/// 网络质量检测所有接口失败回调
- (void)networkQualityDetector:(ZNetworkQualityDetector *)detector didFailWithError:(NSError *)error {
    if (!UserManager.isLogined) {
