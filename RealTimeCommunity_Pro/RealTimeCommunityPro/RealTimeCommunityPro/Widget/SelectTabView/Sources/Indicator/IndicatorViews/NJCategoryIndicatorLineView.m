...
#pragma mark - NJCategoryIndicatorProtocol
...
- (void)jx_contentScrollViewDidScroll:(NJCategoryIndicatorParamsModel *)model {
    ...
    if (self.lineStyle == JXCategoryIndicatorLineStyle_Normal) {
        targetX = [NJCategoryFactory interpolationFrom:leftX to:rightX percent:percent];
        if (self.indicatorWidth == JXCategoryViewAutomaticDimension) {
            targetWidth = [NJCategoryFactory interpolationFrom:leftWidth to:rightWidth percent:percent];
        }
        ...
        if (percent <= 0.5) {
            targetX = leftX;
            targetWidth = [NJCategoryFactory interpolationFrom:leftWidth to:maxWidth percent:percent*2];
        }else {
            targetX = [NJCategoryFactory interpolationFrom:leftX to:rightX percent:(percent - 0.5)*2];
            targetWidth = [NJCategoryFactory interpolationFrom:maxWidth to:rightWidth percent:(percent - 0.5)*2];
        }
        ...
        if (percent <= 0.5) {
            targetX = [NJCategoryFactory interpolationFrom:leftX to:leftX + offsetX percent:percent*2];;
            targetWidth = [NJCategoryFactory interpolationFrom:leftWidth to:maxWidth percent:percent*2];
        }else {
            targetX = [NJCategoryFactory interpolationFrom:(leftX + offsetX) to:rightX percent:(percent - 0.5)*2];
            targetWidth = [NJCategoryFactory interpolationFrom:maxWidth to:rightWidth percent:(percent - 0.5)*2];
        }
    }
