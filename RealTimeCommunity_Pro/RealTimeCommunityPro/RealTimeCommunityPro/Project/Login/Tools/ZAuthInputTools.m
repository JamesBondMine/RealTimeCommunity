...
@implementation ZAuthInputTools

#pragma mark - 校验手机号
+ (BOOL)loginCheckPhoneWithText:(NSString *)text {
    if (text.length <= 0) {
        [HUD showMessage:MultilingualTranslation(@"请输入有效手机号")];
        return NO;
    }
...
+ (BOOL)registerCheckPhoneWithText:(NSString *)text {
    if (text.length <= 0) {
        [HUD showMessage:MultilingualTranslation(@"请输入有效的手机号码")];
        return NO;
    }
    NSString *numberRegex = @".*[0-9]+.*";
    NSPredicate *numberTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", numberRegex];
...
+ (BOOL)loginCheckEmailWithText:(NSString *)text {
    if (text.length <= 0) {
        [HUD showMessage:MultilingualTranslation(@"请输入有效邮箱")];
        return NO;
    }
...
+ (BOOL)registerCheckEmailWithText:(NSString *)text {
    if (text.length <= 0) {
        [HUD showMessage:MultilingualTranslation(@"请输入正确的邮箱格式，如：google@mail.com")];
        return NO;
    }
...
+ (BOOL)loginCheckAccountWithText:(NSString *)text {
    ...
    if (text.length < 6 || text.length > 16) {
        [HUD showMessage:MultilingualTranslation(@"账号长度为6-16位")];
        return NO;
    }
...
+ (BOOL)checkInviteCodeWithText:(NSString *)text {
    if (text.length <= 0) {
        [HUD showMessage:MultilingualTranslation(@"邀请码不能为空")];
        return NO;
    }
    return YES;
}
