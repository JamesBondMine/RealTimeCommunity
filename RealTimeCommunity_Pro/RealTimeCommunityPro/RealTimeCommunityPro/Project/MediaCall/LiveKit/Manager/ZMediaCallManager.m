...
#import "ZAppPermissionTipView.h"//权限获取提示框
#import "ZMediaCallSingleVC.h"//单人
#import "ZMediaCallMoreVC.h"//多人
#import "NTNavigationController.h"


@interface ZMediaCallManager ()
...
@implementation ZMediaCallManager

#pragma mark - 单例>>>>>>
+ (instancetype)sharedManager {
    static ZMediaCallManager *_manager = nil;
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    return [ZMediaCallManager sharedManager];
}
// 防止外部调用copy
...
- (void)mediaCallRequestWith:(ZMediaCallOptions *)callOptions onSuccess:(ZMediaCallSuccessCallBack)onSuccess onFailure:(ZMediaCallFailureCallback)onFailure {
...
- (void)callRequestWith:(ZMediaCallOptions *)callOptions onSuccess:(ZMediaCallSuccessCallBack)onSuccess onFailure:(ZMediaCallFailureCallback)onFailure {
    if (callOptions) {
        ...
        if (callRoom.connectionState == ConnectionStateDisconnected) {
            ...
            [HUD showErrorMessage:MultilingualTranslation(@"你当前正在通话中")];
        }
        
    }else {
        [HUD showErrorMessage:MultilingualTranslation(@"传入参数不合法")];
    }
}
#pragma mark - 接受音视频通话的邀请(被邀请者同意邀请)
- (void)mediaCallAcceptWith:(ZMediaCallOptions *)callOptions onSuccess:(ZMediaCallSuccessCallBack)onSuccess onFailure:(ZMediaCallFailureCallback)onFailure {
    if (callOptions) {
        if (callOptions.callType == LingIMCallTypeAudio) {
            //音频通话
            //检查麦克风权限
...
- (void)callAcceptWith:(ZMediaCallOptions *)callOptions onSuccess:(ZMediaCallSuccessCallBack)onSuccess onFailure:(ZMediaCallFailureCallback)onFailure {
    Room *callRoom = [IMSDKManager imSdkCallRoom];
...
- (void)mediaCallGroupRequestWith:(ZMediaCallOptions *)callOptions onSuccess:(ZMediaCallSuccessCallBack)onSuccess onFailure:(ZMediaCallFailureCallback)onFailure {
    if (callOptions.callType == LingIMCallTypeAudio) {
        //音频通话
        //检查麦克风权限
        BOOL microphoneState = [ZTOOL checkMicrophoneState];
...
//多人通话
- (void)callGroupRequestWith:(ZMediaCallOptions *)callOptions onSuccess:(ZMediaCallSuccessCallBack)onSuccess onFailure:(ZMediaCallFailureCallback)onFailure {
    if (callOptions) {
        Room *callRoom = [IMSDKManager imSdkCallRoom];
...
- (void)mediaCallGroupAcceptWith:(ZMediaCallOptions *)callOptions onSuccess:(ZMediaCallSuccessCallBack)onSuccess onFailure:(ZMediaCallFailureCallback)onFailure {
...
- (void)callGroupAcceptWith:(ZMediaCallOptions *)callOptions onSuccess:(ZMediaCallSuccessCallBack)onSuccess onFailure:(ZMediaCallFailureCallback)onFailure {
    Room *callRoom = [IMSDKManager imSdkCallRoom];
...
#pragma mark - 连接房间
- (void)mediaCallConnectRoomWith:(LingIMCallOptions *)callOptions delegate:(id <RoomDelegateObjC>)roomDelegate {
    //先判断是否有上次的计时器需要销毁
    [self deallocCurrentCallDurationTimer];
    //连接音视频房间
...
- (void)showPermissionTip {
    [ZTOOL doInMain:^{
        ZAppPermissionTipView *viewTip = [ZAppPermissionTipView new];
