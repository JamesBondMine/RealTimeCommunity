//
//  SystemZZMessagePendReviewCell.m
//  CIMKit
//
...
@interface SystemZZMessagePendReviewCell()
...
@implementation SystemZZMessagePendReviewCell
...
- (void)setModel:(MSSSMessageModel *)model {
    ...
    NSString *contentText = [NSString stringWithFormat:MultilingualTranslation(@"%@邀请%@进群"), applyUserNick, applyedUserNick];
    NSMutableAttributedString *text = [[NSMutableAttributedString alloc] initWithString:contentText];
    [text configAttStrLightColor:COLOR_33 darkColor:COLOR_33_DARK range:NSMakeRange(0, contentText.length)];
    //适配暗黑模式
    UIColor *color;
    ...
    [text yy_setTextHighlightRange:[contentText rangeOfString:applyedUserNick] color:COLOR_4791FF backgroundColor:COLOR_CLEAR tapAction:^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
        //跳转到被邀请用户个人主页
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(systemMessageCellClickNickNameAction:)]) {
    ...
    if (self.fromType == ZGroupHelperFormTypeSessionList) {
        ...
        NSString *groupInfoContent = [NSString stringWithFormat:MultilingualTranslation(@"申请加入%@"), groupName];
        ...
        [groupInfoAtt configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:groupInfoContent appointStr:groupName];
