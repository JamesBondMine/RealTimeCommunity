//
//  ZIntegralDetailViewController.m
//  CIMKit
//
//  Created by cusPro on 2024/1/9.
//

#import "ZIntegralDetailViewController.h"
#import "ZIntergralDetailCell.h"
#import "ZFDatePickerView.h"
#import <DZNEmptyDataSet/UIScrollView+EmptyDataSet.h>

@interface ZIntegralDetailViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@implementation ZIntegralDetailViewController

- (void)viewDidLoad {
    [super viewDidLoad];
   
    self.view.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
    self.navTitleStr = MultilingualTranslation(@"积分明细");
    [self setupUI];
...
- (void)setupUI {
    ...
    handleTimeLabel.text = MultilingualTranslation(@"操作时间");
    ...
    handleTypeLabel.text = MultilingualTranslation(@"操作类型");
    ...
    intergralLabel.text = MultilingualTranslation(@"积分");
    ...
    [self.baseTableView registerClass:[ZIntergralDetailCell class] forCellReuseIdentifier:NSStringFromClass([ZIntergralDetailCell class])];
}
...
- (void)requestIntergralDetailRecordWithSelectYear:(NSString*)year selectMonth:(NSString*)month
{
    ...
    [IMSDKManager imSignInWithIntergralDetail:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        ...
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            weakSelf.usedIntergralLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"使用：%ld"), userIntergralNum];
            weakSelf.obtainIntergralLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"获得：%ld"), obtainIntergralNum];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZIntergralDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ZIntergralDetailCell class]) forIndexPath:indexPath];
...
- (UIButton *)downButtonMenu {
    if (!_downButtonMenu) {
        ...
        [_downButtonMenu setImage:ImgNamed(@"remi_signDownBtn") forState:UIControlStateNormal];
        ...
        [_downButtonMenu setTkThemeTitleColor:@[COLOR_33, COLOR_99] forState:UIControlStateNormal];
