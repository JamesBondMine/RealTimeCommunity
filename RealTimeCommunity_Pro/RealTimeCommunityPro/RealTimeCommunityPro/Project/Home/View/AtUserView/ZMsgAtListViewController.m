//
//  ZMsgAtListViewController.m
//  CIMKit
//
//  Created by cusPro on 2022/12/5.
//

#import "ZMsgAtListViewController.h"
#import "ZToolManager.h"
#import "MainSearchView.h"
#import "ZAtUsetListCell.h"

#define Back_Max_Height        DWScale(560)
#define Back_Head_Height       DWScale(103)

@interface ZMsgAtListViewController ()<MainSearchViewDelegate,UITableViewDataSource,UITableViewDelegate>
...
@implementation ZMsgAtListViewController
...
- (void)setupUI {
    ...
    self.backView.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    [self.backView rounded:20];
    [self.view addSubview:self.backView];
    
    UILabel *titleLbl = [[UILabel alloc] init];
    titleLbl.text = MultilingualTranslation(@"选择提醒的人");
    titleLbl.font = FONTN(18);
    titleLbl.tkThemetextColors = @[COLOR_33, COLORWHITE];
    ...
    [closeBtn setTkThemeImage:@[ImgNamed(@"regro_g_arrow_down"), ImgNamed(@"regro_g_arrow_down_dark")] forState:UIControlStateNormal];
    ...
    self.baseTableView.tkThemeseparatorColors = @[COLORWHITE, COLOR_33];
    ...
    [self.baseTableView registerClass:[ZAtUsetListCell class] forCellReuseIdentifier:NSStringFromClass([ZAtUsetListCell class])];
}
...
- (void)requestAtUserReq{
    ...
    if (_chatType == CIMChatType_SingleChat) {  //单聊
        ...
        if (friendUserModel) {
            //对方
            ZUserModel *friendModel = [[ZUserModel alloc] init];
        ...
        if (self.reqAtList.count > 0) {
            ...
            allUser.userNickname = MultilingualTranslation(@"所有人");
            allUser.showName = MultilingualTranslation(@"所有人");
            allUser.nicknameInGroup = MultilingualTranslation(@"所有人");
            allUser.remarks = MultilingualTranslation(@"所有人");
            allUser.userUid = @"-1";
...
- (void)requestMemberListList {
    ...
    if (![NSString isNil:_searchStr]) {
        ...
        [self.showAtList removeAllObjects];
        if (_chatType == CIMChatType_SingleChat) {  //单聊
            for (ZUserModel *userModel in self.reqAtList) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZAtUsetListCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ZAtUsetListCell class]) forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_chatType == CIMChatType_SingleChat) {
        ZUserModel *clickModel = (ZUserModel *)clickModel;
    }
