...
#import "ZCallGroupFloatView.h"
#import "UIButton+Gradient.h"

@interface ZCallGroupVC () <UICollectionViewDataSource, UICollectionViewDelegate, MediaZZCallMoreVideoItemDelegate>
{
    dispatch_queue_t _callMemberQueue;
}
//我是 群聊 被邀请者 显示邀请者信息 UI
@property (nonatomic, strong) UIView *viewBaseBg;//被邀请者UI
@property (nonatomic, strong) ZCallUserModel *inviterUserModel;//邀请者用户信息
...
- (void)viewDidLoad {
    ...
    [currentCallOptions.callMemberList enumerateObjectsUsingBlock:^(ZCallUserModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)updateGroupCallUIWithCallState {
    ...
    if ([ZCallManager sharedManager].callState == ZCallStateCalling) {
        //当前通话已连接成功
        ...
        if (currentCallOptions.zgCallOptions.callType == LingIMCallTypeAudio) {
            ...
            //开启扬声器
            [[ZCallManager sharedManager] callRoomSpeakerMute:NO];
        }
...
- (void)updateGroupCallUIWithUserModel {
    ZCallUserModel *inviterUserModel = [ZCallManager sharedManager].currentCallOptions.inviterUserModel;
...
- (void)updateGroupCallUIWithMembers {
    ...
    dispatch_async(self->_callMemberQueue, ^{
        ...
        //当前音视频通话房间成员
        NSArray *currentCallMemberList = [ZCallManager sharedManager].currentCallOptions.callMemberList;
        
        [currentCallMemberList enumerateObjectsUsingBlock:^(ZCallUserModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            MCMediaCallGroupMemberModel *model = [MCMediaCallGroupMemberModel new];
            model.memberState = obj.callState;//用户通话状态
            model.callType = [ZCallManager sharedManager].currentCallOptions.zgCallOptions.callType;//通话类型
            model.userUid = obj.userUid;//用户ID
            model.groupID = [ZCallManager sharedManager].currentCallOptions.groupID;//群组ID
...
- (void)updateBtnStateWithLoacalUserModel {
    if ([ZCallManager sharedManager].callState == ZCallStateCalling) {
        ...
        if ([ZCallManager sharedManager].currentCallOptions.zgCallOptions.callType == LingIMCallTypeAudio) {
            //扬声器
            ...
            [[ZCallManager sharedManager] callRoomSpeakerMute:speakerState == LingIMCallSpeakerMuteStateOn ? YES : NO];
        }else {
            //摄像头
...
- (void)mediaCallMoreVideoItemDelete:(MCMediaCallGroupMemberModel *)model {
    if (model) {
        ...
        dispatch_async(self->_callMemberQueue, ^{
            ...
            [tempCallMemberList enumerateObjectsUsingBlock:^(ZCallUserModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                NSString *objUserUid = [NSString stringWithFormat:@"%@", obj.userUid];
                if ([userUid isEqualToString:objUserUid]) {
                    //删除该用户
...
- (void)btnMiniClick {
    ...
    ZCallGroupFloatView *viewFloat = [[ZCallGroupFloatView alloc] initWithFrame:CGRectMake(0, 0, DWScale(86), DWScale(101))];
...
- (void)btnInviteClick {
    ...
    ZCallOptions *currentOptions = [ZCallManager sharedManager].currentCallOptions;
...
- (void)btnEndClick {
    ...
    if (currentCallOptions) {
        ...
        //退出房间
        [[ZCallManager sharedManager] callRoomLogout];
        //清空本次通话的配置
        [[ZCallManager sharedManager] clearManagerConfig];
...
- (void)btnMutedAudioClick {
    if ([ZCallManager sharedManager].callState != ZCallStateCalling) return;
    self.btnMutedAudio.selected = !self.btnMutedAudio.selected;
    [[ZCallManager sharedManager] callRoomMicrophoneMute:self.btnMutedAudio.selected];
    _localUserModel.micState = [ZCallManager sharedManager].callRoomMirophoneState;
...
- (void)btnMutedVideoClick {
    ...
    [[ZCallManager sharedManager] callRoomCameraMute:self.btnMutedVideo.selected];
    _localUserModel.cameraState = [ZCallManager sharedManager].callRoomCameraState;
...
- (void)btnExternalClick {
    ...
    [[ZCallManager sharedManager] callRoomSpeakerMute:!self.btnExternal.selected];
    
    _localUserModel.speakerState = [ZCallManager sharedManager].callRoomSpeakerState;
...
- (void)btnCameraSwitchClick {
    ...
    [[ZCallManager sharedManager] callRoomCameraUseFront:self.btnCameraSwitch.selected];
    _localUserModel.cameraDirection = [ZCallManager sharedManager].callRoomCameraDirection;
}
...
- (void)callRoomJoin {
    ...
    //第一步 初始化单例
    [[ZCallManager sharedManager] callRoomCreateEngineWithOptions:currentCallOptions.zgCallOptions];
    //第二步 登录房间
    [[ZCallManager sharedManager] callRoomLogin:^(int errorCode, NSDictionary * _Nullable extendedData) {
        if (errorCode == 0) {
            //第三步 开始推流
            [ZCallManager sharedManager].callState = ZCallStateCalling;
            [[ZCallManager sharedManager] callRoomStartPublish];
            //界面更新
            [weakSelf updateGroupCallUIWithCallState];
            [weakSelf updateBtnStateWithLoacalUserModel];
            [weakSelf updateGroupCallUIWithMembers];
            //通话计时器
            [[ZCallManager sharedManager] createCurrentCallDurationTimer];
            [[ZCallManager sharedManager] createCallHeartBeatTimer];

            [HUD showMessage:MultilingualTranslation(@"连接成功")];
        }else {
            [HUD showMessage:MultilingualTranslation(@"传入参数不合法")];
        }
    }];
}
//群聊成员状态变化
