//
//  ZSafeCodeSettingViewController.m
//  CIMKit
//
...
@interface ZSafeCodeSettingViewController ()

@property (nonatomic, strong)MainInputTextView *safeCodeInput;
@property (nonatomic, strong)UILabel *safeCodeInputTipsLbl;
@property (nonatomic, strong)MainInputTextView *confimSafeCodeInput;
@property (nonatomic, strong)UILabel *confimSafeCodeInputTipsLbl;

@end

@implementation ZSafeCodeSettingViewController
...
- (void)viewDidLoad {
    ...
    self.navTitleStr = MultilingualTranslation(@"设备安全码");
    [self setupNavBarUI];
    [self setupUI];
}
...
- (void)setupUI {
    ...
    tipsTitleLab.text = MultilingualTranslation(@"为加强安全防护，请设置你的设备安全码，新设备首次登录时须输入6位安全码，同时包含字母、数字");
    ...
    newTitleLab.text = MultilingualTranslation(@"请输入安全码");
    newTitleLab.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    confirmTitleLab.text = MultilingualTranslation(@"再次输入安全码");
    confirmTitleLab.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (void)listenSafeCodeInput {
    if ([ZSafeSettingTools checkInputDeviceSafeCodeEndWithText:self.safeCodeInput.inputText.text]) {
...
- (void)requestSaveDeviceSafeCodeWithEncryptkey:(NSString *)encryptKey {
    ...
    [IMSDKManager authSaveSecurityCodeWith:params onSuccess:^(id  _Nullable data, NSString * _Nullable traceId) {
        ...
        if (result) {
            [HUD showMessage:MultilingualTranslation(@"设置成功") inView:self.view];
            NSArray *viewControllers = self.navigationController.viewControllers;
            for (UIViewController *vc in viewControllers) {
                if ([vc isKindOfClass:[NMSafeSettingViewController class]]) {
                    NMSafeSettingViewController *safeCodeSettingVC = (NMSafeSettingViewController *)vc;
...
#pragma mark - Lazy
- (MainInputTextView *)safeCodeInput {
    if (!_safeCodeInput) {
        _safeCodeInput = [[MainInputTextView alloc] init];
        _safeCodeInput.clipsToBounds = YES;
        _safeCodeInput.isPassword = YES;
        _safeCodeInput.isShowBoard = NO;
        _safeCodeInput.placeholderText = MultilingualTranslation(@"请输入安全码");
...
- (UILabel *)safeCodeInputTipsLbl {
    if (!_safeCodeInputTipsLbl) {
        ...
        _safeCodeInputTipsLbl.text = MultilingualTranslation(@"请输入6位包含字母、数字的安全码");
...
- (MainInputTextView *)confimSafeCodeInput {
    if (!_confimSafeCodeInput) {
        _confimSafeCodeInput = [[MainInputTextView alloc] init];
        _confimSafeCodeInput.isPassword = YES;
        _confimSafeCodeInput.isShowBoard = NO;
        _confimSafeCodeInput.placeholderText = MultilingualTranslation(@"再次输入安全码");
...
- (UILabel *)confimSafeCodeInputTipsLbl {
    if (!_confimSafeCodeInputTipsLbl) {
        ...
        _confimSafeCodeInputTipsLbl.text = MultilingualTranslation(@"两次安全码需保持一致");
