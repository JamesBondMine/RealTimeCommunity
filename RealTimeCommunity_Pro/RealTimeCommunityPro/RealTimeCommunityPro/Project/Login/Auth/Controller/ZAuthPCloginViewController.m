//
//  ZAuthPCloginViewController.m
//  CIMKit
//
//  Created by cusPro on 2023/4/3.
//

#import "ZAuthPCloginViewController.h"

@interface ZAuthPCloginViewController ()

@end

@implementation ZAuthPCloginViewController
...
#pragma mark - 界面布局
- (void)setupNavUI {
    self.navTitleStr = MultilingualTranslation(@"登录确认");
    self.navLineView.hidden = YES;
}

- (void)setupUI {
    UIImageView *tipsImgView = [[UIImageView alloc] init];
    tipsImgView.image = ImgNamed(@"relogimg_img_pc_auth_login_reb");
    ...
    NSString * st = [NSString stringWithFormat:MultilingualTranslation(@"登录%@ PC端"), [ZTOOL getAppName]];
    UILabel *tipsTitleLbl = [[UILabel alloc] init];
    tipsTitleLbl.text = st;
    tipsTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    [cancelLoginBtn setTitle:MultilingualTranslation(@"取消登录") forState:UIControlStateNormal];
...
- (void)authAuthLoginClick {
    ...
    [IMSDKManager authScanQrCodeForPCLoginWith:params onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        [ZTOOL doInMain:^{
            ...
            if (result) {
                [HUD showMessage:MultilingualTranslation(@"授权成功")];
            } else {
                [HUD showMessage:MultilingualTranslation(@"二维码已过期")];
            }
...
- (void)cancelAuthLoginClick {
    ...
    [IMSDKManager authScanQrCodeForPCLoginWith:params  onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        [ZTOOL doInMain:^{
            [HUD showMessage:MultilingualTranslation(@"授权取消")];
            [self.navigationController popToRootViewControllerAnimated:YES];
        }];
