...
@interface ZIntergralDetailCell()
...
@implementation ZIntergralDetailCell
...
-(void)setupUI{
    ...
    tipImgView.image = ImgNamed(@"remi_signLogoicon");
...
- (void)setIntergralDetailDict:(NSDictionary *)intergralDetailDict {
    if (intergralDetailDict) {
        ...
        if (handleType == 9) {
            //签到
            self.handleTypeLbl.text = MultilingualTranslation(@"签到");
            self.IntergralLbl.tkThemetextColors = @[COLOR_4791FF, COLOR_4791FF];
            self.IntergralLbl.text = [NSString stringWithFormat:@"+%lld", intergralNum];
        } else if (handleType == 18) {
            //系统扣减
            self.handleTypeLbl.text = MultilingualTranslation(@"系统扣减");
            self.IntergralLbl.tkThemetextColors = @[COLOR_FF3333, COLOR_FF3333];
            self.IntergralLbl.text = [NSString stringWithFormat:@"-%lld", intergralNum];
        } else if (handleType == 19) {
            //系统清除
            self.handleTypeLbl.text = MultilingualTranslation(@"系统清除");
            self.IntergralLbl.tkThemetextColors = @[COLOR_FF3333, COLOR_FF3333];
            self.IntergralLbl.text = [NSString stringWithFormat:@"-%lld", intergralNum];
        }  else {
            //其他
            self.handleTypeLbl.text = MultilingualTranslation(@"其他");
