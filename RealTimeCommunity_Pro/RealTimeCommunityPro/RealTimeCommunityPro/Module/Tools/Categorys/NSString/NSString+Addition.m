...
+ (NSString *)convertBannedSendMsgTime:(int64_t)minit {
    ...
    switch (minit) {
        case 10:
            resultStr = MultilingualTranslation(@"禁言10分钟");
            break;
        case 60:
            resultStr = MultilingualTranslation(@"禁言1小时");
            break;
        case 720:
            resultStr = MultilingualTranslation(@"禁言12小时");
            break;
        case 1440:
            resultStr = MultilingualTranslation(@"禁言24小时");
            break;
        case 10080:
            resultStr = MultilingualTranslation(@"禁言7天");
            break;
        case 43200:
            resultStr = MultilingualTranslation(@"禁言30天");
            break;
        case -1:
            resultStr = MultilingualTranslation(@"禁言");
            break;
...
+ (NSString *)timeStringForPastTimeDate:(long long)timeValue {
    ...
    if (second <= 60 * 15) {
        return MultilingualTranslation(@"刚刚");
    }else{
        NSInteger minute = second / 60;
        if (minute < 60) {
            return [NSString stringWithFormat:MultilingualTranslation(@"%ld分钟以前"),minute];
        }else{
            NSInteger hour = second / 3600;
            if (hour < 24) {
                return [NSString stringWithFormat:MultilingualTranslation(@"%ld小时以前"),hour];
            }else{
                NSInteger day = second / (3600 * 24);
                if (day < 2) {
                    return [NSString stringWithFormat:MultilingualTranslation(@"昨天%@"),[self dateStringFromTimeValue:timeValue formatter:@"HH:mm"]];
                }else{
...
+ (NSString *)sizeFormattedWithSize:(NSInteger)size {
    NSString *sizeString = @"";
    if (size == 0) {
        sizeString = MultilingualTranslation(@"文件过小");
        ...
        sizeString = MultilingualTranslation(@"文件过大");
    }
    return sizeString;
}
...
+ (NSString *)getAuthContetnWithAuthType:(int)authType {
    switch (authType) {
        case UserAuthTypePhone:
            return MultilingualTranslation(@"手机号");
            break;
        case UserAuthTypeEmail:
            return MultilingualTranslation(@"邮箱");
            break;
...
+ (NSString *)loadNickNameWithUserStatus:(NSInteger)userStatus realNickName:(NSString *)realNickName {
    if (userStatus == 4) {
        //已注销
        return MultilingualTranslation(@"账号已注销");
    } else {
        return realNickName;
    }
}
