//
//  NMShareInviteViewController.m
//  CIMKit
//
//  Created by cusPro on 2023/6/30.
//
...
@interface NMShareInviteViewController ()
...
@implementation NMShareInviteViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.view.tkThemebackgroundColors = @[COLORWHITE, COLORWHITE_DARK];
    self.navTitleStr = MultilingualTranslation(@"分享邀请");
    [self setupNavUI];
    [self setupUI];
...
- (void)setupNavUI {
    ...
    [self.navBtnRight setTitle:MultilingualTranslation(@"复制链接") forState:UIControlStateNormal];
...
- (void)setupUI {
    ...
    fromAppLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"来%@"), [ZTOOL getAppName]];
    ...
    fromTipsLbl.text = MultilingualTranslation(@"体验新的聊天乐趣");
    ...
    userNameLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    invitedTitleLbl.text = MultilingualTranslation(@"已经邀请");
    invitedTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _invitedNumLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"%d位用户"), 0];
    _invitedNumLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    inviteCodeTitleLbl.text = MultilingualTranslation(@"邀请码");
    inviteCodeTitleLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _inviteCodeLbl.text = @"--";
    _inviteCodeLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    netUrlContentLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"网络地址：%@"), netUrlStr];
    netUrlContentLbl.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    [_shareBtn setTitle:MultilingualTranslation(@"分享") forState:UIControlStateNormal];
    ...
    inviteTipsLbl.text = MultilingualTranslation(@"邀请更多好友来体验");
...
- (void)requestGetShareInviteData {
    ...
    [IMSDKManager getFriendShareInviteInfo:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            //已经邀请人数
            weakSelf.invitedNumLbl.text = [NSString stringWithFormat:MultilingualTranslation(@"%d位用户"), friendNum];
            //邀请码
...
- (UIImageView *)showBackView {
    if (!_showBackView) {
        ...
        _showBackView.image = ImgNamed(@"remi_img_share_bg.png");
    }
