...
#import "LingIMSDKManager+ChatMessage.h"//聊天类型消息处理
#import "LingIMSDKManager+ServiceMessage.h"//系统类型消息处理
#import "LingIMSDKManager+Session.h"//会话处理
#import "LingIMSDKManager+SyncServer.h"//同步数据
#import "LingIMSDKManager+AppInfo.h"//App配置信息或者敏感词相关
#import "LingIMSDKManager+Group.h"
#import "LingIMSDKManager+Logan.h"//日志模块

#import "LingIMDBTool.h"//数据库工具
#import "LingIMHttpManager.h"//Http
#import <MMKV/MMKV.h>
#import "LingIMSocketHostOptions.h"
...
@implementation LingIMSDKManager

#pragma mark - 单例
+ (instancetype)sharedTool {
    static LingIMSDKManager *_manager = nil;
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    return [LingIMSDKManager sharedTool];
}
// 防止外部调用copy
...
- (void)configSDKSocketUserInfo {
    if (_userID.length > 0 && _userToken.length > 0) {
        //用户信息
        LingIMSocketUserOptions *userOptions = [LingIMSocketUserOptions new];
...
- (void)cimMessageSendSuccess:(IMChatMessageACK *)messageACK {
    ...
    if (sessionID.length > 0) {
        //发送的聊天信息成功
        LingIMChatMessageModel *model = [IMSDKManager toolGetOneChatMessageWithMessageID:messageACK.ackMsgId sessionID:sessionID];
...
- (void)cimMessageSendFail:(NSString *)messageID {
    ...
    //忽略掉发送的ToACK消息 的 ACK消息
    if (sessionID.length > 0) {
        LingIMChatMessageModel *model = [IMSDKManager toolGetOneChatMessageWithMessageID:messageID sessionID:sessionID];
...
- (void)cimMessageSystemReceiveWith:(IMServerMessage *)message {
    CIMLog(@"LingIMSDKManager>>>接收到系统消息");
    [IMSDKManager toolDealReceiveServiceMessage:message];
}

- (void)cimMessageSystemCustomEventWith:(IMServerMessage *)message {
    CIMLog(@"LingIMSDKManager>>>接收到系统消息-自定义事件");
    [self imSdkDealReceiveServiceMessageForCustomEvent:message];
}

- (void)cimMessageSystemMessageTimeDeleteWith:(IMServerMessage *)message {
    CIMLog(@"LingIMSDKManager>>>接收到系统消息-消息定时自动删除消息");
    [self imSdkDealReceiveServiceMessageForMessageTimeDelete:message];
}

- (void)cimMessageSystemMessageUpdateSensitiveWith:(IMServerMessage * _Nullable)message {
    CIMLog(@"LingIMSDKManager>>>接收到系统消息-更新敏感词");
    [self syncAppSensitiveFromServer];
}

- (void)cimMessageSystemMessageSynchroMessageWith:(IMServerMessage * _Nullable)message {
    [self syncUserSessionStatus:message];
}

- (void)cimDialogReadTagChangeEventWith:(IMServerMessage * _Nullable)message {
...
#pragma mark - 签到提醒
- (void)cimMessageSystemMessageSignInReminder:(IMServerMessage * _Nullable)message {
...
- (void)cimMessageSystemMessageUserRoleAuthority:(IMServerMessage * _Nullable)message {
...
- (void)cimUserConnectLogoutWithCode:(NSInteger)errorCode messsage:(NSString *)message {
    ...
    if (errorCode == 40015) {
        //该账号被禁用
        [self.userDelegate imSdkUserForceLogout:2 message:message];
...
- (void)cimUserFriendNoneExist:(IMServerMessage *)message {
...
- (void)cimGroupTipServerMessage:(IMServerMessage *)message {
...
//socket连接失败
- (void)cimConnectFailWithError:(NSError *)error {
...
#pragma mark - **********懒加载**********
- (OIMGCDMulticastDelegate<CIMToolConnectDelegate> *)connectDelegate {
    if (!_connectDelegate) {
        _connectDelegate = (OIMGCDMulticastDelegate <CIMToolConnectDelegate> *) [[OIMGCDMulticastDelegate alloc] init];
    }
    return _connectDelegate;
}
- (OIMGCDMulticastDelegate<CIMToolMessageDelegate> *)messageDelegate {
    if (!_messageDelegate) {
        _messageDelegate = (OIMGCDMulticastDelegate <CIMToolMessageDelegate> *) [[OIMGCDMulticastDelegate alloc] init];
    }
    return _messageDelegate;
}
- (OIMGCDMulticastDelegate<CIMToolUserDelegate> *)userDelegate {
    if (!_userDelegate) {
        _userDelegate = (OIMGCDMulticastDelegate <CIMToolUserDelegate> *) [[OIMGCDMulticastDelegate alloc] init];
    }
    return _userDelegate;
}
- (OIMGCDMulticastDelegate<CIMToolSessionDelegate> *)sessionDelegate {
    if (!_sessionDelegate) {
        _sessionDelegate = [(OIMGCDMulticastDelegate <CIMToolSessionDelegate> *) [OIMGCDMulticastDelegate alloc] init];
    }
...
- (OIMGCDMulticastDelegate<LingIMMediaCallDelegate> *)mediaCallDelegate {
    if (!_mediaCallDelegate) {
        _mediaCallDelegate = [(OIMGCDMulticastDelegate <LingIMMediaCallDelegate> *) [OIMGCDMulticastDelegate alloc] init];
    }
