...
@interface ZTeamInviteTeamDetailTopView : UIView

/// 获取到的团队详情信息
@property (nonatomic, strong) IVTTeamDetailModel *teamDetailModel;

/// 背景图
@property (nonatomic, strong) UIImageView *bgImgView;

/// 团队总人数item
@property (nonatomic, strong) ZTeamListHeaderInfoItemView *teamAllPeopleCountItemView;

/// 昨日邀请item
@property (nonatomic, strong) ZTeamListHeaderInfoItemView *yesterdayItemView;

/// 今日邀请item
@property (nonatomic, strong) ZTeamListHeaderInfoItemView *todayItemView;

/// 本月邀请item
@property (nonatomic, strong) ZTeamListHeaderInfoItemView *monthItemView;

@property (nonatomic, copy) ClickAllGroupCountBlock clickAllGroupCountBlock;

@end

@implementation ZTeamInviteTeamDetailTopView
...
- (ZTeamListHeaderInfoItemView *)teamAllPeopleCountItemView {
    if (!_teamAllPeopleCountItemView) {
        _teamAllPeopleCountItemView = [[ZTeamListHeaderInfoItemView alloc] initWithFrame:CGRectZero];
        _teamAllPeopleCountItemView.title = MultilingualTranslation(@"团队成员");
        // 默认0
...
- (ZTeamListHeaderInfoItemView *)yesterdayItemView {
    if (!_yesterdayItemView) {
        _yesterdayItemView = [[ZTeamListHeaderInfoItemView alloc] initWithFrame:CGRectZero];
        _yesterdayItemView.title = MultilingualTranslation(@"昨日邀请");
        // 默认0
...
- (ZTeamListHeaderInfoItemView *)todayItemView {
    if (!_todayItemView) {
        _todayItemView = [[ZTeamListHeaderInfoItemView alloc] initWithFrame:CGRectZero];
        _todayItemView.title = MultilingualTranslation(@"今日邀请");
        // 默认0
...
- (ZTeamListHeaderInfoItemView *)monthItemView {
    if (!_monthItemView) {
        _monthItemView = [[ZTeamListHeaderInfoItemView alloc] initWithFrame:CGRectZero];
        _monthItemView.title = MultilingualTranslation(@"本月邀请");
        // 默认0
...
- (void)setTeamDetailModel:(IVTTeamDetailModel *)teamDetailModel {
    if (!teamDetailModel) {
        return;
    }
...
@interface ZTeamInviteTeamDetailTextView : UIView
...
@implementation ZTeamInviteTeamDetailTextView
...
- (UILabel *)detailLabel {
    if (!_detailLabel) {
        ...
        _detailLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
@interface ZTeamInviteTeamDetailBottomView : UIView
...
/// 团队邀请码
@property (nonatomic, strong) ZTeamInviteTeamDetailTextView *teamInviteCodeView;

/// 下载链接
@property (nonatomic, strong) ZTeamInviteTeamDetailTextView *teamInviteDownloadLinkView;
...
@implementation ZTeamInviteTeamDetailBottomView
...
- (ZTeamInviteTeamDetailTextView *)teamInviteCodeView {
    if (!_teamInviteCodeView) {
        _teamInviteCodeView = [[ZTeamInviteTeamDetailTextView alloc] initWithFrame:CGRectZero];
        _teamInviteCodeView.title = MultilingualTranslation(@"团队邀请码");
...
- (ZTeamInviteTeamDetailTextView *)teamInviteDownloadLinkView {
    if (!_teamInviteDownloadLinkView) {
        _teamInviteDownloadLinkView = [[ZTeamInviteTeamDetailTextView alloc] initWithFrame:CGRectZero];
        _teamInviteDownloadLinkView.title = MultilingualTranslation(@"下载链接");
...
/// 详情页面上方总人数、邀请数量展示页面
@property (nonatomic, strong) ZTeamInviteTeamDetailTopView *teamInfoTopView;

/// 设为详情页面上方总人数、邀请数量容器
@property (nonatomic, strong) UIView *teamInfoTopContainerView;

/// 团队详情页面西方二维码、下载链接等展示页面
@property (nonatomic, strong) ZTeamInviteTeamDetailBottomView *teamInfoBottomView;
...
- (UILabel *)myTeamNameLabel {
    if (!_myTeamNameLabel) {
        ...
        _myTeamNameLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
...
- (ZTeamInviteTeamDetailTopView *)teamInfoTopView {
    if (!_teamInfoTopView) {
        _teamInfoTopView = [[ZTeamInviteTeamDetailTopView alloc] initWithFrame:CGRectZero];
        @weakify(self)
...
- (ZTeamInviteTeamDetailBottomView *)teamInfoBottomView {
    if (!_teamInfoBottomView) {
        _teamInfoBottomView = [[ZTeamInviteTeamDetailBottomView alloc] initWithFrame:CGRectZero];
        @weakify(self)
...
- (void)saveImage:(UIImage *)image {
    //此处需做判断
    @weakify(self)
    ZPresentItem *saveAlbumItem = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"保存相册") textColor:COLOR_33 font:FONTR(17) itemHeight:DWScale(54) backgroundColor:COLORWHITE];
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        if (themeIndex == 0) {
            saveAlbumItem.textColor = COLOR_33;
            saveAlbumItem.backgroundColor = COLORWHITE;
        }else {
            saveAlbumItem.textColor = COLORWHITE;
            saveAlbumItem.backgroundColor = COLOR_33;
        }
    };
    ...
    ZPresentView *viewAlert = [[ZPresentView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DScreenHeight) titleItem:nil selectItems:@[saveAlbumItem] cancleItem:cancelItem doneClick:^(NSInteger index) {
        @strongify(self)
        if (index == 0) {
            [ZTOOL doInMain:^{
