...
@interface ZComplainFromVC () <UICollectionViewDataSource, UICollectionViewDelegate, ZComplainImageCellDelegate, ZImagePickerVCDelegate, UITextFieldDelegate, UITextViewDelegate, ZFileUploadTaskDelegate>
...
@implementation ZComplainFromVC
...
- (void)setupUI {
    ...
    [_btnSubmit setTitle:MultilingualTranslation(@"确认提交投诉") forState:UIControlStateNormal];
...
- (void)setupImgContentView {
    ...
    lblImageTip.text = MultilingualTranslation(@"图片证据(必填)");
    lblImageTip.font = FONTR(12);
    lblImageTip.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _lblImageNumber.text = [NSString stringWithFormat:MultilingualTranslation(@"%d张/9"), 0];
    ...
    [_collectionView registerClass:[ZComplainImageCell class] forCellWithReuseIdentifier:NSStringFromClass([ZComplainImageCell class])];
...
- (void)setupTextContentView {
    ...
    titleLbl.text = MultilingualTranslation(@"投诉内容");
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ZComplainImageCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([ZComplainImageCell class]) forIndexPath:indexPath];
    ...
    if (assetModel) {
        ...
        //+
        cell.ivComplain.image = ImgNamed(@"com_c_gray_add");
...
- (void)cellDeleteImageWith:(NSIndexPath *)cellIndex {
    ...
    _lblImageNumber.text = [NSString stringWithFormat:MultilingualTranslation(@"%d张/9"), _imageList.count];
}
...
//        [HUD showMessage:[NSString stringWithFormat:MultilingualTranslation(@"最多只能选择%ld张照片"), 9]];
//    }
...
- (void)selectComplainReason {
    ...
    NSArray *complainList = @[
        @{
            @"comName" : MultilingualTranslation(@"发布违法有害信息"),
            @"comType" : @"1"
        },
        @{
            @"comName" : MultilingualTranslation(@"发布垃圾广告"),
            @"comType" : @"2"
        },
        @{
            @"comName" : MultilingualTranslation(@"种族歧视"),
            @"comType" : @"3"
        },
        @{
            @"comName" : MultilingualTranslation(@"存在文化歧视"),
            @"comType" : @"4"
        },
        @{
            @"comName" : MultilingualTranslation(@"辱骂骚扰"),
            @"comType" : @"5"
        },
        @{
            @"comName" : MultilingualTranslation(@"帐号可能被盗"),
            @"comType" : @"6"
        },
        @{
            @"comName" : MultilingualTranslation(@"存在欺诈行为"),
            @"comType" : @"8"
        },
        @{
            @"comName" : MultilingualTranslation(@"其他"),
            @"comType" : @"7"
        }
    ];
    ...
    for (NSDictionary *comDict in complainList) {
        ...
        ZPresentItem *item = [ZPresentItem creatPresentViewItemWithText:complainReason textColor:COLOR_33 font:FONTR(17) itemHeight:DWScale(56) backgroundColor:COLORWHITE];
    ...
    WeakSelf
    ZPresentView *viewAlert = [[ZPresentView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DScreenHeight) titleItem:nil selectItems:complainItemList cancleItem:cancelItem doneClick:^(NSInteger index) {
...
- (void)handleComplainImage {
    [HUD showMessage:MultilingualTranslation(@"处理中...")];
    WeakSelf
    ...
    if (imgInfoDict.count > 0) {
        ...
        NSBlockOperation * blockOperation = [NSBlockOperation blockOperationWithBlock:^{
            ...
            for (NSString *taskId in imgInfoDict.allKeys) {
                ZFileUploadTask *uploadTask = [[ZFileUploadManager sharedInstance] getTaskWithId:taskId];
        ...
        for (NSString *imageName in imgInfoDict.allKeys) {
            ...
            ZFileUploadTask *task = [[ZFileUploadTask alloc] initWithTaskId:imageName filePath:imagePath originFilePath:@"" fileName:imageName fileType:@"" isEncrypt:YES dataLength:imageData.length uploadType:ZHttpUploadTypeUniversal beSendMessage:nil delegate:nil];
...
- (void)clearUIContent {
    ...
    //投诉类型 默认1
    self.comType = @"1";
    [self.itemContentBtn setTitle:MultilingualTranslation(@"发布违法有害信息") forState:UIControlStateNormal];
    //邮箱
...
- (UITextView *)contentTextView {
    if (!_contentTextView) {
        ...
        _contentTextView.attributedPlaceholder = [[NSAttributedString alloc]initWithString:MultilingualTranslation(@"请输入投诉内容")];
...
- (UIButton *)itemContentBtn {
    if (!_itemContentBtn) {
        ...
        [_itemContentBtn setTitle:MultilingualTranslation(@"发布违法有害信息") forState:UIControlStateNormal];
...
- (UITextField *)emailTextField {
    if (!_emailTextField) {
        ...
        _emailTextField.attributedPlaceholder =[[NSAttributedString alloc]initWithString:MultilingualTranslation(@"请输入您的邮箱")];
...
- (UITextField *)domainTextField {
    if (!_domainTextField) {
        ...
        _domainTextField.placeholder = MultilingualTranslation(@"请输入企业号/域名");
        ...
        if (![NSString isNil:ssoModel.liceseId]) {
            //企业号
            _domainTextField.text = [NSString stringWithFormat:@"%@：%@", MultilingualTranslation(@"企业号"), ssoModel.liceseId];
        }
        if (![NSString isNil:ssoModel.ipDomainPortStr]) {
            // IP/Doamin
            _domainTextField.text = [NSString stringWithFormat:@"%@：%@", MultilingualTranslation(@"域名"), ssoModel.ipDomainPortStr];
        }
