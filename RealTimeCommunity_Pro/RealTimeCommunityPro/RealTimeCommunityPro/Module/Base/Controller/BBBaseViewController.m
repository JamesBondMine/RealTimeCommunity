...
@interface BBBaseViewController () <UIGestureRecognizerDelegate>
...
- (void)setupNavView{
    ...
    _navView.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    ...
    _navTitleLabel.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    [_navTitleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.leading.equalTo(_navView.mas_leading).offset(100);
    }];
...
- (BBBaseTableView *)baseTableView {
    if (!_baseTableView) {
        _baseTableView = [[BBBaseTableView alloc] initWithFrame:CGRectZero style:_baseTableViewStyle];
    }
