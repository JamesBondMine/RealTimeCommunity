//
//  ZNetworkDetectionHandle.m
//  RealTimeCommunityPro
//
//  Created by phl on 2025/10/15.
//  TODO: 网络检测工具类

#import "ZNetworkDetectionHandle.h"
#import "NworkDetectionMessageModel.h"
#import "AliyCloundDNSDecoder.h"
#import <pdns-sdk-ios/DNSResolver.h>
#import "ZNetworkQualityDetector.h"
#import "ZFallbackEndpointStore.h"

@interface ZNetworkDetectionHandle ()
...
/// oss直连获取到的url列表
@property (nonatomic, strong) NSMutableArray <ZUrlHostModel *> *ossUrlList;

/// 导航tcp节点结果
@property (nonatomic, strong) NSMutableArray <ZNetRacingItemModel *> *tcpRacingResultArr;
...
@implementation ZNetworkDetectionHandle
...
- (NSMutableArray<ZUrlHostModel *> *)ossUrlList {
    if (!_ossUrlList) {
...
- (NSMutableArray<ZNetRacingItemModel *> *)tcpRacingResultArr {
...
- (instancetype)initWithCurrentSsoNumber:(NSString *)ssoNumber {
    ...
    if (self) {
        ...
        NworkDetectionMessageModel *networkConnectStatusMessageModel = [self createMessageModelWithTitle:MultilingualTranslation(@"网络连接情况") sectionType:ZNetworkDetectionNetworkConnectSectionType];
        NworkDetectionMessageModel *domainNameResolutionMessageModel = [self createMessageModelWithTitle:MultilingualTranslation(@"域名解析检测") sectionType:ZNetworkDetectionDomainNameResolutionSectionType];
        NworkDetectionMessageModel *navConnectDetectionMessageModel = [self createMessageModelWithTitle:MultilingualTranslation(@"导航连接检测") sectionType:ZNetworkDetectionNavConnectDetectionSectionType];
        NworkDetectionMessageModel *serverConnectDetectionMessageModel = [self createMessageModelWithTitle:MultilingualTranslation(@"服务器连接检测") sectionType:ZNetworkDetectionServerConnectDetectionSectionType];
...
/// 创建带标题的消息模型
- (NworkDetectionMessageModel *)createMessageModelWithTitle:(NSString *)title
                                                   sectionType:(ZNetworkDetectionSectionType)sectionType {
    NworkDetectionMessageModel *model = [NworkDetectionMessageModel new];
...
- (NworkDetectionMessageModel *)getSectionModelWithIndex:(NSInteger)section {
...
- (ZNetworkDetectionSubResultModel *)getCellModelWithIndexPath:(NSIndexPath *)indexPath {
    NSInteger section = indexPath.section;
    // 第一行展示主要信息，所以子任务结果展示从第2个开始
    NSInteger row = indexPath.row - 1;
    NworkDetectionMessageModel *sectionModel = [self getSectionModelWithIndex:section];
    if (!sectionModel) {
        return nil;
    }
...
- (NSInteger)getAllUnPassSubResultCount {
    NSInteger count = 0;
    for (NworkDetectionMessageModel *messageModel in self.tableDataSource) {
...
- (void)processData {
    ...
    RACDisposable *d5 = [self.navDetectionEventSubject subscribeNext:^(id  _Nullable x) {
        ...
        if ([x isKindOfClass:[NSDictionary class]]) {
            if (self.tableDataSource.count < 2) {
             ...
             NworkDetectionMessageModel *navConnectDetectionMessageModel = self.tableDataSource[2];
            ...
            if ([event isEqualToString:@"navDetectionCompleted"]) {
                ...
                // 导航链接解析结果
                ZNetworkDetectionSubResultModel *navDetectionResultModel = [ZNetworkDetectionSubResultModel new];
                ZUrlHostModel *urlHostModel = [eventDic objectForKey:@"urlModel"];
                ...
                if (([[ZFallbackEndpointStore shared].domesticUrls containsObject:urlStr] ||
                     [[ZFallbackEndpointStore shared].overseasUrls containsObject:urlStr]) && [urlHostModel.type isEqualToString:@"5"]) {
                    if (isSuccess) {
                        resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"导航%@请求成功，耗时%.0f毫秒"), encryptUrlStr, duration];
                    }else {
                        if ([code isEqualToString:@"-999999"]) {
                            resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"导航%@请求失败，请求超时"), encryptUrlStr];
                            CIMLog(@"[网络链路检测] 域名解析返回事件:%@", [NSString stringWithFormat:MultilingualTranslation(@"导航%@请求失败，请求超时"), encryptUrlStr]);
                        }else if ([code isEqualToString:@"-888888"]) {
                            resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"导航%@请求失败，无法连接至服务器"), encryptUrlStr];
                            CIMLog(@"[网络链路检测] 域名解析返回事件:%@", [NSString stringWithFormat:MultilingualTranslation(@"导航%@请求失败，请求超时"), encryptUrlStr]);
                        }else {
                            resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"导航%@请求失败，返回错误信息%@"), encryptUrlStr, code];
                            CIMLog(@"[网络链路检测] 域名解析返回事件:%@", [NSString stringWithFormat:MultilingualTranslation(@"导航%@请求失败，返回错误信息%@"), encryptUrlStr, code]);
                        }
                    }
                }else {
                    if (isSuccess) {
                        resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"%@请求成功，耗时%.0f毫秒"), encryptUrlStr, duration];
                    }else {
                        if ([code isEqualToString:@"-999999"]) {
                            resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"%@请求失败，请求超时"), encryptUrlStr];
                            CIMLog(@"[网络链路检测] 域名解析返回事件:%@", [NSString stringWithFormat:MultilingualTranslation(@"%@请求失败，请求超时"), encryptUrlStr]);
                        }else if ([code isEqualToString:@"-888888"]) {
                            resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"%@请求失败，请求超时"), encryptUrlStr];
                            CIMLog(@"[网络链路检测] 域名解析返回事件:%@", [NSString stringWithFormat:MultilingualTranslation(@"%@请求失败，无法连接至服务器"), encryptUrlStr]);
                        }else {
                            resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"%@请求失败，返回错误信息%@"), encryptUrlStr, code];
                            CIMLog(@"[网络链路检测] 域名解析返回事件:%@", [NSString stringWithFormat:MultilingualTranslation(@"%@请求失败，返回错误信息%@"), encryptUrlStr, code]);
                        }
                    }
                }
                ...
                navDetectionResultModel.resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"导航请求失败，无可用地址，耗时%.0f毫秒"), duration];
    ...
    RACDisposable *d6 = [self.serverDetectionEventSubject subscribeNext:^(id  _Nullable x) {
        ...
        if ([x isKindOfClass:[NSDictionary class]]) {
            ...
            NworkDetectionMessageModel *serverConnectDetectionMessageModel = self.tableDataSource[index];
            ...
            if ([event isEqualToString:@"serverDetectionCompleted"]) {
                ...
                // TCP服务器链接事件
                ZNetworkDetectionSubResultModel *serverDetectionResultModel = [ZNetworkDetectionSubResultModel new];
                BOOL isSuccess = [[eventDic objectForKey:@"isSuccess"] boolValue];
                double duration = [[eventDic objectForKey:@"duration"] doubleValue];
                ...
                NSString *resultTitleStr = @"";
                if (isSuccess) {
                    resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"TCP节点%@请求成功，耗时%.0f毫秒"), encryptHostStr, duration];
                    CIMLog(@"[网络链路检测] 收到服务器检测事件，%@", [NSString stringWithFormat:MultilingualTranslation(@"TCP节点%@请求成功，耗时%.0f毫秒"), [NSString stringWithFormat:@"%@:%@", host, port], duration]);
                }else {
                    if (status == LingIMSDKManagerProbeECDHExChangeKeyFail) {
                        // 秘钥交换失败
                        resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"TCP节点%@请求失败，密钥交换失败"), encryptHostStr];
                        CIMLog(@"[网络链路检测] 收到服务器检测事件，%@", [NSString stringWithFormat:MultilingualTranslation(@"TCP节点%@请求失败，密钥交换失败"), [NSString stringWithFormat:@"%@:%@", host, port]]);
                    }else {
                        // 无法连接服务器
                        resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"TCP节点%@请求失败，无法连接至服务器"), encryptHostStr];
                        CIMLog(@"[网络链路检测] 收到服务器检测事件，%@", [NSString stringWithFormat:MultilingualTranslation(@"TCP节点%@请求失败，无法连接至服务器"), [NSString stringWithFormat:@"%@:%@", host, port]]);
                    }
                }
                ...
                serverDetectionResultModel.resultTitleStr = [NSString stringWithFormat:MultilingualTranslation(@"TCP节点请求失败，无法连接至服务器"), duration];
...
- (void)checkNetworkPermission {
    ...
    // 检测网络权限
    NworkDetectionMessageModel *networkConnectStatusMessageModel = self.tableDataSource[0];
    // 将网络权限置为检测中
    ...
    // 设备已联网、设备未联网
    ZNetworkDetectionSubResultModel *networkConnectResultModel = [ZNetworkDetectionSubResultModel new];
    // 设备网络状态正常、异常
    ZNetworkDetectionSubResultModel *networkDeviceNetStatusResultModel = [ZNetworkDetectionSubResultModel new];
    ...
    if (isConnect) {
        ...
        networkConnectResultModel.resultTitleStr = MultilingualTranslation(@"设备已联网");
        
        networkDeviceNetStatusResultModel.isPass = YES;
        networkDeviceNetStatusResultModel.resultTitleStr = MultilingualTranslation(@"网络连接状态正常");
    }else {
        CIMLog(@"[网络链路检测] 当前不能联网，网络权限检测未通过");
        networkConnectResultModel.isPass = NO;
        networkConnectResultModel.resultTitleStr = MultilingualTranslation(@"设备未联网");
        
        networkDeviceNetStatusResultModel.isPass = NO;
        networkDeviceNetStatusResultModel.resultTitleStr = MultilingualTranslation(@"网络连接状态异常");
    }
    @weakify(self)
    [self scheduleTaskAfter:1.0 block:^{
        @strongify(self)
...
- (void)checkDomainNameResolution {
    ...
    // 域名解析
    NworkDetectionMessageModel *domainNameResolutionMessageModel = self.tableDataSource[1];
    // 将网络权限置为检测中
    ...
    // 子任务
    // 主域名解析结果
    ZNetworkDetectionSubResultModel *mainDomainNameResultModel = [ZNetworkDetectionSubResultModel new];
    // 副域名解析结果
    ZNetworkDetectionSubResultModel *spareDomainNameResultModel = [ZNetworkDetectionSubResultModel new];
    ...
    [self domainNameResolutionWithMainDomainComplete:^(BOOL isSuccess) {
        ...
        mainDomainNameResultModel.resultTitleStr = isSuccess ? MultilingualTranslation(@"主域名DNS解析正常") : MultilingualTranslation(@"主域名DNS解析失败");
        ...
        spareDomainNameResultModel.resultTitleStr = isSuccess ? MultilingualTranslation(@"备域名DNS解析正常") : MultilingualTranslation(@"备域名DNS解析失败");
        
        CIMLog(@"[网络链路检测] %@", isSuccess ? @"副域名DNS解析正常" : @"副域名DNS解析失败");
    } FinishComplete:^(NSArray<ZUrlHostModel *> *ossUrlList) {
        @strongify(self)
        [self scheduleTaskAfter:1.0 block:^{
            @strongify(self)
            // 主副域名全部结果
            ...
            NSMutableArray<ZUrlHostModel *> *uniqueOssUrlList = [NSMutableArray array];
            ...
            for (ZUrlHostModel *urlModel in ossUrlList) {
...
- (void)domainNameResolutionWithMainDomainComplete:(void(^)(BOOL isSuccess))mainDomainComplete
                               SpareDomainComplete:(void(^)(BOOL isSuccess))spareDomainComplete
                                    FinishComplete:(void(^)(NSArray<ZUrlHostModel *> *ossUrlList))complete {
    // 1. 配置 DNS 解析器
    [self configureDNSResolver];
    
    // 2. 准备并发任务
    NSMutableArray<ZUrlHostModel *> *ossUrlList = [NSMutableArray new];
    ...
    [self resolveDomain:DirectNormarDomain
          withMainTypes:@[@"1", @"2"]
             completion:^(NSArray<ZUrlHostModel *> *mainUrlList) {
    ...
    [self resolveDomain:DirectSpareDomain
          withMainTypes:@[@"3", @"4"]
             completion:^(NSArray<ZUrlHostModel *> *spareUrlList) {
    ...
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        // 添加内置的两个兜底地址
        // 国内兜底地址
        for (NSString *domesticUrlStr in [ZFallbackEndpointStore shared].domesticUrls) {
            if([domesticUrlStr hasSuffix:@"8088"]) {
                continue;
            }
            ZUrlHostModel *fallbackDomesticUrlmodel = [self getUrlHostModelWithUrl:domesticUrlStr type:@"5"];
            [ossUrlList addObject:fallbackDomesticUrlmodel];
        }
        // 国外兜底地址
        for (NSString *overseasUrlStr in [ZFallbackEndpointStore shared].overseasUrls) {
            if([overseasUrlStr hasSuffix:@"8088"]) {
                continue;
            }
            ZUrlHostModel *fallbackOverseasUrlmodel = [self getUrlHostModelWithUrl:overseasUrlStr type:@"5"];
...
- (void)resolveDomain:(NSString *)domain
        withMainTypes:(NSArray<NSString *> *)types
           completion:(void(^)(NSArray<ZUrlHostModel *> *urlList))completion {
    WeakSelf
    ...
    [[DNSResolver share] getIpv6DataWithDomain:domain complete:^(NSArray<NSString *> *dataArray) {
        ...
        NSArray<ZUrlHostModel *> *urlList = @[];
        if (analysisDomain) {
            // 解析成功，处理 URL 列表
            NSMutableArray<ZUrlHostModel *> *tempList = [NSMutableArray array];
...
- (void)processOssUrls:(NSString *)analysisDomain
            ...
            ossUrlList:(NSMutableArray<ZUrlHostModel *> *)ossUrlList {
    WeakSelf
    ...
    [ossBucketArr enumerateObjectsUsingBlock:^(NSString *urlString, NSUInteger idx, BOOL *stop) {
        ...
        // 创建并添加 URL 模型
        ZUrlHostModel *model = [weakSelf getUrlHostModelWithUrl:urlString type:type];
        [ossUrlList addObject:model];
    }];
}

- (ZUrlHostModel *)getUrlHostModelWithUrl:(NSString *)urlString type:(NSString *)type {
    ZUrlHostModel *model = [[ZUrlHostModel alloc] init];
...
- (void)navConnectDetectionWithOssList:(NSArray<ZUrlHostModel *> *)ossList {
    if (self.tableDataSource.count < 2) {
        return;
    }
...
/// @param group 并发组
- (void)executeNavDetectionWithUrlModel:(ZUrlHostModel *)urlModel
    ...
    [manager executeWithSuccess:^(IMServerListResponseBody * _Nonnull serverResponse) {
        ...
        for (ZNetRacingItemModel *existingItem in self.tcpRacingResultArr) {
        ...
        NSMutableArray <ZNetRacingItemModel *>* tcpArray = [NSMutableArray new];
        [tcpEndPointArray enumerateObjectsUsingBlock:^(IMServerEndpoint * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            ZNetRacingItemModel *model = [ZNetRacingItemModel new];
            model.ip = obj.ip;
            model.sort = obj.port;
...
- (void)serverConnectDetection {
    ...
    if (UserManager.isLogined) {
        ...
        NSArray *tcpServers = [[ZNetworkQualityDetector sharedDetector].tcpServers copy];
        
        // 用于去重
        NSMutableSet<NSString *> *existingTcpKeys = [NSMutableSet set];
        
        // 转换为 ZNetRacingItemModel
        NSMutableArray <ZNetRacingItemModel *>* tempTcpServer = [NSMutableArray new];
    ...
    @weakify(self)
    for (ZNetRacingItemModel *tcpItem in tcpRacingResultArr) {
        dispatch_group_enter(group);
...
- (void)executeServerConnectivityCheckWithHost:(NSString *)host
                                          port:(NSString *)port
                                       tcpItem:(ZNetRacingItemModel *)tcpItem
                                         group:(dispatch_group_t)group {
    // 记录开始时间（每个请求独立的时间戳）
    NSDate *startTime = [NSDate date];
...
- (void)checkTcpConnectivityWithHost:(NSString *)host
    ...
    [[LingIMSDKManager sharedTool] probeECDHConnectivityWithHost:host 
                                                            port:portValue 
                                                         timeout:-1 
                                                            type:0 
