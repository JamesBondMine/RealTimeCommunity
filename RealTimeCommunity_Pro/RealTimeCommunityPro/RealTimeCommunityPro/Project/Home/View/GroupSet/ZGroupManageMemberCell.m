...
- (void)setupUI {
    ...
    [_cancelBtn setTitle:MultilingualTranslation(@"解除禁言") forState:UIControlStateNormal];
...
- (void)cellConfigWithmodel:(HomeGroupNotalkMemberModel *)model{
    ...
    NSString * showStr;
    if ([timeStr isEqualToString:MultilingualTranslation(@"禁言")]) {
        showStr = MultilingualTranslation(@"永久禁言");
    }else{
        showStr = [NSString stringWithFormat:MultilingualTranslation(@"约%@后解除禁言"),timeStr];
    }
