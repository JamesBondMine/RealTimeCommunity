...
//  MUIKit
//
//  Created by cusPro on 2023/12/15.
//
...
- (instancetype)initWithTaskId:(NSString *)taskId
                ...
                originFilePath:(NSString *)originFilePath
                 ...
                 beSendMessage:(LingIMChatMessageModel *)beSendMessage
    ...
    if (self.uploadModelType == FileUploadModelTypeThirdOss) {
        ...
        __block NSURLSessionDataTask *publicKeyTask = [manager dataTaskWithHTTPMethod:@"POST" URLString:urlStr parameters:nil headers:nil uploadProgress:^(NSProgress * _Nonnull uploadProgress) {
            ...
            if (resp.isHttpSuccess) {
                ...
                [HUD showMessage:MultilingualTranslation(@"获取私钥失败")];
            ...
            //            [HUD showMessage:MultilingualTranslation(@"网络异常~")];
...
- (void)startUploadSingleFileWithPublicKey:(NSString *)publicKey {
    if(self.session && self.status == FileUploadTaskStatus_Paused){
        ...
        NSString *bucketName = [ZFileUploadTools getMinioBucketNameFormUploadType:self.uploadType];
        NSString *mimeType = [ZFileUploadTools getMimeTypeFormUploadType:self.uploadType withFilePath:self.filePath];
        ...
        self.session =  [manager POST:urlStr parameters:nil headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
            ...
            if (resp.isHttpSuccess) {
                id descryptData = [resp responseDataDescryptWithDataString:resp.data url:urlStr];
                ZFileUploadModel *fileUploadModel = [ZFileUploadModel mj_objectWithKeyValues:descryptData];
...
- (void)uploadFragmentGetUploadIdWithPublicKey:(NSString *)publicKey {
    ...
    __block NSURLSessionDataTask *uploadIdTask = [manager dataTaskWithHTTPMethod:@"POST" URLString:urlStr parameters:nil headers:nil uploadProgress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if (resp.isHttpSuccess) {
            ...
            [HUD showMessage:MultilingualTranslation(@"获取UploadId失败")];
            [uploadTask setStatus:FileUploadTaskStatus_Failed error:nil];
            if (uploadTask) {
                dispatch_semaphore_signal(uploadTask.semaphore);
            }
...
- (void)startUploadAliyunFile {
    ...
    //文件存储路径+文件名
    NSString *objectKey = [NSString stringWithFormat:@"%@%@", [ZFileUploadTools getAliyunObjectKeyPathFormUploadType:self.uploadType], self.fileName];
    //上传单个文件
    //每次都要重新创建client，防止token失效后，client里存的还是已失效的token
    id<OSSCredentialProvider> credentialProvider = [[OSSStsTokenCredentialProvider alloc] initWithAccessKeyId:[ZFileUploadManager sharedInstance].stsInfo.aliyunAccessKeyId secretKeyId:[ZFileUploadManager sharedInstance].stsInfo.aliyunSecretKeyId securityToken:[ZFileUploadManager sharedInstance].stsInfo.aliyunSecurityToken];
    if (self.client == nil) {
...
- (void)startUploadAWSFile {
    ...
    endPointUrl = [endPointUrl stringByReplacingOccurrencesOfString:@"https://" withString:@""];
    //objectKey
    NSString *objectKey = [NSString stringWithFormat:@"%@%@", [ZFileUploadTools getAWSS3ObjectKeyPathFormUploadType:self.uploadType], self.fileName];
    
    //临时性的accessKey、secretKey，需要增加一个参数sessionToken
    AWSBasicSessionCredentialsProvider *credentialsProvider = [[AWSBasicSessionCredentialsProvider alloc] initWithAccessKey:[ZFileUploadManager sharedInstance].stsInfo.awss3AccessKey secretKey:[ZFileUploadManager sharedInstance].stsInfo.awss3SecretKey sessionToken:[ZFileUploadManager sharedInstance].stsInfo.awss3sessionToken];
    //endPoint
...
- (void)startUploadTencentFile {
    ...
    self.status = FileUploadTaskStatus_Upload;
    __block NSString * taskId = self.taskId;
    if ([ZFileUploadManager sharedInstance].stsInfo.tencentsecretId == nil || [ZFileUploadManager sharedInstance].stsInfo.tencentsecretKey == nil || [ZFileUploadManager sharedInstance].stsInfo.tencentToken == nil) {
        ZFileUploadTask * uploadTask = [[ZFileUploadManager sharedInstance] getTaskWithId:taskId];
        if (uploadTask.status != FileUploadTaskStatus_Paused) {
            [uploadTask setStatus:FileUploadTaskStatus_Failed error:nil];
        }
    ...
    endPointUrl = [endPointUrl stringByReplacingOccurrencesOfString:@"https://" withString:@""];
    //objectKey
    NSString *objectKey = [NSString stringWithFormat:@"%@%@", [ZFileUploadTools getTencentCosObjectKeyPathFormUploadType:self.uploadType], self.fileName];
    ...
    credential.secretID = [ZFileUploadManager sharedInstance].stsInfo.tencentsecretId;
    credential.secretKey = [ZFileUploadManager sharedInstance].stsInfo.tencentsecretKey;
    credential.token = [ZFileUploadManager sharedInstance].stsInfo.tencentToken;
    // 设置临时密钥
...
- (void)startUploadHuaWeiOBSFile {
    ...
    self.status = FileUploadTaskStatus_Upload;
    __block NSString * taskId = self.taskId;
    if ([ZFileUploadManager sharedInstance].stsInfo.obsAccessKey == nil || [ZFileUploadManager sharedInstance].stsInfo.obsSecretKey == nil) {
        ZFileUploadTask * uploadTask = [[ZFileUploadManager sharedInstance] getTaskWithId:taskId];
        if (uploadTask.status != FileUploadTaskStatus_Paused) {
            [uploadTask setStatus:FileUploadTaskStatus_Failed error:nil];
        }
    ...
    //文件存储路径+文件名
    NSString *objectKey = [NSString stringWithFormat:@"%@%@", [ZFileUploadTools getHuaWeiOBSObjectKeyPathFormUploadType:self.uploadType], self.fileName];
    
    // 初始化身份验证
    OBSStaticCredentialProvider *credentialProvider = [[OBSStaticCredentialProvider alloc] initWithAccessKey:[ZFileUploadManager sharedInstance].stsInfo.obsAccessKey secretKey:[ZFileUploadManager sharedInstance].stsInfo.obsSecretKey];
    credentialProvider.securityToken = [ZFileUploadManager sharedInstance].stsInfo.obsToken;
    // 初始化服务配置
...
- (void)huaweiOBSMultipartUploadWithUploadId:(NSString *)uploadId client:(OBSClient *)client objectkey:(NSString *)objectkey {
    ...
    __block NSInteger httpFinishCount = 0;
    __block NSMutableArray *eTagList = [NSMutableArray array];
    ZFileUploadTask * uploadTask = [[ZFileUploadManager sharedInstance] getTaskWithId:self.taskId];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        for (int i = 1; i <= self.truncks; i++) {
            //DLog(@"======== 分片上传 总共 %ld 片", (long)self.truncks);
            //DLog(@"======== 分片上传 第二步 第 %d 片 开始上传", i);
...
- (BOOL)isEqual:(id)object{
    ...
    return [self.taskId isEqualToString:((ZFileUploadTask *)object).taskId];
}
