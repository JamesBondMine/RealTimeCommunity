...
@interface ZWindowFloatView () <UIGestureRecognizerDelegate>
...
@implementation ZWindowFloatView
...
- (void)clickDragViewTap {
    if (_delegate && [_delegate respondsToSelector:@selector(clickFloatView:)]) {
        [_delegate clickFloatView:self];
    }
}
//拖拽手势
- (void)dragViewPan:(UIPanGestureRecognizer *)pan {
    ...
    switch (pan.state) {
        case UIGestureRecognizerStateBegan://开始拖动
            {
                if (_delegate && [_delegate respondsToSelector:@selector(beganDragFloatView:)]) {
                    [_delegate beganDragFloatView:self];
                }
        ...
        {
            if (_delegate && [_delegate respondsToSelector:@selector(duringDragFloatView:)]) {
                [_delegate duringDragFloatView:self];
            }
        ...
        {
            [self keepBounds];
            if (_delegate && [_delegate respondsToSelector:@selector(endDragFloatView:)]) {
                [_delegate endDragFloatView:self];
            }
        }
            break;
