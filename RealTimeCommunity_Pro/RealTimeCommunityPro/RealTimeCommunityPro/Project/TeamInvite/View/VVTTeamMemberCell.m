...
@implementation VVTTeamMemberCell
...
- (void)setupUI {
    ...
    liveImage.image = ImgNamed(@"retm_team_live_time_icon");
    ...
    dateImage.image = ImgNamed(@"retm_team_join_time_icon");
...
#pragma mark - 界面赋值
- (void)setMemberModel:(IVTTeamMemberModel *)memberModel {
    _memberModel = memberModel;
    self.lblName.text = memberModel.nickname;
    self.lblDate.text = [NSString stringWithFormat:@"%@: %@",MultilingualTranslation(@"加入时间"),memberModel.joinTime];
    ...
    if (memberModel.latestOfflineTime > 0) {
        ...
        latestOffLineTimeString = MultilingualTranslation(@"在线");
    } else {
        latestOffLineTimeString = @"-";
    }
    self.liveLabel.text = [NSString stringWithFormat:@"%@: %@",MultilingualTranslation(@"最近在线时间"),latestOffLineTimeString];
}
...
- (UILabel *)liveLabel {
    if (_liveLabel == nil) {
        ...
        _liveLabel.tkThemetextColors = @[COLOR_33,COLOR_33_DARK];
...
- (UILabel *)lblDate {
    if (_lblDate == nil) {
        ...
        _lblDate.tkThemetextColors = @[COLOR_33,COLOR_33_DARK];
