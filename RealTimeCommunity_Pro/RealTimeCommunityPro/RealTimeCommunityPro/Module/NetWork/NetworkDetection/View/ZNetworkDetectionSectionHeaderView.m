//
//  ZNetworkDetectionSectionHeaderView.m
...
@interface ZNetworkDetectionSectionHeaderView ()
...
@implementation ZNetworkDetectionSectionHeaderView
...
- (UILabel *)waitDetectLabel {
    if (!_waitDetectLabel) {
        ...
        _waitDetectLabel.text = MultilingualTranslation(@"待检测");
    }
...
- (void)setModel:(NworkDetectionMessageModel *)model {
    if (!model) {
    ...
    _statusDisposable = [[_model.changeStatusSubject takeUntil:self.rac_willDeallocSignal] subscribeNext:^(id  _Nullable x) {
        @strongify(self)
        // 更新UI
        if ([self.model.sectionTitle isEqualToString:MultilingualTranslation(@"网络连接情况")]) {
...
- (void)setUpUI {
    ...
    [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.trailing.lessThanOrEqualTo(self.waitDetectLabel.mas_leading).offset(-16);
    }];
...
//        self.model.isFold = !self.model.isFold;
        ...
        // 立即修改箭头状态
        BOOL foldStatus = !self.model.isFold;
        if (foldStatus) {
            // 折叠
            self.arrowImgView.image = [UIImage imageNamed:@"c_arrow_right_gray"];
            [self.arrowImgView mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        // 调用 block 回调，在回调中会修改数据源
        if (self.changeSectionFolderStatusBlock) {
            self.changeSectionFolderStatusBlock();
        }
    }];
}
