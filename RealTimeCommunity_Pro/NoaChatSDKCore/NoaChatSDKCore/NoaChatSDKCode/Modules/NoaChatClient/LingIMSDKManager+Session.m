//
//  LingIMSDKManager+Session.m
//  NoaChatSDKCore
//
//  Created by cusPro on 2022/10/27.
//

#import "LingIMSDKManager+Session.h"
// 统一的签到助手会话ID（避免重复会话）
static NSString * const kSignInSessionID = @"SIGNIN_ASSISTANT";
#import "LingIMSDKManager+User.h"//用户相关
#import "LingIMSDKManager+Friend.h"//好友相关
#import "LingIMSDKManager+Group.h"//群组相关
#import "LingIMSDKManager+ChatMessage.h"//聊天相关
#import "LingIMSDKManager+MessageRemind.h"//消息提醒相关



@implementation LingIMSDKManager (Session)

#pragma mark - 新增/更新会话列表
- (BOOL)toolInsertOrUpdateSessionWith:(LingIMChatMessageModel *)message isRemind:(BOOL)isRemind {
    ...
    if (sessionID.length > 0) {
        ...
        if (sessionModel) {
            ...
            //1.创建某一会话表
            [DBTOOL createTableWithName:sessionTableName model:LingIMChatMessageModel.class];
...
- (BOOL)toolInsertSessionForCloseGroupRemindWith:(LingIMChatMessageModel *)message {
    //表名称 CIMDB_myUserID_toUserID_Table
...
- (BOOL)toolDeleteAllMySession {
    return [DBTOOL deleteAllObjectWithName:CIMDBSessionTableName];
}
...
- (void)privateGetFriendInfoWithReceiveMessage:(LingIMChatMessageModel *)message userUid:(NSString *)userUid {
    if (userUid.length > 0) {
...
- (void)privateGetGroupInfoWithReceiveMessage:(LingIMChatMessageModel *)message groupID:(NSString *)groupID {
    if (groupID.length > 0) {
        NSMutableDictionary *params = [NSMutableDictionary dictionary];
