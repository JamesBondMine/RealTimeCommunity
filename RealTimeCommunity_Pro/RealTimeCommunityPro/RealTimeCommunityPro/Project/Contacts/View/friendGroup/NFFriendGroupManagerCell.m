...
- (void)setupUI {
    ...
    [_btnDelete setImage:ImgNamed(@"recon_c_fg_delete") forState:UIControlStateNormal];
    ...
    _tfTitle.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    _ivSelect = [[UIImageView alloc] initWithImage:ImgNamed(@"remi_icon_selected_blue")];
...
- (void)configCellWith:(LingIMFriendGroupModel *)friendGroupModel canEdit:(BOOL)canEdit {
    ...
    _tfTitle.text = ![NSString isNil:friendGroupModel.ugName] ? friendGroupModel.ugName : MultilingualTranslation(@"默认分组");
    ...
    if (canEdit) {
        ...
        _ivSelect.image = ImgNamed(@"recon_c_fg_move");
        _ivSelect.hidden = NO;
        ...
        //根据选中情况展示
        _ivSelect.image = ImgNamed(@"remi_icon_selected_blue");
...
- (void)updateFriendGroupName {
    NSString *modelFriendGroupName = ![NSString isNil:_friendGroupModel.ugName] ? _friendGroupModel.ugName : MultilingualTranslation(@"默认分组");
