...
@interface ZMessageModel() <ZFileUploadTaskDelegate>

@end

@implementation ZMessageModel

- (instancetype)initWithMessageModel:(LingIMChatMessageModel *)message {
    self = [super init];
    if (self) {
        _message = message;
...
- (instancetype)initWithMessageModel:(LingIMChatMessageModel *)message isSelf:(BOOL)isSelf {
    self = [super init];
    if (self) {
        _message = message;
        _isSelf = isSelf;
...
- (void)transformTimestampToDataTime {
    if (_message) {
        ...
        _dataTime = [ZMessageTimeTool getTimeStringAutoShort2:msgTime mustIncludeTime:YES];
    }
}
    ...
    if (_message && _isSelf) {
        if (_message.messageSendType == CIMChatMessageSendTypeSending) {
            ...
            if (_message.messageType == CIMChatMessageType_ImageMessage) {
                ...
                //缩略图
                ZFileUploadTask *thumbImgTask = [[ZFileUploadManager sharedInstance] getTaskWithId:[NSString stringWithFormat:@"%@_thumb", _message.msgID]];
                if (thumbImgTask) {
                    ...
                    ZFileUploadTask *thumbImgTask = [[ZFileUploadTask alloc] initWithTaskId:[NSString stringWithFormat:@"%@_thumb", _message.msgID] filePath:localThumbImgPath originFilePath:imagePath fileName:_message.localthumbImgName fileType:@"" isEncrypt:YES dataLength:thumbImgData.length uploadType:ZHttpUploadTypeImageThumbnail beSendMessage:_message delegate:self];
                    thumbImgTask.messageTaskType = FileUploadMessageTaskTypeThumbImage;
                    [taskArray addObject:thumbImgTask];
                }
            
                //图片
                ZFileUploadTask *imgTask = [[ZFileUploadManager sharedInstance] getTaskWithId:_message.msgID];
                if (imgTask) {
                    imgTask.beSendMessage = _message;
                    [imgTask addDelegate:self];
                } else {
                    NSString *imagePath = [NSString getPathWithImageName:_message.localImgName CustomPath:customPath];
                    imgTask = [[ZFileUploadTask alloc] initWithTaskId:_message.msgID filePath:imagePath originFilePath:@"" fileName:_message.localImgName fileType:@"" isEncrypt:YES dataLength:_message.imgSize uploadType:ZHttpUploadTypeImage beSendMessage:_message delegate:self];
                ...
                NSString *customPath = [NSString stringWithFormat:@"%@-%@", UserManager.userInfo.userUID, _message.toID];
                //视频封面上传
                ZFileUploadTask *coverTask = [[ZFileUploadManager sharedInstance] getTaskWithId:[NSString stringWithFormat:@"%@_cover", _message.msgID]];
                if (coverTask) {
                    ...
                    coverTask = [[ZFileUploadTask alloc] initWithTaskId:[NSString stringWithFormat:@"%@_cover", _message.msgID] filePath:coverPath originFilePath:@"" fileName:_message.localVideoCover fileType:@"" isEncrypt:YES dataLength:_message.videoCoverSize uploadType:ZHttpUploadTypeImage beSendMessage:_message delegate:self];
                    coverTask.messageTaskType = FileUploadMessageTaskTypeCover;
                    [taskArray addObject:coverTask];
                }
                //视频文件上传
                ZFileUploadTask *videoTask = [[ZFileUploadManager sharedInstance] getTaskWithId:_message.msgID];
                if (videoTask) {
                    ...
                    videoTask = [[ZFileUploadTask alloc] initWithTaskId:_message.msgID filePath:videoPath originFilePath:@"" fileName:_message.localVideoName fileType:@"" isEncrypt:YES dataLength:_message.videoSize uploadType:ZHttpUploadTypeVideo beSendMessage:_message delegate:self];
                    videoTask.messageTaskType = FileUploadMessageTaskTypeVideo;
                    [taskArray addObject:videoTask];
                }
            } else if (_message.messageType == CIMChatMessageType_VoiceMessage) {
                //音频
                ZFileUploadTask *voiceTask = [[ZFileUploadManager sharedInstance] getTaskWithId:_message.msgID];
                if (voiceTask) {
                    ...
                    voiceTask = [[ZFileUploadTask alloc] initWithTaskId:_message.msgID filePath:_message.localVoicePath originFilePath:@"" fileName:_message.localVoiceName fileType:@"" isEncrypt:NO dataLength:audioData.length uploadType:ZHttpUploadTypeVoice beSendMessage:_message delegate:self];
                ...
                ZFileUploadTask *fileTask = [[ZFileUploadManager sharedInstance] getTaskWithId:_message.msgID];
                if (fileTask) {
                    ...
                    fileTask = [[ZFileUploadTask alloc] initWithTaskId:_message.msgID filePath:filePath originFilePath:@"" fileName:_message.fileName fileType:_message.fileType isEncrypt:YES dataLength:_message.fileSize uploadType:ZHttpUploadTypeFile beSendMessage:_message delegate:self];
                ...
                ZFileUploadTask *geoImgTask = [[ZFileUploadManager sharedInstance] getTaskWithId:_message.msgID];
                if (geoImgTask) {
                    ...
                    geoImgTask = [[ZFileUploadTask alloc] initWithTaskId:_message.msgID filePath:geoImgPath originFilePath:@"" fileName:_message.localImgName fileType:@"" isEncrypt:YES dataLength:geoImgData.length uploadType:ZHttpUploadTypeImage beSendMessage:_message delegate:self];
            ...
            if (taskArray.count > 0) {
                ...
                for (ZFileUploadTask * task in taskArray) {
...
- (void)calculateTextMessage {
    ...
    if (self.isSelf) {
        self.attStr = [[EMChatInputEmojiManager sharedManager] yy_emojiAttributedString:_message.textContent];
        [self.attStr configAttStrLightColor:COLORWHITE darkColor:COLORWHITE range:NSMakeRange(0, self.attStr.length)];
    } else {
        self.attStr = [[EMChatInputEmojiManager sharedManager] yy_emojiAttributedString:![NSString isNil:_message.translateContent] ? _message.translateContent : _message.textContent];
        [self.attStr configAttStrLightColor:COLOR_33 darkColor:COLORWHITE range:NSMakeRange(0, self.attStr.length)];
    }
    ...
    if (!shouldShowTranslate_text) {
        ...
        if (self.isSelf) {
            if (![NSString isNil:_message.translateContent]) {
                //此处需要将表情内容"[表情]"替换为表情图片的富文本
                self.translateAttStr = [[EMChatInputEmojiManager sharedManager] yy_emojiAttributedString:_message.translateContent];
                ...
                if (self.isSelf) {
                    ...
                    [self.translateAttStr configAttStrLightColor:COLOR_33 darkColor:COLORWHITE range:NSMakeRange(0, self.translateAttStr.length)];
                }
            ...
            if (![NSString isNil:_message.againTranslateContent]) {
                //此处需要将表情内容"[表情]"替换为表情图片的富文本
                self.translateAttStr = [[EMChatInputEmojiManager sharedManager] yy_emojiAttributedString:![NSString isNil:_message.againTranslateContent] ? _message.againTranslateContent : _message.translateContent];
                [self.translateAttStr addAttributes:dict range:NSMakeRange(0, self.translateAttStr.length)];
                if (self.isSelf) {
                    [self.translateAttStr configAttStrLightColor:COLORWHITE darkColor:COLORWHITE range:NSMakeRange(0, self.translateAttStr.length)];
                } else {
...
- (void)calculateAtUserMessage {
    ...
    if (_isSelf) {
        _message.showContent = [ZMessageTools atContenTranslateToShowContent:![NSString isNil:_message.atContent] ? _message.atContent : @"" atUsersDictList:_message.atUsersInfoList withMessage:_message isGetShowName:YES];
    } else {
        _message.showContent = [ZMessageTools atContenTranslateToShowContent:![NSString isNil:_message.atTranslateContent] ? _message.atTranslateContent : _message.atContent atUsersDictList:_message.atUsersInfoList withMessage:_message isGetShowName:YES];
    }
    
    if (_isSelf) {
        _message.showTranslateContent = [ZMessageTools atContenTranslateToShowContent:![NSString isNil:_message.atTranslateContent] ? _message.atTranslateContent : @"" atUsersDictList:_message.atUsersInfoList withMessage:_message isGetShowName:YES];
    } else {
        _message.showTranslateContent = [ZMessageTools atContenTranslateToShowContent:![NSString isNil:_message.againAtTranslateContent] ? _message.againAtTranslateContent : _message.atTranslateContent atUsersDictList:_message.atUsersInfoList withMessage:_message isGetShowName:YES];
    }
   
    //此处需要将表情内容"[表情]"替换为表情图片的富文本
    self.attStr = [[EMChatInputEmojiManager sharedManager] yy_emojiAttributedString:_message.showContent];
    [self.attStr addAttributes:dict range:NSMakeRange(0, self.attStr.length)];
    
    self.translateAttStr = [[EMChatInputEmojiManager sharedManager] yy_emojiAttributedString:_message.showTranslateContent];
    [self.translateAttStr addAttributes:dict range:NSMakeRange(0, self.translateAttStr.length)];
    
    //非自己发送的消息
    //原文
    if (!_isSelf) {
        [self.attStr configAttStrLightColor:COLOR_33 darkColor:COLOR_33_DARK range:NSMakeRange(0, self.attStr.length)];
        //改变被 @用户 字体为蓝色
        for (NSDictionary *atUserDic in _message.atUsersInfoList) {
            ...
            if ([atKey isEqualToString:UserManager.userInfo.userUID]) {
                // @我
                [self.attStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:self.attStr.string appointStr:MultilingualTranslation(@"@我")];
            } else if ([atKey isEqualToString:@"-1"]) {
                // @所有人
                [self.attStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:self.attStr.string appointStr:MultilingualTranslation(@"@所有人")];
            } else {
                // @其他用户 显示 @昵称
                ...
                [self.attStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:self.attStr.string appointStr:resultValue];
            }
        }
    } else {
        [self.attStr configAttStrLightColor:COLORWHITE darkColor:COLORWHITE range:NSMakeRange(0, self.attStr.length)];
    ...
    if (!shouldShowTranslate_at) {
        ...
        if (_isSelf) {
            ...
            if (![NSString isNil:_message.againAtTranslateContent]) {
                [self.translateAttStr configAttStrLightColor:COLOR_33 darkColor:COLOR_33_DARK range:NSMakeRange(0, self.translateAttStr.length)];
                //改变被 @用户 字体为蓝色
                for (NSDictionary *atUserDic in _message.atUsersInfoList) {
                    NSArray *atKeyArr = [atUserDic allKeys];
                    NSString *atKey = (NSString *)[atKeyArr firstObject];
                    ...
                    if ([atKey isEqualToString:UserManager.userInfo.userUID]) {
                        // @我
                        [self.translateAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:self.translateAttStr.string appointStr:MultilingualTranslation(@"@我")];
                    } else if ([atKey isEqualToString:@"-1"]) {
                        // @所有人
                        [self.translateAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:self.translateAttStr.string appointStr:MultilingualTranslation(@"@所有人")];
                    } else {
                        // @其他用户 显示 @昵称
                        NSString *resultValue;
                        if(_message.chatType == CIMChatType_GroupChat){
                        ...
                        [self.translateAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:self.translateAttStr.string appointStr:resultValue];
                    }
                }
                CGSize translateSize = [YYTextLayout layoutWithContainerSize:CGSizeMake(DScreenWidth - 140, CGFLOAT_MAX) text:self.translateAttStr].textBoundingSize;
                //CGSize translateSize = [self.translateAttStr boundingRectWithSize:CGSizeMake(DScreenWidth - 140, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin context:nil].size;
...
- (void)calculateLocationMessage{
    self.attStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[图片]")];
    CGFloat imgW;
    CGFloat imgH;
    //图片
...
- (void)calculateStickersMessage {
    self.attStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[表情]")];
    CGFloat imgW;
    CGFloat imgH;
    //图片
...
- (void)calculateVideoMessage {
    self.attStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[视频]")];
    CGFloat imgW;
    CGFloat imgH;
    //视频封面
...
- (void)calculateVoiceMessage {
    self.attStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[语音]")];
...
- (void)calculateFileMessage {
    self.attStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[文件]")];
...
- (void)calculateGroupNoticeMessage {
    ...
    if (![NSString isNil:_message.groupNoticeTranslateContent]) {
        ...
        groupNoticeStr = ![NSString isNil:_message.groupNoticeContent] ? _message.groupNoticeContent : MultilingualTranslation(@"群公告");
    }
    ...
    if (customEvent.type == 101) {
        ...
        if ([mediaCallModel.discard_reason isEqualToString:@"disconnect"]) {
            ...
            if ([mediaCallModel.from_id isEqualToString:UserManager.userInfo.userUID]) {
                //我是 邀请者
                textContentStr = [NSString stringWithFormat:MultilingualTranslation(@"%@ 通话中断"), [NSString getTimeLength:mediaCallModel.duration]];
            } else {
                //我是 被邀请者
                textContentStr = [NSString stringWithFormat:MultilingualTranslation(@"通话中断 %@"), [NSString getTimeLength:mediaCallModel.duration]];
            }
            ...
            if ([mediaCallModel.from_id isEqualToString:UserManager.userInfo.userUID]) {
                //我是 邀请者
                textContentStr = MultilingualTranslation(@"对方无应答");
            } else {
                //我是 被邀请者
                textContentStr = MultilingualTranslation(@"超时未应答");
            }
            ...
            if ([mediaCallModel.from_id isEqualToString:UserManager.userInfo.userUID]) {
                //我是 邀请者
                textContentStr = MultilingualTranslation(@"已取消");
            } else {
                //我是 被邀请者
                textContentStr = MultilingualTranslation(@"对方已取消");
            }
            ...
            if ([mediaCallModel.from_id isEqualToString:UserManager.userInfo.userUID]) {
                //我是 邀请者
                textContentStr = MultilingualTranslation(@"对方已拒绝");
            } else {
                //我是 被邀请者
                textContentStr = MultilingualTranslation(@"已拒绝");
            }
            ...
            if (![mediaCallModel.from_id isEqualToString:UserManager.userInfo.userUID]) {
                //我是 被邀请者
                textContentStr = MultilingualTranslation(@"已在其他设备接听");
            }
        } else {
            //通话正常挂断
            //告知 邀请者 展示 如：10:00通话
            ...
            if ([mediaCallModel.from_id isEqualToString:UserManager.userInfo.userUID]) {
                textContentStr = [NSString stringWithFormat:MultilingualTranslation(@"%@ 通话结束"), [NSString getTimeLength:mediaCallModel.duration]];
            } else {
                textContentStr = [NSString stringWithFormat:MultilingualTranslation(@"通话结束 %@"), [NSString getTimeLength:mediaCallModel.duration]];
            }
        }
        ...
        if (actionModel.mode == 0) {
            callMode = MultilingualTranslation(@"视频通话");
        } else {
            callMode = MultilingualTranslation(@"语音通话");
        }
        ...
        if (groupMemberModel) {
            if ([actionModel.action isEqualToString:@"new"]) {
                ...
                textContentStr = [NSString stringWithFormat:MultilingualTranslation(@"%@已经结束"), callMode];
            }
        } else {
            if ([actionModel.action isEqualToString:@"new"]) {
                textContentStr = [NSString stringWithFormat:@"%@", callMode];
    ...
    if (_message.netCallChatType == 1) {
        ...
        switch (_message.netCallStatus) {
            ...
            case 5://通话挂断结束
            {
                if ([_message.netCallRoomCreateUser isEqualToString:UserManager.userInfo.userUID]) {
                    //我是邀请者
                    textContentStr = [NSString stringWithFormat:MultilingualTranslation(@"%@ 通话结束"), [NSString getTimeLength:_message.netCallDuration]];
                }else {
                    //我是被邀请者
                    textContentStr = [NSString stringWithFormat:MultilingualTranslation(@"通话结束 %@"), [NSString getTimeLength:_message.netCallDuration]];
                }
            ...
            case 7://通话中断
            {
                if ([_message.netCallRoomCreateUser isEqualToString:UserManager.userInfo.userUID]) {
                    //我是邀请者
                    textContentStr = [NSString stringWithFormat:MultilingualTranslation(@"%@ 通话中断"), [NSString getTimeLength:_message.netCallDuration]];
                }else {
                    //我是被邀请者
                    textContentStr = [NSString stringWithFormat:MultilingualTranslation(@"通话中断 %@"), [NSString getTimeLength:_message.netCallDuration]];
                }
...
- (void)calculateNameCardMessage {
    self.attStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[个人名片]")];
...
- (void)calculateGeoLocationMessage {
    self.attStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[地理位置]")];
...
- (void)calculateMergeMsgRecordMessage {
    self.attStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"[会话记录]")];
    ...
    if (_message.messageType == CIMChatMessageType_BackMessage) {
        ...
        if (_message.chatType == CIMChatType_GroupChat && ![NSString isNil:_message.backDeleteExt]) {
            if ([_message.backDeleteExt isEqualToString:UserManager.userInfo.userUID]) {
                resutlContent = [NSString stringWithFormat:MultilingualTranslation(@"您的消息已被%@撤回"),_message.fromNickname];
                ...
                if (originMessageUserModel != nil) {
                    //群主视角
                    resutlContent = [NSString stringWithFormat:MultilingualTranslation(@"您已撤回了“%@”的消息"), originMessageUserModel.showName];
                }
            } else {
                LingIMGroupMemberModel *originMessageUserModel = [IMSDKManager imSdkCheckGroupMemberWith:_message.backDeleteExt groupID:_message.toID];
                if (originMessageUserModel != nil) {
                    resutlContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”撤回了“%@”的消息"), _message.fromNickname, originMessageUserModel.showName];
                }
            }
            NSMutableParagraphStyle *style = [[NSMutableParagraphStyle alloc] init];
            style.lineSpacing = 2;
            ...
            if ([_message.backDeleteExt isEqualToString:UserManager.userInfo.userUID]) {
                [self.attStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[resutlContent rangeOfString:_message.fromNickname]];
            } else if ([UserManager.userInfo.userUID isEqualToString:_message.fromID]) {
                LingIMGroupMemberModel *originMessageUserModel = [IMSDKManager imSdkCheckGroupMemberWith:_message.backDeleteExt groupID:_message.toID];
                if (originMessageUserModel) {
                    [self.attStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[resutlContent rangeOfString:originMessageUserModel.showName]];
                }
            } else {
                LingIMGroupMemberModel *originMessageUserModel = [IMSDKManager imSdkCheckGroupMemberWith:_message.backDeleteExt groupID:_message.toID];
                if (originMessageUserModel) {
                    [self.attStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[resutlContent rangeOfString:_message.fromNickname]];
                    [self.attStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[resutlContent rangeOfString:originMessageUserModel.showName == nil ? @"" : originMessageUserModel.showName]];
                }
            }
        } else {
            if ([_message.fromID isEqualToString:UserManager.userInfo.userUID]) {
                resutlContent = MultilingualTranslation(@"你撤回了一条消息");
            } else {
                if (_message.chatType == CIMChatType_SingleChat) {
                    resutlContent = MultilingualTranslation(@"对方撤回了一条消息");
                    ...
                    resutlContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”撤回了一条消息"), groupMemberModel ? groupMemberModel.showName : _message.fromNickname];
                } else {
                    resutlContent = @"";
                }
            }
            ...
            if (_message.chatType == CIMChatType_GroupChat) {
                LingIMGroupMemberModel *groupMemberModel = [IMSDKManager imSdkCheckGroupMemberWith:_message.fromID groupID:_message.toID];
                // 用户昵称需变为蓝色
                [self.attStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[resutlContent rangeOfString:groupMemberModel ? groupMemberModel.showName : _message.fromNickname]];
            }
        }
...
- (void)calculateReferenceMessage {
    ...
    if (_message.messageType == CIMChatMessageType_TextMessage) {
        ...
        //emoij表情
        self.attStr = [[EMChatInputEmojiManager sharedManager] attributedString:_message.showContent];
        [self.attStr addAttributes:dict range:NSMakeRange(0, self.attStr.length)];
        ...
        self.attStr = [[EMChatInputEmojiManager sharedManager] attributedString:_message.textContent];
        [self.attStr addAttributes:dict range:NSMakeRange(0, self.attStr.length)];
    }
            ...
            if (_message.messageType == CIMChatMessageType_AtMessage) {
                if (![NSString isNil:_message.atTranslateContent]) {
                    if ([NSString isNil:_message.showTranslateContent]) {
                        _message.showTranslateContent = [ZMessageTools atContenTranslateToShowContent:_message.atTranslateContent == nil ? @"" : _message.atTranslateContent atUsersDictList:_message.atUsersInfoList withMessage:_message isGetShowName:YES];
                    }
                    self.translateAttStr = [[EMChatInputEmojiManager sharedManager] yy_emojiAttributedString:_message.showTranslateContent];
                    [self.translateAttStr addAttributes:dict range:NSMakeRange(0, self.translateAttStr.length)];
    ...
    if (_referenceMsg == nil) {
        ...
        self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"该引用不存在")];
        ...
        //昵称
        referenceNickWidth = [MultilingualTranslation(@"未知") widthForFont:FONTN(12)];
    } else {
        if (_referenceMsg.messageType == CIMChatMessageType_TextMessage) {
            ...
            if (_referenceMsg == nil) {
                ...
                if (_referenceMsg.messageStatus == 1) { //正常
                    if ([_referenceMsg.fromID isEqualToString:UserManager.userInfo.userUID]) {
                        self.referenceAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:_referenceMsg.textContent];
                    } else {
                        NSString *resultStr = @"";
                        ...
                        self.referenceAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:resultStr];
                    }
                } else if (_referenceMsg.messageStatus == 0) {  //已删除
                    self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"该引用已删除")];
                } else if (_referenceMsg.messageStatus == 2) {  //已撤回
                    self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"该引用已撤回")];
                }  else {
                    self.referenceMsg.fromNickname = MultilingualTranslation(@"未知");
                    self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"未知错误")];
                }
            }
            [self.referenceAttStr addAttributes:referenceDict range:NSMakeRange(0, self.referenceAttStr.length)];
            if (!_isSelf) {
            ...
            if (_referenceMsg == nil) {
                ...
                if (_referenceMsg.messageStatus == 1) { //正常
                    ...
                    if ([_referenceMsg.fromID isEqualToString:UserManager.userInfo.userUID]) {
                        atContentStr = [ZMessageTools atContenTranslateToShowContent:_referenceMsg.atContent atUsersDictList:_referenceMsg.atUsersInfoList withMessage:_referenceMsg isGetShowName:YES];
                        self.referenceAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:atContentStr];
                    } else {
                        NSString *resultStr = @"";
                        ...
                        atContentStr = [ZMessageTools atContenTranslateToShowContent:resultStr atUsersDictList:_referenceMsg.atUsersInfoList withMessage:_referenceMsg isGetShowName:YES];
                        self.referenceAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:atContentStr];
                    }
                } else if (_referenceMsg.messageStatus == 0) {  //已删除
                    self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"该引用已删除")];
            ...
            if (_referenceMsg == nil) {
                ...
                if (_referenceMsg.messageStatus == 1) { //正常
                    ...
                    if (_referenceMsg.messageType == CIMChatMessageType_VoiceMessage) {
                        referenceContentStr = MultilingualTranslation(@"[语音]");
                    } else {
                        referenceContentStr = _referenceMsg.showFileName;
                    }
                    self.referenceAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:referenceContentStr];
                } else if (_referenceMsg.messageStatus == 0) {  //已删除
                    self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"该引用已删除")];
                } else if (_referenceMsg.messageStatus == 2) {  //已撤回
                    self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"该引用已撤回")];
            ...
            if (_referenceMsg == nil) {
                ...
                if (_referenceMsg.messageStatus == 1) { //正常
                    NSString *referenceContentStr = [NSString stringWithFormat:@"%@%@", MultilingualTranslation(@"[个人名片]"), _referenceMsg.cardNickName];
                    self.referenceAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:referenceContentStr];
                } else if (_referenceMsg.messageStatus == 0) {  //已删除
                    self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"该引用已删除")];
                } else if (_referenceMsg.messageStatus == 2) {  //已撤回
            ...
            if (_referenceMsg == nil) {
                ...
                if (_referenceMsg.messageStatus == 1) { //正常
                    NSString *referenceContentStr = [NSString stringWithFormat:@"%@%@", MultilingualTranslation(@"[位置]"), _referenceMsg.geoName];
                    self.referenceAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:referenceContentStr];
                } else if (_referenceMsg.messageStatus == 0) {  //已删除
                    self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"该引用已删除")];
                } else if (_referenceMsg.messageStatus == 2) {  //已撤回
            ...
            if (_referenceMsg == nil) {
                ...
                if (_referenceMsg.messageStatus == 1) { //正常
                    NSString *referenceContentStr = [NSString stringWithFormat:@"%@%@", MultilingualTranslation(@"[会话记录]"), _referenceMsg.forwardMessage.title];
                    self.referenceAttStr = [[EMChatInputEmojiManager sharedManager] attributedString:referenceContentStr];
                } else if (_referenceMsg.messageStatus == 0) {  //已删除
                    self.referenceAttStr = [[NSMutableAttributedString alloc] initWithString:MultilingualTranslation(@"该引用已删除")];
                } else if (_referenceMsg.messageStatus == 2) {  //已撤回
...
- (void)calculateServerMsgHeight {
    ...
    switch (_message.serverMessage.sMsgType) {
        ...
        {
            NSString *msgContent = MultilingualTranslation(@"您还不是对方好友，请先添加好友。");
            ...
            //需要突出的地方设置为 COLOR_4791FF
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:MultilingualTranslation(@"添加好友")]];
        }
            break;
        case IMServerMessage_ServerMsgType_BlackFriendMessage://好友黑名单
        {
            ...
            if (friendBlack.type == 1) {
                //我拉黑好友
                msgContent = MultilingualTranslation(@"已拉黑");
            }else {
                //好友拉黑我
                msgContent = MultilingualTranslation(@"对方已拒绝接受你的消息");
            }
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
        }
        ...
        case IMServerMessage_ServerMsgType_UserAccountClose://好友 账号已注销
        {
            ...
            NSString *msgContent;
            msgContent = MultilingualTranslation(@"账号已注销");
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
        }
            break;
        case IMServerMessage_ServerMsgType_CreateGroupMessage://创建群聊
        {
            ...
            if ([createModel.uid isEqualToString:UserManager.userInfo.userUID]) {
                //我创建的群聊
                user = MultilingualTranslation(@"你邀请了 ");
            }
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"%@%@ 加入了群聊"),user, invitedUserNameStr];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            
            //富文本处理，颜色处理
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            if (![createModel.uid isEqualToString:UserManager.userInfo.userUID]) {
                [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberCreateModel ? groupMemberCreateModel.showName : createModel.nick]];
            }
            for (UserInfo *obj in invitedMemberArr) {
                if (![obj.uId isEqualToString:createModel.uid]) {
                    LingIMFriendModel *friendModel = [IMSDKManager toolCheckMyFriendWith:obj.uId];
                    if (friendModel) {
                        [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:msgContent appointStr:friendModel ? friendModel.showName: obj.uNick];
                    } else {
                        LingIMGroupMemberModel *memberModel = [IMSDKManager imSdkCheckGroupMemberWith:obj.uId groupID:createModel.gid];
                        [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:msgContent appointStr:memberModel ? memberModel.showName : obj.uNick];
                    }
                }
            }
        }
            ...
            if (inviteModel.type == 4) {
                ...
                [invitedMemberArr enumerateObjectsUsingBlock:^(UserInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if ([obj.uId isEqualToString:UserManager.userInfo.userUID]) {
                        [invitedUserNameStr appendFormat:@"%@", MultilingualTranslation(@"你")];
                    }else {
                        LingIMFriendModel *friendModel = [IMSDKManager toolCheckMyFriendWith:obj.uId];
                        if (friendModel) {
                            [invitedUserNameStr appendFormat:@"“%@”", friendModel ? friendModel.showName: obj.uNick];
                ...
                NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"%@通过扫描二维码加入群聊"), invitedUserNameStr];
                serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
                //富文本处理，颜色处理
                [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
                for (UserInfo *obj in invitedMemberArr) {
                    if (![obj.uId isEqualToString:UserManager.userInfo.userUID]) {
                        ...
                        if (friendModel) {
                            ...
                            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF fullStr:msgContent appointStr:obj.uNick];
                        }
                    }
                }
            } else {
                ...
                if ([inviteModel.uid isEqualToString:UserManager.userInfo.userUID]) {
                    ...
                    user = [NSString stringWithFormat:MultilingualTranslation(@"“%@”邀请了 "), groupMemberInviteModel ? groupMemberInviteModel.showName : inviteModel.nick];
                }
                ...
                [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
                if (![inviteModel.uid isEqualToString:UserManager.userInfo.userUID]) {
                    [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberInviteModel ? groupMemberInviteModel.showName : inviteModel.nick]];
                }
                for (UserInfo *obj in invitedMemberArr) {
                    if (![obj.uId isEqualToString:UserManager.userInfo.userUID]) {
                        LingIMFriendModel *friendModel = [IMSDKManager toolCheckMyFriendWith:obj.uId];
        ...
        {
            NSString *msgContent = MultilingualTranslation(@"该群开启了全员禁言");
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
        }
            break;
        ...
        {
            NSString *msgContent = MultilingualTranslation(@"群组已解散");
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
        }
            break;
        ...
        {
            NSString *msgContent = MultilingualTranslation(@"你已不在该群");
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
        }
            break;
        ...
        {
            NSString *msgContent = MultilingualTranslation(@"你已被禁言");
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
        }
            break;
        case IMServerMessage_ServerMsgType_KickGroupMessage://群成员被踢
        {
            ...
            if ([kickmember.uid isEqualToString:UserManager.userInfo.userUID]) {
                msgContent = [NSString stringWithFormat:@"%@", MultilingualTranslation(@"你已被移出群聊")];
            }else {
                msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”将“%@”移出群聊"),groupMemberOperateModel ? groupMemberOperateModel.showName : kickmember.operateNick, groupMemberKickModel ? groupMemberKickModel.showName : kickmember.nick];
            }
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            //富文本处理，颜色处理
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            ...
            if (![kickmember.uid isEqualToString:UserManager.userInfo.userUID]) {
                ...
                [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberOperateModel ? groupMemberOperateModel.showName : kickmember.operateNick]];
                [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberKickModel ? groupMemberKickModel.showName : kickmember.nick]];
            }
        }
            break;
        case IMServerMessage_ServerMsgType_OutGroupMessage://群成员退群
        {
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”退出了群聊"),groupMemberModel ? groupMemberModel.showName : outGroupMember.nick];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : outGroupMember.nick]];
        }
            break;
        case IMServerMessage_ServerMsgType_TransferOwnerMessage://转让群主
        {
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"群主变更为“%@”"),groupMemberModel ? groupMemberModel.showName : groupOwnerTransfer.nick];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : groupOwnerTransfer.nick]];
        }
            break;
        case IMServerMessage_ServerMsgType_EstoppelGroupMessage://告知全部群成员 群禁言 开启/关闭
        {
            ...
            if (groupStatus.status == 1) {
                msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”开启了全员禁言"), userName];
            }else {
                msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”关闭了全员禁言"), userName];
            }
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:userName]];
        }
            break;
        ...
        case IMServerMessage_ServerMsgType_IsShowHistoryMessage://新成员是否可查看历史消息
        {
            ...
            if (groupStatusMessage.status == 1) {
                msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@” 开启了新成员可查看历史消息"), userName];
            }else {
                msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@” 关闭了新成员可查看历史消息"), userName];
            }
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:userName]];
        ...
        case IMServerMessage_ServerMsgType_AdminGroupMessage://变更管理员
        {
            ...
            [adminInfoArr enumerateObjectsUsingBlock:^(UserInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([obj.uId isEqualToString:UserManager.userInfo.userUID]) {
                    [adminStr appendFormat:@"%@", MultilingualTranslation(@"你")];
                }else {
            ...
            if (groupAdmin.type == 1) {
                msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”将%@设为管理员"), groupMemberOperateModel ? groupMemberOperateModel.showName : groupAdmin.operateNick, adminStr];
            }else {
                msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”将%@从管理员中移除"),groupMemberOperateModel ? groupMemberOperateModel.showName : groupAdmin.operateNick, adminStr];
            }
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            ...
            [adminInfoArr enumerateObjectsUsingBlock:^(UserInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if (![obj.uId isEqualToString:UserManager.userInfo.userUID]) {
                    LingIMGroupMemberModel *groupMemberModel = [IMSDKManager imSdkCheckGroupMemberWith:obj.uId groupID:groupAdmin.gid];
                    [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : obj.uNick]];
                }
            }];
            
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberOperateModel ? groupMemberOperateModel.showName : groupAdmin.operateNick]];
        }
            break;
        case IMServerMessage_ServerMsgType_NameGroupMessage://群名称修改
        {
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”将群名称修改为：%@"),groupMemberModel ? groupMemberModel.showName : groupName.nick, groupName.gName];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            //颜色
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : groupName.nick]];
        }
            break;
        case IMServerMessage_ServerMsgType_NoticeGroupMessage://群公告设置
        {
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”设置了群公告"),groupMemberModel ? groupMemberModel.showName : groupNotice.nick];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            //颜色
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : groupNotice.nick]];
        }
            break;
        case IMServerMessage_ServerMsgType_GroupSingleForbidMessage://群主或管理员 禁言某个群成员
        {
            ...
            if ([memberBanned.toUid isEqualToString:UserManager.userInfo.userUID]) {
                if (memberBanned.status == 1) {
                    //"你被“%@”禁言%lld分钟"
                    msgContent = [NSString stringWithFormat:MultilingualTranslation(@"你被“%@”%@"),groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick, [NSString convertBannedSendMsgTime:memberBanned.expireTime]];
                } else {
                    msgContent = [NSString stringWithFormat:MultilingualTranslation(@"你被“%@”解除禁言"),groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick];
                }
            }else {
                if (memberBanned.status == 1) {
                    //"“%@”被“%@”禁言%lld分钟"
                    msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”被“%@”%@"),groupMemberToModel ? groupMemberToModel.showName : memberBanned.toNick, groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick, [NSString convertBannedSendMsgTime:memberBanned.expireTime]];
                } else {
                    msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”被“%@”解除禁言"),groupMemberToModel ? groupMemberToModel.showName : memberBanned.toNick, groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick];
                }
            }
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            //富文本颜色
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            if ([memberBanned.toUid isEqualToString:UserManager.userInfo.userUID]) {
                [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick]];
            } else {
                [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberToModel ? groupMemberToModel.showName : memberBanned.toNick]];
                [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberFromModel ? groupMemberFromModel.showName : memberBanned.fromNick]];
            }
        }
            break;
        case IMServerMessage_ServerMsgType_DelGroupMessage://解散群组  该消息只转发给在线的所有群成员
        {
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”解散了该群"), groupMemberModel ? groupMemberModel.showName : groupDissolve.nick];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : groupDissolve.nick]];
        }
            break;
        case IMServerMessage_ServerMsgType_InviteJoinGroupNoFriendMessage://邀请好友进群，但是好友不存在，该消息只转发给邀请加入的用户
        {
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”未添加你为好友，无法邀请进入群聊"), groupMemberModel ? groupMemberModel.showName : inviteJoinGroupNoFriend.operateNick];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : inviteJoinGroupNoFriend.operateNick]];
        }
            break;
        case IMServerMessage_ServerMsgType_InviteJoinGroupBlackFriendMessage://邀请好友进群，但是已被拉黑，该消息只转发给邀请加入的用户
        {
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”拒绝加入群聊"), groupMemberModel ? groupMemberModel.showName : inviteJoinGroupBlackFriend.operateNick];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : inviteJoinGroupBlackFriend.operateNick]];
        }
            break;
        case IMServerMessage_ServerMsgType_AvatarGroupMessage://变更群头像  该消息只转发给在线的所有群成员
        {
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”修改了群头像"), groupMemberModel ? groupMemberModel.showName : avatarGroup.nick];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : avatarGroup.nick]];
        }
            break;
        ...
        case IMServerMessage_ServerMsgType_DelGroupNotice://删除群公告
        {
            ...
            NSString *msgContent = [NSString stringWithFormat:MultilingualTranslation(@"“%@”删除了群公告"),groupMemberModel ? groupMemberModel.showName : groupNoticeDel.nick];
            serverMsgAttStr = [[NSMutableAttributedString alloc] initWithString:msgContent];
            //颜色
            [serverMsgAttStr configAttStrLightColor:COLOR_66 darkColor:COLOR_66_DARK range:NSMakeRange(0, msgContent.length)];
            [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[msgContent rangeOfString:groupMemberModel ? groupMemberModel.showName : groupNoticeDel.nick]];
        }
            break;
        case IMServerMessage_ServerMsgType_ScheduleDeleteMessage://消息定时自动删除
        {
            ...
            if (messageTimeDelete.chatType == ChatType_SingleChat) {
                ...
                if ([messageTimeDelete.userId isEqualToString:UserManager.userInfo.userUID]) {
                    userName = MultilingualTranslation(@"你");
                }else {
                    //单聊
            ...
            switch (messageTimeDelete.freq) {
                case 1:
                    timeDeleteInfo = MultilingualTranslation(@"已设置自动删除1天前发送的消息");
                    break;
                case 7:
                    timeDeleteInfo = MultilingualTranslation(@"已设置自动删除7天前发送的消息");
                    break;
                case 30:
                    timeDeleteInfo = MultilingualTranslation(@"已设置自动删除30天前发送的消息");
                    break;
                    
                default:
                    timeDeleteInfo = MultilingualTranslation(@"关闭了自动删除");
                    break;
            }
        ...
        case IMServerMessage_ServerMsgType_JoinVerifyGroupMessage://开启了入群验证
        {
            ...
            if (groupStatus.status == 1) {
                groupStatusInfo = [NSString stringWithFormat:MultilingualTranslation(@"“%@”已启用群里邀请确认"), userName];
            }else {
                groupStatusInfo = [NSString stringWithFormat:MultilingualTranslation(@"“%@”已关闭群里邀请确认"), userName];
            }
            ...
            if (![groupStatus.uid isEqualToString:UserManager.userInfo.userUID]) {
                //颜色
                [serverMsgAttStr configAttStrLightColor:COLOR_4791FF darkColor:COLOR_4791FF range:[groupStatusInfo rangeOfString:userName]];
            }
...
- (instancetype)copyWithZone:(NSZone *)zone {
    ...
    ZMessageModel *model = [[ZMessageModel allocWithZone:zone] init];
