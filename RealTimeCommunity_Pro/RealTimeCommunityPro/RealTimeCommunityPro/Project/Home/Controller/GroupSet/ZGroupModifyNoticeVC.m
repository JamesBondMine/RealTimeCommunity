...
#import "ZGroupNoticeTranslateVC.h"
#import "ZMessageAlertView.h"
//#import "ZCharacterBindViewController.h"
...
@interface ZGroupModifyNoticeVC ()
...
@implementation ZGroupModifyNoticeVC
...
- (void)viewDidLoad {
    ...
    self.languageNameList = [NSMutableArray arrayWithObject:MultilingualTranslation(@"英文")];
...
- (void)setupUI {
    ...
    tipLabel.text = MultilingualTranslation(@"群公告（仅群主和管理员可编辑）");
    ...
    self.textView.tkThemetextColors = @[COLOR_33, COLOR_33_DARK];
    ...
    self.placeHoderLabel.text = MultilingualTranslation(@"编辑群公告");
    ...
    if (self.groupInfoModel.userGroupRole == 1 ||self.groupInfoModel.userGroupRole == 2) {
        ...
        [self.translateButton setTitle:MultilingualTranslation(@"提交翻译") forState:UIControlStateNormal];
        [self.translateButton setTkThemeTitleColor:@[COLORWHITE, COLORWHITE] forState:UIControlStateNormal];
        [self.translateButton setTkThemebackgroundColors:@[COLOR_4791FF, COLOR_4791FF]];
        ...
        [self.releaseButton setTitle:MultilingualTranslation(@"发布原文") forState:UIControlStateNormal];
        [self.releaseButton setTkThemeTitleColor:@[COLOR_4791FF, COLOR_4791FF] forState:UIControlStateNormal];
        [self.releaseButton setTkThemebackgroundColors:@[COLORWHITE, COLOR_33]];
        self.releaseButton.titleLabel.font = FONTN(14);
        [self.releaseButton rounded:DWScale(8) width:0.8 color:COLOR_4791FF];
        ...
        translateTopTipLabel.text = MultilingualTranslation(@"选择群公告翻译通道和语种");
        ...
        channelLbl.tkThemetextColors = @[COLOR_33, COLORWHITE];
        channelLbl.font = FONTR(14);
        channelLbl.text = MultilingualTranslation(@"通道");
        ...
        [self.channelInputView setTextInputClick:^{
            ...
            [weakSelf.channelDropView setChannelSelectedBlock:^(NSString * _Nonnull itemId, NSString * _Nonnull itemName) {
                ...
                [weakSelf.languageNameList addObject:MultilingualTranslation(@"英文")];
        ...
        languageLbl.tkThemetextColors = @[COLOR_33, COLORWHITE];
        languageLbl.font = FONTR(14);
        languageLbl.text = MultilingualTranslation(@"语种");
        ...
        [self.languageInputView setTextInputClick:^{
            ...
            if (weakSelf.channelCode.length <= 0) {
                [HUD showMessage:MultilingualTranslation(@"请先选择翻译通道")];
                return;
            }
            if (!weakSelf.languageDropView) {
        ...
        translateBottomTipLabel.text = MultilingualTranslation(@"注：未选择语种默认展示英文");
...
- (void)requestGetTranslateChannel {
    ...
    [IMSDKManager imSdkTranslateGetChannelLanguage:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSArray class]]) {
            ...
            NSArray *chanelModelList = [MeTranslateChannelLanguageModel mj_objectArrayWithKeyValuesArray:dataArr];
            for (MeTranslateChannelLanguageModel *tempChannelModel in chanelModelList) {
...
- (void)requestGetTranslateLanguage {
    ...
    [IMSDKManager imSdkTranslateGetChannelLanguage:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSArray class]]) {
            ...
            NSArray *dataList = [MeTranslateChannelLanguageModel mj_objectArrayWithKeyValuesArray:dataArr];
            for (MeTranslateChannelLanguageModel *model in dataList) {
            ...
            for (NSString *languageCode in weakSelf.languageCodeList) {
                for (ZTranslateLanguageModel *tempLanguageModel in languageModelList) {
...
- (void)releaseOriginalNoticeAction {
    ...
    if ([NSString isNil:self.textView.text]) {
        [HUD showMessage:MultilingualTranslation(@"公告不能为空")];
        return;
    }
    
    WeakSelf
    ZPresentItem * sendToSessionItem= [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"发送至会话") textColor:COLOR_33 font:FONTR(17) itemHeight:DWScale(54) backgroundColor:COLORWHITE];
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        if (themeIndex == 0) {
            sendToSessionItem.textColor = COLOR_33;
            sendToSessionItem.backgroundColor = COLORWHITE;
        }else {
            sendToSessionItem.textColor = COLORWHITE;
            sendToSessionItem.backgroundColor = COLOR_33;
        }
    };
    
    ZPresentItem *displayTopSessionItem = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"在会话内置顶显示") textColor:COLOR_33 font:FONTR(17) itemHeight:DWScale(54) backgroundColor:COLORWHITE];
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        if (themeIndex == 0) {
            displayTopSessionItem.textColor = COLOR_33;
            displayTopSessionItem.backgroundColor = COLORWHITE;
        }else {
            displayTopSessionItem.textColor = COLORWHITE;
            displayTopSessionItem.backgroundColor = COLOR_33;
        }
    };
    
    ZPresentItem *sendTOSessionTop = [ZPresentItem creatPresentViewItemWithText:MultilingualTranslation(@"发送至会话并置顶") textColor:COLOR_33 font:FONTR(17) itemHeight:DWScale(54) backgroundColor:COLORWHITE];
    self.tkThemeChangeBlock = ^(id  _Nullable itself, NSUInteger themeIndex) {
        if (themeIndex == 0) {
            sendTOSessionTop.textColor = COLOR_33;
            sendTOSessionTop.backgroundColor = COLORWHITE;
        }else {
            sendTOSessionTop.textColor = COLORWHITE;
            sendTOSessionTop.backgroundColor = COLOR_33;
        }
    };
    ...
    ZPresentView *viewAlert = [[ZPresentView alloc] initWithFrame:CGRectMake(0, 0, DScreenWidth, DScreenHeight) titleItem:nil selectItems:@[sendToSessionItem, displayTopSessionItem,sendTOSessionTop] cancleItem:cancelItem doneClick:^(NSInteger index) {
...
- (void)confirmTranslateNoticeAction {
    ...
    if (self.textView.text.length <= 0) {
        [HUD showMessage:MultilingualTranslation(@"请输入群公告内容！")];
        return;
    }
    if (self.channelCode.length <= 0 || self.languageCodeList.count <= 0) {
        [HUD showMessage:MultilingualTranslation(@"选择群公告翻译通道和语种")];
        return;
    }
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setObjectSafe:UserManager.userInfo.userUID forKey:@"userUid"];
    
    ZGroupNoticeTranslateVC *vc = [[ZGroupNoticeTranslateVC alloc] init];
...
- (void)translateNoBindAccountTips {
    //弹出新的弹窗
    WeakSelf
    ZMessageAlertView *translateAlertView = [[ZMessageAlertView alloc] initWithMsgAlertType:ZMessageAlertTypeNomal supView:nil];
    translateAlertView.lblContent.text = MultilingualTranslation(@"您尚未绑定字符账号，无法使用翻译功能，请绑定后使用。");
    translateAlertView.lblContent.textAlignment = NSTextAlignmentCenter;
    [translateAlertView.btnSure setTitle:MultilingualTranslation(@"绑定账户") forState:UIControlStateNormal];
    [translateAlertView.btnSure setTkThemeTitleColor:@[COLORWHITE, COLORWHITE] forState:UIControlStateNormal];
    translateAlertView.btnSure.tkThemebackgroundColors = @[COLOR_4791FF, COLOR_4791FF];
    [translateAlertView.btnCancel setTitle:MultilingualTranslation(@"取消") forState:UIControlStateNormal];
    ...
    translateAlertView.sureBtnBlock = ^(BOOL isCheckBox) {
        //绑定账户-跳转到绑定账户VC
        ZCharacterBindViewController *vc = [[ZCharacterBindViewController alloc] init];
    ...
    if (![NSString isNil:self.groupInfoModel.groupNotice.noticeId]) {
        if (![NSString isNil:self.textView.text]){
             ...
             * TODO: 老版本代码如果没有输入内容，直接删除
             ...
             [IMSDKManager groupDeleteGroupNoticeWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
                 ...
                 [HUD showMessage:MultilingualTranslation(@"群公告移除成功")];
             } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
                 [HUD showMessageWithCode:code errorMsg:msg];
             }];
             */
        ...
        if (![NSString isNil:self.textView.text]) {
            ...
            [IMSDKManager groupCreateGroupNoticeWith:dict onSuccess:^(id _Nullable data, NSString * _Nullable traceId) {
                ...
                [weakSelf backAction];
                [HUD showMessage:MultilingualTranslation(@"发布成功")];
            } onFailure:^(NSInteger code, NSString * _Nullable msg, NSString * _Nullable traceId) {
                [HUD showMessageWithCode:code errorMsg:msg];
            }];
        } else {
...
- (void)backAction {
    ...
    [viewControllers enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if([obj isKindOfClass:[NHChatViewController class]]){
            NHChatViewController *chatCtrl = (NHChatViewController *)obj;
            [self.navigationController popToViewController:chatCtrl animated:YES];
            *stop = YES;
        }else if ([obj isKindOfClass:[ZGroupNoticeListVC class]]) {
            ZGroupNoticeListVC *noticeList = (ZGroupNoticeListVC *)obj;
            // 刷新
