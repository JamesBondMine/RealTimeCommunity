...
- (void)configDB {
    ...
    //红点展示
    if ([self.window.rootViewController isKindOfClass:[NTTabBarController class]]) {
        
        NTTabBarController *tab = (NTTabBarController *)self.window.rootViewController;
        //通讯录红点先取本地展示
...
- (void)cimToolUserFriendInvite:(FriendInviteMessage *)message {
    ...
    CMFriendReqModel *model = [CMFriendReqModel new];
    ...
    for (int i = 0; i < list.count; i++) {
        CMFriendReqModel *req = (CMFriendReqModel *)list[i];
...
- (void)imSdkUserForceLogout:(NSInteger)type message:(NSString *)message {
    ...
    if (type == 888) {
        [HUD showMessage:MultilingualTranslation(@"用户已经注销")];
    }
    if (type == 90018) {
        //登录不在白名单内，需展示IP地址
        [HUD showMessage:[NSString stringWithFormat:MultilingualTranslation(@"登录IP：%@ 不在白名单内"), message]];
    }
    ZUserModel *userModel = [ZUserModel getUserInfo];
...
- (void)cimUserUnableHttpNode:(NSString *)tipsContent {
    [HUD showMessage:MultilingualTranslation(tipsContent)];
}
...
//删除会话 + 清空聊天内容 + 删除群组
- (void)deleteSessionAndChatMessage:(LingIMChatMessageModel *)messageModel {
...
- (void)imSdkUserCloseAutoTranslateAndErrorCode:(NSInteger)errorCode errorMsg:(NSString *)errorMsg sessionModel:(LingIMSessionModel *)sessionModel {
    if (errorCode == Translate_yuuee_no_balance_code) {
        ...
        //提示
        [HUD showMessage:MultilingualTranslation(@"当前账户字符数不足，已关闭翻译功能，请增加字符后使用。")];
    } else if (errorCode == Translate_yuuee_unbind_error_code) {
        [HUD showMessage:MultilingualTranslation(@"您尚未绑定字符账号，无法使用翻译功能，请绑定后使用。")];
    } else {
...
- (void)updateFriendApplyCountFromService {
    ...
    [IMSDKManager getFriendSyncReqListWith:param onSuccess:^(id  _Nullable data, NSString * _Nullable traceId) {
        if ([data isKindOfClass:[NSDictionary class]]) {
            ...
            NSMutableArray<CMFriendReqModel *> *reqList = [NSMutableArray array];
            [reqArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                CMFriendReqModel *model = [CMFriendReqModel mj_objectWithKeyValues:obj];
            ...
            if ([NSString isNil:lastSyncTime]) {
                ...
                for (int i = 0; i < reqList.count; i++) {
                    ...
                    for (int j = 0; j < list.count; j++) {
                        CMFriendReqModel *model = (CMFriendReqModel *)list[j];
                        CMFriendReqModel *reqModel = (CMFriendReqModel *)reqList[i];
            ...
            [IMSDKManager toolUpdateFriendApplyCount:friendApplyCount];
            //更新通讯录红点
            if ([weakSelf.window.rootViewController isKindOfClass:[NTTabBarController class]]) {
                NTTabBarController *tab = (NTTabBarController *)weakSelf.window.rootViewController;
                [tab setBadgeValue:1 number:friendApplyCount];
                //通讯录好友申请，红点更新
                [[NSNotificationCenter defaultCenter] postNotificationName:@"FriendApplyCountChange" object:nil];
            }
