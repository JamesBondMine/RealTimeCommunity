...
//  CIMKit
//
//  Created by cusPro on 2022/10/9.
//

#import "ZImagePickerVC.h"
#import <MobileCoreServices/MobileCoreServices.h>
#import "ZImagePickerCell.h"//图片展示
#import "ZToolManager.h"//工具类
#import "ZClipImageVC.h"//图片裁剪
#import <UniformTypeIdentifiers/UniformTypeIdentifiers.h>

//下载iCloud资源
typedef void(^ZDownloadAsset) (PHAsset *);

@interface ZImagePickerVC () <UICollectionViewDataSource,UICollectionViewDelegate,ZImagePickerCellDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate, ZClipImageVCDelegate>
{
    BOOL _isPhotoOk;
}
...
- (void)requestAlbumAll {
    ...
    [ZTOOL doAsync:^{
        ...
        [HUD showActivityMessage:MultilingualTranslation(@"加载中...")];
...
- (void)setupNavUI {
    ...
    _navView.tkThemebackgroundColors = @[COLOR_33, COLOR_33];
    ...
    _navTitleLabel.text = _isSignlePhoto ? MultilingualTranslation(@"选择") : MultilingualTranslation(@"选择图片");
    ...
    [_navBtnBack setImage:[UIImage imageNamed:@"repicker_image_picker_back"] forState:UIControlStateNormal];
...
- (void)setupUI {
    if (_isSignlePhoto) {
        self.view.tkThemebackgroundColors = @[COLOR_33, COLOR_33];
    } else {
    ...
    _collectionView.tkThemebackgroundColors = @[COLOR_33, COLOR_33];
    _collectionView.showsVerticalScrollIndicator = NO;
    _collectionView.dataSource = self;
    _collectionView.delegate = self;
    [_collectionView registerClass:[ZImagePickerCell class] forCellWithReuseIdentifier:NSStringFromClass([ZImagePickerCell class])];
...
//    [_btnOriginal setImage:ImgNamed(@"repicker_image_picker_select_no") forState:UIControlStateNormal];
//    [_btnOriginal setImage:ImgNamed(@"repicker_image_picker_select_yes") forState:UIControlStateSelected];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ZImagePickerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([ZImagePickerCell class]) forIndexPath:indexPath];
    PHAsset *asset;
    if (_hasCamera) {
            ...
            [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:PHImageManagerMaximumSize contentMode:PHImageContentModeAspectFit options:nil resultHandler:^(UIImage *resultImage, NSDictionary *info) {
                if ([[info valueForKey:@"PHImageResultIsDegradedKey"] integerValue] == 0){
                    ...
                    if ([UserManager.userRoleAuthInfo.upImageVideoFile.configValue isEqualToString:@"true"] && ((imgSize / (1024 * 1024.0)) > [UserManager.userRoleAuthInfo.upImageVideoFile.configData integerValue])) {
                        [HUD showMessage:MultilingualTranslation(@"所选资源超过限制")];
                        return;
                    } else {
...
- (void)imagePickerCellCamera {
    if (!_isSignlePhoto) {
        if (IMAGEPICKER.zSelectedAssets.count >= _maxSelectNum) {
            [HUD showMessage:[NSString stringWithFormat:MultilingualTranslation(@"最多只能选择%ld张照片"),_maxSelectNum]];
            return;;
        }
    }
    [self openCamera];
...
- (void)calculateSelectImageSize {
    ...
    // 先更新UI显示，避免用户看到"加载中"
    [ZTOOL doInMain:^{
        weakSelf.lblTotalMB.text = [NSString stringWithFormat:MultilingualTranslation(@"共%ld个文件"),IMAGEPICKER.zSelectedAssets.count];
        [weakSelf.navBtnRight setTitle:[NSString stringWithFormat:MultilingualTranslation(@"%ld/%ld发送"),IMAGEPICKER.zSelectedAssets.count,weakSelf.maxSelectNum] forState:UIControlStateNormal];
        if (IMAGEPICKER.zSelectedAssets.count > 0) {
            [weakSelf.navBtnRight setBackgroundColor:COLOR_81D8CF];
        }else {
    ...
    [self getPhotoBytesWith:[IMAGEPICKER.zSelectedAssets copy] completion:^(NSString *totalBytes) {
        [ZTOOL doInMain:^{
            weakSelf.lblTotalMB.text = [NSString stringWithFormat:MultilingualTranslation(@"共%@"),totalBytes];
...
- (void)openCamera {
    ...
    if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied){
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:MultilingualTranslation(@"温馨提示") message:MultilingualTranslation(@"请在设置中开启相机权限使用此功能") preferredStyle:UIAlertControllerStyleAlert];
        
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:MultilingualTranslation(@"下次开启") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
            
        }];
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:MultilingualTranslation(@"立即开启") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)showImagePicker{
    ...
    if([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]){
        ...
        [HUD showMessage:MultilingualTranslation(@"相机权限未开启，请在设置中选择当前应用，开启相机权限")];
    }
}
#pragma mark - 相机拍照回调
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:(NSString *)kUTTypeImage]) {
        ...
        if (exifDic) {
            ...
            localIdentifier = [NSString stringWithFormat:@"%lld-%@", [NSDate currentTimeIntervalWithMillisecond], [ZDeviceTool appUniqueIdentifier]];
        }
        ...
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            ...
            [ZTOOL doInMain:^{
                if (success) {
                    ...
                    [HUD showErrorMessage:MultilingualTranslation(@"操作失败，请重试")];
                }
                [HUD hideHUD];
            }];
        }];
...
- (void)downloadAssetFromCloud:(PHAsset *)phAsset complate:(ZDownloadAsset)complate {
    [HUD showActivityMessage:MultilingualTranslation(@"从iCloud下载...")];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (asset1) {
                    ...
                    if (![info[PHImageCancelledKey] boolValue]) {
                        // 如果不是取消，弹窗提示
                        [HUD showMessage:MultilingualTranslation(@"资源下载失败")];
                    }
