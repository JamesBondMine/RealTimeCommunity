...
#import "ZFilePickerVC.h"
#import "ZFileSourceView.h"
#import "ZFilePickerHeaderView.h"
#import "ZFilePickerCell.h"
#import "ZToolManager.h"//工具类

@interface ZFilePickerVC () <UITableViewDelegate, UITableViewDataSource, UIDocumentPickerDelegate>
{
    BOOL _isPhotoOk;
}
...
- (void)viewDidLoad {
    ...
    if (![NSString isNil:self.sessionFoldPath]) {
        ...
        NSMutableDictionary *receivedFileDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                                MultilingualTranslation(@"接收到的其他文件"),@"title",
                                                [NSNumber numberWithBool:NO],@"unfold",
                                                @[],@"itemArr",nil];
...
- (void)setupNavUI {
    ...
    NSString * st = [NSString stringWithFormat:MultilingualTranslation(@"%@中的文件"), [ZTOOL getAppName]];
    [_navTitleBtn setTitle:st forState:UIControlStateNormal];
    [_navTitleBtn setTkThemeTitleColor:@[COLOR_33, COLOR_33_DARK] forState:UIControlStateNormal];
    [_navTitleBtn setImage:ImgNamed(@"com_c_circle_arrow") forState:UIControlStateNormal];
    ...
    [_navBtnRight setTitle:MultilingualTranslation(@"发送") forState:UIControlStateNormal];
    [_navBtnRight setTkThemeTitleColor:@[COLORWHITE, COLOR_CCCCCC_DARK] forState:UIControlStateNormal];
    _navBtnRight.hidden = YES;
    _navBtnRight.tkThemebackgroundColors = @[COLOR_81D8CF, COLOR_81D8CF_DARK];
...
- (void)requestAlbumAll {
    ...
    [ZTOOL doAsync:^{
        ...
        for (PHAsset *tempAsset in albumVideoArr) {
            ...
            [[PHImageManager defaultManager] requestImageDataForAsset:tempAsset options:options resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                if ([[info objectForKey:PHImageResultIsInCloudKey] boolValue] == NO) {
                    ZFilePickModel *albumVideoModel = [[ZFilePickModel alloc] init];
        ...
        [albumDic setObjectSafe:[NSString stringWithFormat:@"%@（%lu）",MultilingualTranslation(@"相册视频"), (unsigned long)albumVideoArr.count] forKey:@"title"];
...
- (void)getSessionLocalFile {
    ...
    for (NSString *tempFileName in allFileList) {
        ...
        if (![[tempFileName substringToIndex:1] isEqualToString:@"."]) {
            ...
            ZFilePickModel *singleFileModel = [[ZFilePickModel alloc] init];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    NSMutableDictionary *sectionDic = (NSMutableDictionary *)[self.fileList objectAtIndex:section];
    ZFilePickerHeaderView *viewHeader = [tableView dequeueReusableHeaderFooterViewWithIdentifier:NSStringFromClass([ZFilePickerHeaderView class])];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZFilePickerCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ZFilePickerCell class]) forIndexPath:indexPath];
    NSMutableDictionary *sectionDic = (NSMutableDictionary *)[self.fileList objectAtIndex:indexPath.section];
    if (indexPath.section == 0) {
        NSArray *itemArr = (NSArray *)[sectionDic objectForKey:@"itemArr"];
        ZFilePickModel *tempAssetModel = (ZFilePickModel *)[itemArr objectAtIndex:indexPath.row];
        cell.videoAsset = tempAssetModel.videoAsset;
        cell.isSelected = tempAssetModel.isSelected;
    } else {
        NSArray *itemArr = (NSArray *)[sectionDic objectForKey:@"itemArr"];
        ZFilePickModel *tempFileModel = (ZFilePickModel *)[itemArr objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZFilePickerCell *cell = [self.fileTableView cellForRowAtIndexPath:indexPath];
    ...
    if ([UserManager.userRoleAuthInfo.upFile.configValue isEqualToString:@"true"] && ((cell.currentFileSize / (1024 * 1024)) > [UserManager.userRoleAuthInfo.upFile.configData integerValue])) {
        ...
        [_navBtnRight setTitle:[NSString stringWithFormat:MultilingualTranslation(@"发送(%d)"), self.selectFileList.count] forState:UIControlStateNormal];
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls {
    ...
    if (fileAut) {
        ...
        [fileCoordinator coordinateReadingItemAtURL:urls.firstObject options:0 error:&error byAccessor:^(NSURL *newURL) {
            ...
            if (error) {
                [HUD showMessage:MultilingualTranslation(@"未能打开文件")];
                return;
            }
            ...
            if ([UserManager.userRoleAuthInfo.upFile.configValue isEqualToString:@"true"] && (fileSize > [UserManager.userRoleAuthInfo.upFile.configData integerValue])) {
                ...
                ZFilePickModel *phoneFileModel = [[ZFilePickModel alloc] init];
...
- (void)navTitleBtnClicked {
    ZFileSourceView *sourceView = [[ZFileSourceView alloc] init];
    ...
    [sourceView setSelectClick:^(NSInteger index) {
        ...
        if (index == 2) {
            //手机储存
            [weakSelf.navTitleBtn setTitle:MultilingualTranslation(@"手机储存") forState:UIControlStateNormal];
...
- (UITableView *)fileTableView {
    if (!_fileTableView) {
        ...
        _fileTableView.tkThemebackgroundColors = @[COLOR_F5F6F9, COLOR_33];
        _fileTableView.bounces = NO;
        _fileTableView.separatorColor = COLOR_CLEAR;
        
        [_fileTableView registerClass:[ZFilePickerHeaderView class] forHeaderFooterViewReuseIdentifier:NSStringFromClass([ZFilePickerHeaderView class])];
        [_fileTableView registerClass:[ZFilePickerCell class] forCellReuseIdentifier:NSStringFromClass([ZFilePickerCell class])];
    }
