...
- (void)createCalendarViewWith:(NSDate *)date{
    ...
    CGFloat itemH     = DWScale(44);
    
    // 周
    NSArray *array = @[MultilingualTranslation(@"星期日"), MultilingualTranslation(@"星期一"), MultilingualTranslation(@"星期二"), MultilingualTranslation(@"星期三"), MultilingualTranslation(@"星期四"), MultilingualTranslation(@"星期五"), MultilingualTranslation(@"星期六")];
    UIView *weekBg = [[UIView alloc] init];
    weekBg.tkThemebackgroundColors = @[COLORWHITE, COLOR_33];
    weekBg.frame = CGRectMake(0, 0, self.frame.size.width, DWScale(35));
    ...
    for (int i = 0; i < 7; i++) {
        ...
        week.tkThemetextColors = @[COLOR_99, COLOR_99];
        week.frame    = CGRectMake(itemW * i, 6, itemW, DWScale(30));
        week.textAlignment   = NSTextAlignmentCenter;
        week.backgroundColor = [UIColor clearColor];
        week.numberOfLines = 2;

        [weekBg addSubview:week];
    }
    
    // 天 (1-31)
    CGFloat btnW = DWScale(30);
    for (int i = 0; i < 42; i++) {
        
        int x = (i % 7) * itemW + (itemW-btnW)*0.5;
        int y = (i / 7) * itemH + CGRectGetMaxY(weekBg.frame) + (DWScale(64)-btnW)*0.5;
        
        UIButton *dayButton = _daysArray[i];
        dayButton.frame = CGRectMake(x, y, btnW, btnW);
        dayButton.titleLabel.font = FONTR(12);//[UIFont systemFontOfSize:12.0];
        ...
        if ([KLSignInTool month:date] == [KLSignInTool month:[NSDate date]]) {
            ...
            if (i < todayIndex && i >= firstWeekday) {
                ...
                [self setSign:i andBtn:dayButton isToday:false];
            }
            if(i == todayIndex){
                [self setSign:i andBtn:dayButton isToday:YES];
            }
        }
    }
}
...
#pragma mark 设置已经签到
- (void)setSign:(int)i andBtn:(UIButton*)dayButton isToday:(BOOL)istoday{
    WEAKSELF;
    if(_signArray.count != 0){
        [_signArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if (now2 == now) {
                [weakSelf setStyleSigned:dayButton isToday:istoday];
                *stop = YES;
            }else{
                if(istoday){
                    [self setStyleTodayUnSigned:dayButton];
                }
            }
        }];
    }else{
...
{
    btn.enabled = NO;
    [btn setTkThemeTitleColor:@[COLORWHITE, COLOR_33] forState:UIControlStateNormal];
}
...
{
    btn.enabled = NO;
    [btn setTkThemeTitleColor:@[HEXCOLOR(@"CACACA"), HEXCOLOR(@"CACACA")] forState:UIControlStateNormal];
}
//今日
- (void)setStyleToday:(UIButton *)btn
{
    btn.enabled = NO;
    [btn setTkThemeTitleColor:@[COLORWHITE, COLORWHITE] forState:UIControlStateNormal];
    [btn setBackgroundColor:HEXCOLOR(@"7154F5")];
}
//今日已签到
- (void)setStyleTodaySigned:(UIButton *)btn
{
    btn.enabled = NO;
    [btn setTkThemeTitleColor:@[HEXCOLOR(@"7154F5"), HEXCOLOR(@"7154F5")] forState:UIControlStateNormal];
...
{
    btn.enabled = NO;
    btn.layer.borderColor = HEXCOLOR(@"7154F5").CGColor;
...
{
    btn.enabled = NO;
    [btn setTkThemeTitleColor:@[COLOR_66, COLOR_66] forState:UIControlStateNormal];
}
//已经签过的 日期style
- (void)setStyleSigned:(UIButton *)btn isToday:(BOOL)istoday
{
    btn.enabled = NO;
